...
%% &= \colim{m \in \nat} {T^m_{G, \Eq_{\rho[\overline{\alpha := A}]}}}\,K_0
%%   \,(\ol{\relsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%%   \Eq_{\rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}}) \\ 
%% &= \colim{m \in \nat} {T^m_{G, \Eq_{\rho[\overline{\alpha := A}]}}}\,K_0
%%   \,\ol{\Eq_{\setsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\ 
%% &= \colim{m \in \nat} (\Eq_{{(T^{\set}_{G, \rho[\overline{\alpha := A}]}})^m\,K_0})^*
%%   \,\ol{\Eq_{\setsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\ 
%% &= (\Eq_{\colim{m \in \nat} {(T^{\set}_{G, \rho[\overline{\alpha := A}]}})^m\,K_0})^*
%%   \,\ol{\Eq_{\setsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\ 
%% &= (\Eq_{{\mu T^{\set}_{G, \rho[\overline{\alpha := A}]}}})^*
%%   \,\ol{\Eq_{\setsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\
%% &= \Eq_{{\mu T^{\set}_{G, \rho[\overline{\alpha := A}]}}
%%   \,\ol{\setsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\
%=& \Eq_{\setsem{\Gamma; \Phi, \ol\gamma, \ol{\alpha}, \ol{\gamma} \vdash (\mu \psi.\, \lambda \beta.\, G) \ol{\gamma}}
%  \rho[\overline{\alpha := A}][\ol{\gamma := \setsem{\Gamma; \Phi, \ol\gamma, \phi, \ol{\alpha} \vdash K}
%  \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}]} \\
...
for~\eqref{eq:helper} on $m$. The fourth equality holds because $\phi$
either does not apear in $G$, or must have arity $0$, in which case
...
\item 
$\underline{\Gamma;\Phi , \ol\gamma \,|\, \Delta \vdash t_{\overline K} s: G
  [\overline{\alpha := K}]}$\; 
  To see that $\setsem{\Gamma;\Phi , \ol\gamma\,|\,
  \Delta \vdash t_{\overline K} s: G [\overline{\alpha := K}]}$ is a
  natural transformation from $\setsem{\Gamma; \Phi , \ol\gamma \vdash \Delta}$ to
  $\setsem{\Gamma;\Phi , \ol\gamma \vdash G [\overline{\alpha := K}]}$ we must
  show that, for every $\rho : \setenv$, $\setsem{\Gamma;\Phi , \ol\gamma \,|\,
    \Delta \vdash t_{\overline K} s: G [\overline{\alpha := K}]}\rho$
  is a morphism from $\setsem{\Gamma; \Phi , \ol\gamma \vdash \Delta}\rho$ to
  $\setsem{\Gamma;\Phi , \ol\gamma \vdash G [\overline{\alpha := K}]}\rho$, and
  that this family of morphisms is natural in $\rho$. Let $d :
  \setsem{\Gamma; \Phi , \ol\gamma \vdash \Delta}\rho$. Then
  \[\begin{array}{ll}
  & \setsem{\Gamma;\Phi , \ol\gamma \,|\, \Delta \vdash t_{\overline K} s: G
  [\overline{\alpha := K}]}\,\rho\,d\\
= & (\eval \circ \langle (\setsem{\Gamma;\ol\gamma \,|\, \Delta \vdash
  t : \Nat^{\overline{\alpha}} \,F \,G}\rho\;
\_)_{\overline{\setsem{\Gamma;\Phi, \ol\gamma\vdash K}\rho}},\,
\setsem{\Gamma;\Phi, \ol\gamma \,|\, \Delta \vdash s: F [\overline{\alpha :=
      K}]}\rho \rangle)\,d\\
= & \eval ((\setsem{\Gamma;\ol\gamma \,|\, \Delta \vdash t :
  \Nat^{\overline{\alpha}} \,F \,G}\rho\;
\_)_{\overline{\setsem{\Gamma;\Phi, \ol\gamma \vdash K}\rho}} \,d,\,
\setsem{\Gamma;\Phi, \ol\gamma \,|\, \Delta \vdash s: F [\overline{\alpha :=
      K}]}\rho\, d)\\
= & \eval ((\setsem{\Gamma;\ol\gamma\,|\, \Delta \vdash t :
  \Nat^{\overline{\alpha}} \,F \,G}\rho\;
d)_{\overline{\setsem{\Gamma;\Phi, \ol\gamma \vdash K}\rho}},\,
\setsem{\Gamma;\Phi, \ol\gamma \,|\, \Delta \vdash s: F [\overline{\alpha :=
      K}]}\rho\, d)\\
\end{array}\]
The induction hypothesis ensures that $(\setsem{\Gamma;\ol\gamma\,|\,
  \Delta \vdash t : \Nat^{\overline{\alpha}} \,F \,G}\rho\;
d)_{\overline{\setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho}}$ has type
$\setsem{\Gamma; \ol{\alpha} \vdash F}\rho[\ol{\alpha :=
    \setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho}] \to \setsem{\Gamma;
  \ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha := \setsem{\Gamma;\Phi , \ol\gamma \vdash
      K}\rho}]$.  Since, in addition, $\setsem{\Gamma;\Phi , \ol\gamma \,|\,
  \Delta \vdash s: F [\overline{\alpha := K}]}\rho\, d :
\setsem{\Gamma; \Phi , \ol\gamma \vdash F[\ol{\alpha := K}]}\rho =
\setsem{\Gamma; \Phi, \ol\gamma, \ol{\alpha} \vdash F}\rho[\ol{\alpha :=
    \setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho}] = \setsem{\Gamma;
  \ol{\alpha} \vdash F}\rho[\ol{\alpha := \setsem{\Gamma;\Phi , \ol\gamma \vdash
      K}\rho}]$ by Equation~(6),
we have that $\setsem{\Gamma;\Phi , \ol\gamma \,|\, \Delta
  \vdash t_{\overline K} s: G [\overline{\alpha :=
      K}]}\,\rho\,d : \setsem{\Gamma; \Phi, \ol\gamma ,\ol{\alpha} \vdash
  G}\rho[\ol{\alpha := \setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho}] =
\setsem{\Gamma; \Phi , \ol\gamma \vdash G[\ol{\alpha := K}]}\rho$, as desired.

\vspace*{0.1in}

To see that the family of maps comprising $\setsem{\Gamma;\Phi , \ol\gamma \,|\,
  \Delta \vdash t_{\overline K} s: G [\overline{\alpha := K}]}$
is natural in $\rho$
we need to show that, if $f : \rho \to \rho'$ in $\setenv$, then the
following diagram commutes, where $g = \setsem{\Gamma;\ol\gamma\,|\,
  \Delta \vdash t : \Nat^{\overline{\alpha}} \,F \,G}$ and $h =
\setsem{\Gamma;\Phi , \ol\gamma \,|\, \Delta \vdash s: F [\overline{\alpha :=
      K}]}$:
{\footnotesize
\[\begin{tikzcd}
\setsem{\Gamma;\Phi , \ol\gamma \vdash \Delta}\rho \ar[r, "{\setsem{\Gamma;\Phi
      , \ol\gamma \vdash \Delta}f}"] \ar[d, "{\langle g \rho, h \rho\rangle}"']
& \setsem{\Gamma;\Phi , \ol\gamma \vdash 
  \Delta}\rho' \ar[d, "{\langle g \rho', h \rho' \rangle}"]\\
\setsem{\Gamma;\ol\gamma \vdash \Nat^{\overline{\alpha}} \,F \,G}\rho
\times \setsem{\Gamma;\Phi , \ol\gamma \vdash F [\overline{\alpha := K}]}\rho
\ar[d, "{\eval \circ ((-)_{\overline{\sem{\Gamma;\Phi, \ol\gamma \vdash K}\rho}} \times
    \id)}"']
\ar[r, bend left = 5, "{\setsem{\Gamma;\ol\gamma\vdash
      \Nat^{\overline{\alpha}} \,F \,G}f\, \times\, \setsem{\Gamma;\Phi
      , \ol\gamma \vdash F [\overline{\alpha := K}]}f}"] &
\setsem{\Gamma;\ol\gamma\vdash \Nat^{\overline{\alpha}} \,F \,G}\rho'
\times \setsem{\Gamma;\Phi , \ol\gamma \vdash F [\overline{\alpha := K}]}\rho'
\ar[d, "{\eval \circ ((-)_{\overline{\sem{\Gamma;\Phi, \ol\gamma \vdash
          K}\rho'}} \times \id)}"] \\
\setsem{\Gamma;\Phi , \ol\gamma \vdash G [\overline{\alpha := K}]}\rho
\ar[r, "{\setsem{\Gamma;\Phi , \ol\gamma \vdash G [\overline{\alpha := K}]}f}"']
&
\setsem{\Gamma;\Phi , \ol\gamma \vdash G [\overline{\alpha := K}]}\rho'
\end{tikzcd}\]}
...
\setsem{\Gamma; \ol{\alpha} \vdash F}\rho[\ol{\alpha :=
    \setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho}] \ar[r,
  "{\;\;\;\eta_{\ol{\setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho}}\;\;\; }"]
\ar[d, "{\setsem{\Gamma; \ol{\alpha} \vdash F} \id_{\rho} [\ol{\alpha := 
        \setsem{\Gamma;\Phi , \ol\gamma \vdash K}f}]}"']
& \setsem{\Gamma;
  \ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha := \setsem{\Gamma;\Phi , \ol\gamma \vdash
      K}\rho}]
\ar[d, "{\setsem{\Gamma; \ol\gamma,\ol{\alpha} \vdash G} \id_{\rho} [\ol{\alpha := 
        \setsem{\Gamma;\Phi , \ol\gamma \vdash K}f}]}"]\\
\setsem{\Gamma; \ol{\alpha} \vdash F}\rho[\ol{\alpha :=
    \setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho'}] \ar[r,
  "{\eta_{\ol{\setsem{\Gamma;\Phi , \ol\gamma \vdash K}\rho'}} }"]
...
\[
\setsem{\Gamma;\ol\gamma, \ol{\alpha} \vdash G} f[\overline{\alpha := \id_{\setsem{\Gamma;\Phi, \ol\gamma \vdash K}\rho'}}]
\circ \setsem{\Gamma;\ol\gamma, \ol{\alpha} \vdash G} \id_{\rho} [\overline{\alpha := \setsem{\Gamma;\Phi, \ol\gamma \vdash K}f}]
\]
}
