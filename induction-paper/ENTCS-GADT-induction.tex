...
\item If $\mathsf{F\,G\,\ol{B} = H\, (\ol{F_k\,G\,\ol{B}})}$, if
  ...
  $\mathsf{H^\wedge}$ has a map function $\mathsf{H^\wedge Map =
    \forall (\ol{C : Set}) (\ol{Q_C\;Q_C' : C \to Set}) \to
...
deep induction rules, we might expect that truly nested GADTs would as
well. Surprisingly, however, they do not. That is, our results from
...
model interpreting it, and deel induction for a truly nested type or a
ruly nested GADT also relies on this interpretation being functorial.
But, whereas nested types can admit a functorial parametric semantics,
GADTs cannot admit both functorial and parametric semantics at the
same time~\cite{jgj21}. In this section we show how deep induction for
truly nested GADTs nesting goes wrong by analyzing the following
concrete example of such a type.\looseness=-1
...
deep induction for such GADTs.\looseness=-1
