...
proved by induction outright or rely on lemmas that are.  {\color{red}
  ...
  Coq}\footnote{{\color{red} Algebraic data types are 
  defined using the $\mathsf{Inductive}$ keyword in Coq.}}
  % called inductive
  %   types in Coq.}}
    , an induction rule is automatically generated for
it.\looseness=-1
...
bushes
% \footnote{To define $\mathsf{Bush}$ and other truly nested types in Agda, 
% we must use the $\mathsf{NO\_POSITIVITY\_CHECK}$
% pragma . Similar flags are required in Coq.}
~\cite{bm98}:
...
{\small
\[\begin{array}{lll}
\mathsf{LType^{\wedge}\,A\,Q_A\,(tbool\,B\,e)} & = &\mathsf{\exists
...
= & \mathsf{capp\,A\,B\,Q_A\, Q_B\, t_{BA}\, t_B\, p_{BA} \, p_B}\\
  \mathsf{dIndLTerm \, P\, cvar \, cabs\, capp\, listc \, A\, Q_A\,
    (list \,B \,e \, ts) \, (Q_B , liftE', lift_{List})} & = & 
  \mathsf{listc \,A\,B\,Q_A\, Q_B\, e\, ts\, liftE'\, p_{List} }
\end{array}\]}
...
deep induction for truly nested GADTs nesting goes wrong by analyzing
the following very simple nested proper GADT:\looseness=-1
...
\[ \mathsf{getTypeProof \,A \,t = 
    dIndLTerm\, P \, cvar\, cabs\, capp\, listc\, A\, K_\top\, t\,
...
%  LLTy^\wedge B\,
L\, (just \,T_C) = just \, (tarr\, B\, C\, e\, T_B \,T_C)}$ otherwise.
In the third clause, $\mathsf{capp\, A \,B \, Q_A \, Q_B \, t_{BA}\,
  t_A \, (just \, (tarr\, B \,A\, refl\, T_B\, T_A))\, mb = just\,
...
the type of $\mathsf{lift_{ts}}$ becomes $\mathsf{List^{\wedge}\,
