...
This way, deep induction only inspects the structure of the data type and not its content, just like structural induction does.
...
\[
\mathsf{SIEqual\,Q\,srefl\,a\,b\,e = DIEqual\,P\,srefl\,a\,b\,K_1\,K_1\,e\,L_E}
\]
where $\mathsf{srefl: \forall (c : Set) \to P\,c\,c\,Refl}$
...
\item the left-most $\mathsf{K_1}$ is the constantly $\mathsf{1}$-valued predicate on $\mathsf{a}$;
\item the right-most $\mathsf{K_1}$ is the constantly $\mathsf{1}$-valued predicate on $\mathsf{b}$;
\item $\mathsf{L_E : Equal^{\wedge}\,a\,b\,K_1\,K_1\,e}$ is defined by pattern matching,
i.e., in case $\mathsf{a = b}$ and $\mathsf{e}$ is $\mathsf{Refl}$,
...
We also need the lifting of predicates over the polymorphic type of pairs, $\mathsf{Pair = \forall (b\,c : Set) \to b \times c}$, which is 
\[
...
\]
%The only requirement on the $\mathsf{K_i}$ is that they need to have a predicate-lifting function.
...
In the definition above, $\mathsf{F}$ has type $\mathsf{(Set \to Set) \to Set^{\beta} \to Set}$
...
\begin{multline*}
\mathsf{CC = \forall (P : \forall (a : Set) \to (a \to Set) \to G\,a \to Set)} \\
...
%\subsection{General case (Daniel's revision)}
%
%%We restrict our scope to GADTs not featuring nesting,
%%in the sense that their constructors contain no nested calls of the recursive variable,
%%as truly nested types do.
...
%\end{equation}
%is defined by a finite number of constructors $\mathsf{C_i}$.
%In the definition above, $\mathsf{F_i}$ has type $\mathsf{(Set^{\alpha} \to Set) \to Set^{\beta} \to Set}$
%and each $\mathsf{K_i}$ has type $\mathsf{Set^{\beta} \to Set}$.
%The overline notation denotes a finite list:
%%and the vertical bars notation denotes the length of a list:
%$\mathsf{G}$ depends on a finite list of types $\mathsf{\ol{a}}$ of length $\mathsf{\alpha}$,
%each of the $\mathsf{\alpha}$-many $\mathsf{K_i}$ depends on a finite list of types $\mathsf{\ol{b}}$ of length $\mathsf{\beta}$,
%and notice that the length of the list of types $\mathsf{\ol{a}}$ matches that of the list of type constructors $\mathsf{\ol{K_i}}$.
%We allow each $\mathsf{F_i}$ to be inductively built in the following ways (and with the following restrictions):
%\begin{itemize}
%\item $\mathsf{F_i = F_i' \times F_i''}$ where $\mathsf{F_i'}$ and $\mathsf{F_i''}$ have the same type as $\mathsf{F_i}$.
%\item $\mathsf{F_i = F_i' + F_i''}$ where $\mathsf{F_i'}$ and $\mathsf{F_i''}$ have the same type as $\mathsf{F_i}$.
%\item $\mathsf{F_i = F_i' \to F_i''}$
%where $\mathsf{F_i'}$ does not contain the recursive variable,
%i.e., $\mathsf{F_i' : Set^\beta \to Set}$,
%and $\mathsf{F_i''}$ has the same type as $\mathsf{F_i}$.
%\item $\mathsf{F_i\,G\,\ol{b} = G(\ol{F_a\,\ol{b}})}$
%where none of the $\mathsf{F_a}$ contains the recursive variable,
%i.e., $\mathsf{F_a : Set^\beta \to Set}$
%for each $\mathsf{a}$.
...
%where $\mathsf{H}$ is a type constructor not containing the recursive variable,
%i.e., $\mathsf{H : Set^\gamma \to Set}$,
%and $\mathsf{F_c}$ has the same type as $\mathsf{F_i}$, for every $\mathsf{c = 1 \dots \gamma}$.
%Moreover, we assume that $\mathsf{H}$ is not a GADT itself (but we allow it to be an ADT or even a nested type).
%\end{itemize}
%The only requirement on the $\mathsf{K_i}$ is that they need to have a predicate-lifting function.
%
%Consider such a generic GADT
...
%= \exists (\ol{Q_b : b \to Set})
%\to F^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,{\ol{Q_b}}\,x}
%\]
%where $\mathsf{Q_a : a \to Set}$
...
%\mathsf{\to F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x
%	\to P\,\ol{a}\,\ol{Q_a}\,(C\,\ol{b}\,\ol{e_a}\,x)}
%\end{multline*}
%to the constructor $\mathsf{C}$.
%
...
%Define
%\[
%\mathsf{DIG\,P\,cc\,\ol{a}\,\ol{Q_a}\,(C\,\ol{b}\,\ol{e_a}\,x)\,(\ol{Q_b}, L_F)
%= cc\,\ol{a}\,\ol{b}\,\ol{Q_a}\,\ol{Q_b}\,\ol{e_a}\,x\,(p\,x\,L_F)}
%\]
%where
%\begin{itemize}
%\item $\mathsf{cc : CC\,P}$;
...
%and $\mathsf{x : F\,G\,\ol{b}}$;
%\item $\mathsf{(\ol{Q_b}, L_F) : G^{\wedge}\,\ol{a}\,\ol{Q_a} (C\,\ol{b}\,\ol{e_a}\,x)}$,
%i.e.,
%$\mathsf{Q_b : b \to Set}$ for each $\mathsf{b}$,
%and $\mathsf{L_F : F^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,{\ol{Q_b}}\,x}$.
%\end{itemize}
...
%%Thus, we define $\mathsf{p\,x\,L_F\,L_1 = p_2\,(L_F\,L_1)}$
%%for $\mathsf{L_1 : F_1^{\wedge}\,\ol{b}\,\ol{Q_b}\,x}$.
%Notice that $\mathsf{F_1}$ does not containing the recursive variable
%is a necessary restriction, as the proof relies on
%$\mathsf{F^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,\ol{Q_b}\,x}$
%and $\mathsf{F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x}$
%having the same domain.
