...
% on the structure of $\mathsf{F}$ and $\mathsf{K}$ respectively.


%Associate the type
%\begin{multline}
%\mathsf{PC = \forall (\ol{a : Set}) (\ol{b : Set}) (\ol{Q_a : a \to Set}) (\ol{Q_b : b \to Set}) (\ol{e_a : Equal\,a\,(K\,\ol{b})}) (x : F\,G\,\ol{b})} \\
...
%\subsection{General case}
%
%%Outline general heuristics for this process, generalizing process in deep induction paper.
%%\begin{itemize}
%%    \item Lifting for GADT (possibly a restricted class?)
%%    \item Deep induction rule
%%    \item Prove deep induction (define term)
%%\end{itemize}
%
%TODO:
%\begin{itemize}
...
%The overline notation denotes a finite list:
%$\mathsf{G}$ depends on a finite list of types $\mathsf{a}$,
%each $\mathsf{K_i}$ depends on a finite list of types $\mathsf{b}$,
%and notice that the length of the list of $\mathsf{a}$ matches that of the list of $\mathsf{K_i}$.
%With that in mind, each constructor $\mathsf{C_i}$ can be rewritten as
%\[
...
%as
%\[
%\mathsf{DIG\,P\,\ol{pc_i}\,\ol{a}\,\ol{Q_a}\,(C_i\,\ol{b}\,\ol{e_a}\,x)\,(\ol{Q_b}, \ol{E_a}, L_F)
%= pc_i\,\ol{a}\,\ol{b}\,\ol{Q_a}\,\ol{Q_b}\,\ol{e_a}\,x\,\ol{E_a}\,(p\,L_F)}
%\]
%where $\mathsf{\ol{pc_i : PC_i}}$,
%$\mathsf{\ol{Q_a : a \to Set}}$,
%$\mathsf{\ol{e_a : Equal\,a\,(K_i\,\ol{b})}}$,
%$\mathsf{x : F_i\,G\,\ol{b}}$,
%$\mathsf{\ol{Q_b : b \to Set}}$,
%$\mathsf{\ol{E_a : Equal^{\wedge}\,a\,(K_i\,\ol{b})\,Q_a\,(K_i^{\wedge}\,\ol{b}\,\ol{Q_b})\,e_a}}$,
%$\mathsf{L_F : F_i^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,{\ol{Q_b}}\,x}$,
%and $\mathsf{p}$ is defined in the following lemma.
%
%\begin{lemma}
...
%We have that $\mathsf{F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x = F_1^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x \times F_2^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x}$.
%By inductive hypothesis, there exist functions
%$\mathsf{p_1 : F_1^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,\ol{Q_b}\,x_1 \to F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x_1}$ and
%$\mathsf{p_2 : F_2^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,\ol{Q_b}\,x_2 \to F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x_2}$.
%Thus, we define $\mathsf{p (L_1, L_2) = (p_1\,L_1) \times (p_2\,L_2)}$
%%%%
%\item Case $\mathsf{F = F_1 + F_2}$. Analogous to case $\mathsf{F = F_1 \times F_2}$.
%%%%
%\item Case $\mathsf{F = F_1 \to F_2}$ where $\mathsf{F_1}$ does not contain the recursive variable.
%We have that
%\begin{gather*}
%\mathsf{F^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,\ol{Q_b}\,x = F_1^{\wedge}\,\ol{b}\,\ol{Q_b}\,x \to F_2^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,\ol{Q_b}\,x} \\
%\mathsf{F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x = F_1^{\wedge}\,\ol{b}\,\ol{Q_b}\,x \to F_2^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x}
%\end{gather*}
%By inductive hypothesis, there exist a function
%$\mathsf{p_2 : F_2^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,\ol{Q_b}\,x_2 \to F_2^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x_2}$.
%Thus, we define $\mathsf{p\,L\,l = p_2\,(L\,l)}$ for
%$\mathsf{L : F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x}$
%and $\mathsf{l : F_1^{\wedge}\,\ol{b}\,\ol{Q_b}\,x}$.
%
%{\color{blue} TODO: explain why $\mathsf{F_1}$ does not contain the recursive variable.}
%%%%
%\item Case $\mathsf{F\,G\,\ol{b} = G(\ol{F_a\,\ol{b}})}$.
%Thus, $\mathsf{F^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b} = P (\ol{F_a\,\ol{b}}) (\ol{F_a^{\wedge}\,\ol{b}\,\ol{Q_b}})}$.
%So, $\mathsf{p}$ is defined as
%\[
%\mathsf{p\,L_F = DIG\,P\,\ol{pc_i}\,(\ol{F_a\,\ol{b}})\,(\ol{F_a^{\wedge}\,\ol{b}\,\ol{Q_b}})\,x\,L_F}
%\]
%Note: No nesting!
%\end{itemize}
%\end{proof}

%Define $\mathsf{p_b}$ by structural induction on $F_i$.
%
%Case $\mathsf{F_i\,G\,\ol{b} = F'_i\,G\,\ol{b} \times F''_i\,G\,\ol{b}}$.
%We have $\mathsf{F_i^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,(x', x'') = (F'_i)^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x' \times (F''_i)^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x''}$.
%Thus define
%\[
%\mathsf{
%p_b\,P\,PC_i\,\ol{a}\,\ol{Q_a}\,\ol{b}\,\ol{e_a}\,(x', x'')\,\ol{e'_a}\,(l', l'')
%=
%(
%p'_b\,P\,PC_i\,\ol{a}\,\ol{Q_a}\,\ol{b}\,\ol{e_a}\,x'\,\ol{e'_a}\,l',
%p''_b\,P\,PC_i\,\ol{a}\,\ol{Q_a}\,\ol{b}\,\ol{e_a}\,x''\,\ol{e'_a}\,l''
%)
%} 
%\]
%where $\mathsf{p'_b}$ is the function associated to $\mathsf{F'_i}$
%and $\mathsf{p''_b}$ is the function associated to $\mathsf{F''_i}$.
%
%
%Case $\mathsf{F_i\,G\,\ol{b} = H(\ol{F'_i\,G\,\ol{b}})}$ with $\mathsf{H}$ data type with constructors $\mathsf{D_i}$.
%We have
%\[
%\mathsf{F_i^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x = H^{\wedge}\,(\ol{F'_i\,G\,\ol{b}}) (\ol{(F'_i)^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}}) x}
%\]
%Thus define
%\[
%\mathsf{
%p_b\,P\,PC_i\,\ol{a}\,\ol{Q_a}\,\ol{b}\,\ol{e_a}\,x\,\ol{e'_a}\,l
%=
%} 
%\]
...
%\subsection{General case (with product)}
%
%Note: case with one constructor to make things simpler
%
%Consider a generic GADT
%\begin{equation}\label{eq:gadt}
%\begin{array}{l}
%\mathsf{data\, G\,(\ol{a : Set}) : Set\,where}\\
%\mathsf{\;\;\;\;\;\;\;\;C\, :\, F_1\,G\,\ol{b} \times \dots \times F_n\,G\,\ol{b} \to G (\ol{K\,\ol{b}})}
%\end{array}
%\end{equation}
%where each $\mathsf{F_i}$ has type $\mathsf{(Set^{\lvert\ol{a}\rvert} \to Set) \to Set^{\lvert\ol{b}\rvert} \to Set}$
%and $\mathsf{K}$ has type $\mathsf{Set^{\lvert\ol{b}\rvert} \to Set}$.
%The constructor $\mathsf{C}$ can be rewritten as
%\[
%\mathsf{C : \exists (\ol{b : Set}) \to \ol{Equal\,a\,(K\,\ol{b})} \to F_1\,G\,\ol{b} \to F_n\,G\,\ol{b} \to G\,\ol{a}}
%\]
%which is the form we shall use from now on.
%
%We define the lifting of $\mathsf{G}$
...
%as
%\begin{multline*}
%\mathsf{G^{\wedge}\,\ol{a}\,\ol{Q_a}\,(C\,\ol{b}\,\ol{e_a}\,x_1 \dots x_n)} \\
%\mathsf{= \exists (\ol{Q_b : b \to Set})
%\to \ol{Equal^{\wedge}\,a\,(K\,\ol{b})\,Q_a\,(K^{\wedge}\,\ol{b}\,\ol{Q_b})\,e_a}
%\times F_1^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,{\ol{Q_b}}\,x_1
%\dots
%\times F_n^{\wedge}\,G\,\ol{b}\,G^{\wedge}\,{\ol{Q_b}}\,x_n}
%\end{multline*}
%where $\mathsf{Q_a : a \to Set}$, $\mathsf{e_a : Equal\,a\,(K\,\ol{b})}$ and $\mathsf{x_i : F_i\,G\,\ol{b}}$,
%and the liftings
%\begin{multline*}
%\mathsf{F_i^{\wedge} : \forall (G : Set^{\lvert\ol{a}\rvert} \to Set) (\ol{b : Set})
%\to (\forall (\ol{a : Set}) \to (\ol{a \to Set}) \to G\,\ol{a} \to Set)} \\
...
%\begin{multline}
%\mathsf{DIG : \forall (P : \forall (\ol{a : Set}) \to (\ol{a \to Set}) \to G\,\ol{a} \to Set)} \\
%\mathsf{\to \bigtimes_i \big( \forall (\ol{a : Set}) (\ol{b : Set}) (\ol{Q_a : a \to Set}) (\ol{Q_b : b \to Set}) (\ol{e_a : Equal\,a\,(K\,\ol{b})}) (x_i : F_i\,G\,\ol{b})} \\
%\mathsf{\to \ol{Equal^{\wedge}\,a\,(K\,\ol{b})\,Q_a\,(K^{\wedge}\,\ol{b}\,\ol{Q_b})\,e_a}
%	\to F_1^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x_1 \to \dots \to F_n^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x_n
%	\to P\,\ol{a}\,\ol{Q_a}\,(C\,\ol{b}\,\ol{e_a}\,x_1 \dots x_n) \big)} \\
%\mathsf{\to \forall (\ol{a : Set})(\ol{Q_a : a \to Set})(x : G\,\ol{a}) \to G^{\wedge}\,\ol{a}\,\ol{Q_a}\,x \to P\,\ol{a}\,\ol{Q_a}\,x}
%\end{multline}
%as
%\[
%\mathsf{DIG\,P\,PC\,\ol{a}\,\ol{Q_a}\,(C\,\ol{b}\,\ol{e_a}\,x_1 \dots x_n)\,(\ol{Q_b}, \ol{e'_a}, l_{F_1}, \dots, l_{F_n})
%= PC\,\ol{a}\,\ol{b}\,\ol{Q_a}\,\ol{Q_b}\,\ol{e_a}\,x_1 \dots x_n\,\ol{e'_a}\,p_1 \dots p_n}
%\]
%where
%\[
%\mathsf{p_i = DIG\,P\,PC\,\ol{a}\,\ol{Q_a} ? : F_i^{\wedge}\,G\,\ol{b}\,P\,\ol{Q_b}\,x_i}
%\]
...
%\begin{array}{l}
%\mathsf{data\, G\,(\ol{a : Set}) : Set\,where}\\
%\mathsf{\;\;\;\;\;\;\;\;C_i\, :\, f_i[G/\phi,\ol{b/\beta}] \to G (\ol{k_i[\ol{b/\beta}]})}
%\end{array}
%\end{equation}
%with a finite number of constructors $\mathsf{C_i}$.
%Each constructor $\mathsf{C_i}$ is defined by type expressions $\mathsf{f_i}$,
%containing a type constructor variable $\mathsf{\phi}$ of arity matching the number of type arguments of $G$,
%and type variables $\mathsf{\ol{\beta}}$;
%and $\mathsf{\ol{g_i}}$ containing the type variables $\mathsf{\ol{\beta}}$.
%Each constructor $\mathsf{C_i}$ can be rewritten as
%\[
%\mathsf{C_i : \exists (\ol{b : Set}) \to \ol{Equal\,a\,(k_i[\ol{b/\beta}])} \to f_i[G/\phi,\ol{b/\beta}] \to G\,\ol{a}}
%\]
%which is the form we shall use from now on.
%
%We define the types
%\begin{gather*}
%	\mathsf{F_i : (Set^{\lvert\ol{a}\rvert} \to Set) \to Set^{\lvert\ol{b}\rvert} \to Set} \\
%	\mathsf{F_i }
%\end{gather*}
%
%
%Define the lifting of $\mathsf{G}$
%\[
%\mathsf{G^{\wedge} : \forall (\ol{a : Set}) \to (\ol{a \to Set}) \to G\,\ol{a} \to Set}
%\]
%as
%\[
%\mathsf{G^{\wedge}\,\ol{a}\,\ol{P_a}\,(C_i\,\ol{b}\,\ol{e_a}\,x)
%= \exists (\ol{Q_b : b \to Set})
%\to \ol{Equal^{\wedge}\,a\,k_i[\ol{b/\beta}]\,P_a\,(k_i^{\wedge}\,\ol{b}\,\ol{Q_b})\,e_a}
%\times f_i[G/\phi]^{\wedge}\,\ol{b}\,{\ol{Q_b}}\,x}
%\]
%where $\mathsf{P_a : a \to Set}$, $\mathsf{e_a : Equal\,a\,(k_i\,\ol{b})}$ and $\mathsf{x : f_i\,\ol{b}}$,
%and the liftings $\mathsf{f_i^{\wedge}}$ and $\mathsf{k_i^{\wedge}}$
%are defined the same way by induction on the structure of the type.
