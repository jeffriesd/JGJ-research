...
\title[(Deep) Induction Rules for GADTs]{(Deep) Induction Rules for GADTs}
  ...
  \email{johannp,ghiorzie@appstate.edu}
...
implemention of them is available in the accompanying code.\looseness=-1
%at~\cite{web-page}.
...
predicates but otherwise follows the familiar pattern. It is given
by the first expression in Figure~\ref{fig:ptree-and-bush}. There,
$\mathsf{Pair^{\wedge} : \forall (A\; B: Set) \to (A \to Set) \to (B
  \to Set)\; \to\;}$ $\mathsf{A \times B \to}$ $\mathsf{Set}$
lifts predicates
...
$\mathsf{\,: \,\forall (A : Set) \to (A \to Set) \to PTree\,A \to Set}$
lifts a predicate $\mathsf{Q}$ on data of type $\mathsf{A}$ to a
predicate on data of type $\mathsf{PTree\,A}$ asserting that
$\mathsf{Q}$ holds for every element of type $\mathsf{A}$ contained in
its perfect tree argument.\looseness=-1
