...
Church encodings in a higher-kinded extension of System F, we adopt
...
over lists according to the inductive structure of the input
...
induction rules for bushes and other truly nested types, but has also
...
variable contexts to sets or relations, as appropriate. But to ensure
...
It is well-known that there are no set-based parametric models of
System F~\cite{rey84}. So it is worth noting that our calculus does
not contain all System F types, and is not impredicative, so a
set-based parametric model is not {\em a priori} precluded. In fact,
local finite presentability of $\set$ makes it an obvious choice for
interpreting nested types. We return to this point at several places
below.
...
each Church encoding is interpreted as the least
...
Constructions rather than in a semantic category. His
...
primitives. However, the $\mathit{fmap}$ functions associated to 
Atkey's functors must be {\em given} together with their
underlying type constructors, and he need not 
impose cocontinuity conditions on his model to ensure that 
fixpoints of his functors exist. Unfortunately, Atkey does not 
indicate which
type constructors support the $\mathit{fmap}$ functions
needed for them to be functors, but we suspect spelling this
...
\Nat^\emptyset F \,G$. Since $\Gamma; \emptyset \vdash \Nat^{\ol
  \alpha}F\,G$ represents a natural transformation in $\ol \alpha$
from $F$ to $G$, the type $\Gamma;\emptyset \vdash \Nat^\emptyset F
\,G$ represents the arrow type $\Gamma \vdash F \to G$ in our
calculus. The type $\Gamma; \emptyset \vdash \Nat^{\ol\alpha} \,\onet
\,F$ similarly represents the $\forall$-type $\Gamma; \emptyset \vdash
\forall \ol\alpha . F$.  Note, however, that
%not even all
...
value on $\ast$ and consider a set environment to map a type
variable of arity $0$ to
...
objects since $\set$ is locally small.  We note that $\setsem{\Gamma;
  \emptyset \vdash \Nat^{\ol\alpha} F\,G}$ is $\omega$-cocontinuous because
...
we have that, if $\rho, \rho' : \setenv$, \,$f : \rho \to \rho'$,
