...
\Nat^\emptyset \ (\List\,\alpha)\,(\Nat^\emptyset \alpha\,(\List
    (\mu \beta. 1 + \beta)))$, all of whose argument types are
    parameterized over the same type $\alpha$. This is not possible
    for nested types, even when they are semantically
    equivalent to ADTs.
...
return type of such a function can be parameterized over only 
%any or all of
the type variables occurring in its final argument's type.
%
...
%its first argument.
Moreover, even some recursive functions of a single proper
...
fixpoints studied here. Enhancing the expressivity of our calculus is
discussed further in Section~\ref{sec:conclusion}.
...
an object-level {\em left} Kan extension construct. This suggests
extending our term calculus with such object-level constructs.  It was
shown in~\cite{jp19} that augmenting the types in our calculus with a
carefully designed left Kan extension preserves the cocontinuity
needed for GADTs to have well-defined interpretations --- but not
necessarily in
%However, this will not, in general, be
%the case in
locally finitely presentable catgories. Indeed, as shown
in~\cite{jp19}, local $\lambda$-presentability for $\lambda >
\aleph_1$ is required to interpret even commonly used GADTs. This
suggests
...
Another possibility is to add term-level fixpoints
%, as in~\cite{pit00},
...
domains to replace $\set$, is not locally finitely presentable.
