...
{\color{blue} Can we use $\setsem{\Gamma;\Phi \vdash F}\rho$
instead of an arbitrary set $A$ below?}
...
\begin{itemize}
\item {\color{blue} We abuse the $\Rightarrow$ notation in the second
  step.} The case $J = \Nat^{\ol\beta} G \,K$ is proved by observing
  that
\[
\begin{split}
...
&= \relsem{\Gamma; \ol\beta \vdash
    G}\Eq_{\rho}\ol{[\beta := \_]} \Rightarrow \relsem{\Gamma; \Phi, \phi,
...
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash \Nat^{\ol\beta} G
  ...
  \overline{[\alpha := \Eq_A]} \\
\end{split}\]
Here, the second equality holds because $\phi$ must have arity $0$
(since only functorial variables of arity $0$ can appear free in
bodies of $\mu$ types), and thus $\ol\alpha$ must be empty, in order
for $\Gamma;\Phi, \phi,\ol\alpha \vdash \Nat^{\ol\beta} G\,K$ to be
well-typed. The third equality is by
...
\item The case $J = (\mu \psi.\, \lambda \ol{\beta}.\, G) \ol{K}$
...
&= T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha
      ...
      \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha :=
        \Eq_A}]}\,K_0) \\
&= \lambda \ol{R}. \relsem{\Gamma; \Phi, \phi, \ol\alpha, \psi,
...
must have arity $0$, and thus $\ol\alpha$ must be empty, if $\phi$
appears in $G$, and uses~\eqref{eq:iel-fix-point-intermediate2} for
$G$ when $\phi$ has arity $0$.
\end{itemize}
