...
(Note that we can use an arbitrary set $A$ in the statements below, or
$\setsem{\Gamma;\Phi \vdash F}\rho$ as in the original statements in
supp.tex, if we like.)
...
{\color{blue} $J$}: the only interesting cases, though, are when $J =
\Nat^{\ol\beta} G \,K$, when $J = \phi \ol{G}$ and when $J = (\mu \psi.\, \lambda \ol{\beta}.\, G) \ol{K}$.

The case $J = \Nat^{\ol\beta} G \,K$ is proved by observing that
\[
\begin{split}
& \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash \Nat^{\ol\beta} G
    \,K} \Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0]
  \overline{[\alpha := \Eq_A]} \\
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha},\ol\beta \vdash
    G}\Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0] \overline{[\alpha
      := \Eq_A]}\,\ol{[\beta := \_]} \Rightarrow\\
& \hspace*{0.4in} \relsem{\Gamma; \Phi, \phi,
    \ol{\alpha},\ol\beta \vdash \,K}\Eq_{\rho} [\phi :=
    T^{n}_{H,\Eq_{\rho}} K_0] \overline{[\alpha := \Eq_A]}\,\ol{[\beta := \_]} \\
&= \relsem{\Gamma; \ol\beta \vdash
    G}\Eq_{\rho}\ol{[\beta := \_]} \Rightarrow \relsem{\Gamma; \Phi, \phi^0,
    \ol\beta \vdash \,K}\Eq_{\rho} [\phi :=
    T^{n}_{H,\Eq_{\rho}} K_0]\ol{[\beta := \_]} \\
&= \relsem{\Gamma; \ol\beta \vdash
    G}\Eq_{\rho}[\phi
    := \Eq_{(T^\set_{H,\rho})^n K_0}]\ol{[\beta := \_]} \Rightarrow\\
& \hspace*{0.4in} \relsem{\Gamma; \Phi, \phi^0,
    \ol\beta \vdash \,K}\Eq_{\rho}[\phi
...
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash \Nat^{\ol\beta} G
  ...
  \overline{[\alpha := \Eq_A]} \\
\end{split}\]
Here, the second equality holds because $\Phi$ must consist entirely
of arity $0$ variables, $\phi$ must have arity $0$, and $\ol\alpha$
must be empty for $\Gamma;\Phi, \phi,\ol\alpha \vdash \Nat^{\ol\beta}
G\,K$ to be well-typed, and the third is by
Equation~\ref{eq:iel-fix-point-intermediate2} when $\phi$ has arity
$0$.
...
\[
\begin{split}
& \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash (\mu \psi.\, \lambda \ol{\beta}.\, G) \ol{K}}
  \Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0]
  [\overline{\alpha := \Eq_A}] \\
&= (\mu {T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}})
  \,\ol{\relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash K}\Eq_{\rho}
...
%%   \,(\ol{\relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash K}
%%   \Eq_{\rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}}) \\ 
%% &= \colim{m \in \nat} {T^m_{G, \Eq_{\rho[\overline{\alpha := A}]}}}\,K_0
%%   \,\ol{\Eq_{\setsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash K}
...
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\
%% &= \Eq_{{\mu T^{\set}_{G, \rho[\overline{\alpha := A}]}}
%%   \,\ol{\setsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash K}
%%   \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}} \\
%=& \Eq_{\setsem{\Gamma; \Phi, \ol{\alpha}, \ol{\gamma} \vdash (\mu \psi.\, \lambda \beta.\, G) \ol{\gamma}}
%  \rho[\overline{\alpha := A}][\ol{\gamma := \setsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash K}
%  \rho[\phi := (T^{\set}_{H,\rho})^n K_0][\overline{\alpha := A}]}]} \\
...
&= (\mu {T_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}})
  \,\ol{\relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash K}\Eq_{\rho}
  [\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]} \\ 
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash (\mu \psi.\, \lambda \beta.\, G) \ol{K}}
  \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_{A}}]
\end{split}
\]
where
\begin{itemize}
\item the third equality is by the induction hypothesis for~\eqref{eq:iel-fix-point-intermediate2} on $K$,
\item the fourth equality is because we can prove that for all $m \in \nat$, \\
      $T^m_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}\,K_0 \
      = T^m_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0$. \\
      The base case is trivial because \\
      $T^0_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}\,K_0 \
      = K_0 = T^0_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0$. \\
      The inductive case is proven by: \\
      \begin{align*}
        & T^{m+1}_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}\,K_0 \\
        &= T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]} 
         (T^{m}_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}\,K_0) \\
        &= T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]} 
         (T^{m}_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0) \\
        &= \lambda \ol{R}. \relsem{\Gamma; \Phi, \phi, \ol\alpha, \psi, \ol\beta \vdash G} 
          \Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}][\psi := T^{m}_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0][\overline{\beta := R}] \\
        &= \lambda \ol{R}. \relsem{\Gamma; \Phi, \phi, \ol\alpha, \psi, \ol\beta \vdash G} 
          \Eq_{\rho} [\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}][\psi := T^{m}_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0][\overline{\beta := R}] \\
        &= T_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]} 
         (T^{m}_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0) \\
        &= T^{m+1}_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}\,K_0 
      \end{align*}

      where the second equality is by the induction hypothesis on $m$ and the fourth
      equality is because $\phi$ does not appear in $G$.
