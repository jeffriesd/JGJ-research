...
functors. In Section~\ref{sec:conclusion} we discuss how more
expressive types might be accommodated. 
...
\lambda d\,\ol{B}\,
{\color{red}\ol{C}}.\,(\mathit{in}_{{T}^X_{H,\rho{\color{red}[\ol{\gamma :=
...
\llbracket \Gamma;\emptyset \,|\, \emptyset \vdash
  \fold^F_H : \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F) & = &  
\lambda d\,\eta\,\ol{B}{\color{red}\,\ol{C}}.\,
...
\section{Free Theorems for Nested Types}\label{sec:free-theorems}

\subsection{Naturality and Its Consequences}\label{sec:Nat-type-terms}

If $\Gamma; \ol{\alpha} \vdash F$ then the {\em identity} $\id_F$ on
$F$ is defined by $\id_F = \Gamma;\emptyset~|~\emptyset \vdash
L_{\ol{\alpha}}x.x : \Nat^{\ol{\alpha}} F\,F$, and if $\Gamma;
\emptyset \,|\, \Delta \vdash t: \Nat^{\overline{\alpha}} F\,G$ and
$\Gamma; \emptyset \,|\, \Delta \vdash s: \Nat^{\overline{\alpha}}
G\,H$ are terms then the {\em composition} $s \circ t$ of $t$ and $s$
is defined by $s \circ t = \Gamma; \emptyset\,|\, \Delta \vdash
L_{\overline{\alpha}} x. s_{\overline{\alpha}}(t_{\overline{\alpha}}x):
...
$\underline{\Gamma;\emptyset \,|\, \emptyset \vdash \tin_H :
  ...
  naturality of $\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}}$ ensure
...
morphism from $\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}} \vdash (\mu
  \phi.\lambda \overline \alpha.H)\overline \beta}\rho[\ol{\beta :=
    B}]{\color{red}[\ol{\gamma := C}]} \,=\,(\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}})
\ol{B}$\\ to $\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}} \vdash
...
\[(\setsem{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  ...
  \alpha.H)\overline \beta\,F)}\,\rho\,d\,\eta)_{\ol{B}{\color{red}\,\ol{C}}}\] is a
morphism from
$\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}} \vdash (\mu
  \phi.\lambda \overline \alpha.H)\overline \beta}\rho[\ol{\beta :=
    B}]{\color{red}[\ol{\gamma := C}]}\,=\, (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}})
\ol{B}$ to $\setsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} \vdash
  F}\rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]}$.
To see that this family of morphisms is natural in $\ol{B}$
{\color{red} and
$\ol{C}$}, we observe that the following diagram commutes for all
$\ol{f : B \to B'}$ {\color{red} and $\ol{g : C \to C'}$}:
{\tiny
...
\ar[r,"{(\mathit{fold}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
            C'}]}}}\,(\lambda \ol{A}.\,\eta_{\ol{A}{\color{red}\,\ol{C'}}}))_{\ol{B}}\,}" ] 
\ar[d, "{(\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}) \, \ol{f}}"'] & 
...
by naturality of $\mathit{fold}_{T^\set_{H,\rho[{\color{red}\ol{\gamma
          := C'}]}}}\,(\lambda
\ol{A}.\,\eta_{\ol{A}{\color{red}\,\ol{C'}}})$.
%
%ensures that the bottom diagram commutes. {\color{red} BOTTOM!}
