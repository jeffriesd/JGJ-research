...
in Equation~\ref{eq:colim-form}. Then for any $h$-tuple of functions
...
(\mathit{Lan}_KF) \ol{g} \circ j_{\ol{S},\, \ol{f}} = j_{\ol{S},\,
    \ol{g\, \circ f}}
  \end{equation}
holds.  We can similarly represent left Kan extensions of functors
over $\rel$ in terms of colimits. In that setting we will denote the
morphism mapping the cocone into the colimit $\iota$. We will make
good use of both of these representations in
Section~\ref{sec:term-sem} below.
...
is defined to be 
\begin{multline*}
\big(\lambda \ol{S}.\,\textit{Lan}_{\ol{\setsem{\Gamma; \ol\alpha
      \vdash K}\rho[\ol{\alpha := S}]}}\,  
      \setsem{\Gamma; \Phi, \ol\alpha \vdash F}f[\ol{\alpha := \id_S}]\big)\,
    \ol{\setsem{\Gamma; \Phi \vdash A}\rho'} \\
\circ\;
  \big(\lambda \ol{S}.\,\textit{Lan}_{\ol{\setsem{\Gamma; \ol\alpha
        \vdash K}\rho[\ol{\alpha := S}]}}\,  
      \setsem{\Gamma; \Phi, \ol\alpha \vdash F}\rho[\ol{\alpha := S}]\big)\,
...
\vdash (\Lan^{\alpha}_{\ol{K}} F)\ol{A}$. This is proved in
...
unique morphism such that, for any $\ol{R : \rel_0}$ and $\ol{m :
  ...
  \relsem{ \Gamma; \Phi \vdash A } \rho}$, we have $h \circ (\pi_1
\iota_{\ol{R},\, \ol{m}}) = j_{\ol{\pi_1 R}, \,\ol{\pi_1 m}}$.
...
$\ol{(\id_{\pi_1 R}, !) : (\pi_1 R, 0, 0) \to R}$, we have that $\ol m
\,\circ\, \relsem{ \emptyset; \ol\alpha \vdash K } \rho[\ol{\alpha :=
    (\id_{\pi_1 R}, !)}] = (\ol{\pi_1 m}, !)$ in the indexing category
for the colimit representation of the codomain of $k$. This in turn
implies that {\color{red} $\iota_{\ol{R},\, \ol{m}} =
  \iota_{\ol{(\pi_1 R, 0, 0)},\, \ol{(\pi_1 m, !)}}$}, so that $k
\circ h \circ (\pi_1 \iota_{\ol{R},\, \ol{m}}) = \pi_1
\iota_{\ol{R},\, \ol{m}}$ for all $\ol R$ and $\ol m$, and thus $h
\circ k = \id$.

To prove Equation~\ref{eq:latter}, let $\iota$ be the cocone for the
colimit $\relsem{ \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}}
  F)\ol{A} } \rho$, let $j$ be the cocone for the colimit $\setsem{
  \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}} F)\ol{A} }
(\pi_i\rho)$, and let $h$ and $k$ be the isomorphisms showing that
$\pi_i(\setsem{ \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}}
  F)\ol{A} } \rho) = \setsem{ \Gamma; \Phi \vdash (\Lan^{\ol
    \alpha}_{\ol{K}} F)\ol{A} } (\pi_i\rho)$. Analogously, let
$\iota'$, $j'$, $h'$ and $k'$ be as above but for interpretations with
respect to $\rho'$ rather than $\rho$.  By definition, $\relsem{
  \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}} F)\ol{A} } f$ is the
unique morphism of relations such that, for any $\ol{R : \rel_0}$ and
$\ol{m : \relsem{ \emptyset; \ol\alpha \vdash K } \rho[\ol{\alpha :=
      R}] \to \relsem{ \Gamma; \Phi \vdash A } \rho}$,
\[
(\relsem{ \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}} F)\ol{A} }
f) \circ \iota_{\ol{R},\, \ol{m}} = \iota'_{\ol{R},\, \ol{\relsem{ \Gamma;
      \Phi \vdash A } f \, \circ\, m}}
\]
holds. Projecting onto the $i^{th}$ component of both sides thus gives
that
\[
\pi_i (\relsem{ \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}}
  F)\ol{A} } f) \circ \pi_i \iota_{\ol{R}, \,\ol{m}} = \pi_i
\iota'_{\ol{R},\, \ol{\relsem{ \Gamma; \Phi \vdash A } f \,\circ\, m}}
\]
Now, using the facts from above that $h \circ (\pi_i \iota_{\ol{R},
  \,\ol{m}}) = j_{\ol{\pi_i R},\, \ol{\pi_i m}}$ and, analogously,
that $h' \circ (\pi_i \iota'_{\ol{R},\, \ol{m}}) = j'_{\ol{\pi_i R},
  \,\ol{\pi_i m}}$, we get
{\color{red}
\[
\pi_i (\relsem{ \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}}
  F)\ol{A} } f) \circ k \circ j_{\ol{\pi_i R},\, \ol{\pi_i m}} = k'
\circ j'_{\ol{\pi_i R},\, \ol{\relsem{ \Gamma; \Phi \vdash A } (\pi_i f)
  \, \circ\, (\pi_i m)}}
\]
i.e., $\pi_i (\relsem{ \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}}
  F)\ol{A} } f)$ satisfies the universal property for $\setsem{
  \Gamma; \Phi \vdash (\Lan^{\ol \alpha}_{\ol{K}} F)\ol{A} } (\pi_i
f)$.} We therefore have that $\pi_i (\relsem{ \Gamma; \Phi \vdash
  (\Lan^{\ol \alpha}_{\ol{K}} F)\ol{A} } f) = \setsem{ \Gamma; \Phi
  \vdash (\Lan^{\ol \alpha}_{\ol{K}} F)\ol{A} } (\pi_i f)$ (up to
natural isomorphism) as desired.
\end{proof}
