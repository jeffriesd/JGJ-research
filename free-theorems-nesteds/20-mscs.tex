...
paper~\cite{jgj21}. Compared to that paper, this one includes all
proofs, as well as more examples. The discussion of GADTs in
Section~\ref{sec:GADTs} is also entirely new.
...
$\ol{\alpha}$, in the body $F$. {\color{red} Can bind variables of
  arity greater than 0 in Nat-types even just for nesteds? Improves
  expressibility? Worth it? Worth mentioning?}
...
\alpha_{\ol{S}}$, where $j$ is the cocone into $(\mathit{Lan}_K F)
\ol{A}$ and $j'$ is the cocone into $(\mathit{Lan}_K F') \ol{A}$. We
...
{\color{red} Relate $k$ to arities in $\Phi$.}
\begin{align*}
...
= \mu
\]
respectively, where, for each $\ol{N : RT_k}$ {\color{red} Relate
  $k$ to arities in $\Phi$.}, $\eta_{\ol{N}}$ is the natural
transformation associated with the left Kan extension
...
\end{tikzcd}\]
and, for all $\ol{N : RT_k}$ {\color{red} Relate $k$ to arities in
  $\Phi$.} and $d : \setsem{\Gamma;\emptyset \vdash \Delta}$, 
\[\begin{array}{l}
...
Let $F : \set^h \to \set$ and $\ol{K : \set^h \to \set^k}$ be functors.
...
\end{proof}
Thus, {\color{red} CHECK!} if $L$ were a restriction of
$\setsem{\Gamma; \Phi,\ol\alpha \vdash (\Lan^{\ol\alpha}_{\ol K}
  F){\ol K}}\rho$, if the functorial action of $L$ were a restriction
of that of $\setsem{\Gamma; \Phi,\ol\alpha \vdash
  (\Lan^{\ol\alpha}_{\ol K} F){\ol K}}\rho$, and if
$L(\ol{\setsem{\Gamma;\ol\alpha \vdash K}\rho \ol N \, \ol A})$ were
to contain $\setsem{\Gamma;\emptyset~|~\emptyset \vdash \int_{\ol K,F}
  : \Nat^{\Phi, \ol\alpha}\, F\, (\Lan^{\ol\alpha}_{\ol{K}}\, F)
  \ol{K}}\rho\,d\,\ol N\,\ol A\,y$ for all $\ol N$, $\ol A$, and $y$,
then $L$ would have to be the entirety of $\mathit{Lan}_{\ol{\lambda
    \ol A.\, \setsem{\Gamma; \ol\alpha \vdash K}\rho {[\ol{\alpha :=
          A} ]}}}\, \lambda \ol A.\, \setsem{\Gamma; \Phi, \ol\alpha
  \vdash F}\rho {[\Phi, \ol \alpha := \ol N, \ol A ]}$. An analogous
result holds for relational interpretations of $\Lan$-types.
...
when interpreted using left Kan extensions, our model construction
does not extend to give a parametric model for them. It may be
possible to modify the categories of relations and relation
transformers so that these are always surjective on their domans and
codomains; this would prohibit situations like that in
Example~\ref{ex:no-rel-transf}, and might therefore allow us to
recover our IEL and, ultimately, our Abstraction Theorem for GADTs.
If this turns out to be possible, then extending this new model
construction to locally $\lambda$-presentable categories for $\lambda
> \omega$ would make it possible to handle broader classes of
GADTs. (Interestingly, $\lambda > \aleph_1$ is required to interpret
even common GADTs.) We could even attempt to carry out our
...
appropriately fibred over $\cal C$. This would give a framework for
constructing parametric models for calculi with primitive GADTs that
is based on locally $\lambda$-presentable fibrations, for some
appropriate definition thereof.
...
represented in our calculus. Another important direction for future
work would therefore be to improve its expressivity by adding, say,
generalized folds~\cite{bp99}, Mendler iterators~\cite{amu05}, or
otherwise extend standard folds to express computations whose results
are not natural transformations. In particular, we may wish to add
term-level fixpoints as, e.g., in~\cite{pit00}. This would require the
categories interpreting types to be not just locally
$\lambda$-presentable, but to support some kind of domain structure as
well. At the moment it seems that such an endeavor will have to wait
for a generalization of the results presented here to at least locally
$\omega_1$-presentable categories: $\omega$-CPO, the most natural
category of domains to replace $\set$, is not locally finitely
presentable.
