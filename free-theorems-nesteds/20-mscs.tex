...
$\alpha$ must both be non-functorial variables. {\color{red} It is in
  fact possible to allow ``extra'' $0$-ary functorial variables in the
  body of $\mu$-types (functorial variables of higher arity are the
  real problem), which would allow the first-order representations of
  ADTs to be functorial.  However, it simplifies the formation rule
  for $\mu$-types considerably to disallow even these $0$-ary
  functorial variables. Since we can always use an ADT's (semantically
  equivalent) second-order representation when functoriality is
  needed, this restriction allows us to simplify the type syntax
  without sacrificing the expressivity of the calculus.}
%The representation of $\mathit{List}\,\alpha$ as the ADT $\mu
%\beta. \onet + \alpha \times \beta$ cannot be functorial in
%$\alpha$. By contrast, its representation as the nested type $(\mu
%\phi. \lambda \beta.\,\onet + \beta \times \phi \beta)\,\alpha$ can be
%considered either functorial in $\alpha$ or not, according to whether
%$\alpha$ is in the functorial or non-functorial context in which this
%type is formed. If $\emptyset;\alpha \vdash \mathit{List} \,\alpha$,
%then $\vdash \Nat^\alpha \onet\, (\mathit{List}\,\alpha)$ is
%well-formed; if $\alpha;\emptyset \vdash \List \,\alpha$, then it is
%not.  Similarly, if $\mathit{Tree}\,\alpha\,\gamma = \mu \,\beta.\,
%\alpha + \beta \times \gamma \times \beta$, then $\gamma;\emptyset
%\vdash \Nat^\alpha (\mathit{List}\, \alpha) \, (\mathit{Tree} \,
%\alpha\, \gamma)$ is well-formed, but $\emptyset;\gamma \vdash
%\Nat^\alpha (\mathit{List}\, \alpha) \, (\mathit{Tree} \, \alpha\,
%\gamma)$ is not. The body $F$ of a type $(\mu \phi.\lambda
%\ol\alpha.F)\ol G$ can only be functorial in $\phi$ and the variables
%in $\ol\alpha$. As a result, both $\phi$ and the variables in
%$\ol\alpha$ must be non-functorial in $\mathit{GRose}\, \phi \, \alpha
%= \mu\beta . \alpha \times \phi \beta$, $\mathit{GRose}\,\phi\,\alpha$
%cannot be the (co)domain of a $\Nat$-type binding any variables, and
%no type of the form $(\mu \phi. \lambda
%\alpha. \mathit{GRose}\,\phi\,\alpha)\tau$ is well-formed in our
%system.
These restrictions ensure that the set and relational interpretations
of types in Section~\ref{sec:type-interp} are functors. In
Section~\ref{sec:conclusion} we discuss how more expressive types
...
$G$. 
%due to typing rule restrictions for the $\mu$ types, $\phi$ either
%does not appear in $G$, or must have arity $0$, in which case
%$\ol\alpha$ must be empty, if $\phi$ appears in $G$, and uses
%Equation~\ref{eq:iel-fix-point-intermediate2} for $G$ when $\phi$ has
%arity $0$. {\color{red} Note that if functorial variables of arity
%  greater than $0$ were allowed to appear in the bodies of
%  $\mu$-types, then the aboe clause of the IEL would fail.}
\end{itemize}
...
has, for every $\rho \in \setenv$, a set interpretation
$\setsem{\Gamma;\Phi~|~\Delta \vdash t : F}\rho$ as a natural
transformation from $\setsem{\Gamma; \Phi \vdash \Delta}\rho$ to
$\setsem{\Gamma; \Phi \vdash F}\rho$, and, for every $\rho \in
\relenv$, a relational interpretation $\relsem{\Gamma;\Phi~|~\Delta
  \vdash t : F}\rho$ as a natural transformation from
...
\lambda
d{\color{red}\,\ol{B}\,\ol{C}.\,(}\mathit{in}_{{T}^X_{H,\rho{\color{red}[\ol{\gamma
          := 
        C}]}}}{\color{red})_{\ol{B}}} \;\;\mbox{ where } X \mbox{ is } \set \mbox{ when } \\ 
...
\llbracket \Gamma;\emptyset \,|\, \emptyset \vdash
  \fold^F_H : \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F) & = &  
\lambda d\,\eta{\color{red}\,\ol{B}\,\ol{C}.\,
(}\mathit{fold}_{T^X_{H,\rho{\color{red}[\ol{\gamma := C}]}}} \, (\lambda
\ol{A}.\,\eta_{\ol{A}\,\ol{C}}){\color{red})_{\ol{B}}}\\ 
...
The return type for the semantic fold is
  $\dsem{\Gamma;\ol\beta,{\color{red}\ol\gamma} \vdash F}\rho{\color{red}[\ol{\gamma :=
    C}]}[\ol{\beta := B}]$.
...
and terms --- i.e., \[\dsem{\Gamma;\Phi\vdash (L_\alpha x.t)_\alpha
  \top : F} = \dsem{\Gamma;\Phi \vdash t : F}\] and
\[\dsem{\Gamma;\Phi\vdash (L_\alpha x.t)_\alpha x : F} =
\dsem{\Gamma;\Phi \vdash t : F}\] --- then follow (assuming both sides
of these equations are defined).
...
types. If $\Gamma; \ol{\alpha} \vdash F$ then define the {\em
  identity} $\id_F$ on $F$ by $\id_F = \Gamma;\emptyset~|~\emptyset
\vdash L_{\ol{\alpha}}x.x : \Nat^{\ol{\alpha}} F\,F$, and if $\Gamma;
\emptyset \,|\, \Delta \vdash t: \Nat^{\overline{\alpha}} F\,G$ and
$\Gamma; \emptyset \,|\, \Delta \vdash s: \Nat^{\overline{\alpha}}
G\,H$ are terms then define the {\em composition} $s \circ t$ of $t$
and $s$ by $s \circ t = \Gamma; \emptyset\,|\, \Delta \vdash
L_{\overline{\alpha}}
...
\Nat^{\overline{\beta}{\color{red}, \overline{\gamma}}}\, H[\phi :=
  (\mu \phi. \lambda
...
\[= \setsem{ \Gamma; \emptyset \,|\, x: \Nat^{\overline{\beta}{\color{red},
...
\Nat^{\overline{\beta}{\color{red}, \overline{\gamma}}}\, (\mu \phi. \lambda
...
\Nat^{\overline{\beta}{\color{red}, \overline{\gamma}}} H[\phi := (\mu
...
i.e.,
%{\color{red} by remembering the following facts,
%\begin{align*}
%\setsem{\Gamma;\Phi \vdash F[\ol{\alpha := K}]}\rho
%&= \setsem{\Gamma; \ol{\alpha} \vdash F}\rho[\ol{\alpha :=
%    \setsem{\Gamma;\Phi \vdash K}\rho}] \\
...
$\underline{\Gamma;\emptyset \,|\, \emptyset \vdash \tin_H :
  \Nat^{\ol{\beta}{\color{red},\ol{\gamma}}} \, H[\phi := (\mu \phi.\lambda
  ...
  is in $\setsem{\Gamma;\emptyset \vdash
    \Nat^{\ol{\beta}{\color{red},\ol{\gamma}}} \, H[\phi := (\mu \phi.\lambda
      {\overline \alpha}.H){\overline \beta}][\ol{\alpha := \beta}]
    \;(\mu \phi.\lambda {\overline \alpha}.H){\overline \beta}}\,
  \rho$, we first note that, for all $\ol{B}$ {\color{red} and $\ol{C}$},
  \begin{align*}
    &(\setsem{\Gamma;\emptyset \,|\, \emptyset \vdash \tin_H :
    \Nat^{\ol{\beta}{\color{red},\ol{\gamma}}} \, H[\phi := (\mu \phi.\lambda
      {\overline \alpha}.H){\overline \beta}][\ol{\alpha := \beta}]
    \;(\mu \phi.\lambda {\overline \alpha}.H){\overline \beta}}\,
  \rho\,d)_{\ol{B}{\color{red}\,\ol{C}}}\, \\
    =\,
    &(\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}})_{\ol{B}}
  \end{align*}
  maps
  \begin{align*}
    &\setsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} \vdash H[\phi := (\mu
      \phi.\lambda {\overline \alpha}.H){\overline \beta}][\ol{\alpha
        := \beta}]}\rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]} 
        \\ = \,
    &T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}\, (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma
        := C}]}}) \, \ol{B}
  \end{align*}
        to $\setsem{\Gamma;\ol{\beta},{\color{red}\ol{\gamma}}
    \vdash (\mu \phi.\lambda {\overline \alpha}.H){\overline \beta}}
  \rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]} = (\mu
  T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}) \, \ol{B}$. 
  Secondly, we observe
  that
  \begin{align*}
    &\setsem{\Gamma;\emptyset \,|\, \emptyset \vdash \tin_H :
    ...
    \;(\mu \phi.\lambda {\overline \alpha}.H){\overline
      \beta}}\,\rho\,d \\ 
      = \, &\lambda
  \ol{B}{\color{red}\,\ol{C}}.\,(\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
          C}]}}})_{\ol{B}}
  \end{align*}
  is natural in $\ol{B}$ {\color{red} and $\ol{C}$}, since
{\color{red}  naturality of $\mathit{in}$ with respect to its functor argument and}
  naturality of $\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}}$ ensures
  that the following diagram commutes for all $\ol{f : B \to B'}$
  {\color{red} and
  $\ol{g : C \to C'}$}:
...
T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}\, (\mu
T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\, \ol{B} \ar[d, "{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
          C'}]}}\, (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\, \ol{f}}"]
\ar[r,"{(\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})_{\ol{B}}}}" ] & 
  (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\, \ol{B}
\ar[d,"{\mu {T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}}\, {\ol{f}}}" ] 
\\
T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}\, (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\,
\ol{B'} \ar[r, "{(\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
            C'}]}}})_{\ol{B'}}}"] & (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\,
\ol{B'}
\end{tikzcd}\]
}
...
\[\setsem{\Gamma;\emptyset \,|\, \emptyset \vdash
  \tin_H : \Nat^{\ol{\beta}{\color{red},\ol{\gamma}}} \, H[\phi := (\mu \phi.\lambda
    {\overline \alpha}.H){\overline \beta}][\ol{\alpha := \beta}]
  \;(\mu \phi.\lambda {\overline \alpha}.H){\overline
...
\[\hspace*{-0.2in}
 ...
 & (\,(\setsem{\Gamma;\emptyset \,|\, \emptyset \vdash \tin_H :
      \Nat^{\ol{\beta}{\color{red},\ol{\gamma}}} \, H[\phi := (\mu \phi.\lambda
        {\overline \alpha}.H){\overline \beta}][\ol{\alpha := \beta}]
      \;(\mu \phi.\lambda {\overline \alpha}.H){\overline
        \beta}}\,\rho\,d)_{\ol{B}{\color{red},\ol{C}}},\,\\ & \hspace*{0.5in}(\setsem{\Gamma;\emptyset
      \,|\, \emptyset \vdash \tin_H : \Nat^{\ol{\beta}{\color{red},\ol{\gamma}}} \,
      H[\phi := (\mu \phi.\lambda {\overline \alpha}.H){\overline
          \beta}][\ol{\alpha := \beta}] \;(\mu \phi.\lambda {\overline
        \alpha}.H){\overline
        \beta}}\,\rho\,d)_{\ol{B'}{\color{red},\ol{C'}}}\,)\\ &= (\,
    (\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}})_{\ol{B}},
    (\mathit{in}_{T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}})_{\ol{B'}}\,)
\end{array}\]
has type
\[\begin{array}{ll}
& (\, T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}\, (\mu
T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}) \, 
\ol{B} \to (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}}) \, \ol{B}, \, \\
& \hspace*{0.5in}T^\set_{H,\rho{\color{red} [\ol{\gamma := C'}]}}\, (\mu
T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}) \, 
\ol{B'} \to (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}) \,\ol{B'} \, )\\
= &
 \relsem{\Gamma;\ol{\beta},{\color{red}\ol{\gamma}} \vdash H[\phi := (\mu
    \phi.\lambda {\overline \alpha}.H){\overline \beta}][\ol{\alpha :=
      \beta}]}\Eq_\rho[\ol{\beta := R}]{\color{red}[\ol{\gamma := S}]} \to\\
 & \hspace*{0.5in} \relsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} \vdash (\mu
  \phi.\lambda \ol{\alpha}.H)\ol{\beta}} \Eq_\rho[\ol{\beta:=
    R}]{\color{red}[\ol{\gamma :=S}]}
\end{array}\]
...
$\underline{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
  (\Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}} }\,(\mu \phi.\lambda \overline
  ...
  $\setsem{\Gamma;\emptyset \vdash \emptyset}$ to \[\setsem{\Gamma;
    \emptyset \vdash \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red},
      \ol{\gamma}}}\,H[\phi :=_{\ol{\beta}} F][\ol{\alpha :=
        \beta}]\,F)\; (\Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}} }\,(\mu
  ...
  \setsem{\Gamma; \emptyset \vdash \Nat^{\ol{\beta}{\color{red},
      \ol{\gamma}}}\,H[\phi :=_{\ol{\beta}} F][\ol{\alpha :=
        \beta}]\,F} \rho$,
\[ \setsem{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
...
$\setsem{\Gamma; \emptyset \vdash \Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}}
  }\,(\mu \phi.\lambda \overline \alpha.H)\overline \beta\,F}\,\rho$,
i.e., for any $\ol{B}$ {\color{red} and $\ol{C}$},
\[(\setsem{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
  (\Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\,F)}\,\rho\,d\,\eta)_{\ol{B}{\color{red}\,\ol{C}}}\] is a
morphism from $\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}} \vdash (\mu
  \phi.\lambda \overline \alpha.H)\overline \beta}\rho[\ol{\beta :=
    B}]{\color{red}[\ol{\gamma := C}]} \,=\,(\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}})_{\ol{B}}$ to $\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}} \vdash
  F}\rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]}$.  To see this, note
that $\eta$ is a natural transformation from
\[\begin{array}{ll}
 & \lambda \ol{B}{\color{red}\,\ol{C}}.\,\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}}
  \vdash H[\phi := F][\ol{\alpha := \beta}]}\rho[\ol{\beta :=
    B}]{\color{red}[\ol{\gamma := C}]}\\
= & \lambda \ol{B}{\color{red}\,\ol{C}}.\,T^\set_{H,\rho{\color{red}[\ol{\gamma:=
     C}]}}\,(\lambda \ol{A}. \, \setsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} 
  \vdash F}\rho[\ol{\beta := A}]{\color{red}[\ol{\gamma := C}]}) \, \ol{B}
\end{array}\]
to
\[\begin{array}{ll}
 & \lambda \ol{B}{\color{red}\,\ol{C}}.\,(\lambda
\ol{A}.\,\setsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} \vdash F}\rho[\ol{\beta
    := A}]{\color{red}[\ol{\gamma := C}]}) \ol{B}\\
= & \lambda
\ol{B}{\color{red}\,\ol{C}}.\,\setsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} \vdash
  F}\rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]}
\end{array}\]
and thus for each $\ol{B}$ {\color{red} and $\ol{C}$},
\[(\setsem{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
  (\Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}} }\,(\mu \phi.\lambda
  \overline \alpha.H)\overline
  \beta\,F)}\,\rho\,d\,\eta)_{\ol{B}{\color{red}\,\ol{C}}}\] is a
morphism from $\setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}}
  \vdash (\mu \phi.\lambda \overline \alpha.H)\overline
  \beta}\rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]}\,=\, (\mu
T^\set_{H,\rho{\color{red}[\ol{\gamma := C}]}})_{\ol{B}}$ to
$\setsem{\Gamma;\ol{\beta}{\color{red},\ol{\gamma}} \vdash
  F}\rho[\ol{\beta := B}]{\color{red}[\ol{\gamma := C}]}$.  To see
that this family of morphisms is natural in $\ol{B}$ {\color{red} and
  $\ol{C}$}, we observe that the following diagram commutes for all
$\ol{f : B \to B'}$ {\color{red} and $\ol{g : C \to C'}$}: {\tiny
...
%    g}][\ol{\beta := \id_B}]}"]\\
(\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\, \ol{B} 
\ar[r,"{(\mathit{fold}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
            C'}]}}}\,{\color{red}(\lambda \ol{A}.\,}\eta{\color{red}_{\ol{A}\,\ol{C'}})})_{\ol{B}}\,}" ] 
\ar[d, "{(\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}}) \, \ol{f}}"'] & 
 \setsem{\Gamma; \ol{\beta}{\color{red},\ol{\gamma}} \vdash F}\rho{\color{red}[\ol{\gamma :=
    C'}]}[\ol{\beta := B}]\ar[d, "{\setsem{\Gamma;
      \ol{\beta}{\color{red},\ol{\gamma}} \vdash F}\id_\rho{\color{red}[\ol{\gamma := 
    \id_{C'}}]}[\ol{\beta := f}]}"]\\
 (\mu T^\set_{H,\rho{\color{red}[\ol{\gamma := C'}]}})\, \ol{B'} 
\ar[r,"{(\mathit{fold}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
            C'}]}}}{\color{red}\,(\lambda \ol{A}.\,}\eta{\color{red}_{\ol{A}\,\ol{C'}})})_{\ol{B'}}\,}"
...
\end{tikzcd}\]}
{\color{red} Indeed,}
by naturality of $\mathit{fold}_{T^\set_{H,\rho[{\color{red}\ol{\gamma
          := C'}]}}}{\color{red}\,(\lambda
\ol{A}.\,}\eta{\color{red}_{\ol{A}\,\ol{C'}}})$.
{\color{red} ensures that the bottom diagram commutes. 
To see that the top one commutes we first observe that, given a
...
$\mathit{fold}_{H}(\Theta\,(\mu K) \circ \mathit{in}_{K})$, i.e., the
unique morphism making the following diagram commute:\label{page:dia1}
{\footnotesize
...
\end{equation}}
{\color{red} Next, note that the action of the functor
\[\lambda \ol{B}.{\color{red} \lambda \ol{C}.} \setsem{\Gamma; \ol{\beta}{\color{red},
  \ol{\gamma}} \vdash H[\phi := F][\ol{\alpha := \beta}]}\rho
        [\ol{\beta := B}] {\color{red}[\ol{\gamma := C}]}\]
on the morphisms $\ol{f : B \to B'}{\color{red}, \ol{g : C \to C'}}$ is given by
\[\begin{array}{ll}
 & \setsem{\Gamma; \ol{\beta{\color{red}, \ol{\gamma}}} \vdash H[\phi :=
      F][\ol{\alpha := \beta}]} \id_{\rho} [\ol{\beta := f}]
           {\color{red}[\ol{\gamma := g}]}\\
= & \setsem{\Gamma; \phi, \ol{\alpha}{\color{red}, \ol{\gamma}} \vdash H}
\id_{\rho}[\ol{\alpha := f}] {\color{red}[\ol{\gamma := g}]}[\phi := \lambda
  \ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \id_{\rho
    [\ol{\beta := A}]}{\color{red} [\ol{\gamma := g}]}] \\ 
=& \setsem{\Gamma; \phi, \ol{\alpha}{\color{red}, \ol{\gamma}} \vdash H} \id_{\rho
  {\color{red}[\ol{\gamma := C'}]}[\phi := \lambda \ol{A}. \setsem{\Gamma;
      \ol{\beta} {\color{red}, \ol{\gamma}} \vdash F} \rho [\ol{\beta := A}]
   {\color{red} [\ol{\gamma := C'}]}]} [\ol{\alpha := f}] \\
&\hspace{3em} \circ \setsem{\Gamma; \phi, \ol{\alpha}{\color{red}, \ol{\gamma}}
  \vdash H} \id_{\rho [\ol{\alpha := B}][\phi := \lambda
    \ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \rho
       [\ol{\beta := A}]{\color{red} [\ol{\gamma := C'}]}]} {\color{red}[\ol{\gamma := g}]} \\
&\hspace{3em} \circ \setsem{\Gamma; \phi, \ol{\alpha} {\color{red}, \ol{\gamma}}
  \vdash H} \id_{\rho [\ol{\alpha := B}] {\color{red}[\ol{\gamma := C}]}} [\phi :=
  \lambda \ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F}
  \id_{\rho [\ol{\beta := A}]}{\color{red} [\ol{\gamma := g}]}] \\
= & T^{\set}_{H,\rho {\color{red}[\ol{\gamma := C'}]}} (\lambda
\ol{A}. \setsem{\Gamma; \ol{\beta} {\color{red}, \ol{\gamma}} \vdash F} \rho
   [\ol{\beta := A}]{\color{red} [\ol{\gamma := C'}]}) \ol{f} \\
&\hspace{3em} \circ \big( T^{\set}_{H,\id_{\rho}{\color{red}[\ol{\gamma := g}]}}
   (\lambda \ol{A}. \setsem{\Gamma; \ol{\beta} {\color{red}, \ol{\gamma}} \vdash F}
   \rho [\ol{\beta := A}] {\color{red}[\ol{\gamma := C'}]}) \big)_{\ol{B}} \\
&\hspace{3em} \circ \big( T^{\set}_{H,\rho {\color{red}[\ol{\gamma := C}]}} (\lambda
   \ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma} }\vdash F} \id_{\rho
     [\ol{\beta := A}]} {\color{red}[\ol{\gamma := g}]}) \big)_{\ol{B}}
\end{array}\]
So if $\eta$ is a natural transformation such that $\eta_{\ol B {\color{red}, \ol
  C}}$ has type
\[
\setsem{\Gamma; \ol{\beta},
  {\color{red}\ol{\gamma}} \vdash H[\phi := F][\ol{\alpha := \beta}]}\rho
[\ol{\beta := B}] {\color{red}[\ol{\gamma := C}]}
\to
\setsem{\Gamma; \ol{\beta},
  {\color{red} \ol{\gamma}} \vdash F}\rho [\ol{\beta := B}]
   {\color{red}    [\ol{\gamma := C}]}\] 
then, by naturality,
\[\begin{array}{ll}
 & \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \id_{\rho}
       [\ol{\beta := f}] {\color{red}[\ol{\gamma := g}]} \circ \eta_{\ol{B}{\color{red},
         \ol{C}}} \\ 
= & \eta_{\ol{B'}{\color{red}, \ol{C'}}} \circ T^{\set}_{H,\rho{\color{red} [\ol{\gamma := C'}]}}
(\lambda \ol{A}. \setsem{\Gamma; \ol{\beta} {\color{red}, \ol{\gamma}} \vdash F}
\rho [\ol{\beta := A}]{\color{red} [\ol{\gamma := C'}]}) \ol{f} \\
& \circ \big( T^{\set}_{H,\id_{\rho}{\color{red}[\ol{\gamma := g}]}} (\lambda
\ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \rho
   [\ol{\beta := A}]{\color{red} [\ol{\gamma := C'}]}) \big)_{\ol{B}} \\
& \circ \big( T^{\set}_{H,\rho {\color{red}[\ol{\gamma := C}]}} (\lambda
   \ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \id_{\rho
     [\ol{\beta := A}]} {\color{red}[\ol{\gamma := g}]}) \big)_{\ol{B}}
\end{array}\]
...
\begin{split}
  & \lambda \ol{B}. \setsem{\Gamma; \ol{\beta} {\color{red}, \ol{\gamma}} \vdash F}
\id_{\rho [\ol{\beta := B}]} {\color{red}[\ol{\gamma := g}]} \circ \lambda
\ol{B}.\eta_{\ol{B}{\color{red}, \ol{C}}} \\  
=\;\;\; & \lambda \ol{B}.\eta_{\ol{B}{\color{red}, \ol{C'}}} \circ
T^{\set}_{H,\id_{\rho}{\color{red}[\ol{\gamma := g}]}} (\lambda
\ol{A}. \setsem{\Gamma; \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \rho
   [\ol{\beta := A}]{\color{red} [\ol{\gamma := C'}]}) \\
 & \hspace*{0.5in} \circ
   T^{\set}_{H,\rho {\color{red}[\ol{\gamma := C}]}} (\lambda \ol{A}. \setsem{\Gamma;
     \ol{\beta}{\color{red}, \ol{\gamma}} \vdash F} \id_{\rho [\ol{\beta := A}]}
 {\color{red}  [\ol{\gamma := g}]}) 
\end{split}
\end{equation}
...
\[\setsem{\Gamma; \emptyset~|~\emptyset
  \vdash \fold^F_H : \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red},
    \ol{\gamma}}}\,H[\phi :=_{\ol{\beta}} F][\ol{\alpha :=
      \beta}]\,F)\; (\Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}} }\,(\mu
...
{\color{red} and $\ol{S : \rel(C,C')}$}.  Since $\eta$ satisfies the
additional condition needed for it to be in $\setsem{\Gamma; \emptyset
  \vdash \Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,(H[\phi := F][\ol{\alpha :=
      \beta}])\,F} \rho$,
\[\begin{array}{ll}
 & (\,(\mathit{fold}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
        C}]}}}{\color{red}\,(\lambda \ol{A}.\,}\eta{\color{red}_{\ol{A}\,\ol{C}})})_{\ol{B}},\,
(\mathit{fold}_{T^\set_{H,\rho{\color{red}[\ol{\gamma :=
        C'}]}}}{\color{red}\,(\lambda \ol{A}.}\eta{\color{red}_{\ol{A}\,\ol{C'}})})_{\ol{B'}}\,) 
\end{array}\]
has type
\[\begin{array}{ll}
  & (\mu T_{H,\Eq_\rho{\color{red}[\ol{\gamma := S}]}}) \,\ol{R} \to
\relsem{\Gamma;{\color{red}\ol{\gamma},}\ol{\beta} \vdash F}\Eq_\rho{\color{red}[\ol{\gamma := 
    S}]}[\ol{\beta:= R}]\\ 
= & (\mu T_{H,\Eq_\rho{\color{red}[\ol{\gamma :=
      S}]}})\,\ol{\relsem{\Gamma;{\color{red}\ol{\gamma},}\ol{\beta} 
  \vdash \beta}\Eq_\rho{\color{red}[\ol{\gamma := S}]}[\ol{\beta := R}]} \\ 
  &\hspace{1.5in} \to
\relsem{\Gamma;{\color{red}\ol{\gamma},}\ol{\beta} \vdash F}\Eq_\rho{\color{red}[\ol{\gamma := 
    S}]}[\ol{\beta:= R}]\\ 
= & \relsem{\Gamma; {\color{red}\ol{\gamma},}\ol{\beta} \vdash (\mu \phi. \lambda
  \ol{\alpha}. H)\ol{\beta}}\Eq_\rho{\color{red}[\ol{\gamma := S}]}[\ol{\beta := R}] \\ 
  &\hspace{1.5in}\to
\relsem{\Gamma;{\color{red}\ol{\gamma},}\ol{\beta} \vdash F}\Eq_\rho{\color{red}[\ol{\gamma := 
    S}]}[\ol{\beta:= R}]
\end{array}\]
...
\[\setsem{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  \Nat^\emptyset\;(\Nat^{\ol{\beta}{\color{red}, \ol{\gamma}}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
  (\Nat^{{\ol{\beta}{\color{red},\ol{\gamma}}} }\,(\mu \phi.\lambda
  \overline \alpha.H)\overline \beta \;F)}\] in $\rho$ is trivial.
\end{itemize}
\qed
