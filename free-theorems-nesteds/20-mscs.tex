...
\hspace*{0.4in}\mathtt{sconst\;:\; A \rightarrow List\;A}\\
...
establishing some useful notation surounding them.
...
An alternative presentation of left Kan extensions characterizes them
in terms of an adjunction.

Using this adjunction we can represent the types of the data
constructors for $\mathtt{Seq}$ as:
}

\subsection{Extending the Calculus}

In light of the above, we incorporate GADTs into our calculus by first
adding to Definition~\ref{def:wftypes} the following type formation
rule for $\Lan$-types:
...
\noindent
Here, $\Lan$ binds the variables in $\ol\alpha$, and these variables
must always have arity $0$. In addition, the vectors $\ol K$ and $\ol
...
(\phi \gamma_1 \times \phi \gamma_2) \right) \beta \right. \\ &\qquad
\left. + \left(\Lan^{\gamma}_{\mathtt{Nat} \to \gamma} (\mathtt{Nat} \to \phi
\gamma) \right) \beta + \phi (\mathtt{Nat} \to \beta) \times
\mathtt{Nat} \right) \alpha
\end{align*}
\noindent
where $\mathtt{Nat} = \mu \alpha. \bone + \alpha$ and $\mathtt{Nat}
\to D$ abbreviates $(\Lan^\beta_{\mathtt{Nat}} \bone)D$.} This
representation corresponds exactly to the rewriting of $\mathit{Seq}$
in Section~\ref{sec:lke}. As explained in Section~IV.D of~\cite{jp19},
the more general construct $\Lan^{\ol{\alpha^0}}_{\ol K} F$ allowing
extensions along vectors of functors makes it possible to represent
GADTs with two or more type arguments that depend on one another. Such
GADTs cannot be represented using just unary $\Lan$-types, i.e,
$\Lan$-types of the form $(\Lan^{\ol{\alpha^0}}_K F)\ol A$.
{\color{red} $\Lan$ vs $\Lan$-type.}

To define the term-formation rules for terms of $\Lan$-types we also
need to generalize $\Nat$-types to bind type constructor variables not
just of arity $0$, but of arbitrary arity. {\color{red} This is
  because...WHY????}  Accordingly, we replace the rule for
$\Nat$-types in Definition~\ref{def:wftypes} with
...
To also extend our term calculus to accommodate GADTs we must first
give introduction and elimination rules appropriate to our generalized
$\Nat$-types. We therefore replace the tenth and eleventh rules in
Figure~\ref{fig:terms} with
...
\DisplayProof
\]
respectively. Finally, we add to the term calculus in
Figure~\ref{fig:terms} the following introduction and elimination
rules for terms of $\Lan$-types, respectively:
...
\noindent
Note that these rules do indeed make essential use of our new,
generalized $\Nat$-types.

{\color{red} Say how these rules mirror real LKEs just as for
  products, coproducts, etc.}

\subsection{Extending the Semantics}

It is necessary, of course, to suitably extend the semantic
interpretations of $\Nat$ types and terms.  The type constructor
variables bound by $\Nat$ will be interpreted as either functors or
relation transformers.  The set and relational interpretations for
$\Nat$ types are defined as follows.
\begin{align*}
...
We intend to interpret $\Lan$-types as actual left Kan extensions. But
in order to guarantee parametricity we might think we have to cut down
their set interpretations by restricting to a well-behaved collection
of the sets produced by such interpretations, just as we did both in
our original interpretations of $\Nat$-types and the above
interpretations of our generalized $\Nat$-types.

{\color{red} REORDER!!!!}


Along similar lines, we might try to restrict the semantic
interpretation of $\Lan$ types to a subset of the set produced by the
left Kan extension. {\color{red} WHY?????} We show in the following
proposition that this is not possible, if we want such interpretation
to include the interpretation of the terms given by the
