...
expressive types might be accommodated. {\color{red} Daniel: Check the
  above carefully?}
...
\end{defi}
\noindent
{\color{red} Daniel: Agree?} The idea is that the arguments to $\phi$
get substituted for the variables in $\ol\alpha$ in each $F$ replacing
an occurrence of $\phi$. It is not hard to see that $\Gamma;\Phi
\vdash H[\phi :=_{\ol{\alpha}} F]$. In addition, if \,$\Gamma, \phi^k;
\Phi \vdash H$,\, $\Gamma; \ol\psi,\ol{\alpha} \vdash F$, $|\ol\alpha|
= k$, and $\Phi \cap \ol\psi = \emptyset$, then $\Gamma,\ol\psi';\Phi
\vdash H[\phi :=_{\ol{\alpha}} F[\ol{\psi :== \psi'}]]$.
...
maps, e.g., the perfect tree $((1,2),(3,4))$ to $((4,3),(2,1))$
{\color{red} CHANGE TO $pnil/pnode$ NOTATION?}, and
can be defined as
...
\begin{split}
& \relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash J} \Eq_{\rho} [\phi
    := T^{n}_{H,\Eq_{\rho}} K_0] \overline{[\alpha := \Eq_A]} \\
=\;\; & \relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash J} \Eq_{\rho}
[\phi := \Eq_{(T^\set_{H,\rho})^n K_0}] \overline{[\alpha := \Eq_A]}
\end{split}
\end{equation}
{\color{red}(Notice that we don't know what's in the context
  $\Phi'$.)}  We prove Equation~\ref{eq:iel-fix-point-intermediate1}
by induction on $n$.  The case $n=0$ is trivial, because
$T^0_{H,\Eq_{\rho}} K_0 = K_0$ and $(T^\set_{H,\rho})^0 K_0 = K_0$;
the inductive step is proved by the following sequence of equalities:
\[
\begin{split}
...
\[
\begin{split}
& \relsem{\Gamma; {\color{red} \Phi',} \phi, \ol{\alpha} \vdash \phi
    \ol{G}} \Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0]
  \overline{[\alpha := \Eq_A]}
  \\
&= T^{n}_{H,\Eq_{\rho}} K_0\, \overline{\relsem{\Gamma; {\color{red} \Phi',}
...
&= T^{n}_{H,\Eq_{\rho}} K_0\, \overline{\Eq_{\setsem{\Gamma;
        {\color{red}\Phi',} \phi, \ol{\alpha} \,\vdash G} \rho [\phi :=
        (T^\set_{H,\rho})^{n} K_0] \overline{[\alpha :=
          A]}}} \\ 
...
&= (\Eq_{(T^\set_{H,\rho})^{n} K_0})^* \overline{\relsem{\Gamma;
      {\color{red}\Phi',} \phi, \ol{\alpha} \vdash G} \Eq_{\rho} [\phi :=
      \Eq_{(T^\set_{H,\rho})^{n} K_0}] \overline{[\alpha :=
        \Eq_A]}} \\ 
&= \relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash \phi \ol{G}}
...
\[
\begin{split}
& \relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash (\mu \psi.\, \lambda
    \ol{\beta}.\, G) \ol{K}} 
  \Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0]
  [\overline{\alpha := \Eq_A}] \\
&= (\mu {T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}})
  \,\ol{\relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash K}\Eq_{\rho}
...
&= (\mu {T_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}})
  \,\ol{\relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash K}\Eq_{\rho}
  [\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]} \\ 
&= \relsem{\Gamma; {\color{red}\Phi',} \phi, \ol{\alpha} \vdash (\mu \psi.\, \lambda \beta.\, G) \ol{K}}
...
&= T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha
      ...
      \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha :=
        \Eq_A}]}\,K_0) \\
&= \lambda \ol{R}. \relsem{\Gamma; {\color{red}\phi, \ol\alpha,} \psi,
...
: F$, then we write $\dsem{\vdash t : F}$ instead of $\dsem{\emptyset;
  \emptyset~|~\emptyset \vdash t : F}$.  Moreover, term interpretation
respects substitution for both functorial and non-functorial type
variables, as well as term substitution, i.e., if $\Gamma,\alpha;\Phi
\,|\, \Delta \vdash t : F$ and $\Gamma;\Phi,\alpha \,|\, \Delta \vdash
t' : F$ and $\Gamma;\Phi \vdash G$ then
...
\noindent
Moreover, if $\Gamma;\Phi \,|\, \Delta, x: G \vdash t : F$ and $\Gamma;\Phi
...
\noindent
Direct calculation reveals that term interpretations also satisfy
...
\dsem{\Gamma;\Phi \vdash t : F}$ --- follow (when both sides of these
...
Define, for $\Gamma; \ol{\alpha} \vdash F$, the {\em identity} $\id_F$
on $F$ to be
...
\,|\, \Delta \vdash s: \Nat^{\overline{\alpha}} G\,H$, the {\em
  composition} $s \circ t$ of $t$ and $s$ to be \[s \circ t = \Gamma;
...
\end{thm}
\noindent
%{\color{red} When $s = \mathsf{in}_H$, and
...
%\end{equation}
%\noindent
%stating that $\map$ and $\tin$ commute.}  More generally,
...
$\fold$, and their interpretations, e.g., that uniqueness and the
universal property of the interpretation of $\fold$, and that the
interpretation of $\tin$ is an isomorphism. 

{\color{red} INCLUDE THEOREMS AND PROOFS?}

{\color{red} WAS HERE, BUT CONSEQ OF NAT OR MORE GENERAL?} Further,
our calculus admits no terms with the type $\Nat^\alpha \onet
\,\alpha$ of the polymorphic bottom, and every closed term $g$ of type
...
to prove free theorems, such as that for the type of the standard
$\mathit{filter}$ function for lists, that go beyond mere naturality,
and extend them to those nested types for which analogous functions
can be defined. In particular, we can extend short cut short cut
...
{\color{red} CHANGE? COMPARE AGAINST INTRO.}
