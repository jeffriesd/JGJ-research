...
To prove Equation~\ref{eq:part-one} we first observe that
...
\]
gives that, for all $\ol{R : \rel_0}$ and $\ol{m : \relsem{\Gamma;
    \ol\alpha \vdash K}\rho[\ol{\alpha := R}] \to \relsem{\Gamma; \Phi
    \vdash A}\rho'}$,
\[\begin{array}{lll}
& & \big(\textit{Lan}_{\ol{\lambda \ol{R}.\,\relsem{\Gamma; \ol\alpha
      \vdash K}\rho[\ol{\alpha := R}]}}\, \lambda
...
\end{array}\]
Equation~\ref{eq:cocone-proj} {\color{red} and its analogue for
  $\iota'$ and $j'$ then give --- HOW?????}
\[\begin{array}{lll}
 & & \pi_i \Big( \big(\textit{Lan}_{\ol{\lambda
  ...
  \vdash F}f[\ol{\alpha := \id_R}]\big)\, \ol{\relsem{\Gamma; \Phi
    \vdash A}\rho'} \Big) \,\circ\, {\color{red} j?}f_{\ol{\pi_i R},
  \ol{\pi_i m}} \\[2ex]
...
& = & \pi_i \iota'_{\ol{R}, \ol{\relsem{\Gamma; \Phi \vdash A}f \,\circ\,
    m}} 
\end{array}\]
{\color{red} Equation~\ref{eq:cocone-proj} and its analogue for
