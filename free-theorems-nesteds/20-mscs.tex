...
Generalized algebraic datatypes (GADTs) generalize nested types by...
...
A natural next step in this line of work is to extend our
parametricity results to GADTs.  It was shown in~\cite{jg08} that (the
data of) GADTs can be represented using object-level left Kan
extensions (over discrete categories), and further shown
in~\cite{jp19} that adding a carefully designed left Kan extension
construct to a calculus supporting primitive nested types preserves
the cocontinuity needed for primitive GADTs to have well-defined,
properly functorial interpretations.

This suggests adding GADTs to our calculus by extending
Definition~\ref{def:wftypes} with such object-level constructs, and
extending Figure~\ref{fig:terms} with categorically inspired
constructs to introduce and eliminate terms of these types, along the
same entirely standard lines as we did there for products, coproducts,
and fixpoints.  In this section we outline how our model from
Section~\ref{sec:term-interp} can be extended to a parametric model
when some classes of GADTs are incorporated as primitives in this
way. However, as we argue in~???? below, we suspect this is not
possible in general.
...
To show when this can be done, as well as to give a sense of what goes
wrong in the general case, we begin by recalling that GADTs are
naturally interpreted as initial algebras of left Kan extensions, as
suggested in discrete~\cite{jg08} and



Functorial semantics in~\cite{jp19}.

introducing a new
type, $\Lan$, to represent left Kan extensions.  Indeed, 

To begin with, we add the following type formation rule for $\Lan$
...
\DisplayProof
\]
Notice that the length of $\ol K$ coincides with that of $\ol A$.
Introducing the new type also requires the following additional clause in the inductive definition of higher-order substitution.
\[
((\Lan^{\ol{\alpha}}_{\ol K}\,F) \ol A)[\phi :=_{\ol{\beta}} G]
  = (\Lan^{\ol\alpha}_{\ol K}\,F[\phi :=_{\ol{\beta}} G])\, \ol{A[\phi :=_{\ol{\beta}} G]}
\]

Before we extend the term calculus with the term introduction and elimination rules for $\Lan$ types,
it is necessary to generalize $\Nat$ types to bind type constructor variables of arbitrary arity.
So, the formation rule for $\Nat$ types will be modified as
\[
\AXC{$\Gamma;\Phi \vdash F$}
\AXC{$\Gamma;\Phi  \vdash G$}
...
\DisplayProof
\]
and the term introduction and elimination rules will be
\[
\AXC{$\Gamma; \Phi \vdash F$}
\AXC{$\Gamma; \Phi \vdash G$}
...
\]
respectively.
It is necessary, of course, to suitably extend the semantic interpretations of $\Nat$ types and terms.
The type constructor variables bound by $\Nat$ will be interpreted as either functors
or relation transformers.
The set and relational interpretations for $\Nat$ types are defined as follows.
\begin{align*}
...
We can now give the term introduction and elimination rules for $\Lan$, which are
\[
...
\]
respectively.
Notice that, in these rules, we make essential use of the extended $\Nat$ types.

To give the semantics for $\Lan$ types we follow~\cite{jp19}
and use the fundamental idea that these have to be interpreted as left Kan extensions.
We give the definition of left Kan extension and some useful notation in the following definition.

\begin{defi}
Let $F : \set^k \to \set$ and $K : \set^k \to \set^h$.
Then, the left Kan extension of $F$ along $\ol K$ is given by a functor $\Lan_K\,F : \set^h \to \set$
and a natural transformation $\epsilon : F \to (\Lan_K\,F) \circ K$
which are initial among such pairs of functor and natural transformations, i.e.,
such that for every $L : \set^h \to \set$ and $\eta :  F \to L \circ K$
there exists a unique natural transformation $\mu : \Lan_K\,F \to L$
such that $(\mu K) \circ \epsilon = \eta$, as shown in the following diagram.
\[
...
&& \set \\
& \set^h
\ar[Rightarrow, bend right = 10, from=F, "{\epsilon}" description]
\ar[ur, bend left, "{\Lan_K\,F}"{name=Lan, description}]
\ar[ur, bend right, "{L}"'{name=L, right}, ""'{name=Lr, right}]
\ar[Rightarrow, bend left = 90, from=Fa, to=Lr, "{\eta}"]
\ar[Rightarrow, from=Lan, to=L, "{\mu}"]
\end{tikzcd}
\]
Of course, we can similarly define the left Kan extension for functors on $\rel$.
\end{defi}

One might try to restrict the semantic interpretation of $\Lan$ types to a subset of the set produced by the left Kan extension.
We show in the following proposition that this is not possible, if we want such interpretation to include the interpretation of the terms given by the term-introduction rule.
...
Thus, we are forced to interpret $\Lan$ types as whole left Kan extensions, without any cutting down.
The set interpretation of the $\Lan$ type then is
\[
...
\]
and the relational interpretation is
\[
  \relsem{\Gamma;\Phi \vdash (\Lan^{\ol{\alpha}}_{\ol{K}}\,F)\ol{A}}\rho
  = (\textit{Lan}_{\ol{\relsem{\Gamma; \ol\alpha \vdash
          K}\rho[\ol{\alpha := \_}]}}\, \relsem{\Gamma; \Phi, \ol\alpha
      \vdash F}\rho[\ol{\alpha := \_}])\, \ol{\relsem{\Gamma; \Phi
        \vdash A}\rho}
\]
We also need to define the functorial action of the interpretation on morphisms.
Let $f : \rho \to \rho'$ be a morphism of set environments.
Then
\[
...
\circ\;
  \big(\textit{Lan}_{\ol{\setsem{\Gamma; \ol\alpha \vdash K}\rho[\ol{\alpha := \_}]}}\,
    ...
    \ol{\setsem{\Gamma; \Phi \vdash A}\rho}
\end{multline*}
The functorial action for the relational interpretation is defined analogously.

The left Kan extension can be expressed via a colimit formula~\cite{rie16} as
\[
	(\Lan_K\,F) \ol{A} = \colim{\ol{S} : \set^k_0, \ol{f : K\ol{S} \to A}}{F \ol{S}}
\]
for any $\ol{A} : \set^h$, and where $\set_0$ is the category finite sets
(i.e., the full subcategory of $\omega$-presentable objects in $\set$).
For any $\ol{S} : \set^k_0$ and $\ol{f : K\ol{S} \to A}$, let
$j_{\ol{S}, \ol{f}} : F \ol{S} \to (\Lan_K\,F) \ol{A}$
be the morphism indexed by $\ol{S}$ and $\ol{f}$ of the cocone into the colimit.
Moreover, for any $h$-tuple of functions $\ol{g : A \to B}$,
the functorial action $(\Lan_K\,F) \ol{g}$ is the unique function $(\Lan_K\,F) \ol{A} \to (\Lan_K\,F) \ol{B}$
such that, for every $\ol{S} : \set^k_0$ and $\ol{f : K\ol{S} \to A}$,
we have that $((\Lan_K\,F) \ol{g}) \circ j_{\ol{S}, \ol{f}} = j_{\ol{S}, \ol{g \circ f}}$.
We can similarly express via a colimit formula the left Kan extension of functors on $\rel$,
and in that case we call the cocone over it $\iota$.

We define the semantics for the terms from the introduction and elimination rules for $\Lan$ types as, respectively,
\begin{multline*}
