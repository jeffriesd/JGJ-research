...
\TIC{$\Gamma;\Phi \vdash (\mu \phi^k.\lambda
  \ol{\alpha^0}. \,F {\color{red} [\ol{\gamma := H}]})\,\ol{G}$}  
\DisplayProof
\end{array}\]
...
{\color{red} Note that we use first-order substitution in the
  $\mu$-clause even though it isn't defined until the next
  page. Mutually inductive definition? The purpose of the substitution
  in the $\mu$-type clause of Def 2.1 is to ensure that variables in
  $\Phi$ are not applied to types involving variables from $\ol\alpha$
  in $F$; indeed, the first-order substitution $[\ol{\gamma := H}]$
  constrains the types in $\ol H$ to have arity $0$, and constrains
  the variables in $\Phi$ to already be fully applied to types not
  involving $\ol\alpha$ in $\ol H$, making such applications
  impossible. Of course, rather than intertwining the definitions of
  well-formed types of our calculus and the operation of first-order
  substitution on them we could instead prohibit such applications in
  a side condition to the rule for well-formed $\mu$-types, but we
  favor the precision of the current presentation.} We write $\vdash
F$ for $\emptyset;\emptyset \vdash F$.  Definition~\ref{def:wftypes}
ensures that the expected weakening rules for well-formed types hold,
although weakening does not change the contexts in which types can be
formed. If $\Gamma;\emptyset \vdash F$ and $\Gamma;\emptyset \vdash
G$, then our rules allow formation of the types $\Gamma;\emptyset
\vdash \Nat^\emptyset F \,G$ and $\Gamma; \emptyset \vdash
\Nat^{\ol\alpha} \,\onet \,F$, which represent the System F types
$\Gamma \vdash F \to G$ and $\forall$-type $\Gamma; \emptyset \vdash
\forall \ol\alpha . F$, respectively. However, some System F types,
such as $\forall \alpha. (\alpha \to \alpha) \to \alpha$, are not
representable in our calculus.

{\color{red} CHANGE TEXT FOR NEW $\mu$-RULE.}
Definition~\ref{def:wftypes} allows the formation of all of the
(closed {\color{red} NOT!}) nested types from the introduction:
\[\begin{array}{lll}
...
\end{array}\]
{\color{red} The representation of $\mathit{List}\,\alpha$ as the ADT
  $\mu \beta. \onet + \alpha \times \beta$ cannot be functorial in
  $\alpha$. By contrast, the representation of $\mathit{List}\,\alpha$
  as the nested type $(\mu \phi. \lambda \beta.\,\onet + \beta \times
  \phi \beta)\,\alpha$
%Each of these types
  can be considered either functorial in $\alpha$ or not, according to
  whether $\alpha$ is in the functorial or non-functorial context in
  which this type is formed.} In particular, if $\emptyset;\alpha
\vdash \mathit{List} \,\alpha$, then $\vdash \Nat^\alpha \onet\,
(\mathit{List}\,\alpha)$ is well-formed; if $\alpha;\emptyset \vdash
\List \,\alpha$, then it is not.  Similarly, if
...
\alpha) \, (\mathit{Tree} \, \alpha\, \gamma)$ is not. {\color{red}
  KEEP? Although types can be functorial in variables of arity greater
  than $0$, the body $F$ of a type $(\mu \phi.\lambda \ol\alpha.F)\ol
  G$ cannot.} For example, the type $\mathit{GRose}\, \phi \, \alpha =
\mu\beta . \alpha \times \phi \beta$ {\color{red} can be functorial
  --- NOT!} or not in $\alpha$, but cannot be functorial in
$\phi$. Moreover, $\mathit{GRose}\,\phi\,\alpha$ cannot be the
(co)domain of a $\Nat$-type representing a natural transformation
functorial in $\phi$, and the type $\mu \phi. \lambda
...
in no variables. Functorial variables
...
{\color{red} Check no change needed for new $\mu$-rule in next
  def. Just note that in application case the args to $\phi$ go in for
  the $\alpha$s in $F$s replacing $\phi$.}
  ...
  \begin{adjustbox}{varwidth=5.4in, max width=5in, fbox, center}
       ...
       \BIC{$\Gamma; \emptyset  \,|\, \emptyset \vdash \fold^F_H :
         \Nat^\emptyset\; (\Nat^{\ol{\beta}, {\color{red} \ol{\gamma}}}\,H[\phi
          ...
          {\color{red} \ol{\gamma}}}\,(\mu \phi.\lambda \ol{\alpha}.H)\ol{\beta}\,F)$}
       \hspace*{0.2in}\DisplayProof\vspace*{-0.1in}
       \end{array}\]

       \vspace*{0.05in}

       \caption{Well-formed terms {\color{red} Remove red $\gamma$s?
           Leave rule for map as
           is???}}\label{fig:terms} \vspace*{-0.00in}
\end{adjustbox}
%       \vspace*{-0.25in}
\end{figure*}
...
in the rules for $\map^{\ol{F},\ol{G}}_H$ {\color{red} , $\tin_H$, and
  $\fold^F_H$} (i.e., those variables not affected by the substitution
of $\phi$) deserve comment. They allow us to map {\color{red} or fold}
...
{\color{red} The next discussion should still be OK.}
...
variables occurring in the type of its final argument.  {\color{red}
  CHECK THE FOLLOWING! SHOULD BE OK.}  Even for ADTs there is a
difference between which folds over them we can type when they are
viewed as ADTs {\color{red} (i.e., as fixpoints of first-order
  functors) EARLIER?}  versus as proper nested types {\color{red}
  (i.e., as fixpoints of higher-order functors) EARLIER?}. This is
because, in the return type of $\mathsf{fold}$, the arguments of the
$\mu$-type must be variables bound by $\Nat$.  For ADTs, the
$\mu$-type takes no arguments, making it possible to write recursive
functions, such as a concatenation function for lists of type $\alpha
; \emptyset \vdash \Nat^\emptyset \ (\mu \beta. \onet + \alpha \times
\beta)\, (\Nat^\emptyset (\mu \beta. \onet + \alpha \times \beta) \,
(\mu \beta. \onet + \alpha \times \beta))$.  This is not possible for
...
$\setsem{\Gamma;{\color{red} \ol\gamma},\phi, \ol{\alpha} \vdash H}$, this means
...
where $k = |\ol \alpha|$. This means that for each $\setsem{\Gamma;
 {\color{red} \ol\gamma}, \phi^k, \ol{\alpha} \vdash H}$, the corresponding
...
\item If \,$\Gamma;\Phi\vdash F \times G$ then
  ...
  \setsem{\Gamma;\Phi \vdash F}f \times \setsem{\Gamma;\Phi \vdash
    G}f$
\item If \,$\Gamma;\Phi,{\color{red}\ol\gamma} \vdash (\mu \phi.\lambda
  \ol{\alpha}. H)\ol{G}$ then
  \[\begin{array}{lll}
  \setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash (\mu  \phi.\lambda
    \ol{\alpha}. H)\ol{G}} f &: &\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} 
    \vdash (\mu \phi.\lambda \ol{\alpha}. H)\ol{G}} \rho \to
  \setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash (\mu
    \phi.\lambda\ol{\alpha}. H)\ol{G}} \rho'\\
  &= &(\mu
  T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho} \to (\mu
  T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho'}
  \end{array}\]
  is
  defined by
  \[\begin{array}{ll} & (\mu T^\set_{H,f})\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash
      G}\rho'} \circ (\mu T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma}
      \vdash G}f}\\ = & (\mu T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma}
      \vdash G}f} \circ (\mu T^\set_{H,f})\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma}
...
\begin{CD}
 (\mu T^\set_{H,\rho}) \ol{\setsem{\Gamma;\Phi,{\color{red} \ol\gamma} \vdash G}\rho} @> (\mu
  T^\set_{H,f})_{\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho}} >> (\mu
  T^\set_{H,\rho'}) \ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho} \\ 
 @V(\mu T^\set_{H,\rho}) \ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}f}VV @V  (\mu
 T^\set_{H,\rho'}) \ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}f} VV \\ 
(\mu T^\set_{H,\rho}) \ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho'} @>(\mu
 T^\set_{H,f})_{\ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho'}}>> (\mu
 T^\set_{H,\rho'}) \ol{\setsem{\Gamma;\Phi,{\color{red}\ol\gamma} \vdash G}\rho'}
\end{CD}
...
\item {\color{red} Use vector notation?} By definition,
  $\relsem{\Gamma; \emptyset \vdash \Nat^{\overline\alpha} \,F\,G}
  \Eq_{\rho}$ is the relation on $\setsem{\Gamma; \emptyset \vdash
    \Nat^{\overline\alpha} \,F\,G} \rho$ relating $t$ and $t'$ if, for
  all ${R_1 : \rel(A_1,B_1)},...,{R_k : \rel(A_k,B_k)}$,
  ...
  $\rel$.  To prove that this relation is $\Eq_{\setsem{\Gamma;
      \emptyset \vdash \Nat^{\overline\alpha} \,F\,G} \rho}$ we need
  to show that $(t_{\overline{A}}, t'_{\overline{B}})$ is a morphism
  from $\relsem{\Gamma; \overline\alpha \vdash F}
  \Eq_{\rho}\overline{[\alpha := R]}$ to $\relsem{\Gamma ;
    \overline\alpha \vdash G} \Eq_{\rho}\overline{[\alpha := R]}$ in
...
&=(\mu {T_{H, \Eq_{\rho}}}) \,\ol{\relsem{\Gamma; \Phi, {\color{red} \ol\gamma}  \vdash F}\Eq_{\rho}}\\ 
&= \colim{n \in \nat}{T^n_{H, \Eq_{\rho}} K_0}\, \ol{\relsem{\Gamma; \Phi, {\color{red}\ol\gamma} 
  \vdash F}\Eq_{\rho}}\\
&= \colim{n \in \nat}{ T^n_{H, \Eq_{\rho}} K_0 \,\ol{\Eq_{\setsem{\Gamma;
    \Phi, {\color{red}\ol\gamma}  \vdash F}\rho}}}\\
&= \colim{n \in \nat}{(\Eq_{(T^\set_{H,\rho})^n K_0})^*
  \ol{\Eq_{\setsem{\Gamma; \Phi, {\color{red}\ol\gamma}  \vdash F}\rho}}}\\
&= \colim{n \in \nat}{\Eq_{(T^\set_{H,\rho})^n K_0 \,\ol{\setsem{\Gamma;
        \Phi, {\color{red}\ol\gamma}  \vdash F}\rho}}}\\ 
&= \Eq_{\colim{n \in \nat}{ (T^\set_{H,\rho})^n K_0\,
    \ol{\setsem{\Gamma; \Phi, {\color{red}\ol\gamma}  \vdash F}\rho}}}\\
&= \Eq_{\setsem{\Gamma; \Phi, {\color{red}\ol\gamma}  \vdash (\mu \phi.\lambda
      \ol{\alpha}. H)\ol{F}}\rho}
\end{split}
\]
...
=\;\; & \relsem{\Gamma; \Phi', \phi, \ol{\alpha} \vdash J} \Eq_{\rho}
[\phi := \Eq_{(T^\set_{H,\rho})^n K_0}] \overline{[\alpha := \Eq_A]}
\end{split}
\end{equation}
(Notice that we don't know what's in the context $\Phi'$.)  We prove
Equation~\ref{eq:iel-fix-point-intermediate1} by induction on $n$.
...
\overline{\Eq_A} \\ 
&= \relsem{\Gamma; \phi, \ol{\alpha}, {\color{red}\ol\gamma} \vdash H} \Eq_{\rho} [\phi
...
&= \relsem{\Gamma; \phi, \ol{\alpha}, {\color{red}\ol\gamma} \vdash H} \Eq_{\rho [\phi
    := (T^\set_{H,\rho})^n K_0] \overline{[\alpha :=
      A]}} \\ 
&= \Eq_{\setsem{\Gamma; \phi, \ol{\alpha}, {\color{red}\ol\gamma} \vdash H} \rho [\phi
    := (T^\set_{H,\rho})^n K_0] \overline{[\alpha :=
      A]}} \\ 
...
is proved by the sequence of equalities {\color{red} (where $\Phi' = \Phi'', \ol\gamma$)}:
\[
\begin{split}
& \relsem{\Gamma; {\color{red} \Phi'', \ol\gamma}, \phi, \ol{\alpha} \vdash (\mu \psi.\, \lambda \ol{\beta}.\, G) \ol{K}}
  \Eq_{\rho} [\phi := T^{n}_{H,\Eq_{\rho}} K_0]
  [\overline{\alpha := \Eq_A}] \\
&= (\mu {T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha := \Eq_A}]}})
  \,\ol{\relsem{\Gamma; {\color{red}\Phi'', \ol\gamma}, \phi, \ol{\alpha} \vdash K}\Eq_{\rho}
...
&= (\mu {T_{G, \Eq_{\rho}[\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]}})
  \,\ol{\relsem{\Gamma; {\color{red}\Phi'', \ol\gamma}, \phi, \ol{\alpha} \vdash K}\Eq_{\rho}
  [\phi := \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha := \Eq_A}]} \\ 
&= \relsem{\Gamma; {\color{red}\Phi'', \ol\gamma}, \phi, \ol{\alpha} \vdash (\mu \psi.\, \lambda \beta.\, G) \ol{K}}
...
&= T_{G, \Eq_{\rho}[\phi := T^{n}_{H,\Eq_{\rho}} K_0][\overline{\alpha
      ...
      \Eq_{(T^{\set}_{H,\rho})^n K_0}][\overline{\alpha :=
        \Eq_A}]}\,K_0) \\
&= \lambda \ol{R}. \relsem{\Gamma; {\color{red}\ol\gamma}, \phi, \ol\alpha, \psi,
...
\begin{comment}
\begin{defi}\label{def:rel-interp}
