...
\AXC{$\Gamma;\ol{\gamma^0},\ol{\alpha^0}  \vdash G$}
\BIC{$\Gamma;\ol{\gamma^0} \vdash \Nat^{\ol{\alpha^0}}F \,G$}
\DisplayProof
\\[3ex]
...
\gamma \times \beta$, then both $\gamma;\emptyset \vdash \Nat^\alpha
(\mathit{List}\, \alpha) \, (\mathit{Tree} \, \alpha\, \gamma)$ and
$\emptyset;\gamma \vdash \Nat^\alpha (\mathit{List}\, \alpha) \,
(\mathit{Tree} \, \alpha\, \gamma)$ are well-formed.  Although types
...
functorial in $\phi$.  As a result, $\mathit{GRose}\,\phi\,\alpha$
...
system. These restrictions on higher-arity functoriality are necessary
...
\AXC{$\Gamma; \ol\gamma, \ol{\alpha} \vdash G$}
\AXC{$\Gamma; \ol\gamma, \ol{\alpha} \,|\, \Delta, x : F \vdash t: G$} 
\TIC{$\Gamma; \ol\gamma
  %\emptyset
...
\AXC{$\ol{\Gamma;\Phi,\ol\gamma \vdash K}$}
\AXC{$\Gamma;\Phi,\ol\gamma \,|\, \Delta \vdash s: F[\overline{\alpha := K}]$}
\TIC{$\Gamma;\Phi,\ol\gamma \,|\, \Delta \vdash t_{\ol K} s:
  G[\overline{\alpha := K}]$}
\DisplayProof
\\\\
\AXC{$\Gamma; \ol{\phi}, \ol{\gamma} \vdash H$}
\AXC{$\ol{\Gamma; \ol{\beta},\ol{\gamma} \vdash F}$}
\AXC{$\ol{\Gamma; \ol\alpha, \ol{\beta},\ol{\gamma} \vdash
    G}$\hspace*{0.3in}$\ol{\Gamma;\ol\alpha~|~\Delta \vdash t :
...
\map_{\mathit{PTree}\,\alpha}^{\alpha \times \alpha, \alpha \times
  \alpha} t & : & \Nat^{\alpha} (\mathit{PTree}\,(\alpha \times
...
%\alpha))\, (\mathit{PTree}\,(\alpha \times \alpha)))
\end{array}\]
for $t : \Nat^{\alpha} (\alpha \times \alpha)\, (\alpha \times
\alpha)$, 
and 
...
Because our calclus is designed to ensure functoriality, and
functoriality does not interact with polymorphism as well as might be
hoped, our calculus cannot express nontrivial recursive functions, such
...
We can, however, represent types of functions that take
%    as arguments
%one proper nested type and as many other (possibly proper)
%as many proper nested types as we wish
multiple nested types as arguments, provided the types of those
arguments are parameterized over disjoint sets of type variables. The
return type of the function can be parameterized over any or all of
the type variables occurring in its argument types.  For example, we
can represent the type of a zip-like function for bushes.
...
\AXC{$\Gamma;\emptyset \vdash \mathit{List}\,G$ \hspace*{0.3in}
  $\Gamma;\emptyset~|~\Delta \vdash t
  : \Nat^\emptyset\,(\mathit{PTree}\, F)\,(\mathit{List}\,
  G)$}
\TIC{$\Gamma;\emptyset \,|\, \Delta \vdash \map^{\mathit{PTree}\,F,
    \,\mathit{List}\,G}_{\mathit{List}\,\alpha} \,t :
  \Nat^\emptyset\,(\mathit{List}\, (\mathit{PTree}\,
  F))\,(\mathit{List}\, (\mathit{List}\, G))$} \DisplayProof
\end{array}\]
But in the absence of $\ol \gamma$, if $\Gamma;
\emptyset~|~\Delta\vdash t : \Nat^\beta(\mathit{PTree}\,
\beta)\,(\mathit{List}\, \beta)$ then the instance
\[\Gamma;\emptyset~|~\Delta \vdash
\map^{\mathit{PTree}\,\beta,\mathit{List}\,\beta}_{\mathit{List}\,\alpha}\,t
: \Nat^\beta\,(\mathit{List}\, (\mathit{PTree}\,
...
%\beta))\] of $\mathtt{map}$
required to map the {\em polymorphic}
...
\AXC{$\Gamma;\beta \vdash \mathit{PTree}\,\beta \hspace*{0.3in}
  \Gamma;\beta \vdash \mathit{List}\,\beta$\hspace*{0.3in}
$\Gamma;\emptyset~|~\Delta \vdash t : \Nat^\beta(\mathit{PTree}\,
  \beta)\,(\mathit{List}\, \beta)$}
\BIC{$\Gamma;\emptyset~|~\Delta \vdash
  \map^{F,G}_{\mathit{List}\,\alpha} \, t :
 \Nat^\beta\,(\mathit{List}\,
  (\mathit{PTree}\, \beta))\,(\mathit{List}\, (\mathit{List}\,
  \beta))$}
\DisplayProof
...
%    if } v \in \tvars^0 \\ 
  \setsem{\Gamma; \ol\gamma
    %\emptyset
    \vdash \Nat^{\ol{\alpha}}
    \,F\,G}\rho &= \{\eta : \lambda \ol{A}. \,\setsem{\Gamma;
    \ol{\alpha} \vdash
    F}\rho[\ol{\alpha := A}] 
      \Rightarrow \lambda \ol{A}.\,\setsem{\Gamma; \ol\gamma,
      ...
      \rightarrow \relsem{\Gamma;\ol\gamma, \ol{\alpha} \vdash
...
\Rightarrow \setsem{\Gamma;\ol\gamma,\ol{\alpha} \vdash G}\rho\}$
(which contains $\setsem{\Gamma;\ol\gamma \vdash
  \Nat^{\ol{\alpha}}\,F\,G}\rho$) is a subset of
$\big\{({\setsem{\Gamma;\ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha
...
${\setsem{\Gamma;\ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha :=
      S}]}$. But there are only $\set$-many choices of morphisms
between any two objects since $\set$ is locally small.  That
$\setsem{\Gamma; \ol\gamma \vdash \Nat^\alpha F\,G}$ is
$\omega$-cocontinuous follows from $\omega$-cocontinuity of
$\setsem{\Gamma;\ol\alpha \vdash F}$ and
$\setsem{\Gamma;\ol\gamma,\ol\alpha \vdash G}$. Interpretations of
$\Nat$-types ensure that $\setsem{\Gamma \vdash F \to G}$ and
$\setsem{\Gamma \vdash \forall \alpha. F}$ are as expected in any
parametric model.
...
\hspace*{0.2in} $\lambda t : \setsem{\Gamma;\ol\gamma \vdash
    \Nat^{\ol{\alpha}}\,F\,G}\rho.\, 
  (\lambda \ol{A}. \setsem{\Gamma; \ol\gamma,\ol\alpha \vdash G}f[\ol{\alpha
      := id_A}]) \circ t$  
...
\begin{align*}
  ...
  \end{align*}
  \begin{align*}
  \relsem{\Gamma; \ol\gamma \vdash \Nat^{\ol{\alpha}} \,F\,G}\rho &= \{\eta
  : \lambda \ol{R}.\,\relsem{\Gamma; \ol{\alpha} \vdash
    F}\rho[\ol{\alpha := R}] \Rightarrow \lambda \ol{R}. \,\relsem{
    \Gamma; \ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha := R}]\}\\
  &=
  \{(t,t') \in \setsem{\Gamma; \ol\gamma
    \vdash \Nat^{\ol{\alpha}}
  ...
  (\relsem{\Gamma; \ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha :=
...
That $\relsem{\Gamma; \ol\gamma \vdash \Nat^\alpha F\,G}$ is
$\omega$-cocontinuous follows from $\omega$-cocontinuity of
$\relsem{\Gamma;\ol\alpha \vdash F}$ and
$\relsem{\Gamma;\ol\gamma,\ol\alpha \vdash G}$.  If $\rho \in \relenv$
...
\begin{thm}[IEL]\label{thm:iel}
  ...
  $\relsem{\Gamma; \Phi \vdash F} \Eq_\rho = \Eq_{\setsem{\Gamma;
      \Phi \vdash F}\rho}$.
\end{thm}
The proof is by induction on the structure of $F$. Only the
...
\begin{minipage}[t]{0.5\textwidth}
\[\begin{array}{lll}
\dsem{\Gamma;\emptyset \,|\, \Delta,x :F \vdash x : F} \rho& = &
\pi_{|\Delta|+1}\\
...
%\rangle\\
\dsem{\Gamma;\ol\gamma \,|\, \Delta \vdash L_{\overline \alpha} x.t : \Nat^{\overline
    \alpha} \,F \,G}\rho & = &  \curry (\dsem{\Gamma;\ol\gamma,\overline \alpha
  \,|\, \Delta, x : F \vdash t: G}\rho[\overline{\alpha := \_}])\\
\dsem{\Gamma;\Phi,\ol\gamma \,|\, \Delta \vdash t_{\overline K} s:
  G [\overline{\alpha := K}]}\rho & = & \eval \circ \langle
  \lambda d.\,(\dsem{\Gamma;\ol\gamma \,|\, \Delta \vdash t :
  \Nat^{\overline{\alpha}} \,F \,G}\rho\; d)_{\overline{\dsem{\Gamma;\Phi,\ol\gamma
      \vdash K}\rho}},\\ 
 & & \hspace*{0.5in} \dsem{\Gamma;\Phi,\ol\gamma \,|\,
...
\dsem{\Gamma;\ol\alpha \,|\, \Delta \vdash \map^{\ol{F},\ol{G}}_H
\,\ol t: \Nat^{\ol{\gamma}}\,H[\ol{\phi :=_{\ol{\beta}} F}]\,H[\ol{\phi
      :=_{\ol{\beta}} G}]} \rho
& = & \lambda d\, \ol{C}.\,
\dsem{\Gamma; \ol{\phi},\ol{\gamma}\vdash H}\\ 
...
Straightforward calculation shows that terms of $\Nat$ type behave as
...
If\, $\Gamma; \ol\beta \,|\, \Delta \vdash s : \Nat^{\overline{\alpha},
  \overline{\gamma}} F\,G$ and $\overline{\Gamma; \ol\beta \,|\,
  \Delta \vdash t : \Nat^{\overline{\gamma}} K\, H}$, then
  \[\begin{array}{l}
  \hspace*{0.135in}
\setsem{\Gamma; \ol\beta \,|\, \Delta
  \vdash L_{\overline{\gamma}} z. 
  (\map_G^{\overline{K}, \overline{H}} \,\overline{t})_{\ol\gamma}
  (s_{\overline{K}, \overline{\gamma}} z)
  : \Nat^{\overline{\gamma}} F[\overline{\alpha := K}]\,
  G[\overline{\alpha := H}]}\\
= \setsem{ \Gamma; \ol\beta \,|\, \Delta \vdash
  L_{\overline{\gamma}} z. s_{\overline{H}, \overline{\gamma}}
  ((\map_F^{\overline{K}, \overline{H}} \,
  \overline{t})_{\ol\gamma} z) : \Nat^{\overline{\gamma}} F[\overline{\alpha :=
      K}]\, G[\overline{\alpha := H}]}
\end{array}\]
\end{thm}
\noindent
When $s = \mathsf{in}_H$, and
$\overline{\Gamma; \ol\delta \,|\, t :
...
\ol{\gamma} \vdash H\), \(\ol{\Gamma; \ol{\beta}, \ol{\gamma} \vdash
  F}\), and \(\ol{\Gamma; \Phi, \ol{\beta}, \ol{\gamma} \vdash G}\),
...
In particular, if $\Gamma; \ol{\alpha} \vdash H$, $\ol{\Gamma;
  \emptyset \vdash F}$, and $\ol{\Gamma; \ol\beta \vdash G}$, then
Definition~\ref{def:set-interp} gives, for all $\ol{f :
  \setsem{\Gamma;\ol\beta \vdash \Nat^\emptyset F\,G}\rho}$, that
\[\begin{array}{rl}
&\setsem{ \Gamma;\ol\beta\,|\,\ol{x : \Nat^\emptyset F \, G}
...
\Delta \vdash t: \Nat^{\overline{\alpha}} F\,G$ and $\Gamma; \ol\beta
\,|\, \Delta \vdash s: \Nat^{\overline{\alpha}} G\,H$, the {\em
  composition} $s \circ t$ of $t$ and $s$ to be $\Gamma; \ol\beta
...
$\setsem{\Gamma; \ol\beta \,|\, \Delta \vdash s \circ t:
  \Nat^{\overline{\alpha}} F\,H} = \setsem{\Gamma; \ol\beta \,|\,
  \Delta \vdash s: \Nat^{\overline{\alpha}} G\,H} \circ
\setsem{\Gamma; \emptyset \,|\, \Delta \vdash t:
  \Nat^{\overline{\alpha}} F\,G}$.  We have the expected relationships
...
$\ol{\Gamma; \ol\delta, \ol{\beta}, \ol{\gamma} \vdash G}$, and
$\ol{\Gamma;\ol\delta\,|\,\Delta \vdash t : \Nat^{\ol{\alpha}, \ol{\beta},
    \ol{\gamma}} F\, G}$
then
\[\setsem{\Gamma; \ol\delta \,|\, \Delta \vdash
\map_{H[\ol{\psi := K}]}^{\ol{F}, \ol{G}}\,\ol t : \xi} = \setsem{\Gamma;
  \ol\delta \,|\, \Delta \vdash \map_H^{\ol{K[\ol{\phi := F}]},
    \ol{K[\ol{\phi := G}]}}\, (\ol{\map_K^{\ol{F}, \ol{G}}}\, \ol t) :
  \xi}\]
for $\xi = \Nat^{\ol{\gamma}} \,H[\ol{\psi :=
...
$\overline{\Gamma; \ol\delta, \overline{\alpha}, \overline{\gamma}
  \vdash G}$,\; 
...
\emptyset\,|\,\Delta \vdash s : \Nat^{\overline{\beta},
  \overline{\gamma}} H\, K$ and $\ol{\Gamma; \ol\delta\,|\,\Delta \vdash
  t : \Nat^{\overline{\alpha}, \overline{\gamma}} F\, G}$, then
\[\hspace*{0.13in}\setsem{\Gamma; \ol\delta \,|\, \Delta
    \vdash L_{\overline{\gamma}} z.\,
...
It is also standard to prove that the parametric model constructed
verifies the existence of initial algebras. This is, however,
unnecessary here since initial algebras are built directly into our
model.
...
obtaining our parametric model is the delicate threading of
functoriality and its accompanying naturality conditions throughout
the model construction. 

Note that all recursion in our calculus must be expressed using
...
computations that return natural transformations. Of course our
calculus inherits the constraints standard folds for nested types
impose, and thus is perhaps not as expressive as we would ideally
like. But since this expressivity issue is orthogonal to constructing
parametric models in the presence of nested types built from
primitives, and since it would detreact from the conceptual clarity of
...
Many constructions that enhance standard folds have been
proposed. Perhaps the most significant of these are generalized
folds~\cite{bp99},
%and Mendler iterators~\cite{amu05}.
which are used to express computations whose results are not natural
transformations. Generalized folds are shown in~\cite{jg10} to be
equivalent to standard folds in the presence of {\em right} Kan
extensions.  This suggests extending our term calculus %presented here
with an object-level right Kan extension construct; interpreting such
a construct in $\set$ and $\rel$ should be unproblematic {\color{blue}
  since these categories have all(?) limits}. It was also shown
in~\cite{jg08} that GADTs can be represented using an object-level
{\em left} Kan extension construct, and in~\cite{jp19} that augmenting
the types in our calculus with a carefully designed such construct
preserves the cocontinuity needed for GADTs to have well-defined
interpretations in $\set$ and $\rel$. These observations suggest
augmenting our type formation rules with such a construct.
%natural directions for future work.
Another possibility is to add
term-level fixpoints
%, as in~\cite{pit00},
to our calculus. However,
this would require the categories interpreting types to be not just
locally finitely presentable categories, but to support some kind of
domain structure as well. Unfortunately, $\omega$-CPO, the most
obvious category of domains to replace $\set$, is not locally finitely
presentable.

The constructions of this paper should all be possible in any locally
presentable cartesian closed category (lpccc) $\cal C$ whose category
of (abstract) relations, obtained by pullback as in~\cite{jac99}, is
also a lpccc and is appropriately fibred over $C$.  This would
...
supporting nested types built from primitives that is based on
``locally presentable fibrations'', for some appropriate definition
thereof.
