...
If $\rho$ is a set environment and $\vdash F$ then we may
write $\setsem{\vdash F}$ instead of $\setsem{\vdash F}\rho$
...
\setsem{\Gamma;\ol\gamma,\ol{\alpha} \vdash G}\rho\}$ (which contains
$\setsem{\Gamma;\ol\gamma \vdash \Nat^{\ol{\alpha}}\,F\,G}\rho$) is a
subset of $\big\{({\setsem{\Gamma;\ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha
...
${\setsem{\Gamma;\ol\gamma,\ol{\alpha} \vdash G}\rho[\ol{\alpha := S}]}$. But
there are only $\set$-many choices of morphisms between any
%these (or any)
two objects since $\set$ is locally small. Interpretations of
$\Nat$ types in Definition~\ref{def:set-sem} ensure $\setsem{\Gamma
  \vdash F \to G}$ and $\setsem{\Gamma \vdash \forall
  \alpha. F}$ are as expected in any parametric model.
...
ensure that $\relsem{\Gamma \vdash F \to G}$ and $\relsem{\Gamma
  \vdash \forall \alpha. F}$ are as expected in any parametric model.
%$\relsem{\Gamma \vdash
%  \sigma \to \tau}\rho = \relsem{\Gamma \vdash \sigma} \rho \to
%\relsem{\Gamma \vdash \tau} \rho$.
If $\rho$ is a relational environment and $\vdash F$, then we write
$\relsem{\vdash F}$ instead of $\relsem{\vdash F}\rho$ as for set
interpretations.  For the last clause in Definition~\ref{def:rel-sem}
to be well-defined we need $T_{H,\rho}$ to be an $\omega$-cocontinuous
endofunctor on $RT$ so that, by Lemma~\ref{lem:fp}, it admits a
fixpoint. Since $T_{H,\rho}$ is defined in terms of
$\relsem{\Gamma;\ol\gamma,\phi^k, \ol{\alpha} \vdash H}$, this means that
...
$\setsem{\Gamma; \ol{\alpha} \vdash F}$ is actually in
$[\rel^k,\rel]$ where $k = |\ol \alpha|$.
...
F)_{\ol{R}} = \relsem{\Gamma; \ol\gamma,\phi,\ol{\alpha} \vdash H}f[\phi :=
...
\[\begin{array}{l}
\hspace*{0.135in}\setsem{\Gamma; {\color{blue} \ol\alpha} \,|\, x :
  \Nat^{\overline{\alpha}, 
    \overline{\gamma}} F\,G, \overline{y : \Nat^{\overline{\gamma}}
    K\, H} \vdash (\map_G^{\overline{K},
    \overline{H}} \,\overline{y}) \circ
  (L_{\overline{\gamma}} z. x_{\overline{K}, \overline{\gamma}}
  z) : \Nat^{\overline{\gamma}} F[\overline{\alpha := K}]\,
  G[\overline{\alpha := H}]}\\
= \setsem{ \Gamma; {\color{blue} \ol\alpha} \,|\, x
  : \Nat^{\overline{\alpha}, \overline{\gamma}} F\, G, \overline{y :
    \Nat^{\overline{\gamma}} K\, H} \vdash
  (L_{\overline{\gamma}} z. x_{\overline{H}, \overline{\gamma}} z)
  \circ (\map_F^{\overline{K}, \overline{\tau}} \,
  \overline{y}) : \Nat^{\overline{\gamma}} F[\overline{\alpha :=
      K}]\, G[\overline{\alpha := H}]}
\end{array}\]
\end{thm}
When $x = \mathit{in}_H$ and $\xi = \Nat^{\overline{\gamma}} H[\phi :=
...
\begin{array}{l}
\hspace*{0.135in}\setsem{\Gamma; {\color{blue} \ol\alpha} \,|\, \overline{y :
    \Nat^{\overline{\gamma}} 
    F\, G} \vdash (\map_{(\mu \phi. \lambda
    \overline{\alpha}. H) \overline{\beta}}^{\overline{F},
    \overline{G}}\, \overline{y}) \circ
  (L_{\overline{\gamma}} z. (\tin_{H})_{\overline{F},
    \overline{\gamma}} z) : \xi}\\
= \setsem{\Gamma; {\color{blue} \ol\alpha} \,|\, \overline{y :
  \Nat^{\overline{\gamma}} F\, G} 
  \vdash (L_{\overline{\gamma}} z. (\tin_H)_{\overline{G}, \overline{\gamma}} z)
...
for bushes, e.g., is
\begin{align*}
  &\setsem{\emptyset; \alpha~|~ y : \Nat^\emptyset F \, G
    \vdash (\map_{\mathit{Bush}\,\alpha}^{{F},
      {G}}\, y) \circ (L_{{\emptyset}}
...
\begin{align*}
  &\setsem{\emptyset; \alpha,\gamma~|~y : \Nat^\emptyset F \,
    G \vdash (\map_{\mathit{Tree}\,\alpha \, \gamma}^{F,
      G}\, y) \circ (L_{{\gamma}}
    z. \mathit{reverseTree}_{F, \gamma}\, z) : \Nat^{{\gamma}}
    (\mathit{Tree} \, F \, \gamma) \, (\mathit{Tree} \, G \,
    \gamma) }\\ =\; &\setsem{\emptyset; \alpha,\gamma~|~y :
    \Nat^\emptyset F \, G \vdash (L_{\gamma}
    z. \mathit{reverseTree}_{G, \gamma} \,z) \circ
    (\map_{\mathit{Tree} \, \alpha\, \gamma}^{{F},
      {G}}\, y) : \Nat^{\gamma} (\mathit{Tree} \,
...
$\ol{\Gamma; \Phi, \ol{\beta}, \ol{\gamma} \vdash G}$, and
$\ol{\Gamma;\Phi\,|\,\Delta \vdash t : \Nat^{\ol{\alpha}, \ol{\beta},
    \ol{\gamma}} F\, G}$
then
\[\setsem{\Gamma; \Phi \,|\, \Delta \vdash
\map_{H[\ol{\psi := K}]}^{\ol{F}, \ol{G}}\,\ol t : \xi} = \setsem{\Gamma;
  \Phi \,|\, \Delta \vdash \map_H^{\ol{K[\ol{\phi := F}]},
...
\Phi\,|\,\Delta \vdash s : \Nat^{\overline{\beta},
  \overline{\gamma}} H\, K$ and $\ol{\Gamma; \Phi\,|\,\Delta \vdash
  t : \Nat^{\overline{\alpha}, \overline{\gamma}} F\, G}$, then
\[\hspace*{0.13in}\setsem{\Gamma; \Phi \,|\, \Delta
