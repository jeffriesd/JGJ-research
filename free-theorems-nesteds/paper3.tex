...
\AXC{$\Gamma;\Phi,\ol{\alpha^0}  \vdash G$}
...
\DisplayProof
\\[2ex]
\AXC{$\Gamma;\ol{\beta^0},\ol{\alpha^0},\phi^k \vdash F$}
\AXC{$\quad\quad\ol{\Gamma;\Phi,\ol{\beta^0} \vdash G}$}
\BIC{$\Gamma;\Phi,\ol{\beta^0} \vdash (\mu \phi^k.\lambda
  \ol{\alpha^0}. \,F)\,\ol{G}$} 
\DisplayProof
\end{array}\]
...
transformation in $\phi$, and the type $\mu \phi. \lambda
...
\item If \,$\Gamma;\Phi \vdash (\mu \phi.\lambda
  ...
  \[\begin{array}{ll} & (\mu T^\set_{H,f})\ol{\setsem{\Gamma;\Phi \vdash
      G}\rho'} \circ (\mu T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi
      \vdash G}f}\\ = & (\mu T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi
...
$\relsem{\Gamma;\Phi,\phi^k, \ol{\alpha} \vdash H}$, this means that
...
%\rangle\\
\dsem{\Gamma;\Phi \,|\, \Delta \vdash L_{\overline \alpha} x.t : \Nat^{\overline
    \alpha} \,F \,G}\rho & = &  \curry (\dsem{\Gamma;\Phi,\overline \alpha
...
\dsem{\Gamma;\Phi \,|\, \Delta \vdash t_{\overline K} s:
  ...
  \lambda d.\,(\dsem{\Gamma;\Phi \,|\, \Delta \vdash t :
...
\dsem{\Gamma;\Phi \,|\, \Delta \vdash \map^{\ol{F},\ol{G}}_H
...
\hspace*{0.79in}  & & \hspace*{0.1in}\id_{\rho[\ol{\gamma :=
      C}]}[\ol{\phi := \lambda
    \ol{B}. (\dsem{\Gamma;\Phi\,|\,\Delta \vdash t : 
...
\phi, \alpha \vdash \mathit{GRose}\,\phi\,\alpha$ cannot appear as
the (co)domain of a $\Nat$ type binding both $\phi$ and $\alpha$. As a
...
involved diagram chasing. That this is possible provides strong
