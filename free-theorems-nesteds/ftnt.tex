...
\item To see that if $d : \setsem{\Gamma;\emptyset \vdash \emptyset}
  ...
  \rho\,d\] is in $\setsem{\Gamma;\emptyset \vdash
    Nat^{\ol{\beta},\ol{\gamma}} \, H[\phi := (\mu \phi.\lambda
      {\overline \alpha}.H){\overline \beta}][\ol{\alpha := \beta}]
    \;(\mu \phi.\lambda {\overline \alpha}.H){\overline \beta}}\,
  \rho$, we first show that, for all $\ol{B}$ and $\ol{C}$,
\[\setsem{\Gamma;\emptyset \,|\, \emptyset \vdash \tin_H :
    Nat^{\ol{\beta},\ol{\gamma}} \, H[\phi := (\mu \phi.\lambda
      {\overline \alpha}.H){\overline \beta}][\ol{\alpha := \beta}]
    \;(\mu \phi.\lambda {\overline \alpha}.H){\overline \beta}}\,
\rho\,d\,\ol{B}\,\ol{C}\] does indeed map
\[\begin{array}{ll}
...
%= & \setsem{\Gamma;\ol{\beta},\ol{\gamma},\ol{\alpha} \vdash H[\phi :=
%    (\mu \phi.\lambda {\overline \alpha}.H){\overline
%      \beta}]}\rho[\ol{\beta := B}][\ol{\gamma := C}][\ol{\alpha :=
%    \setsem{\Gamma;\ol{\beta},\ol{\gamma} \vdash \beta}\rho[\ol{\beta
%        := B}][\ol{\gamma := C}]}]\\
...
\end{array}\]
But $(\mathit{in}_{T_{\rho[\ol{\gamma := C}]}})_{\ol{B}}$ certainly
does this.
...
one commutes. We therefore have that $\lambda
\ol{B}\,\ol{C}.\,(\mathit{in}_{T_{\rho[\ol{\gamma := C}]}})_{\ol{B}}$
is natural in $\ol{B}$ and $\ol{C}$ as desired.
\item {\color{red} extra properties for nat transfs that are set
  interps?} 
...
\item {\color{red} extra properties for nat transfs that are rel
  interps?}   
\item Finally, to see that $\pi_i(\relsem{\Gamma;\emptyset \,|\,
...
\item To see that $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
  ...
  morphism from \[\setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash (\mu
    \phi.\lambda \overline \alpha.H)\overline \beta}\rho[\ol{\beta :=
      B}][\ol{\gamma := C}]\] to \[\setsem{\Gamma;
    \ol{\beta},\ol{\gamma} \vdash F}\rho[\ol{\beta := B}][\ol{\gamma
      := C}]\] To see this, let $x : \setsem{\Gamma;
    \ol{\beta},\ol{\gamma} \vdash (\mu \phi.\lambda \overline
    \alpha.H)\overline \beta}\rho[\ol{\beta := B}][\ol{\gamma := C}]
  \;=\; (\mu T^\set_{\rho[\ol{\gamma := C}]}) \ol{B}$.  Using
  Equations~\ref{eq:subs-var} and~\ref{eq:subs-const} for the first
  and second equalities below, together with weakening, the induction
  hypothesis ensures that $(\setsem{\Gamma;\emptyset \,|\, \Delta
    \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi := F][\ol{\alpha
        := \beta}]\;F}\rho\,d)$ is a natural transformation from
\[\begin{array}{ll}
...
$\ol{C}$ as desired.
\item {\color{red} extra properties for nat transfs that are set
  interps?} To see that, for every $\rho$ and $d$, the components of
  $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
  \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\;F}\,\rho\,d$ satisfy the additional
  condition necessary for it to be in $\setsem{\Gamma;\emptyset \vdash
    \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\,\rho$, let $\ol{R : \rel(A,B)}$ and
  \[\begin{array}{lll}
  (u,v) & \in & \relsem{\Gamma;\ol{\gamma},\overline{\beta} \vdash
  (\mu \phi.\lambda \overline \alpha.H)\overline
    \beta}\Eq_\rho[\overline{\alpha := R}]\\
  & = &
(\setsem{\Gamma;\ol{\gamma},\overline{\beta} \vdash (\mu \phi.\lambda
  \overline \alpha.H)\overline \beta}\rho[\overline{\alpha := A}],\;
\setsem{\Gamma;\ol{\gamma},\overline{\beta} \vdash (\mu \phi.\lambda
  \overline \alpha.H)\overline \beta}\rho[\overline{\alpha := B}])
\end{array}\]
We show that if $\eta\, \rho\, d$ satisfies the additional condition
on its components necessary for it to be in $\setsem{\Gamma; \emptyset
  \vdash \Nat^{\ol{\beta}, \ol{\gamma}}\,(H[\phi := F][\ol{\alpha :=
      \beta}])\,F} \rho$ --- i.e., if for all $\overline{R :
  \rel(A,B)}$ and $\ol{S : \rel(C,D)}$ we have
\[\begin{array}{l}
((\eta \,\rho\,d)_{\ol{A}\,\ol{C}}\,, (\eta\,\rho\,d)_{\ol{B}\,\ol{D}}) \in
...
\relsem{\Gamma;\ol{\gamma},\ol{\beta} \vdash F}\Eq_\rho[\ol{\gamma
    := S}][\ol{\beta := R}]
\end{array}\]
--- then $\lambda d\,\ol{R}\,\ol{S}.\,
(\mathit{fold}_{T_{\rho[\ol{\gamma := S}]}} (\lambda \ol{V}.\, \eta
\,\rho\,d)_{\ol{V}\,\ol{S}}))_{\ol{R}}$ satisfies the addition
condition necessary for it to be in $\setsem{\Gamma;\emptyset\vdash
  \Nat^{\ol{\gamma},\ol{\beta}} (\mu \phi.\lambda
  \ol{\alpha}.H)\ol{\beta} \, F}\rho$. We calculate
...
{\color{blue}
  %We now show that if $\eta$ satisfies the additional
  %condition on its components necessary for it to be in
  %$\setsem{\Gamma;\emptyset \vdash \Nat^{\overline\alpha}\,(H[\phi :=
  %    F][\overline{\beta := \alpha}])\,F}\rho$ --- i.e., if for all
  %$\overline{R : \rel(A,B)}$ we have $(\eta_{\ol{A}}, \eta_{\ol{B}})
  %\in \relsem{\Gamma;\ol{\alpha} \vdash H[\phi := F][\overline{\beta
