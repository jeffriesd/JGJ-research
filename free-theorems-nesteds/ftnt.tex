...
{\color{blue} Add pro/demotion theorem here? Give examples of where it
is used?}
...
{\color{blue} Replace section with ftnt-graph-lemma}
...
\Eq_{\rho v}$ for all $v \in \tvars \cup \fvars$. This is exactly the same
...
\end{dfn}
{\color{blue} Add return type for fold in last clause? Should be
  $\relsem{\Gamma;\ol\beta,\ol\gamma \vdash F}\rho]\ol{\gamma := C}]$.}
...
In addition, if we define, for $\Gamma; \ol{\alpha} \vdash F$, the term
...
\end{itemize}
{\color{blue} This gives a family of results that we normally derive
  as free theorems but actually are consequences of naturality. Most
  of Wadler's fall into this family, for example, but not the free
  theorem for filter (even for lists) or short cut fusion.}
...
{\color{blue} Include result in ftnt-graph-lemma, short cut fusion for
  nesteds, and other results.}
