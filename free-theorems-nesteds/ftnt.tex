 ...
 & {\color{red} \mbox{need to interpret forall types if we include them}}\\
  \setsem{\Gamma;\emptyset \vdash \Nat^{\ol{\alpha}}
    ...
    F}\rho[\ol{\alpha := \text{--}}] 
      ...
      \rightarrow_{\rel} \relsem{\Gamma;\ol{\alpha} \vdash
...
\item If $\Gamma;\Phi \vdash \onet$ then $\setsem{\Gamma;\Phi \vdash
  \onet}f = \id_1$.
\item If $\Gamma;\Phi \vdash \phi \ol{A}$, then we have that
  $\setsem{\Gamma;\Phi \vdash \phi \ol{A}} f : \setsem{\Gamma;\Phi
  \vdash \phi \ol{A}}\rho \to \setsem{\Gamma;\Phi \vdash
  \phi\ol{A}}\rho' = (\rho\phi) \ol{\setsem{\Gamma;\Phi \vdash A}\rho}
  \to (\rho'\phi) \ol{\setsem{\Gamma;\Phi \vdash A}\rho'}$ is defined
  by $\setsem{\Gamma;\Phi \vdash \phi \ol{A}} f =
  (f\phi)_{\ol{\setsem{\Gamma;\Phi \vdash A}\rho'}}\, \circ\,
  (\rho\phi) {\ol{\setsem{\Gamma;\Phi \vdash A}f}} = (\rho'\phi)
  {\ol{\setsem{\Gamma;\Phi \vdash A}f}}\, \circ\, (f
  \phi)_{\ol{\setsem{\Gamma;\Phi \vdash A}\rho}}$.  This equality
...
\begin{CD}
  (\rho\phi) \ol{\setsem{\Gamma;\Phi \vdash A}\rho} @> (f\phi)_{
    \ol{\setsem{\Gamma;\Phi \vdash A}\rho}} >> (\rho'\phi)
  \ol{\setsem{\Gamma;\Phi \vdash A}\rho} \\ @V(\rho\phi)
  \ol{\setsem{\Gamma;\Phi \vdash A}f}VV @V (\rho'\phi)
  \ol{\setsem{\Gamma;\Phi \vdash A}f} VV \\ (\rho\phi)
  \ol{\setsem{\Gamma;\Phi \vdash A}\rho'} @>(f\phi)_{
    \ol{\setsem{\Gamma;\Phi \vdash A}\rho'}}>> (\rho'\phi)
...
\item If $E = E_1 \times E_2$ then $\setsem{\Gamma;\Phi \vdash E}f =
  \setsem{\Gamma;\Phi \vdash E_1}f \times \setsem{\Gamma;\Phi \vdash
    E_2}f$.
\item If $\Gamma;\Phi \vdash (\mu \phi.\lambda \ol{\alpha}. H)\ol{A}$ then
%writing
%\[\begin{array}{ccc}
...
\[ F \mapsto \lambda \ol{R}. \setsem{\Gamma;\Phi,\phi,\ol{\alpha} \vdash
...
\[\begin{array}{cl}
 & \setsem{\Gamma;\Phi \vdash (\mu \phi.\lambda
  \ol{\alpha}. H)\ol{A}} f\\
 : & \setsem{\Gamma;\Phi \vdash (\mu \phi.\lambda
  \ol{\alpha}. H)\ol{A}} \rho \,\to \,\setsem{\Gamma;\Phi \vdash (\mu
  \phi.\lambda\ol{\alpha}. H)\ol{A}} \rho'\\
 = & (\mu T^\set_\rho)\ol{\setsem{\Gamma;\Phi \vdash A}\rho}\, \to \, (\mu
T^\set_{\rho'})\ol{\setsem{\Gamma;\Phi \vdash A}\rho'}
\end{array}\]
\noindent
by
\[\begin{array}{ll}
& (\mu \sigma^\set_f)\ol{\setsem{\Gamma;\Phi \vdash
  A}\rho'} \circ (\mu T^\set_\rho)\ol{\setsem{\Gamma;\Phi \vdash A}f}\\
= & (\mu T^\set_{\rho'})\ol{\setsem{\Gamma;\Phi \vdash A}f} \circ (\mu
\sigma^\set_f)\ol{\setsem{\Gamma;\Phi \vdash A}\rho} 
\end{array}\]
...
\begin{CD}
 (\mu T^\set_\rho) \ol{\setsem{\Gamma;\Phi \vdash A}\rho} @> (\mu
  \sigma^\set_f)_{\ol{\setsem{\Gamma;\Phi \vdash A}\rho}} >> (\mu
  T^\set_{\rho'}) \ol{\setsem{\Gamma;\Phi \vdash A}\rho} \\ 
 @V(\mu T^\set_\rho) \ol{\setsem{\Gamma;\Phi \vdash A}f})VV @V  (\mu
 T^\set_{\rho'}) \ol{\setsem{\Gamma;\Phi \vdash A}f} VV \\ 
(\mu T^\set_\rho) \ol{\setsem{\Gamma;\Phi \vdash A}\rho'} @>(\mu
 \sigma^\set_f)_{\ol{\setsem{\Gamma;\Phi \vdash A}\rho'}}>> (\mu
 T^\set_{\rho'}) \ol{\setsem{\Gamma;\Phi \vdash A}\rho'}
\end{CD}
