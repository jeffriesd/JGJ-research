...
{\color{red} Is Soundness Theorem Lemma 29 or Lemma 30???}
...
\begin{itemize}
  ...
  \[
  \begin{split}
    &\hspace{-0.3in}\setsem{\Gamma;\Phi \,|\, \Delta \vdash (t_{\overline\tau}s)[\alpha := \sigma] : G[\overline{\beta := \tau}][\alpha := \sigma]}\rho \\
    &=\setsem{\Gamma;\Phi \,|\, \Delta \vdash t_{\overline{\tau[\alpha := \sigma]}}(s[\alpha := \sigma]) : G[\overline{\beta := \tau[\alpha := \sigma]}]}\rho \\
    &=\eval \circ \langle
      (\setsem{\Gamma;\emptyset \,|\, \Delta \vdash t: \Nat^{\overline{\beta}} \,F \,G}\rho\; \_)_{\overline{\setsem{\Gamma;\Phi\vdash \tau[\alpha := \sigma]}\rho}},
      \setsem{\Gamma;\Phi \,|\, \Delta \vdash s[\alpha := \sigma]: F [\overline{\beta := \tau[\alpha := \sigma]}]}\rho
    \rangle \\
    &=\eval \circ \langle
      (
