...
occurrences of the type variables in $\ol{\alpha}$ and the term
variable $x$ in the body $t$.  Moreover, in the first hypothesis of
the rule for $\tin_H\,t$, the variables in $\ol{\beta}$ are fresh with
respect to $H$ and $\ol{\tau}$. Substitution for terms is the obvious
extension of the usual capture-avoiding textual substitution.
...
in Definition~\ref{def:set-sem-funcs} below. If $\rho$ is a set
...
below.  If $\rho$ is a relational environment and $\vdash \tau : \F$,
then we write $\relsem{\vdash \tau }$ instead of
...
weakenings all have the same set and relational interpretations.
{\color{red} The same for actions on morphisms??? Does this even make
  sense?}
In particular, for any $\rho \in \setenv$,
...
interpretations of terms respect the judgment
\begin{itemize}
\item $\Gamma; \Phi~|~\Delta \vdash (L_{\ol{\alpha}}x.t)_{\ol{\tau}}s
  = t [\ol{\alpha := \tau}][x := s]$
\end{itemize}
{\color{blue} Add equation for fold?}
The set and relational interpretations of terms therefore respect the
congruence closed equational theory obtained by adding this judgment
