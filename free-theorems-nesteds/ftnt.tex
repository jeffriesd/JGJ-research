...
{\color{red}
  Let $\Gamma$ be a {\em (non-functorial) type context}, i.e., a finite set of type
variables in which the resulting type is not functorial, and let $\Phi$ be a {\em functorial type context}, i.e., a
finite set of type variables in which the resulting type is functorial. 
Note that variables of arity greater than 0, which are interpreted as functors,
may appear in the non-functorial type context.
}
...
\Nat^{\ol\beta} H \, K$ is an invalid typing judgment.
}
...
Similarly, if \,$\Gamma, \phi^k; \Phi \vdash F[\phi] : \F$ and $\Gamma;\Phi,
\ol{\alpha} \vdash H : \F$ with $\ol{\alpha} =
\{\alpha_1,...,\alpha_k\}$ and $k \geq 1$, then $\Gamma;\Phi \vdash
F[\phi :=_{\color{red} \ol{\alpha}} H] : \F$. 
The operation
...
$[\set^k,\set]$. A morphism $f : \rho \to \rho'$ from a set
environment $\rho$ to a set environment $\rho'$ with $\rho|_\V =
...
$k$-ary functor $\rho' \phi$ on $\set$. Composition of morphisms on
