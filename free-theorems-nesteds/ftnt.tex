...
% $\phi^k$ to indicate that $\phi \in \tvars^k$. When convenient we may
% write $\alpha, \beta$, etc., rather than $\alpha^0, \beta^0$, etc.,
% for elements of $\tvars^0$. The set of all type constructor variables
% is $\tvars = \bigcup_{k \ge 0} \tvars^k$.  We further assume an
% infinite set $\V$ of {\em type variables} disjoint from $\tvars$.  We
% write $\overline{\zeta}$ for either a set $\{\zeta_1,...,\zeta_n\}$ of
% type variables or a set of type constructor variables when the
% cardinality $n$ of the set is unimportant. If $\Pos$ is a set of type
% constructor variables then we write $\Pos, \overline{\phi}$ for $\Pos
% \cup \overline{\phi}$ when $\Pos \cap \overline{\phi} = \emptyset$.
% We omit the boldface for a singleton set, thus writing $\phi$, rather
% than $\overline{\phi}$, for $\{\phi\}$.
%
% \V becomes \tvars^0 \cup \fvars^0
%

For each $k \ge 0$, we assume countable sets {\color{purple} $\tvars^k$ of \emph{type
constructor variables of arity $k$}, $\fvars^k$ of \emph{functorial variables of arity $k$},
disjoint for distinct $k$ and disjoint from each other.}  
We use lower case Greek letters for both sets of variables, and write
$\phi^k$ to indicate that 
{\color{purple} $\phi \in \tvars^k \cup \fvars^k$.}
When convenient we may write $\alpha, \beta$, etc., rather than $\alpha^0, \beta^0$, etc.,
for elements of {\color{purple} $\tvars^0 \cup \fvars^0$.} 
{\color{purple} The sets of all type constructor variable and functorial variables are 
$\tvars = \bigcup_{k \ge 0} \tvars^k$ and $\fvars = \bigcup_{k \ge 0} \fvars^k$, respectively. 
We use the term \emph{type variable} to refer to variables in $\tvars \cup \fvars$.}
We write $\overline{\zeta}$ for either a set $\{\zeta_1,...,\zeta_n\}$ of
type constructor variables or a set of functorial variables when the
cardinality $n$ of the set is unimportant. If $\Pos$ is a set of type
variables then we write $\Pos, \overline{\phi}$ for $\Pos
\cup \overline{\phi}$ when $\Pos \cap \overline{\phi} = \emptyset$.
We omit the vector notation for a singleton set, thus writing $\phi$, rather
than $\overline{\phi}$, for $\{\phi\}$.

\begin{dfn}
{\color{purple} Let $V$ be a finite subset of\, $\tvars$, 
  let $\Pos$ be a finite subset of\, $\fvars$, 
let $\overline{\alpha}$ be a finite subset of\, $\fvars^0$ disjoint from $\Pos$, and let $\phi^k \in \fvars^k \setminus \Pos$.
 The sets $\T(V)$ of
  {\em {\color{red} type constructor expressions}} over $V$ and $\mcF^\Pos(V)$ of {\em {\color{red} 
  functorial expressions}} over $V$ are given by: }
  %% \[\color{red} \T(V) \; ::= \; V \mid \T(V) \to \T(V) \mid
  %%  {\color{red} \forall v.\, \T(V,v)} \mid
  %%  \color{red} \Nat^{\ol{\alpha}}(\mcF^{\overline{\alpha}}(V),\mcF^{\ol{\alpha}}(V))
  %%  \quad (remove \to and \forall)
  %%  \]
  \[{\color{purple}\T(V) \; ::= \; V \mid 
   \Nat^{\ol{\alpha}} \, \mcF^{\overline{\alpha}}(V) \; \mcF^{\ol{\alpha}}(V) \mid
   V \ol{\T(V)}
   }\]
\noindent
and 
...
\mid {\color{purple} V\, \ol{\mcF^{\Pos}(V)}  }
...
The above notation entails that an application $\tau\tau_1...\tau_k$
is allowed only when $\tau$ is a type variable of arity
$k$, or $\tau$ is a subexpression of the form $(\mu \phi^{k}. \lambda
\alpha_1...\alpha_k.\tau) \tau_1...\tau_k$. Moreover, if $\tau$ has
...
length matches the arity of the {\color{red} type expression} applied to it.
The fact that {\color{red} type expressions} are always in \emph{$\eta$-long
  normal form} avoids having to consider $\beta$-conversion at the
level of type constructors, and the fact that the standard type
formers are all defined pointwise avoids having to relate {\color{red} type 
expressions} at different kinds.
...
If {\color{purple} $\tau \in \mcF^\Pos(V) \cup \T(V)$,} {\color{purple} $\Pos$ and $V$} contain only 
type variables of arity $0$, and if $k=0$ for every occurrence of $\phi^k$
...
\begin{dfn}\label{def:wftypes}
  % changes - Gamma can now contain type constructor variables
  %         (in which the resulting expression is not functorial)
            

%% Let $\Gamma$ be a {\em type context}, i.e., a finite set of type
%% variables, and let $\Phi$ be a {\em type constructor context}, i.e., a
%% finite set of type constructor variables. 

{\color{purple}
  Let $\Gamma$ be a type constructor context, i.e., a finite set of type
constructor variables in which the resulting type is not intended to be functorial, and let $\Phi$ be a 
{\em functorial context}, i.e., a finite set of functorial variables in 
which the resulting type is intended to be functorial. Note that type variables of arity greater 
than 0 may appear in either context.
}

The formation rules for the
set {\color{purple} $\T \subseteq \bigcup_{V \subseteq \tvars}\T(V)$ of\, {\em well-formed
type constructor expressions}} are 
\[\begin{array}{cc}
...
{\color{purple}
\AXC{$\phi^k \in \Gamma$}
\AXC{$\quad\quad\ol{\Gamma;\emptyset \vdash \tau : \T}$}
\BIC{$\Gamma;\emptyset \vdash \phi^k \ol{\tau} : \T $}
\DisplayProof
}
\\
\\
...
\DisplayProof
\end{array}\]
  The formation rules for the set {\color{purple} $\F \subseteq \bigcup_{V \subseteq
 \tvars, \Pos \subseteq \fvars}\F^\Pos(V)$ of\, {\em well-formed functorial
  expressions}} are
\[\begin{array}{cccc}
...
for well-formed {\color{red} type expressions} % and type constructor expressions
hold.  If $\Gamma;\emptyset \vdash \sigma : \T$ and $\Gamma;\emptyset
\vdash \tau : \T$, then our rules allow formation of the type
expression $\Gamma;\emptyset \vdash \Nat^\emptyset\sigma \,\tau$. This
type can be thought of as representing the standard arrow type
$\Gamma \vdash \sigma \to \tau$ in our calculus. 

{\color{purple}
A similar derivation allows one to represent standard for all types
of the form $\Gamma \vdash \forall \ol\alpha . \tau$ by the type 
$\Gamma; \emptyset \vdash \Nat^{\ol\alpha} \,\onet \,\tau :\F$.
Note that whenever $\ol\alpha$ is non-empty, $\tau$ cannot be of the 
form $\Nat^{\ol\beta} H \, K$ because $\Gamma; \ol\alpha \vdash 
\Nat^{\ol\beta} H \, K$ is an invalid typing judgment (except by weakening).
}

Our formation rules
also allow {\color{red} type expressions} like $\List\gamma = (\mu
...
former, then we can derive $\vdash \Nat^\gamma \onet (\List\gamma) :
\T$. If the latter, then we cannot. Our formation rules also allow the
derivation of, e.g., $\delta; \emptyset \vdash \Nat^\gamma (\List
\gamma) \, (\Tree \gamma \delta)$, which represents a natural
transformation between lists and trees that is natural in $\gamma$ but
not in $\delta$.
{\color{purple}
{\color{red} Type expressions} may also be functorial in variables
of arity greater than $0$. For example, the type $\GRose \phi \, \alpha = 
\mu\beta . \alpha \times \phi \beta$ may be functorial in $\phi$ or not,
according to which context $\phi$ belongs. This choice of context for $\phi$ determines whether
one can form types such as $\Nat^\alpha (\GRose \phi \, \alpha) (\List \alpha)$. If $\GRose$ is functorial
in $\phi$, i.e. $\emptyset; \alpha, \phi \vdash \mu \beta . \alpha \times \phi \beta$, then
$\GRose$ cannot be used as a (co)domain in $\Nat$ types. This is due to the fact that
our calculus does not allow naturality in variables of arity greater than $0$.
}


Substitution for first-order {\color{red} type expressions} is the usual
...
for second-order {\color{red} type expressions} is given in the next
definition.

{\color{blue} Add subscript notation in next definition and propagate?
  Omit when clear from context?}


\begin{dfn}\label{def:second-order-subst}

%% If \,$\Gamma; \Phi, \phi^k \vdash F[\phi] : \F$ and $\Gamma;\Phi,
%% \ol{\alpha} \vdash H : \F$ with $\ol{\alpha} =
%% \{\alpha_1,...,\alpha_k\}$ and $k \geq 1$, then $\Gamma;\Phi \vdash
%% F[\phi :=_{\color{red} \ol{\alpha}} H] : \F$, where the operation
%% $(\cdot)[\phi := H]$ of {\em second-order type constructor
%%   substitution} is defined by:

%% % I would write \Gamma; \Phi \vdash F[\phi], and \phi \in \Gamma \cup \Phi
%% % but then it isn't easy to express the context for F[\phi := H] where
%% % the phi is removed. 
%% {\color{red}
%% If \,$\Gamma; \Phi, \phi^k \vdash F[\phi] : \F$ and $\Gamma;\Phi,
%% \ol{\alpha} \vdash H : \F$ with $\ol{\alpha} =
%% \{\alpha_1,...,\alpha_k\}$ and $k \geq 1$, then $\Gamma;\Phi \vdash
%% F[\phi :=_{\color{red} \ol{\alpha}} H] : \F$.
%% Similarly, if \,$\Gamma, \phi^k; \emptyset \vdash F[\phi] : \T$ and $\Gamma, 
%% \ol\alpha; \emptyset ,  \vdash H : \T$ with $\ol{\alpha} =
%% \{\alpha_1,...,\alpha_k\}$ and $k \geq 1$, then $\Gamma;\emptyset \vdash
%% F[\phi :=_{\color{red} \ol{\alpha}} H] : \T$. 
%% The operation
%% $(\cdot)[\phi := H]$ of {\em second-order type constructor
%%   substitution} is defined by:
%% }
...
{\color{red}
If \,$\Gamma; \Phi \vdash F[\phi] : \F$, 
$\phi^k \in \Gamma \cup \Phi$, $k \geq 1$, and
$\Gamma, \ol\beta;\Phi,  \ol{\alpha} \vdash H : \F$ 
with $|\ol\alpha| + |\ol\beta| = k$,
then $\Gamma \setminus \phi^k;\Phi \setminus \phi^k\vdash
  F[\phi :=_{\ol{\alpha}, \ol\beta} H] : \F$.
The operation
$(\cdot)[\phi := H]$ of {\em second-order type constructor
  substitution} is defined by:
}
...
\[\begin{array}{lll}
\color{red} 
  (\Nat^{\ol\beta} F \,G)[\phi := H] & = & \color{red} \Nat^{\ol\beta} F[\phi := H] \,G[\phi := H] \\
% \tau[\phi := H] & = & \tau \mbox{ if } \tau \in \T\\[0.5ex] 
\onet[\phi := H] & = & \onet\\[0.5ex]
\zerot[\phi := H] & = & \zerot\\[0.25ex]
(\psi \ol{\tau})[\phi := H] & = &
\left\{\begin{array}{ll}
\psi \,\ol{\tau[\phi := H]} & \mbox{if } \psi \not = \phi\\
  H[\ol{\alpha  := \tau[\phi := H]}] 
  {\color{red}  [\ol{\beta := \tau[\phi := H]}]} 
  & \mbox{if } \psi = \phi
\end{array}\right.\\
\\[-0.25ex]
(\sigma + \tau)[\phi := H] & = & \sigma[\phi := H]
+ \tau[\phi := H]\\[0.5ex] 
(\sigma \times \tau)[\phi := H] & = & \sigma[\phi := H] \times \tau[\phi
  := H]\\[0.5ex]  
((\mu \psi. \lambda \ol{\beta}.\, G)\ol{\tau}[\phi :=
  H] & = & (\mu \psi. \lambda \ol{\beta}. \,G[\phi :=
  H])\, \ol{\tau[\phi := H]}
\end{array}\]
\end{dfn}

% Note that, since an arity $0$ type constructor is first-order,
% substitution into it is just the usual textual replacement, i.e., is
% the usual notion of substitution, as expected.

\subsection{Terms}
We assume an infinite set $\cal V$ of term variables disjoint from
$\tvars$ and {\color{red} $\fvars$.}

\begin{dfn}
  Let $\Gamma$ be a {\color{red} type constructor context} and $\Phi$ be a {\color{red} functorial 
...
contexts as in {\color{red} type constructor contexts and functorial contexts.}
...
$x$ in $t$. In the rule for $t_{\ol\tau} s$ there is one {\color{red} functorial 
expression} $\tau$ for every {\color{red}functorial} variable $\alpha$. In the
rule for $\map^{\ol{F},\ol{G}}_H$ there is one {\color{red} functorial expression} $F$
and one {\color{red} functorial expression} $G$ for each {\color{red} functorial}
variable $\phi$. Moreover, for each $\phi$ the number of 
{\color{red} functorial} variables $\beta$ in the judgments for its corresponding
$F$ and $G$ is the same as its arity. In the rules for $\tin_H$ and
$\fold^F_H$, the {\color{red} functorial} variables in $\ol{\beta}$ are fresh
...
{\color{blue} The presence of the ``extra'' {\color{red} functorial} variables
  in $\ol{\gamma}$ in the rules for $\map^{\ol{F},\ol{G}}_H$, $\tin_H$, and
  $\fold^F_H$ allows us to, e.g., map or fold polymorphic functions
  over data types. For example, consider the function
  $\mathtt{flatten} : \Nat^\beta\,
  (\mathtt{PTree}\,\beta)\,(\mathtt{List}\,\beta)$ that maps perfect
  trees to lists. Even without the ``extra'' $\gamma$s the term
  representing the mapping of each {\em instance} of
  $\mathtt{flatten}$ acting on perfect trees and lists of types
  involving no {\color{red} functorial} variables is well-typed:
\[\begin{array}{l}
\AXC{$\Gamma;\alpha \vdash \mathtt{List} \, \alpha$}
\AXC{$\Gamma;\emptyset \vdash \sigma \hspace*{0.3in} \Gamma;\emptyset
  \vdash \tau$}
\AXC{$\Gamma;\emptyset \vdash \mathtt{PTree}\,\sigma \hspace*{0.3in}
  \Gamma;\emptyset \vdash \mathtt{List}\,\tau$}
\TIC{$\Gamma;\emptyset \,|\, \emptyset \vdash
  \map^{F,G}_{\mathtt{List}\,\alpha} :
  \Nat^\emptyset\;(\Nat^\emptyset\,(\mathtt{PTree}\,
  \sigma)\,(\mathtt{List}\, \tau))\; (\Nat^\emptyset\,(\mathtt{List}\,
  (\mathtt{PTree}\, \sigma))\,(\mathtt{List}\, (\mathtt{List}\,
  \tau)))$}
\DisplayProof
\end{array}\]
...
$\mathtt{flatten}$ is not. Indeed, the polymorphic function
$\mathtt{flatten}$, whose type contains the {\color{red} functorial} variable
$\beta$, say, cannot have a type of the form $\Nat^\emptyset\, F'\,
G'$ for any $F'$ and $G'$ as is required in the conclusion of the rule
for $\map^{F,G}_{\mathtt{List}\,\alpha}$. However, in a polymorphic
calculus of course we want to be able to manipulate terms representing
polymorphic functions in their entirety rather than just their
monomorphic instantiations. The rules with the ``extra'' $\gamma$s
recover this capability. For example, with these rules the term
representing the mapping of polymorphic $\mathtt{flatten}$ is typeable
as follows:
\[\begin{array}{l}
\AXC{$\Gamma;\alpha,\beta \vdash \mathtt{List} \, \alpha$}
\AXC{$\Gamma;\beta \vdash \mathtt{PTree}\,\beta \hspace*{0.3in}
  \Gamma;\beta \vdash \mathtt{List}\,\beta$}
\BIC{$\Gamma;\emptyset \,|\, \emptyset \vdash
  \map^{F,G}_{\mathtt{List}\,\alpha} :
  \Nat^\emptyset\;(\Nat^\beta\,(\mathtt{PTree}\,
  \beta)\,(\mathtt{List}\, \beta))\; (\Nat^\beta\,(\mathtt{List}\,
  (\mathtt{PTree}\, \beta))\,(\mathtt{List}\, (\mathtt{List}\,
  \beta)))$}
\DisplayProof
\end{array}\]
}
...
% Note that $\Delta[\alpha := \sigma] also needs to be defined
%\begin{dfn}
%Let $\Gamma, \alpha;\Phi \,|\, \Delta \vdash t : \tau$ and $\Gamma;\Phi
%\vdash \sigma : \F$.  We define $\Gamma;\Phi \,|\, \Delta [\alpha :=
%  \sigma] \vdash t[\alpha := \sigma] : \tau [\alpha := \sigma]$ by
%structural induction on $t$ as follows:
%\begin{align*}
%x[\alpha := \sigma] &= x \\
...
%(L_{\overline \beta} x.t)[\alpha := \sigma] &= \left\{
%\begin{array}{ll}
%L_{\overline \beta} x.(t[\alpha := \sigma]) & \mbox{ if } \alpha \not \in
%\ol{\beta}\\ 
%L_{\overline \beta} x.t & \mbox{ otherwise }
%\end{array} \right.\\
%(\fold_{H,F}\, t)[\alpha := \sigma] &= \fold_{H[\alpha :=
%    \sigma],F[\alpha := \sigma]}\, (t[\alpha := \sigma]) 
%\end{align*}
%All cases should be defined below. It's just that if $\alpha$ doesn't
%appear in the term, then there is no substitution to be done.
%Now let $\Gamma;\Phi,\alpha \,|\, \Delta \vdash t : \tau$ and $\Gamma;\Phi
%\vdash \sigma : \F$.  We define $\Gamma;\Phi \,|\, \Delta [\alpha :=
%  \sigma] \vdash t[\alpha := \sigma] : \tau [\alpha := \sigma]$ by
%structural induction on $t$ as follows:
%\begin{align*}
...
%Let $\Gamma;\Phi \,|\, \Delta, x: \sigma \vdash t : \tau$ and
%$\Gamma;\Phi \,|\, \Delta \vdash s: \sigma$.  We define $\Gamma;\Phi
%\,|\, \Delta \vdash t[x := s] : \tau$ by structural induction on $t$
%as
%\begin{align*}
%x[x := s] &= s \\
...
%%(st)[x := u] &= (s[x := u])(t[x := u]) \\
%(L_{\overline \alpha} y.t)[x := s] &= L_{\overline \alpha} y.(t[x := s]) \\
%(t_{\overline\tau}u)[x := s] &= (t[x := s])_{\overline{\tau}}(u[x := s]) \\
%(\bot_\tau t) [x := s] &= \bot_\tau (t[x := s]) \\
%\top [x := s] &= \top \\
%(t,u)[x := s] &= (t[x := s], u[x := s]) \\
%(\pi_1 t)[x := s] &= \pi_1 (t[x := s]) \\
%(\pi_2 t)[x := s] &= \pi_2 (t[x := s]) \\
%(\case{t}{y \mapsto l}{z \mapsto r})[x := s] &= \case{t[x := s]}{y \mapsto l[x := s]}{z \mapsto r[x := s]} \\   
%(\inl \,t)[x := s] &= \inl\,(t[x := s]) \\
%(\inr \,t)[x := s] &= \inr\,(t[x := s]) \\
%(\tin\,t)[x := s] &= \tin(t[x := s]) \\
...
\begin{dfn}
{\color{red} A {\em set environment} maps each type variable 
in $\tvars^k \cup \fvars^k$ to an element of $[\set^k,\set]$.}
A morphism $f : \rho \to \rho'$ from a set
environment $\rho$ to a set environment 
{\color{red} $\rho'$ with $\rho|_\tvars =
\rho'|_\tvars$ maps each functorial variable $\phi^k \in \fvars$ to a natural transformation from 
the $k$-ary functor $\rho \phi^k$ on $\set$ to the $k$-ary functor $\rho' \phi^k$ on $\set$
and maps each type constructor variable $\psi^k \in \tvars$ to the identity natural
transformation on $\rho \psi^k = \rho'\psi^k$. 
}

Composition of morphisms on
...
maps a {\color{red} type} variable of arity $0$ to an
$\omega$-cocontinuous functor from $\set^0$ to $\set$ --- i.e., to a
set. % --- just as it does a type variable. 
If $\ol{\alpha} =
...
environment such that $\Eq_\rho v = \Eq_{\rho v}$ for every {\color{red} type
variable } $v$; see
...
\begin{align*}
  \setsem{\Gamma;\emptyset \vdash v} \rho &= \rho v \mbox{
    if } {\color{red} v \in \tvars^0} \\ 
  \color{red} \setsem{\Gamma; \emptyset \vdash \phi\ol\tau} \rho &= (\rho \phi) \ol{\setsem{\Gamma; \emptyset \vdash \tau} \rho} \\
...
environment and $\vdash \tau : \F$ then we may write $\setsem{\vdash
  \tau}$ instead of $\setsem{\emptyset; \emptyset \vdash \tau}\rho$
since the environment is immaterial. Definition~\ref{def:set-sem}
ensures that
\[\setsem{\Gamma;\Phi \vdash F \ol{\tau}}\rho =
  ...
  the {\color{red} third} clause does indeed define a
...
In fact, for each $\setsem{\Gamma; \Phi, \phi^k, \ol{\alpha} \vdash
...
action of each functor interpreting a type on morphisms between
set environments.

\begin{dfn}\label{def:set-sem-funcs}
Let $f: \rho \to \rho'$ for set environments $\rho$ and $\rho'$ such
  that {\color{red} $\rho|_\tvars = \rho'|_\tvars$.} The action $\setsem{\Gamma;\Phi \vdash
...
{\color{red} \item If $\Gamma; \emptyset \vdash \phi\ol\tau$ then $\setsem{\Gamma; \emptyset \vdash \phi\ol\tau}f = \id_{(\rho\phi) \ol{\setsem{\Gamma; \emptyset \vdash \tau} \rho} }$. }
...
{\color{red} Note that the second bullet in this definition is a special case of this rule
in which $\rho \phi = \rho' \phi$ and $f \phi = \id_{\rho\phi}$. Due to these
restrictions the naturality condition is trivial. The intuition here is that
the functor given by $\setsem{\Gamma; \emptyset \vdash \tau} : \, [\setenv, \set]$ 
acts as a discrete functor for any \,$\tau$ that is well-formed in this context.
}
...
\item If $\Gamma;\Phi \vdash (\mu \phi.\lambda
  \ol{\alpha}. H)\ol{\tau}$
%  then letting $\sigma^\set_f : T^\set_\rho
%  \to T^\set_{\rho'}$ be the map
...
A {\em relation environment} maps each % type variable to a relation, and
  each {\color{red} type variable in $\tvars^k \cup \fvars^k$ } to an
$\omega$-cocontinuous $k$-ary relation transformer.  A morphism $f :
\rho \to \rho'$ from a relation environment $\rho$ to a relation
  environment $\rho'$ such that {\color{red}$\rho|_\tvars = \rho'|_\tvars$} 
{\color{red}
maps each functorial variable $\phi^k \in \fvars$ to a natural transformation
from the $k$-ary relation transformer $\rho \phi$ to the $k$-ary relation transformer
$\rho' \phi$ and maps each type constructor variable $\psi^k \in \tvars$ to the identity
natural transformation on $\rho \psi^k = \rho' \psi^k$.
}
Composition of morphisms on relation environments is
...
relation environment maps a {\color{red} type} variable of arity $0$ to
a $0$-ary relation transformer --- i.e., to a relation. % --- just as it does a type variable.  
We write $\rho[\ol{\alpha := R}]$ for the
relation environment $\rho'$ such that $\rho' \alpha_i \, = R_i$ for
$i = 1,...,k$ and $\rho' \alpha = \rho\alpha$ if $\alpha \not \in
\{\alpha_1,...,\alpha_k\}$. 
If $\rho$ is a relation environment, we
{\color{red} 
write $\pi_1 \rho$ for the set environment mapping each 
% type variable $\beta$ to $\pi_1 (\rho \beta)$ and each type constructor variable
type variable 
...
Considering $\relenv$ as a product $\Pi_{\phi^k \in {\color{red} \tvars \cup \fvars}}
...
\begin{align*}
  ...
  } v \in {\color{red} \tvars^0} \\ 
  \color{red} \relsem{\Gamma; \emptyset \vdash \phi\ol\tau} \rho &= (\rho \phi) \ol{\relsem{\Gamma; \emptyset \vdash \tau} \rho} \\
...
Let $f: \rho \to \rho'$ for relation environments $\rho$ and $\rho'$
  such that {\color{red} $\rho|_\tvars = \rho'|_\tvars$.} The action $\relsem{\Gamma;\Phi
...
\item 
  {\color{red} \item If $\Gamma; \emptyset \vdash \phi\ol\tau$ then $\relsem{\Gamma; \emptyset \vdash \phi\ol\tau}f = \id_{(\rho\phi) \ol{\relsem{\Gamma; \emptyset \vdash \tau} \rho} }$. }
%\item If $\Gamma;\emptyset \vdash \sigma \to \tau$ then
...
{\color{blue} The substitution $[\phi := H]$ should specify that the
  $\alpha$s in $H$ correspond to the arguments of $\phi$.  For
...
are again analogous. Finally, we prove Equation~\ref{eq:subs-var} for
  substitution for just a single {\color{red} type variable} since
...
\item If $\Gamma; \emptyset \vdash F : \T$, or if $F$ is $\onet$ or
  $\zerot$, then $F$ does not contain any {\color{red} functorial} variables
...
\Eq_{\rho v}$ for all $v \in {\color{red} \tvars \cup \fvars}$. This is exactly the same
...
It follows from Theorem~\ref{thm:iel} that $\relsem{\Gamma \vdash
  \sigma \to \tau} \Eq_{\rho} =
%\relsem{\Gamma; \emptyset
%  \vdash \sigma} \Eq_{\rho} \to \relsem{\Gamma; \emptyset \vdash \tau}
%\Eq_{\rho} = \Eq_{\setsem{\Gamma; \emptyset \vdash \sigma}\rho} \to
%\Eq_{\setsem{\Gamma; \emptyset \vdash \tau}\rho} =
\Eq_{\setsem{\Gamma \vdash \sigma \to \tau}\rho}$, as expected.
...
In addition, direct calculation reveals that the set interpretations
of terms satisfy
\begin{itemize}
...
\end{itemize}
Thus, if we define, for $\Gamma; \ol{\alpha} \vdash F$, the term
...
\end{itemize}
The previous semantic equality reveals that terms of $\Nat$ type
behave as natural transformations with respect to their source and
target functorial types. As the special case when $x = in_H$ we have
\begin{itemize}
...
relational environments. The set and relational interpretations of
terms therefore respect the congruence closed equational theory
obtained by adding these judgments to those generating the usual
congruence closed equational theory induced by the other term formers.
...
%% It follows immediately from Theorem~\ref{thm:at-gen} that 
