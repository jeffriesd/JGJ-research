...
%       [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
%     \end{equation}
%     and
%     \begin{equation}\label{eq:rel-subs-const}
%       \relsem{\Gamma; \Phi \vdash F[\phi := H]}\rho
%       = \relsem{\Gamma; \Phi, \phi \vdash F}\rho
%       [\phi := \relsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
%     \end{equation}
%   \end{itemize}
%   {\color{red} ISSUE: the substitution $[\phi := H]$ should specify that the
%   $\alpha$s in $H$ correspond to the arguments of $\phi$.
%   For example, we could write $[\phi :=_{\alpha} H]$.}
% \end{lemma}
\begin{lemma}\label{lemma:substitution}
  We have that, for any set environment $\rho$ and morphism of set environments $f : \rho \to \rho'$
  \begin{itemize}
  \item for $\Gamma;\Phi,\alpha \vdash F$
  and $\Gamma;\Phi \vdash \tau$,
    ...
    \intertext{and}\label{eq:subs-var-morph}
      \setsem{\Gamma;\Phi \vdash F[\alpha := \tau]}f =
      \setsem{\Gamma;\Phi,\alpha \vdash F}f[\alpha :=
  ...
  \item for $\Gamma;\Phi,\phi^k\vdash F$
    ...
    \intertext{and}\label{eq:subs-const-morph}
      ...
      [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}f[\overline{\alpha := \text{--}}]]
    \end{gather}
  \end{itemize}
  Analogous identities hold for the relational interpretation.
...
are completely analogous, so we shall just give the formers.
Likewise, we shall only prove equations~\ref{eq:subs-var} and~\ref{eq:subs-const},
as the proofs for equations~\ref{eq:subs-var-morph} and~\ref{eq:subs-const-morph}
are analogous.
% make the argument more abstract
% by using a generic interpretation $\sem{-}$.

Although equation~\ref{eq:subs-var} is a special case of equation~\ref{eq:subs-const},
it is convenient to prove it first,
and then use that to prove equation~\ref{eq:subs-const}.
...
\begin{itemize}
  ...
  = \setsem{\Gamma;\Phi,\alpha\vdash\alpha}\rho[\alpha := \setsem{\Gamma;\Phi\vdash\tau}\rho]
  \]
  as both are equal to $\setsem{\Gamma;\Phi\vdash\tau}\rho$.
  \item for $F = \phi \overline{\sigma}$ with $\phi \neq \alpha$, we have that
  \[
  \begin{split}
  \setsem{\Gamma; \Phi \vdash (\phi \overline{\sigma})[\alpha := \tau]}\rho
    ...
    &=(\rho\phi)\overline{
      \setsem{\Gamma;\Phi,\alpha\vdash\sigma}
  ...
  \item for $F = (\mu \psi. \lambda \overline{\beta}. K) \overline{\sigma}$
  we have that 
  \[
  \begin{split}
  &\hspace{-0.3in} \setsem{\Gamma; \Phi
    ...
    &=\mu(\setsem{\Gamma; \Phi, \psi, \overline{\beta} \vdash K[\alpha := \tau]}
      \rho[\psi := \text{--}][\overline{\beta := \text{--}}])
      (\overline{\setsem{\Gamma;\Phi \vdash \sigma [\alpha := \tau]}\rho}) \\
    &=\mu(
      \setsem{\Gamma; \Phi, \psi, \overline{\beta}, \alpha \vdash K}
      \rho
      [\alpha := \setsem{\Gamma;\Phi \vdash \tau}\rho]
      [\psi := \text{--}]
      [\overline{\beta := \text{--}}]
    ) \\
  ...
  &=\setsem{\Gamma;\Phi,\alpha\vdash(\mu\psi.\lambda\overline{\beta}.K)\overline{\sigma}}\rho
      [\alpha := \setsem{\Gamma;\Phi \vdash \tau}\rho]
  \end{split}
  \]
  where the third equality is by the induction hypothesis.
...
We prove equation~\ref{eq:subs-const}.
We proceed by induction on the structure of $F$.
\begin{itemize}
\item $F : \T$ does not contain any type constructor variable to replace,
    so there is nothing to prove.
...
\item for $F = F_1 \times F_2, F_1 + F_2$,
    the substitution distributes over the product/sum
    and it then suffices to apply the induction hypothesis.
\item for $F = \phi \overline{\tau}$, we have that
  \[
  \begin{split}
  &\hspace{-0.3in} \setsem{\Gamma; \Phi \vdash (\phi \overline{\tau})[\phi := H]}\rho \\
  &=\setsem{\Gamma; \Phi \vdash H[\overline{\alpha:= \tau[\phi := H]}]}\rho \\
  &=\setsem{\Gamma; \Phi \vdash H}\rho
    [\overline{\alpha:= \setsem{\Gamma;\Phi\vdash\tau[\phi := H]}\rho}] \\
  &=\setsem{\Gamma; \Phi \vdash H}\rho
    [\overline{
      \alpha:= \setsem{\Gamma;\Phi,\phi\vdash\tau}
      \rho
      [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
    }] \\
  &=\setsem{\Gamma; \Phi, \phi \vdash \phi \overline{\tau}}\rho
    [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
  \end{split}
  \]
  where the second equality is for equation~\ref{eq:subs-var},
  and the third equality is by the induction hypothesis.
\item for $F = \psi \overline{\tau}$ with $\psi \neq \phi$,
  we have a case similar to the previous one but simpler,
  as $\phi$ only needs to be substituted in the arguments
  $\tau$ of $\psi$.
\item for $F = (\mu \psi. \lambda \overline{\beta}. K) \overline{\tau}$
  we have that 
  \[
  \begin{split}
  &\hspace{-0.3in} \setsem{\Gamma; \Phi
  ...
  &=\mu(\setsem{\Gamma; \Phi, \psi, \overline{\beta} \vdash K[\phi := H]}
    \rho[\psi := \text{--}][\overline{\beta := \text{--}}])
    (\overline{\setsem{\Gamma;\Phi \vdash \tau[\phi := H]}\rho}) \\
  &=\mu(
      \setsem{\Gamma; \Phi, \psi, \overline{\beta}, \phi \vdash K}
      \rho
      [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
      [\psi := \text{--}]
      [\overline{\beta := \text{--}}]
    ...
    &\hspace{0.5in} (\overline{
      \setsem{\Gamma;\Phi,\phi \vdash \tau}
      \rho
      [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}
        \rho[\overline{\alpha := \text{--}}]]
    }) \\
  &=\setsem{\Gamma;\Phi,\phi\vdash(\mu\psi.\lambda\overline{\beta}.K)\overline{\tau}}\rho
      [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
  \end{split}
  \]
  where the third equality is by the induction hypothesis.
...
\item For the $\fold$ case, we shall check only the set interpretation,
  as the relational interpretation is analogous.
  $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
...
\item
  \[
  \begin{split}
    \pi_i(\relsem{\Gamma;\emptyset \,|\, \Delta,x :\tau \vdash x : \tau} \rho)
    &= \pi_i(
...
\item
  ...
  \end{split}
  \]
\item $\Gamma;\Phi \,|\, \Delta,x :\tau \vdash x : \tau$ is analogous to
...
\item
  ...
  \end{split}
  \]
\item $\Gamma;\Phi \,|\, \Delta \vdash \top : \onet$ is analogous to
...
\item
  ...
  \end{split}
  \]
\item $\Gamma;\Phi \,|\, \Delta \vdash \pi_2 t : \sigma$
...
\item
  ...
  \end{split}
  \]
\item $\Gamma;\Phi \,|\, \Delta \vdash \inr \,t: \sigma + \tau$
...
\item
  ...
  \begin{split}
    ...
    &= \setsem{\Gamma;\emptyset \,|\, \Delta \vdash
      \fold_{H,F}\, t : \Nat^{\overline \alpha}\,((\mu
      \phi.\lambda \overline \beta.H)\overline \alpha)\,F}(\pi_i\rho)
  \end{split}
  \]
\end{itemize}
\end{proof}
