...
T_\rho F \to T_{\rho'} F$ whose compnent at $\ol{R}$ is
    \[ (\sigma_f F)_{\ol{R}} = \relsem{\Gamma; \Phi,\phi,\ol{\alpha}
      \vdash H}f[\phi := \id_F][\ol{\alpha := R}]\] The next
...
\end{itemize}
Moreover, if we define, for $\Gamma; \ol{\alpha} \vdash F$, the term
...
\end{itemize}
The last semantic equality reveals that terms of $\Nat$ type behave as
natural transformations with respect to their source and target
functorial types. As the special case when $x = in_H$ we have
\begin{itemize}
...
$\overline{\Gamma; \phi, \overline{\psi}, \overline{\gamma} \vdash
  \tau}$,\; $\overline{I} = \overline{F}, H$,\; and $\overline{J} =
  \overline{G}, K$, then
...
\phi. \lambda \overline{\alpha}. H)\overline{\beta}$
\item $\hspace*{0.13in}\setsem{\Gamma; \emptyset \,|\, \emptyset \vdash (\fold_{H,
    H[\phi := (\mu \phi. \lambda
      \overline{\alpha}. H)\overline{\beta}]})_{\emptyset}
  ((\map_{H}^{H[\phi := (\mu \phi. \lambda
      \overline{\alpha}. H)\overline{\beta}][\overline{\alpha :=
...
%$\ol{\sigma}$ and $\ol{\tau}$.
Analogous results hold for the functorial actions of set
interpretations of terms, as well as for the actions on objects and
functorial actions of the relational interpretations of terms.  The
set and relational interpretations of terms therefore respect the
congruence closed equational theory obtained by adding these judgments
to those generating the usual congruence closed equational theory
induced by the other term formers.
