...
\BIC{$\Gamma;\Phi \,|\, \Delta \vdash \tin\,t : (\mu \phi^k.\lambda 
 ...
 & {\color{red} \mbox{need to interpret forall types if we include them}}\\
  \setsem{\Gamma;\emptyset \vdash \Nat^{\bm \alpha}
    \,F\,G}\rho &= \{\eta : \setsem{\Gamma; \bm\alpha \vdash F}\rho[\bm \alpha := \text{--}]
      ...
      &\hspace{0.2in}|~\forall \overline{R : \rel(A, B)}.\\
...
\[\setsem{\Gamma;\Phi \vdash F \tau_1...\tau_k}\rho =
  ...
  $\omega$-cocontinuity of $\setsem{\Gamma;\bm \alpha \vdash
    F}\rho$ ensure that $\{\eta : \setsem{\Gamma;\bm \alpha
    \vdash F}\rho \Rightarrow \setsem{\Gamma;\bm \alpha \vdash
    G}\rho\}$ (which contains $\setsem{\Gamma;\emptyset \vdash \Nat^{\bm \alpha}\,F\,G}\rho$) is equal to
...
\begin{align*}
 ...
 & {\color{red} \mbox{need to interpret forall types if we include them}}\\
  ...
  &= \pi_3(\mu T_{\rho})(\relsem{\Gamma;\Phi \vdash
     A_1}\rho)...(\relsem{\Gamma;\Phi \vdash A_k}\rho) \\
  \text{where }	T_{\rho}
    &= (T^\set_{\rho}, T^\set_{\rho}, T^\rel_{\rho}) \\
...
\item If $\Gamma;\Phi \vdash \phi^k A_1...A_k$, then we have that
  ...
  \relsem{\Gamma;\Phi \vdash \phi^k A_1...A_k}\rho' =
         ...
         \pi_3( \pi_1\rho\phi) (\bm {\relsem{\Gamma;\Phi \vdash A}f}) =
...
% \begin{definition}
% If $F : [\set^k, \set]$, then $\Eq_F^* : [\rel^k, \rel]$ is defined by
% \[\begin{array}{lll}
% \Eq_F^* \overline{R} & := & \bigcup_{\overline{f : A \to B},
%   \overline{f|_{\dom R}} \subseteq \overline{R}} \langle
% F\overline{f}|_{F \overline{\dom R}}\rangle\\
% & = & \{ (F\overline{i}x, y) | x \in F \overline{\dom R} \text{ and }
% \exists \overline{f : A \to B}. \overline{f|_{\dom R}} \subseteq
% \overline{R}, y = F\overline{f}(F\overline{i}x) \}
% \end{array}\]
% for all $\overline{R : \rel(A, B)}$, where, $\overline{i} :
% \overline{\dom\, R} \hookrightarrow \overline{A}$ is the subobject
% defined by the domain of $\overline{R}$, and 
% \[\begin{array}{lll}
% \Eq_F^*(\alpha, \beta) & := & (F\alpha, F\beta)
% \end{array}\]
% for all $\overline{R : \rel(A, B)}$, $\overline{S : \rel(C, D)}$, and
% $\overline{(\alpha, \beta) : R \to S}$.
% \end{definition}

% \begin{lemma}\label{lem:eq-reln-functors}
% If $F : [\set^k,\set]$ then the triple $\Eq_F = (F, F, \Eq_F^*)$ is a
% relation transformer.
% \end{lemma}
% \begin{proof}
% For all $\overline{R : \rel(A, B)}$, we have that $\Eq_F^* \overline
% {R} : \rel(F\overline{A}, F\overline{B})$.

% We first show that for all $\overline{R : \rel(A, B)}$, $\overline{S :
%   \rel(C, D)}$, and $\overline{(\alpha, \beta) : R \to S}$, we have
% that $\Eq_F^*(\alpha, \beta)$ is a morphism in $\rel$.  To see this,
% let $(F\overline{i}x, y) \in \Eq_F^* \overline{R}$. Then there
% exists $\overline{f : A \to B}$ such that
% $\overline{f}|_{\overline{\dom R}} \subseteq \overline{R}$, $x \in
% F(\dom R)$, and $y = F\overline{f}(F\overline{i}x)$, where $i :
% \overline{\dom R} \hookrightarrow \overline{A}$ is the subobject
% defined by the domain of $\overline{R}$.  We need to show that $(F
% \overline{\alpha}(F\overline{i} x), F \overline{\beta} y) \in \Eq_F^*
% \overline{S}$, i.e., that $F \overline{\alpha}(F\overline{i} x) = F
% \overline{j} x'$ for some $x' \in F(\dom S)$ where $\overline{j : \dom
%   S \hookrightarrow C}$ is the subobject defined by the domain of
% $\overline{S}$, and there exists $\overline{g : C \to D}$ such that
% $\overline{g|_{\dom S}} \subseteq \overline{S}$,
% % $(F \alpha) x \in F(\dom S)$,
% and $F \overline{\beta}y = (F\overline{g}) (F \overline{j} x')$.

% Next, observe that the assumption that $(\alpha, \beta)$ is a morphism
% in $\rel$ means, in particular, that for every $a \in \dom R$ we have
% that $\alpha (i a) \in \dom S$.  Then, there are functions
% \[
% \dom R
% \xrightarrow{\alpha' = \alpha|_{\dom R}}
% \alpha(\dom R)
% \xrightarrow{k}
% \dom S
% \]
% such that, for every $a \in \dom R$, we have that $\alpha (i a) = j (
% k (\alpha' a))$.

% We show that $F \overline{\alpha}(F\overline{i} x) = F \overline{j}
% x'$ for some $x' \in F(\dom S)$.
% % that there is a function $\alpha' \colon \dom R \to \dom S$ such that $\alpha i = j \alpha'$,
% % which makes the inclusion $\alpha(\dom R) \subseteq \dom S$ explicit.
% Observe that $F \overline{\alpha}(F\overline{i} x) = F
% (\overline{\alpha \circ i}) x = F (j \circ k \circ \alpha') x$, so let
% $x'$ be $F (k \circ \alpha') x$.

% We show that there exists $\overline{g : C \to D}$ such that
% $\overline{g|_{\dom S}} \subseteq \overline{S}$, and $F
% \overline{\beta}y = (F\overline{g}) (F \overline{j} x')$.  Let
% $\overline{\gamma : \alpha(\dom R) \to \dom R}$ be a right inverse of
% $\alpha'$
% % \[\ove.rline{\alpha|_{\dom R}^{\alpha(\dom R)} : \dom R \to \alpha(\dom R)}\]
% (which exists assuming the Axiom of Choice).
% Observe that, for every $c \in \alpha(\dom R)$, since $\gamma c \in \dom R$,
% we have that $(\alpha(i(\gamma c)), \beta (f ( i (\gamma c)))) \in S$.
% Then, it is possible to choose a function $g : C \to D$ such that
% $g \circ j \circ k = \beta \circ f \circ i \circ \gamma$
% % $g|_{\alpha(\dom R)} = \beta \circ \alpha \circ \gamma$
% and $g_{\dom S} \subseteq S$
% (again, thanks to the Axiom of Choice).
% % Observe that $g \circ \alpha \circ i = \beta \circ f \circ i$.
% Then,
% \[
% \begin{split}
% (F\overline{g}) (F \overline{j} x')
% %&=(Fg)((F\alpha)(F i x)) \\
% %&=(Fg)((F\alpha)|_{F \dom R}x) \\
% %&=(Fg)((F\alpha|_{\dom R})x) \\
% &=F(g \circ j \circ k \circ \alpha')x \\
% &=F(\beta \circ f \circ i \circ \gamma \circ \alpha')x \\
% &=??? \\
% &=F(\beta \circ f \circ i)x \\
% %&=(F\beta)((F f)x) \\
% &=(F\beta)((F f)(F i x)) \\
% &=(F\beta)y
% \end{split}
...
If $F$ is a functor from $\set^k$ to $\set$, we define $\Eq_F^* :
\rel^k \to \rel$ as follows. Given $R_1 : \rel(A_1, B_1), ..., R_k :
\rel(A_k,B_k)$, let $\iota_{R_i} : R_i \hookrightarrow A_i \times B_i$
be the inclusion of $R_i$ as a subset of $A_i \times B_i$ for $i =
1,...,k$. We define $\Eq_F^* \overline{R} : \rel(F\overline{A},
F\overline{B})$ by $(x,y) \in \Eq_F^* \overline{R}$
%to be the relation that relates $x : F\overline{A}$
%and $y : F\overline{B}$
if and only if there exists a $z : F\overline{R}$ such that
$F(\overline{\pi_1 \circ \iota_R})z = x$ and $F(\overline{\pi_2 \circ
  \iota_R})z = y$.  If $(\alpha_1, \beta_1) : R_1 \to S_1, ...,
(\alpha_k, \beta_k) : R_k \to S_k$ are morphisms in $\rel$ then we
define $\Eq_F^*\overline{(\alpha, \beta)}$ by
$\Eq_F^*\overline{(\alpha, \beta)} = (F\overline\alpha,
F\overline\beta)$.
\end{dfn}

\begin{lemma}\label{lem:eq-reln-functors}
If $F$ is a functor from $\set^k$ to $\set$, then the triple $\Eq_F =
(F, F, \Eq_F^*)$ is in $RT_k$.
\end{lemma}
\begin{proof}
If $R_1 : \rel(A_1, B_1), ..., R_k : \rel(A_k,B_k)$, then
Definition~\ref{def:eq-reln-functors} ensures that $\Eq_F^*
\overline{R} : \rel(F\overline{A}, F\overline{B})$.

Moreover, if $(\alpha_1, \beta_1) : R_1 \to S_1, ..., (\alpha_k,
\beta_k) : R_k \to S_k$ are morphisms in $\rel$, and if $x :
F\overline{A}$ and $y : F\overline{B}$ are such that $(x, y) \in
\Eq_F^*\overline{R}$, then there must exist a $z : F\overline{R}$ such
that $F(\overline{\pi_1 \circ \iota_R})z = x$ and $F(\overline{\pi_2
  \circ \iota_R})z = y$.  We want to show that $(F\overline{\alpha}x,
F\overline{\beta}y) \in \Eq_F^*\overline{S}$, i.e., that there exists a
$w : F\overline{S}$ such that $F(\overline{\pi_1 \circ \iota_S})w =
F\overline{\alpha}x$ and $F(\overline{\pi_2 \circ \iota_S})w =
F\overline{\beta}y$. To see this, first consider the $\set$-morphisms
$\gamma_1 : R_1 \to S_1, ..., \gamma_k : R_k \to S_k$ defined by
$\gamma_i(a, b) = (\alpha_i a, \beta_i b)$ for all $(a,b) : R_i$ and
$i = 1,...,k$. These are well-defined because each $(\alpha_i,
\beta_i)$ is a morphism in $\rel$. Next, observe that, for all $i =
1,...,k$, we have that $\pi_1 \circ \iota_{S_i} \circ \gamma_i =
\alpha_i \circ \pi_1 \circ \iota_{R_i}$ and $\pi_2 \circ \iota_{S_i}
\circ \gamma_i = \beta_i \circ \pi_2 \circ \iota_{R_i}$. Let $w = F
\overline{\gamma} z$. Then
\[\begin{array}{lll}
      F(\overline{\pi_1 \circ \iota_S})w
& = & F(\overline{\pi_1 \circ \iota_S})(F \overline{\gamma} z)\\
& = & F(\overline{\pi_1 \circ \iota_S \circ \gamma}) z\\
& = & F(\overline{\alpha \circ \pi_1 \circ \iota_R}) z\\
& = & F\overline{\alpha}(F(\overline{\pi_1 \circ \iota_R})z)\\
& = & F\overline{\alpha}x\\
\end{array}\]
and, analogously, $F(\overline{\pi_2 \circ \iota_S})w = F\overline{\beta}y$.
\end{proof}

If $A_1,...,A_k$ are sets, the write $\overline{\Eq_A}$ for the
sequence of relations $\Eq_{A_1}....\Eq_{A_k}$.

\begin{lemma}
If $F$ is a functor from $\set^k$ to $\set$ and $A_1, ..., A_k$ are
sets, then $\Eq_F^* \overline{\Eq_A} = \Eq_{F\overline{A}}$.
\end{lemma}
\begin{proof}
Let $x, x' : F\overline{A}$. Then $(x, x') \in \Eq_F^*
\overline{\Eq_A}$ if and only if there exists $z : F\overline{\Eq_A}$
such that $F(\overline{\pi_1 \circ \iota_{\Eq_A}})z = x$ and
$F(\overline{\pi_2 \circ \iota_{\Eq_A}})z = x'$.  For each $A : \set$
there is an isomorphism $\phi_A : A \to \Eq_A$ in $\set$ such that
$\iota_{\Eq_A} \circ \phi_A : A \to A \times A$ is the diagonal
$\Delta_A$. Then
\[
  F(\overline{\pi_1 \circ \iota_{\Eq_A}})z
= F(\overline{\pi_1 \circ \iota_{\Eq_A} \circ \phi_A \circ \phi_A^{-1}})z
= F(\overline{\pi_1 \circ \Delta_A})(F \overline{\phi_A^{-1}} z)
= F\overline{\id_A}(F \overline{\phi_A^{-1}} z)
= F \overline{\phi_A^{-1}} z
\]
and, analogously, $F(\overline{\pi_2 \circ \iota_{\Eq_A}})z = F
\overline{\phi_A^{-1}} z$.  Thus, $(x, x') \in \Eq_F^*
\overline{\Eq_A}$ if and only if there exists $z : F\overline{\Eq_A}$
such that $F \overline{\phi_A^{-1}} z = x$ and $F
\overline{\phi_A^{-1}} z = x'$, i.e., if and only if $x = x'$ (by
letting $z = F \overline{\phi_A} x = F \overline{\phi_A} x'$). We
therefore have that $\Eq_F^* \overline{\Eq_A}$ is the equality
relation $\Eq_{F\overline{A}}$ on $F\overline{A}$, as desired.
\end{proof}

We now show that an Identity Extension Lemma holds for the
interpretation given in Sections~\ref{sec:set-interp}
and~\ref{sec:rel-interp}. First, define $\Eq_A : \rel(A,A)$ for any
set $A$ to be the relation $\{(x,x)~|~ x \in A\}$, and $\Eq_F$ for any
$F : \rel^k \to \rel$ as in
Definition~\ref{def:eq-reln-functors}. Furthermore, if $\rho$ is a set
environment, define $\Eq_\rho$ to be the relation environment such
that $\Eq_\rho x = \Eq_{\rho x}$ for all $x \in \V \cup \tvars$. The
Identity Extension Lemma can then be stated and proved as follows:

\begin{thm}\label{thm:iel}
  If $\bm \alpha = \{\alpha_1,...,\alpha_k\}$, $\bm \beta =
  \{\beta_1,...,\beta_m\}$, $\rho$ is a set environment, and $\bm
  \alpha; \bm \beta \vdash \tau : \F$, then $\relsem{\bm \alpha; \bm
    \beta \vdash \tau} \Eq_\rho = \Eq_{\setsem{\bm \alpha; \bm \beta
      \vdash \tau}\rho}$. 
%  [\alpha_1 := \Eq_{\alpha_1
%      \rho},...,\alpha_k := \Eq_{\alpha_k \rho}, \beta_1 :=
%    \Eq_{\beta_1 \rho} ,...,\beta_m := \Eq_{\beta_m \rho}] =
%  \Eq_{\setsem{\bm \alpha; \bm \beta \vdash \tau}\rho}$.
\end{thm}
\begin{proof}
{\color{red} REVISE}
By induction on the structure of $\tau$.
% Write $\rho'$ for the
% relation environment $\rho[\alpha_1 := \Eq_{\alpha_1 \rho}, ...,\alpha_k
%   := \Eq_{\alpha_k \rho}, \beta_1 := \Eq_{\beta_1 \rho}, ...,\beta_m
%   := \Eq_{\beta_m \rho}]$.
\begin{itemize}
...
\item $\relsem{\bm \alpha; \bm \beta \vdash
  \phi^k\tau_1...\tau_k}\rho' = \phi^k\rho' (\relsem{\bm \alpha; \bm
  \beta\vdash \tau_1}\rho')... (\relsem{\bm \alpha; \bm \beta\vdash
  \tau_k}\rho') = \Eq_{\phi^k\rho} \Eq_{\setsem{\bm \alpha; \bm
    \beta\vdash \tau_1}\rho}...$ $\Eq_{\setsem{\bm \alpha; \bm
    \beta\vdash \tau_k}\rho} = \Eq_{\phi^k\rho (\setsem{\bm \alpha;\bm
    \beta\vdash \tau_1}\rho)... (\setsem{\bm \alpha; \bm \beta\vdash
    \tau_k}\rho)} = \Eq_{\setsem{\bm \alpha; \bm \beta \vdash
    \phi^k\tau_1...\tau_k}\rho}$
\item {\color{red} $\relsem{\bm \alpha; \bm \beta \vdash (\mu
    \phi^k.\lambda \gamma_1...\gamma_k.\sigma)\tau_1...\tau_k}\rho' =
  (\mu (F \mapsto \lambda R_1...R_k. \relsem{ \bm \alpha; \bm
    \beta,\bm \gamma \vdash H}\rho'[\phi := F][\bm \gamma := \bm
    R]))(\relsem{\bm \alpha; \bm \beta \vdash
    \tau_1}\rho')...(\relsem{\bm \alpha; \bm \beta \vdash
    \tau_k}\rho') = (\mu (F \mapsto \lambda
  R_1...R_k. \Eq_{\relsem{\bm \alpha; \bm \beta,\bm \gamma \vdash
      H}\rho[\phi := F][\bm \gamma := \bm R]}))$ $\Eq_{\setsem{\bm \alpha;
    \bm \beta \vdash \tau_1}\rho}...\Eq_{\setsem{\bm \alpha; \bm \beta
      \vdash \tau_k}\rho} = HERE!!!! = 
  \Eq_{\setsem{\bm \alpha; \bm \beta \vdash (\mu
    \phi^k.\lambda \gamma_1...\gamma_k.\sigma)\tau_1...\tau_k}\rho}$}
...
{\color{blue} We will need to go back and add typing rules for
  ...
  functoial actions), and cases for $\map$ to all of our proofs thus
...
\item RULE FOR SUMS  
\item $\fold\,k\,(in\,t) = k\,(\map\,(\fold\,k)\,t)$
\item $\map \,\overline{(L_{\bm \alpha} x.x)} = L_{\bigcup \bm \alpha} x.x$
...
\item $\lambda x. \map^\T_G\, \overline{f}\, (\eta_{\overline \sigma} x) =
  \lambda x.\eta_{\overline \tau}\, (\map^\T_F \,\overline{f}\, x)$ 
\item and other conversion rules as on page 18 of MFPS paper
\end{itemize}
Note that there is no analogue of the last conversion rulewhen the
first vector of arguments to $\map^\F_H$ consists of natural
transformations because natural transformations can only be atural in
type variables (i.e., type constructor variables of arity $0$.
...
The show that we have initial algebras for all functors definable in
our syntax (perhaps in two steps, first by showing that we have a weak
initial algebra, and then showing uniqueness), and possibly that we
have final coalgebras as well.
