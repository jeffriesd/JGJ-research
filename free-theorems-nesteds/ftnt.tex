...
\end{array}\]
To this end, we note that, for any $\ol{B}$ and all $\ol{\eta :
  \setsem{\Gamma; \emptyset \vdash \Nat^{\ol{\beta},\ol{\gamma}}\,F\,G}}$,
\[\begin{array}{ll}
  & (\setsem{\Gamma; \emptyset~|~\emptyset \vdash
\map^{\ol{F},\ol{G}}_H :
\Nat^\emptyset\;(\ol{\Nat^{\ol{\beta},\ol{\gamma}}\,F\,G})\;
...
= & \setsem{\Gamma ;\ol{\gamma}\;\ol{\phi} \vdash H}\rho[\ol{\gamma
      := B}][\ol{\phi := \lambda \ol{A}.\setsem{\Gamma;
        \ol{\gamma},\ol{\beta} \vdash G}\rho[\ol{\gamma :=
          B}][\ol{\beta := A}]}]
\end{array}\]
{\color{red} Does this use Enrico's proof that 
$\setsem{\Gamma ;\ol{\gamma} \vdash H[\ol{\phi := G}]}$ acts like
  $\mathit{map}$ semantically?}
...
& \setsem{\Gamma ;\ol{\gamma} \vdash H[\ol{\phi := G}]}\rho[\ol{\gamma
      := B'}] 
\end{tikzcd}\]
This diagram commutes by {\color{red} Enrico's proof that interp of
  map is natural?} We therefore have that 
