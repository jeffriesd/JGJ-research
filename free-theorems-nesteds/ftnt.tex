...
For each $k \ge 0$, we assume countable sets {\color{red} $\tvars^k$ of \emph{type
...
{\color{red} $\phi \in \tvars^k \cup \fvars^k$.}
When convenient we may write $\alpha, \beta$, etc., rather than $\alpha^0, \beta^0$, etc.,
for elements of {\color{red} $\tvars^0 \cup \fvars^0$.} 
{\color{red} The sets of all type constructor variable and functorial variables are 
...
cardinality $n$ of the set is unimportant. If $\Pos$ is a set of type
constructor (or functorial) variables then we write $\Pos, \overline{\phi}$ for $\Pos
\cup \overline{\phi}$ when $\Pos \cap \overline{\phi} = \emptyset$.
We omit the vector notation for a singleton set, thus writing $\phi$, rather
than $\overline{\phi}$, for $\{\phi\}$.

\begin{dfn}
{\color{red} Let $V$ be a finite subset of\, $\tvars$, 
  let $\Pos$ be a finite subset of\, $\fvars$ (where $\Pos^{~k}$ denotes the set $\Pos \cap \fvars^k$), and 
let $\overline{\alpha}$ be a finite subset of\, $\fvars^0$, disjoint from $\Pos$. 
 The sets $\T(V)$ of
...
\mid {\color{red} V\, \ol{\mcF^{\Pos}(V)}  }
\mid \mcF^{\Pos}(V)\! +\, \mcF^{\Pos}(V)
\mid \mcF^{\Pos}(V)\! \times\, \mcF^{\Pos}(V)\\
&\color{red} \mid \left(\mu \Pos^{~k}. \lambda \alpha_1...\alpha_k. 
...
is allowed only when $\tau$ is a type constructor variable of arity
...
length matches the arity of the functorial expression applied to it.
The fact that functorial expressions are always in \emph{$\eta$-long
...
formers are all defined pointwise avoids having to relate functorial
...
If {\color{red} $\tau \in \mcF^\Pos(V) \cup \T(V)$,} {\color{red} $\Pos$ and $V$} contain only 
...
variables in which the resulting type is not functorial, and let $\Phi$ be a 
{\em functorial context}, i.e., a finite set of type variables in 
which the resulting type is functorial. Note that variables of arity greater 
than 0, which are interpreted as functors, may appear in either context.

Should we state $\Gamma \subseteq \tvars$ and $\Phi \subseteq \fvars$?
}


The formation rules for the
set {\color{red} $\T \subseteq \bigcup_{V \subseteq \tvars}\T(V)$ of\, {\em well-formed
...
\DisplayProof
\end{array}\]
  The formation rules for the set {\color{red} $\F \subseteq \bigcup_{V \subseteq
...
for well-formed type expressions and type constructor expressions
...
Our formation rules
also allow type constructor expressions like $\List\gamma = (\mu
...
not in $\delta$.
{\color{red}
Type constructor expressions may also be functorial in variables
...
% the phi is removed. 
  %
  %
  % also current version suggests one rule for t types, one rule for ftypes
  % with corresponding contexts. 
  % but I think we need mixing, e.g. the alphas that appear in H
  % may not need to be all to one side of the semicolon.. 
% -- Daniel 
{\color{red}
If \,$\Gamma; \Phi, \phi^k \vdash F[\phi] : \F$ and $\Gamma;\Phi,
\ol{\alpha} \vdash H : \F$ with $\ol{\alpha} =
...
H[\ol{\alpha := \tau[\phi := H]}] & \mbox{if } \psi = \phi
\end{array}\right.\\
\\[-0.25ex]
...
{\color{red} Need to update to $\tvars$ and $\fvars$}
...
$x$ in $t$. In the rule for $t_\tau s$ there is one type constructor
expression $\tau$ for every type constructor variable $\alpha$. In the
rule for $\map^{\ol{F},\ol{G}}_H$ there is one type constructor expression $F$
and one type constructor expression $G$ for each type constructor
variable $\phi$. Moreover, for each $\phi$ the number of type
constructor variables $\beta$ in the judgments for its corresponding
$F$ and $G$ is the same as its arity. In the rules for $\tin_H$ and
$\fold^F_H$, the type constructor variables in $\ol{\beta}$ are fresh
...
{\color{blue} The presence of the ``extra'' type constructor variables
  ...
  involving no type constructor variables is well-typed:
\[\begin{array}{l}
...
$\mathtt{flatten}$, whose type contains the type constructor variable
...
\[\setsem{\Gamma;\Phi \vdash F \ol{\tau}}\rho =
  ...
  the second clause does indeed define a
...
\item To see that $\setsem{\Gamma; \emptyset~|~\emptyset \vdash
  ...
  type context is empty, we need only show that, for every $\rho :
  \setenv$,
  \[\setsem{\Gamma; \emptyset~|~\emptyset \vdash
  \map^{\ol{F},\ol{G}}_H :
  \Nat^\emptyset\;(\ol{\Nat^{\ol{\beta},\ol{\gamma}}\,F\,G})\;
...
\item To see that $\setsem{\Gamma; \emptyset~|~\emptyset \vdash
  ...
  the type context is empty, we need only show that, for every $\rho :
  \setenv$,
\[\setsem{\Gamma; \emptyset~|~\emptyset \vdash
  \fold^F_H : \Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
