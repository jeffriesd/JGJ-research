...
\lambda d\,\ol{B}\,\ol{C}.\, (\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C}]}})_{\ol{B}}\\
%(\lambda \ol{A}.\, (\setsem{\Gamma;\emptyset \,|\,
%  \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
%    F]\;F[\overline{\beta :=
%      \alpha}]}\rho\,d)_{\ol{A}\,\ol{C}}))_{\ol{B}}\\
...
\end{array}\]
\end{itemize}
\item {\color{red} Check this case of AT} We now consider
...
= & \setsem{\Gamma ;\ol{\gamma},\ol{\phi} \vdash H}\rho[\ol{\gamma
      := B}][\ol{\phi := \lambda \ol{A}.\setsem{\Gamma;
        \ol{\gamma},\ol{\beta} \vdash G}\rho[\ol{\gamma :=
          B}][\ol{\beta := A}]}]
\end{array}\]
{\color{red} since $\setsem{\Gamma ;\ol{\phi}, \ol{\gamma} \vdash H}$
  is a functor from $\setenv$ to $\set$ by~\cite{jp19} and
  $\id_{\rho[\ol{\gamma := B}]}[\ol{\phi := \lambda
      \ol{A}. \eta_{\ol{A}\,\ol{B}}}]$ is a morphism in $\setenv$ from
  $\rho[\ol{\gamma := B}][\ol{\phi := \lambda \ol{A}.\setsem{\Gamma;
        \ol{\gamma},\ol{\beta} \vdash F}\rho[\ol{\gamma :=
          B}][\ol{\beta := A}]}]$ to $\rho[\ol{\gamma := B}][\ol{\phi
      := \lambda \ol{A}.\setsem{\Gamma; \ol{\gamma},\ol{\beta} \vdash
        G}\rho[\ol{\gamma := B}][\ol{\beta := A}]}]$. }
...
& \setsem{\Gamma ;\ol{\gamma} \vdash H[\ol{\phi := G}]}\rho[\ol{\gamma
      := B'}] 
\end{tikzcd}\]
This diagram commutes by {\color{red} the facts that $\setsem{\Gamma;
    ol{\phi}, \ol{\gamma} \vdash H}$ is a functor from $\setenv$ to
  $\set$, and that ???} We therefore have that
...
\end{split}\]
\end{itemize}
\item {\color{red} Check this case of AT}
...
\end{array}\]
To this end we note that, for any $\ol{B}$ and $\ol{C}$,
\[(\setsem{\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H :
  \Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
  (\Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
...
to 
\[\lambda \ol{B}\,\ol{C}.\,\setsem{\Gamma;\ol{\beta},\ol{\gamma}
  \vdash F}\rho[\ol{\beta := B}][\ol{\gamma := C}]\] so that if $x :
\setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash (\mu \phi.\lambda
...
= & (\mathit{fold}_{T^\set_{\rho[{\gamma :=
      C}]}})_{\ol{B}}\,\eta_{\ol{B}\,\ol{C}}\,x\\ 
...
\ar[d, "{(\mu \sigma_{\id_\rho[\ol{\gamma := g}]}) \, \ol{B}}"'] \ar[r, 
  "{(\mathit{fold}_{T^\set_{\rho[\ol{\gamma := C}]}})_{\ol{B}}\,\eta_{\ol{B}\,\ol{C}}}"]
...
naturality of $(\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C'}]}})_{\ol{B'}}$ and $\eta_{\ol{B}\,\ol{C'}}$ {\color{red}
  and composition} ensures that the bottom one commutes.  We therefore
have that
...
\[\begin{array}{ll}
 & (\,(\lambda e\,\ol{Y}\,\ol{Z}.\,
(\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        Z}]}})_{\ol{Y}})\,d\,\ol{B}\,\ol{C}\,\eta_{\ol{B}\,\ol{C}},
(\lambda e\,\ol{Y}\,\ol{Z}.\, (\mathit{fold}_{T^\set_{\rho[\ol{\gamma
        := Z}]}})_{\ol{Y}})\,d\,\ol{B'}\,\ol{C'}\,
\eta_{\ol{B'}\,\ol{C'}}\,)\\
= & (\,(\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C}]}})_{\ol{B}}\,\eta_{\ol{B}\,\ol{C}},\,
(\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        B'}]}})_{\ol{C'}}\,\eta_{\ol{B'}\,\ol{C'}}\,) 
\end{array}\]
has type
\[\begin{array}{ll}
...
=& \lambda e\,\ol{R}\,\ol{S}.\,(\mathit{fold}_{T_{(\pi_i\rho)[\ol{\gamma
        := \pi_i S}]}})_{\ol{\pi_i R}}\\
=& \lambda d\,\ol{B}\,\ol{C}.\,(\mathit{fold}_{T_{(\pi_i\rho)[\ol{\gamma
        := C}]}})_{\ol{B}}\\
=& \relsem{\Gamma; \emptyset~|~\emptyset \vdash
  \fold^F_H : \Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
