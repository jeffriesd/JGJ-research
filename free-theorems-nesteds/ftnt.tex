...
\TIC{$\Gamma;\emptyset~|~\emptyset \vdash \map^{F,G}_H :
...
rule for $\map^{F,G}_H$ there is one type constructor expression $F$
...
{\color{blue} The presence of the ``extra'' type constructor variables
  in $\ol{\gamma}$ in the rules for $\map^{F,G}_H$, $\tin_H$, and
...
\llbracket \Gamma;\emptyset \,|\, \emptyset \vdash \map^{F,G}_H :
\Nat^\emptyset\;(\Nat^{\ol{\beta},\ol{\gamma}}\,F\,G)\;&
...
\end{split}\]
\end{itemize}
\item We now consider $\Gamma; \emptyset~|~\emptyset \vdash \fold^F_H
  : \Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
  :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
...
\item To see that $\setsem{\Gamma; \emptyset~|~\emptyset \vdash
  ...
  \[\setsem{\Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
    (\Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
...
\[\relsem{\Gamma; \emptyset~|~\emptyset \vdash
  ...
  \alpha.H)\overline \beta\,F}\] is a natural transformation from
  $\relsem{\Gamma;\emptyset \vdash \emptyset}$ to
  \[\relsem{\Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
    (\Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
...
=& \relsem{\Gamma; \emptyset~|~\emptyset \vdash
  \fold^F_H : \Nat^\emptyset\;(\Nat^{\ol{\beta}, \ol{\gamma}}\,H[\phi
    :=_{\ol{\beta}} F][\ol{\alpha := \beta}]\,F)\;
  (\Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\,F}(\pi_1 \rho)\\
