...
F\overline{i_R}$. Define $F^\wedge\overline{R}$ to be the subobject
...
&&F\overline{A} \times F\overline{B} \\
&F^\wedge\overline{R}
\ar[ur, hookrightarrow, "{i_{F^\wedge\overline{R}}}"']
\end{tikzcd}
\]
%
...
\end{tikzcd}
\]
commutes. We therefore get the following commutative diagram:
\[
\begin{tikzcd}
F\overline{R}
...
\end{tikzcd}
\]
Then, by the left-lifting property of $q_{F^\wedge\overline{R}}$
with respect to $\iota_{F^\wedge\overline{S}}$
...
\end{lemma}
\begin{proof}
Each $\Eq_A : \rel$ has as its third component $\Delta_{A} : A \to A
\times A$.  Since $h_{\overline{A \times A}}$ is the unique morphism
making the bottom triangle of the following diagram commute
\[
\begin{tikzcd}[row sep = large]
&F\overline{A}
...
\item The application case is proved by the following sequence of
  equalities, where the second equality is given by the induction
  hypothesis, and the third by Lemma~\ref{lem:eq-reln-equalities}:
\[
\begin{split}
...
\end{split}
\]
Here, the third equality is by induction hypothesis, the sixth is by
