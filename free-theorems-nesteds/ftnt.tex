...
\item To see that $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
  ...
  \[\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
    ...
    \alpha.H)\overline \beta\;F} \rho\,d\] is a morphism in $\set$
  ...
  \rho$, $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\, \rho$ is a natural transformation
  from \[\lambda \ol{B}\,\ol{C}.\, \setsem{\Gamma;
    \ol{\beta},\ol{\alpha} \vdash (\mu \phi.\lambda \overline
    \alpha.H)\overline \beta}\rho[\ol{\beta := B}][\ol{\gamma := C}]\]
  to \[\lambda \ol{B}\,\ol{C}.\, \setsem{\Gamma;
    \ol{\beta},\ol{\alpha} \vdash F}\rho[\ol{\beta := B}][\ol{\gamma
      := C}]\] We first observe that, for any $\ol{B}$ and $\ol{C}$,
  \[(\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\, \rho\,d)_{\ol{B}\,\ol{C}}\] is a
  morphism from \[\setsem{\Gamma; \ol{\beta},\ol{\alpha} \vdash (\mu
    \phi.\lambda \overline \alpha.H)\overline \beta}\rho[\ol{\beta :=
      B}][\ol{\gamma := C}]\] to \[\setsem{\Gamma;
    \ol{\beta},\ol{\alpha} \vdash F}\rho[\ol{\beta := B}][\ol{\gamma
      := C}]\] Indeed, if $x : \setsem{\Gamma; \ol{\beta},\ol{\alpha}
    \vdash (\mu \phi.\lambda \overline \alpha.H)\overline
    \beta}\rho[\ol{\beta := B}][\ol{\gamma := C}] \;=\; (\mu
  T^\set_{\rho[\ol{\gamma := C}]}) \ol{B}$, then 
\[\begin{array}{ll}
 & (\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
...
:& \setsem{\Gamma;\ol{\alpha},\ol{\gamma} \vdash F}\rho[\ol{\beta :=
    B}][\ol{\gamma := C}]
\end{array}\]
So 
\[(\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\;F}\, \rho\,d)_{\ol{B}\,\ol{C}}\] gives a
family of morphisms from \[\setsem{\Gamma; \ol{\beta},\ol{\alpha}
  \vdash (\mu \phi.\lambda \overline \alpha.H)\overline
  \beta}\rho[\ol{\beta := B}][\ol{\gamma := C}]\] to
\[\setsem{\Gamma;\ol{\alpha},\ol{\gamma} \vdash F}\rho[\ol{\beta :=
    B}][\ol{\gamma := C}]\]

{\color{red} NEED IH FOR $t$ in above. Used it there, so note it.

Follow structure for $L$-case.}
...
Will also need:
if $d : \setsem{\Gamma;\emptyset \vdash \Delta}
  \rho$ then \[\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
  \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu
  \phi.\lambda \overline \alpha.H)\overline \beta\;F}\,\rho\,d\]
...
\setsem{\Gamma;\ol{\alpha} \vdash F}\rho[\ol{\alpha := \_}] ~|~ ...\}
\end{array}\]
Here, the final equality holds because using weakening
