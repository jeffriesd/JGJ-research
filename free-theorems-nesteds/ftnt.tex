...
\end{itemize}
Note that there is no analogue of the conversion rule $\star$ when the
...
Show $\sem{\Gamma; \emptyset \vdash \sigma \to \tau} = \sem{\Gamma;
  \emptyset \vdash \Nat^\emptyset\, \sigma\, \tau}$.

Other sanity checks?

Note that our calculus does not support Church encodings of data types
like pair or sum or list types. But we could (probably) alter our
calculus to admit sum and product and list types because we can tell
metatheoretically that these are fixed points of (positive) functors,
and because all of the ``forall''s are at the top level.
