...
Definitions~\ref{def:set-interp} and~\ref{def:rel-sem} in space but
...
Definition~\ref{def:set-interp} for types of the form
...
Definition~\ref{def:set-interp}, we need to know that $T^\set_{\rho}$
is an $\omega$-cocontinuous endofunctor on $[\set^k, \set]$, so that
it admits a fixed point.  Since $T_\rho^\set$ is defined in terms of
...
from~\cite{jp19} that, for every $\Gamma; \ol{\alpha} \vdash \tau : \F$,
$\setsem{\Gamma; \ol{\alpha} \vdash \tau}$ is actually functorial in
$\ol{\alpha}$ and $\omega$-cocontinuous.  What remains is to define
...
with the relation $(A,B,R)$. We may also write $F^0$ and $F^1$ for
$\pi_1 F$ and $\pi_2 F$. We extend these conventions to relation
environments, introduced in Definition~\ref{def:reln-env} below, as
well.
...
Definition~\ref{def:set-interp}. We can, at last, define:
...
\begin{dfn}
If $F$ is a functor from $\set^k$ to $\set$, define the functor
$\Eq_F^*$ from $\rel^k$ to $\rel$ as follows.  Given $R_1 : \rel(A_1,
B_1),$ $...,R_k : \rel(A_k,B_k)$, let $\overline{i_{R_i} : R_i
  \hookrightarrow A_i \times B_i}$ be the inclusion of $R_i$ as a
...
F\overline{i_R}$.  Then, define $\Eq_F^*\overline{R}$ as the subobject
...
\end{tikzcd}
\]
Note that $\Eq_F^*\overline{R} : \rel(F\overline{A}, F\overline{B})$
by construction.  Moreover, if $\overline{(\alpha, \beta) : R \to S}$
are morphisms in $\rel$, then $\Eq_F^*\overline{(\alpha, \beta)}$ is
defined to be $(F\overline\alpha, F\overline\beta)$.
\end{dfn}
...
\end{lemma}
\begin{proof}
Clearly, $\Eq*_F : [\rel^k,\rel]$. i.e., $Eq^*_F$ is
$\omega$-cocontinuous. 

Now, consider $\overline{(\alpha, \beta) : R \to S}$, where
$\overline{R : \rel(A, B)}$ and $\overline{S : \rel(C, D)}$.  We want
to show that there exists a morphism $\epsilon : \Eq_F^*\overline{R}
\to \Eq_F^*\overline{S}$ such that
\[
\begin{tikzcd}
\Eq_F^*\overline{R}
...
\]
commutes.
By hypothesis, there exist $\overline{\gamma : R \to S}$
...
\end{tikzcd}
\]
commutes. Thus, we get the following commutative diagram:
\[
\begin{tikzcd}
F\overline{R}
...
there exists an $\epsilon$ such that the diagram
\[
\begin{tikzcd}
F\overline{R}
	\ar[d, "{F\overline{\gamma}}"']
...
\end{lemma}
\begin{proof}
The relation $\overline{\Eq_A}$ corresponds to the subobject
$\overline{\Delta_{A} : A \to A \times A}$.  Since
$h_{\overline{\Eq^*_A}}$ is the unique morphism making the diagram
\[
\begin{tikzcd}[row sep = large]
F\overline{A}
&F(\overline{A \times A})
...
\end{tikzcd}
\]
commute, we have $h_{\overline{\Eq^*_A}} = \Delta_{F\overline{A}}$.
Moreover, since $\Delta_{F\overline{A}}$ is a monomorphism, we have
that $i_{\Eq_F^* \overline{\Eq^*_A}} = \Delta_{F\overline{A}}$.  To
conclude we observe that the relation corresponding to the subobject
$\Delta_{F\overline{A}}$ is $\Eq_{F\overline{A}}$.
\end{proof}

We now show that the Identity Extension Lemma holds for the
interpretations given in Definitions~\ref{def:set-interp}
...
\item The application case is proved by the sequence of equalities
\[
\begin{split}
...
\end{split}
\]
where the second equality is given by the induction hypothesis, and
the third by Lemma~\ref{lem:eq-reln-equalities}.
...
&= \colim{n \in \nat}{ \Eq_{(T^\set_\rho)^n K_0}
  \ol{\Eq_{\setsem{\Gamma; \Phi \vdash \tau_1}\rho}}}\\
...
\end{split}
\]
where the third equality is by induction hypothesis, the sixth is by
Lemma~\ref{lem:eq-reln-equalities} and the fifth equality is because,
for every $n \in \nat$, the following two statements can be proved by
simultaneous induction:
\begin{equation}\label{eq:iel-fix-point-intermediate1}
T^n_{\Eq_{\rho}} K_0^{\rel}\, \ol{\Eq_{\setsem{\Gamma; \Phi \vdash
      \tau}\rho}} = \Eq_{(T^\set_\rho)^n K_0^{\set}}
\ol{\Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}}
\end{equation}
and
\begin{multline}\label{eq:iel-fix-point-intermediate2}
\relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash H} \Eq_{\rho} [\phi :=
  T^{n}_{\Eq_{\rho}} K_0^{\rel}] \overline{[\alpha :=
    \Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}]} \\
= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash H} \Eq_{\rho} [\phi
  := \Eq_{(T^\set_\rho)^n K_0^{\set}}] \overline{[\alpha :=
    \Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}]}
\end{multline}
We prove~\eqref{eq:iel-fix-point-intermediate1}.  The case $n=0$ is
trivial, because $T^0_{\Eq_{\rho}} K_0^{\rel} = K_0^{\rel}$ and
$(T^\set_\rho)^0 K_0^{\set} = K_0^{\set}$; the inductive step is
proved by the following sequence of equalities:
\[
\begin{split}
T^{n+1}_{\Eq_{\rho}} K_0^{\rel}\, \overline{\Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}}
&= T^\rel_{\Eq_{\rho}} (T^{n}_{\Eq_{\rho}} K_0^{\rel})
...
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash H} \Eq_{\rho} [\phi
  := \Eq_{(T^\set_\rho)^n K_0^{\set}}] \overline{[\alpha :=
    \Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}]} \\ 
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash H} \Eq_{\rho [\phi
    := (T^\set_\rho)^n K_0^{\set}] \overline{[\alpha :=
      \setsem{\Gamma; \Phi \vdash \tau}\rho]}} \\ 
&= \Eq_{\setsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash H} \rho [\phi
    := (T^\set_\rho)^n K_0^{\set}] \overline{[\alpha :=
      \setsem{\Gamma; \Phi \vdash \tau}\rho]}} \\ 
&= \Eq_{(T^\set_\rho)^{n+1} K_0^{\set} \overline{\setsem{\Gamma; \Phi
      \vdash \tau}\rho}} \\ 
&= \Eq_{(T^\set_\rho)^{n+1} K_0^{\set}} \overline{\Eq_{\setsem{\Gamma;
      \Phi \vdash \tau}\rho}} 
\end{split}
\]
...
& \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash \phi
    \ol{\sigma}} \Eq_{\rho} [\phi := T^{n}_{\Eq_{\rho}} K_0^{\rel}]
  \overline{[\alpha := \Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}]}
  \\
&= T^{n}_{\Eq_{\rho}} K_0^{\rel}\, \overline{\relsem{\Gamma; \Phi,
...
&= T^{n}_{\Eq_{\rho}} K_0^{\rel}\, \overline{\relsem{\Gamma; \Phi,
      \phi, \ol{\alpha} \vdash \sigma} \Eq_{\rho [\phi := (T^\set_\rho)^{n}
        K_0^{\set}] \overline{[\alpha := \setsem{\Gamma; \Phi \vdash
            \tau} \rho]}}} \\ 
&= T^{n}_{\Eq_{\rho}} K_0^{\rel}\, \overline{\Eq_{\setsem{\Gamma;
        \Phi, \phi, \ol{\alpha} \vdash \sigma} \rho [\phi :=
        (T^\set_\rho)^{n} K_0^{\set}] \overline{[\alpha :=
          \setsem{\Gamma; \Phi \vdash \tau} \rho]}}} \\ 
&= \Eq_{(T^\set_\rho)^{n} K_0^{\set}} \,\overline{\Eq_{\setsem{\Gamma;
        \Phi, \phi, \ol{\alpha} \vdash \sigma} \rho [\phi :=
        (T^\set_\rho)^{n} K_0^{\set}] \overline{[\alpha :=
          \setsem{\Gamma; \Phi \vdash \tau} \rho]}}} \\ 
&= \Eq_{(T^\set_{\rho})^{n} K_0^{\set}} \overline{\relsem{\Gamma;
      \Phi, \phi, \ol{\alpha} \vdash \sigma} \Eq_{\rho} [\phi :=
      \Eq_{(T^\set_{\rho})^{n} K_0^{\set}}] \overline{[\alpha :=
        \Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}]}} \\ 
&= \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash \phi \ol{\sigma}}
  \Eq_{\rho} [\phi := \Eq_{(T^\set_{\rho})^{n} K_0^{\set}}]
  \overline{[\alpha := \Eq_{\setsem{\Gamma; \Phi \vdash \tau}\rho}]} 
\end{split}
\]
...
on the $\sigma$s, and the fifth is by the induction hypothesis on $n$
...
\begin{lemma}\label{lemma:substitution}
  We have that:
  \begin{itemize}
  \item for $\Gamma;\Phi,\alpha \vdash F$
  and $\Gamma;\Phi \vdash \tau$,
    \begin{equation}\label{eq:set-subs-var}
      \setsem{\Gamma;\Phi \vdash F[\alpha := \tau]}\rho =
      \setsem{\Gamma;\Phi,\alpha \vdash F}\rho[\alpha :=
      \setsem{\Gamma;\Phi \vdash \tau}\rho]
    \end{equation}
    and
    \begin{equation}\label{eq:rel-subs-var}
      \relsem{\Gamma;\Phi \vdash F[\alpha := \tau]}\rho =
      \relsem{\Gamma;\Phi, \alpha \vdash F}\rho[\alpha :=
      \relsem{\Gamma;\Phi \vdash \tau}\rho]
    \end{equation}
  \item for $\Gamma;\Phi,\phi^k\vdash F$
    and $\Gamma;\Phi,\alpha_1...\alpha_k \vdash H$,
    \begin{equation}\label{eq:set-subs-const}
      \setsem{\Gamma; \Phi \vdash F[\phi := H]}\rho
      = \setsem{\Gamma; \Phi, \phi \vdash F}\rho
      [\phi := \setsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\alpha := \text{--}]]
    \end{equation}
    and
    \begin{equation}\label{eq:rel-subs-const}
      \relsem{\Gamma; \Phi \vdash F[\phi := H]}\rho
      = \relsem{\Gamma; \Phi, \phi \vdash F}\rho
      [\phi := \relsem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\alpha := \text{--}]]
    \end{equation}
  \end{itemize}
\end{lemma}
\begin{proof}
Equations~\ref{eq:set-subs-var} and~\ref{eq:rel-subs-var} are special cases of,
respectively, equations~\ref{eq:set-subs-const} and~\ref{eq:rel-subs-const}.
Though, it is convenient to prove equations~\ref{eq:set-subs-var}
and~\ref{eq:rel-subs-var} first, and then use those to prove
equations~\ref{eq:set-subs-const} and~\ref{eq:rel-subs-const}.

The proofs for the set interpretation and those for the relational interpretation
are completely analogous, so we shall make the argument more abstract
by using a generic interpretation $\sem{-}$.

We prove equations~\ref{eq:set-subs-var} and~\ref{eq:rel-subs-var}.
We proceed by induction on the structure of $F$.
\begin{itemize}
  \item $F : \T$ does not contain any type constructor variable to replace,
    so there is nothing to prove.
  \item $F = \onet, \zerot$ do not contain any type constructor variable to replace,
    so there is nothing to prove.
  \item for $F = F_1 \times F_2, F_1 + F_2$,
    the substitution distributes over the product/sum
    and it then suffices to apply the induction hypothesis.
  \item for $F = \beta$ with $\beta \neq \alpha$, there is nothing to prove.
  \item for $F = \alpha$, we have
  \[
  \sem{\Gamma;\Phi\vdash\alpha[\alpha := \tau]}\rho
  = \sem{\Gamma;\Phi,\alpha\vdash\alpha}\rho[\alpha := \sem{\Gamma;\Phi\vdash\tau}\rho]
  \]
  as both are equal to $\sem{\Gamma;\Phi\vdash\tau}\rho$.
  \item for $F = \phi^k \sigma_1 ... \sigma_k$, we have that
  \[
  \begin{split}
  \sem{\Gamma; \Phi \vdash (\phi \overline{\sigma})[\alpha := \tau]}\rho
    &=\sem{\Gamma; \Phi \vdash \phi (\overline{\sigma[\alpha := \tau]})}\rho \\
    &=(\rho\phi)\overline{\sem{\Gamma;\Phi\vdash\sigma[\alpha := \tau]}\rho} \\
    &=(\rho\phi)\overline{
      \sem{\Gamma;\Phi,\alpha\vdash\sigma}
      \rho [\alpha := \sem{\Gamma;\Phi\vdash\tau}] } \\
    &=\sem{\Gamma; \Phi, \alpha \vdash \phi \overline{\sigma}}
      \rho [\alpha := \sem{\Gamma;\Phi\vdash\tau}]
  \end{split}
  \]
  where the third equality is by the induction hypothesis.
  \item for $F = (\mu \psi. \lambda \beta_1 ... \beta_k. K) \sigma_1 ... \sigma_k$
  we have that 
  \[
  \begin{split}
  &\hspace{-0.3in} \sem{\Gamma; \Phi
      \vdash ((\mu \psi. \lambda \overline{\beta}. K) \overline{\sigma})
      [\alpha := \tau]}\rho \\
    &=\sem{\Gamma; \Phi
      \vdash (\mu \psi. \lambda \overline{\beta}. K[\alpha := \tau])
      (\overline{\sigma [\alpha := \tau]})}\rho \\
    &=\mu(\sem{\Gamma; \Phi, \psi, \overline{\beta} \vdash K[\alpha := \tau]}
      \rho[\psi := \text{--}][\overline{\beta := \text{--}}])
      (\overline{\sem{\Gamma;\Phi \vdash \sigma [\alpha := \tau]}\rho}) \\
    &=\mu(
      \sem{\Gamma; \Phi, \psi, \overline{\beta}, \alpha \vdash K}
      \rho
      [\alpha := \sem{\Gamma;\Phi \vdash \tau}\rho]
      [\psi := \text{--}]
      [\overline{\beta := \text{--}}]
    ) \\
  ...
  &=\sem{\Gamma;\Phi,\alpha\vdash(\mu\psi.\lambda\overline{\beta}.K)\overline{\sigma}}\rho
      [\alpha := \sem{\Gamma;\Phi \vdash \tau}\rho]
  \end{split}
  \]
  where the third equality is by the induction hypothesis.
...
We prove equations~\ref{eq:set-subs-const} and~\ref{eq:rel-subs-const}.
We proceed by induction on the structure of $F$.
\begin{itemize}
  \item $F : \T$ does not contain any type constructor variable to replace,
    so there is nothing to prove.
  ...
  \item for $F = F_1 \times F_2, F_1 + F_2$,
    the substitution distributes over the product/sum
    and it then suffices to apply the induction hypothesis.
  \item for $F = \phi^k \tau_1 ... \tau_k$, we have that
  \[
  \begin{split}
  \sem{\Gamma; \Phi \vdash (\phi \overline{\tau})[\phi := H]}\rho
  &=\sem{\Gamma; \Phi \vdash H[\overline{\alpha:= \tau[\phi := H]}]}\rho \\
  &=\sem{\Gamma; \Phi \vdash H}\rho
    [\overline{\alpha:= \sem{\Gamma;\Phi\vdash\tau[\phi := H]}\rho}] \\
  &=\sem{\Gamma; \Phi \vdash H}\rho
    [\overline{
      \alpha:= \sem{\Gamma;\Phi,\phi\vdash\tau}
      \rho
      [\phi := \sem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
    }] \\
  &=\sem{\Gamma; \Phi, \phi \vdash \phi \overline{\tau}}\rho
    [\phi := \sem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
  \end{split}
  \]
  where the second equality is for equations~\ref{eq:set-subs-var}
  and~\ref{eq:rel-subs-var},
  and the third equality is by the induction hypothesis.
  \item for $F = (\mu \psi. \lambda \beta_1 ... \beta_k. K) \tau_1 ... \tau_k$
  we have that 
  \[
  \begin{split}
  &\hspace{-0.3in} \sem{\Gamma; \Phi
    \vdash ((\mu \psi. \lambda \overline{\beta}. K) \overline{\tau})[\phi := H]}\rho \\
  &=\sem{\Gamma; \Phi
    \vdash (\mu \psi. \lambda \overline{\beta}. K[\phi := H])
    (\overline{\tau[\phi := H]})}\rho \\
  &=\mu(\sem{\Gamma; \Phi, \psi, \overline{\beta} \vdash K[\phi := H]}
    \rho[\psi := \text{--}][\overline{\beta := \text{--}}])
    (\overline{\sem{\Gamma;\Phi \vdash \tau[\phi := H]}\rho}) \\
  &=\mu(
      \sem{\Gamma; \Phi, \psi, \overline{\beta}, \phi \vdash K}
      \rho
      [\phi := \sem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
      [\psi := \text{--}]
      [\overline{\beta := \text{--}}]
    ...
    &\hspace{0.5in} (\overline{
      \sem{\Gamma;\Phi,\phi \vdash \tau}
      \rho
      [\phi := \sem{\Gamma;\Phi,\overline{\alpha}\vdash H}
        \rho[\overline{\alpha := \text{--}}]]
    }) \\
  &=\sem{\Gamma;\Phi,\phi\vdash(\mu\psi.\lambda\overline{\beta}.K)\overline{\tau}}\rho
      [\phi := \sem{\Gamma;\Phi,\overline{\alpha}\vdash H}\rho[\overline{\alpha := \text{--}}]]
  \end{split}
  \]
  where the third equality is by the induction hypothesis.
...
\item We show that
  ...
  \]
  %is a natural transformation in $\overline{A}$...
  ...
  \in
  %\colon \relsem{\Gamma;\overline{\alpha} \vdash  F} \Eq_{\rho}[\overline{\alpha := R}]
  %\to
...
\item To define $\setsem{\Gamma;\Phi \,|\, \Delta \vdash
  ...
  follows from the naturality of $\textit{in}$.
  Indeed the diagram
  % for any morphism of set environments $f : \rho \to \rho'$,
  \[
  \begin{tikzcd}
  \setsem{\Gamma;\Phi \vdash
    H[\phi := \mu \phi.\lambda {\overline \alpha}.H]
  ...
  % \end{tikzcd}
  % \]
  commutes for any morphism of environments $f : \rho \to \rho'$.
  
  We proceed analogously to define the relational interpretation of the same term.
\item For the $\fold$ case, we shall check only the $\rel$-interpretation,
  as the $\set$-interpretation is analogous.
  $\relsem{\Gamma;\emptyset \,|\, \Delta \vdash
  \fold_{H,F}\, t : \Nat^{\overline\alpha}\,((\mu \phi.\lambda \overline{\beta}.H)\overline{\alpha})\,F}\rho$
  is defined as
  \[
  \begin{tikzcd}
  \relsem{\Gamma;\emptyset \vdash \Delta}\rho
  \ar[d, "{\relsem{\Gamma;\emptyset \,|\, \Delta \vdash t : 
    \Nat^{\overline\alpha}\,(H[\phi := F][\overline{\beta := \alpha}])\,F}\rho}"] \\
  \relsem{\Gamma;\emptyset \vdash \Nat^{\overline\alpha}\,(H[\phi := F][\overline{\beta := \alpha}])\,F}\rho
    \ar[d, "{\text{\textit{fold}}}"] \\
  \relsem{\Gamma;\emptyset \vdash \Nat^{\overline\alpha}\,((\mu \phi.\lambda \overline{\beta}.H)\overline{\alpha})\,F}\rho
  \end{tikzcd}
  \]
  We want to show that $\textit{fold}$ is well defined.
  Let $\eta \in \relsem{\Gamma;\emptyset \vdash \Nat^{\overline\alpha}\,(H[\phi := F][\overline{\beta := \alpha}])\,F}\rho$.
  For all $\overline{R : \rel(A, B)}$, we have that
  $\eta_{\overline R}$ is a morphism in $\rel$.
...
\item To define $\setsem{\Gamma;\Phi \,|\, \Delta \vdash t_{\overline \tau} s:
 ...
 \setsem{\Gamma;\Phi \vdash G [\overline{\alpha := \tau}]}\rho
  \end{tikzcd}
  \]
  we need Lemma~\ref{lemma:substitution}.
