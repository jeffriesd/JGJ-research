...
Definition~\ref{def:set-sem}. The action of $T^\set$ on a morphism $f
: \rho \to \rho'$ is the higher-order natural transformation
{\color{blue} May want to use $T^\set_{H,f}$ rather than $\sigma^\set_f$
  in final version.} $\sigma^\set_f : T^\set_\rho \to T^\set_{\rho'}$
whose action on any $F : [\set^k,\set]$ is the natural transformation
$\sigma^\set_f\, F : T^\set_\rho\,F \to T^\set_{\rho'}\,F$ whose
component at $\ol{A}$ is
...
\item $\Gamma; \Phi~|~\Delta \vdash (L_{\ol{\alpha}}x.t)_{\ol{\tau}}s
  = t [\ol{\alpha := \tau}][x := s]$
\item $\fold^F_H\,k \circ \tin_H = k \circ
  \map_H^{\mu H, F}\,(\fold^F_H\,k)$ 
\end{itemize}
The latter is equivalent to
$(\fold^F_H\,k)_{\ol{\sigma}\,\ol{\tau}}\,((\tin_H)_{\ol{\sigma}\,\ol{\tau}}\,t)
= k_{\ol{\sigma}\,\ol{\tau}}\, (
  \map_H^{\mu H, F}\,(\fold^F_H\,k)_{\ol{\sigma}\,\ol{\tau}}\, t)$ for all
...
\[ E_{F,B} = \rho[\ol{\gamma := B}][\ol{\phi := \lambda \ol{A}.\, \setsem{\Gamma;
      \ol{\gamma},\ol{\beta} \vdash F}\rho[\ol{\gamma := B}][\ol{\beta
        := A}]}]\] and
\[ e_F = \id_\rho[\ol{\gamma := f}][\ol{\phi := \lambda \ol{A}.\, \setsem{\Gamma;
...
= & \relsem{\Gamma;\ol{\gamma} \vdash H[\ol{\phi := F}]}\Eq_\rho[\ol{\gamma
    := R}] \to \setsem{\Gamma;\ol{\gamma} \vdash H[\ol{\phi :=
...
\ol{B}\] to \[\setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash
  F}\rho[\ol{\beta := B}][\ol{\gamma := C}]\] To see this, we use
...
\ol{B}\,\ol{C}.\,\setsem{\Gamma;\ol{\beta},\ol{\gamma},\ol{\alpha}  
  \vdash H[\phi := F]}\rho[\ol{\beta := B}][\ol{\gamma :=
    C}][\ol{\alpha := B}]\\ 
= & \lambda \ol{B}\,\ol{C}\,(\lambda \ol{A}.\, \setsem{\Gamma;
...
\ar[r,"{(\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
 ...
 \setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash F}\rho[\ol{\gamma :=
    C'}][\ol{\beta := B'}]
\end{tikzcd}\]
Indeed, {\color{red} naturality of $\mathit{fold}$ with respect to its functor and
...
the bottom one commutes.  We therefore have that
\[\lambda \ol{B}\,\ol{C}.\,(\setsem{\Gamma; \emptyset~|~\emptyset
...
\item To see that, for every $\rho : \setenv$, $d \in \setsem{\Gamma;
  \emptyset \vdash \Delta}\rho$, and $\eta : \setsem{\Gamma; \emptyset
