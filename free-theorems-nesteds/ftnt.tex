...
type can can be thought of as representing the standard arrow type
$\Gamma \vdash \sigma \to \tau$ in our calculus.  Our formation rules
...
\ol{\alpha} \vdash H : \F$ with $\ol{\alpha} = \{\alpha_1,...,\alpha_k\}$
and $k \geq 1$, then $\Gamma;\Phi \vdash F[\phi := H] : \F$, where the
operation $(\cdot)[\phi := H]$ of {\em second-order type constructor
...
$x$ in $t$.  Moreover, in the rule for $\tin_H$, the variables in
$\ol{\beta}$ are fresh with respect to $H$. Substitution for terms is
the obvious extension of the usual capture-avoiding textual
substitution. Definition~\ref{def:well-formed-terms} ensures that the
expected weakening rules for well-formed terms hold.
...
\item If $\Gamma;\Phi \vdash (\mu \phi.\lambda
  \ol{\alpha}. H)\ol{\tau}$ then letting $\sigma^\set_f : T^\set_\rho
...
\end{itemize}
\end{dfn}
The interpretation of $\Nat$-types ensures that if $\Gamma \vdash
\sigma \to \tau$ then $\setsem{\Gamma\vdash \sigma \to \tau}f =
\id_{\setsem{\Gamma \vdash \sigma \to \tau}\rho}$, as expected.
...
\item If $\Gamma;\Phi \vdash (\mu \phi^k.\lambda
  \ol{\alpha}. H)\ol{\tau}$ then letting $\sigma_f : T_\rho \to
  T_{\rho'}$ be the map
...
The interpretation of $\Nat$-types ensures that if $\Gamma;\emptyset
\vdash \sigma \to \tau$ then $\relsem{\Gamma \vdash \sigma \to \tau}f
= \id_{\relsem{\Gamma \vdash \sigma \to \tau}\rho}$, as expected. To
see that the functors given by Definitions~\ref{def:rel-sem}
...
\hspace*{1.24in}(\mu \phi.\lambda {\overline \alpha}.H){\overline
  \beta} \rrbracket^\set \rho & & \\
\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
  \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu
  \phi.\lambda \overline \alpha.H)\overline \beta\;F}\rho & = & 
\lambda d\,\ol{B}\,\ol{C}.\, (\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C}]}} (\lambda \ol{A}.\, (\setsem{\Gamma;\emptyset \,|\,
  \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
    F]\;F[\overline{\beta := \alpha}]}\rho\,d)_{\ol{A}\,\ol{C}}))_{\ol{B}}
\end{array}\]
\end{dfn}
...
Nat^{\ol{\beta},\ol{\gamma}} \, H[\phi := (\mu \phi.\lambda {\overline
    \alpha}.H){\overline \beta}][\ol{\alpha := \beta}] & = &
\lambda d\,\ol{R}\, \ol{S}.\,(\mathit{in}_{T^\set_{\rho[\ol{\gamma := S}]}})_{\ol{R}}\\
\hspace*{1.24in}(\mu \phi.\lambda {\overline \alpha}.H){\overline
  \beta} \rrbracket^\rel \rho & & \\
\relsem{\Gamma;\emptyset \,|\, \Delta \vdash
  \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu
  \phi.\lambda \overline \alpha.H)\overline \beta\;F}\rho & = & 
\lambda d\,\ol{R}\,\ol{S}.\, (\mathit{fold}_{T_{\rho[\ol{\gamma :=
        S}]}} (\lambda \ol{V}.\, (\relsem{\Gamma;\emptyset \,|\,
  \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
    F]\;F[\overline{\beta := \alpha}]}\rho\,d)_{\ol{V}\,\ol{S}}))_{\ol{R}}
%\mathit{fold}_{T_\rho} \circ \relsem{\Gamma;\emptyset \,|\, \Delta
%  \vdash t : \Nat^{\ol \beta, \ol \gamma}\,(H[\phi :=
%    F][\overline{\alpha := \beta}])\,F}\rho 
\end{array}\]
\end{dfn}
...
\end{itemize}
Finally, direct calculation reveals that the set and relational
interpretations of terms respect the judgments
\begin{itemize}
\item $\Gamma; \Phi~|~\Delta \vdash (L_{\ol{\alpha}}x.t)_{\ol{\tau}}s
  = t [\ol{\alpha := \tau}][x := s]$
\item $\fold_{H,F}\,k \circ \tin_H = k \circ
  {\color{blue}map}\,(\fold_{H,F}\,k)$
\end{itemize}
The latter is equivalent to
$(\fold_{H,F}\,k)_{\ol{\sigma}\,\ol{\tau}}\,((\tin_H)_{\ol{\sigma}\,\ol{\tau}}\,t)
= k_{\ol{\sigma}\,\ol{\tau}}\, ({\color{blue}
  map}\,(\fold_{H,F}\,k)_{\ol{\sigma}\,\ol{\tau}}\, t)$ for all
$\ol{\sigma}$ and $\ol{\tau}$.  The set and relational interpretations
of terms therefore respect the congruence closed equational theory
...
\item {\color{red} Perhaps change this bullet point just to say ``The
  proofs that $\relsem{\Gamma;\emptyset \,|\, \Delta \vdash
    L_{\overline{\alpha}} x.t : \Nat^{\overline{\alpha}} \,F \,G}$ is
  a natural transformation from $\relsem{\Gamma;\emptyset \vdash
  ...
  transformation from $\relsem{\Gamma;\ol{\alpha} \vdash
    ...
    G}\rho[\ol{\alpha := \_}]$ that satisfies the additional condition
  necessary for it to be in $\relsem{\Gamma;\emptyset \vdash
    \Nat^{\overline{\alpha}} \,F \,G}\rho$, are analogous.''}  To see that
  $\relsem{\Gamma;\emptyset \,|\, \Delta \vdash L_{\overline{\alpha}}
  x.t : \Nat^{\overline{\alpha}} \,F \,G}$ is a natural transformation
  from $\relsem{\Gamma;\emptyset \vdash \Delta}$ to
...
\item {\color{red} The proof that $\relsem{\Gamma;\Phi \,|\, \Delta
    \vdash t_{\bf \tau} s: G [\overline{\alpha := \tau}]}$ is a
  natural transformation from $\relsem{\Gamma;\Phi \vdash \Delta}$ to
  $\relsem{\Gamma;\Phi \vdash G [\overline{\alpha := \tau}]}$
is analogous.}
...
\item To see that $\setsem{\Gamma;\emptyset \,|\, \emptyset \vdash
  ...
  it to be in \[\setsem{\Gamma;\emptyset \vdash
    ...
    \;(\mu \phi.\lambda {\overline \alpha}.H){\overline
      \beta}}\,\rho\]
  let $\ol{R : \rel(B,C)}$ and $\ol{S : \rel(B',C')}$. Then
\[\begin{array}{ll}
...
T_{\rho[\ol{\gamma := C'}]}\, (\mu T_{\rho[\ol{\gamma := C'}]}) \,
\ol{B'} \to (\mu T_{\rho[\ol{\gamma := C'}]}) \,\ol{B'} \, )\\
= & \relsem{\Gamma;\ol{\beta},\ol{\gamma} \vdash (\mu \phi.\lambda
  \ol{\alpha}.H)\ol{\beta}} \Eq_\rho[\ol{\beta:= R}][\ol{\gamma :=S}]
\end{array}\]
\item {\color{red} The proofs that $\relsem{\Gamma;\emptyset \,|\,
  ...
  \ol{R}\,\ol{S}.\,\relsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash
    ...
    \ol{\beta},\ol{\gamma} \vdash (\mu \phi.\lambda {\overline
      ...
      S}]$ that satisfies the additional condition necessary for it to
  be in $\relsem{\Gamma;\emptyset \vdash Nat^{\ol{\beta},\ol{\gamma}}
    \, H[\phi := (\mu \phi.\lambda {\overline \alpha}.H){\overline
        \beta}][\ol{\alpha := \beta}] \;(\mu \phi.\lambda {\overline
      \alpha}.H){\overline \beta}}\rho$, are analogous.}
\item Finally, to see that $\pi_i(\relsem{\Gamma;\emptyset \,|\,
  \emptyset \vdash \tin_H : Nat^{\ol{\beta},\ol{\gamma}} \, H[\phi :=
    (\mu \phi.\lambda {\overline \alpha}.H){\overline
      \beta}][\ol{\alpha := \beta}] \;(\mu \phi.\lambda {\overline
...
\item To see that $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
  \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu
  \phi.\lambda \overline \alpha.H)\overline \beta\;F}$ is a natural
  transformation from $\setsem{\Gamma;\emptyset \vdash \Delta}$ to
  $\setsem{\Gamma;\emptyset \vdash \Nat^{{\ol{\beta},\ol{\gamma}}
    }\,(\mu \phi.\lambda \overline \alpha.H)\overline \beta\;F}$,
  since the functorial part $\Phi$ of the type context is empty, we
  need only show that, for every $\rho : \setenv$,
  \[\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
    \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F} \rho\] is a morphism in $\set$
  from $\setsem{\Gamma;\emptyset \vdash \Delta}\rho$ to
  $\setsem{\Gamma;\emptyset \vdash \Nat^{{\ol{\beta},\ol{\gamma}}
    }\,(\mu \phi.\lambda \overline \alpha.H)\overline \beta\;F}\rho$,
  i.e., that, for any $d : \setsem{\Gamma;\emptyset \vdash \Delta}
  \rho$, $\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\, \rho\,d$ is a natural transformation
  from \[\lambda \ol{B}\,\ol{C}.\, \setsem{\Gamma;
    \ol{\beta},\ol{\gamma} \vdash (\mu \phi.\lambda \overline
    \alpha.H)\overline \beta}\rho[\ol{\beta := B}][\ol{\gamma := C}]\]
  to \[\lambda \ol{B}\,\ol{C}.\, \setsem{\Gamma;
    \ol{\beta},\ol{\gamma} \vdash F}\rho[\ol{\beta := B}][\ol{\gamma
      := C}]\] We first observe that, for any $\ol{B}$ and $\ol{C}$,
  \[(\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\, \rho\,d)_{\ol{B}\,\ol{C}}\] is a
  ...
  \ol{B}\] to \[\setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash
    F}\rho[\ol{\beta := B}][\ol{\gamma := C}]\] To see this, let $x :
  \setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash (\mu \phi.\lambda
    \overline \alpha.H)\overline \beta}\rho[\ol{\beta :=
      B}][\ol{\gamma := C}]$.  Using Equations~\ref{eq:subs-var}
  and~\ref{eq:subs-const} for the first and second equalities below,
  together with weakening, the induction hypothesis ensures that
  $(\setsem{\Gamma;\emptyset \,|\, \Delta \vdash t : \Nat^{\ol \beta,
      \ol \gamma}\,H[\phi := F][\ol{\alpha := \beta}]\;F}\rho\,d)$ is
...
\ol{B}\,\ol{C}.\,\setsem{\Gamma;\ol{\beta},\ol{\gamma},\ol{\alpha}  
  \vdash H[\phi := F]}\rho[\ol{\beta := B}][\ol{\gamma :=
    C}][\ol{\alpha := B}]\\ 
= & \lambda \ol{B}\,\ol{C}\,\ol{A}.\, \setsem{\Gamma;
  \ol{\beta},\ol{\gamma},\ol{\alpha},\phi \vdash H}\rho
[\ol{\beta := B}][\ol{\gamma :=
    C}][\ol{\alpha := B}]\\
 & \hspace*{0.5in}[\phi := \setsem{\Gamma;\ol{\beta},
    \ol{\gamma},\ol{\alpha} \vdash F}\rho[\ol{\beta := B}][\ol{\gamma
      := C}][\ol{\alpha := B}][\ol{\beta := A}]]\\ 
= & \lambda \ol{B}\,\ol{C}\,\ol{A}.\, \setsem{\Gamma;
  \ol{\gamma},\ol{\alpha},\phi \vdash H}\rho[\ol{\gamma :=
    C}][\ol{\alpha := B}][\phi := \setsem{\Gamma;\ol{\beta},
    \ol{\gamma} \vdash F}\rho[\ol{\beta := B}][\ol{\gamma
      := C}][\ol{\beta := A}]]\\ 
= & \lambda \ol{B}\,\ol{C}\,\ol{A}.\, \setsem{\Gamma;
  \ol{\gamma},\ol{\alpha},\phi \vdash H}\rho[\ol{\gamma :=
    C}][\ol{\alpha := B}][\phi := \setsem{\Gamma;
    \ol{\gamma},\ol{\beta} \vdash F}\rho[\ol{\gamma := C}][\ol{\beta
      := A}]]\\ 
=& \lambda \ol{B}\,\ol{C}.\,T^\set_{\rho[\ol{\gamma:=
      C}]}\,(\setsem{\Gamma;\ol{\beta},\ol{\gamma} 
  \vdash F}\rho[\ol{\beta := B}][\ol{\gamma := C}])
\end{array}\]
to 
\[\lambda \ol{B}\,\ol{C}.\,\setsem{\Gamma;\ol{\beta},\ol{\gamma}
  \vdash F}\rho[\ol{\beta := B}][\ol{\gamma := C}]\]
...
\[\begin{array}{ll}
 & (\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\, \rho\,d)_{\ol{B}\,\ol{C}}\,x\\
=& (\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C}]}} (\lambda \ol{A}.\, (\setsem{\Gamma;\emptyset \,|\,
  \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
    F][\ol{\alpha := \beta}]\;F}\rho\,d)_{\ol{A}\,\ol{C}}))_{\ol{B}}\,x\\
:&\setsem{\Gamma;\ol{\beta},\ol{\gamma} \vdash F}\rho[\ol{\beta :=
    B}][\ol{\gamma := C}]
\end{array}\]
i.e., that for each $\ol{B}$ and $\ol{C}$
\[(\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
    : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\;F}\, \rho\,d)_{\ol{B}\,\ol{C}}\] is a
...
\[\begin{array}{ll}
 & (\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
  \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\;F}\,\rho\,d)_{\ol{B}\,\ol{C}}\\
=& (\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C}]}} (\lambda \ol{A}.\, (\setsem{\Gamma;\emptyset \,|\,
  \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
    F][\ol{\alpha :=
      \beta}]\;F}\rho\,d)_{\ol{A}\,\ol{C}}))_{\ol{B}}
\end{array}\]
Writing $t_{\ol{C}}$ for $\lambda \ol{A}.\, (\setsem{\Gamma;\emptyset
  \,|\, \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
    F][\ol{\alpha := \beta}]\;F}\rho\,d)_{\ol{A}\,\ol{C}})$, we
observe that the following diagram commutes for all $\ol{f : B \to
  B'}$ and $\ol{g : C \to C'}$:
\[\begin{tikzcd}[column sep=2.5in, row sep=0.75in]
(\mu T^\set_{\rho[\ol{\gamma := C}]})\, \ol{B}
\ar[d, "{(\mu \sigma_{\id_\rho[\ol{\gamma := g}]}) \, \ol{B}}"'] \ar[r, 
  "{(\mathit{fold}_{T^\set_{\rho[\ol{\gamma := C}]}}\,t_{\ol{C}})_{\ol{B}}}"]
& \setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash F}\rho[\ol{\gamma :=
    C}][\ol{\beta := B}]\ar[d, "{\setsem{\Gamma;
      \ol{\beta},\ol{\gamma} \vdash F}\id_\rho[\ol{\gamma := 
    g}][\ol{\beta := \id_B}]}"]\\
(\mu T^\set_{\rho[\ol{\gamma := C'}]})\, \ol{B} 
\ar[r,"{(\mathit{fold}_{T^\set_{\rho[\ol{\gamma := C'}]}}\,t_{\ol{C'}})_{\ol{B}}\,}" ]
...
\ar[r,"{(\mathit{fold}_{T^\set_{\rho[\ol{\gamma := C'}]}}\,t_{\ol{C'}})_{\ol{B'}}\,}" ] & 
 \setsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash F}\rho[\ol{\gamma :=
    \id_{C'}}][\ol{\beta := B'}]
\end{tikzcd}\]
Indeed, naturality of $\mathit{fold}$ with respect to its functor and
function arguments ensures that the top diagram commutes and
naturality of $\mathit{fold}_{T^\set_{\rho[\ol{\gamma :=
        C'}]}}\,t_{\ol{C'}}$ ensures that the bottom one commutes.  We
therefore have that $\lambda
\ol{B}\,\ol{C}.\,(\setsem{\Gamma;\emptyset \,|\, \Delta \vdash
  \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu
  \phi.\lambda \overline \alpha.H)\overline
  \beta\;F}\,\rho\,d)_{\ol{B}\,\ol{C}}$ is natural in $\ol{B}$ and
$\ol{C}$ as desired.
\item
To see that, for every set environment $\rho$ and $d \in
\setsem{\Gamma; \emptyset \vdash \Delta}\rho$,
\[\setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
    \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\,\rho\,d\] satisfies the additional
  condition necessary for it to be in $\setsem{\Gamma;\emptyset \vdash
    \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}\,\rho$, let $\ol{R : \rel(B,C)}$ and
  $\ol{S : \rel(B',C')}$. By the induction hypothesis,
  \[\eta = \setsem{\Gamma;\emptyset \,|\,
  \Delta \vdash t : \Nat^{\ol \beta, \ol \gamma}\,H[\phi :=
    F]\;F[\overline{\beta := \alpha}]}\rho\,d\]
  satisfies
  the additional condition necessary for it to be in $\setsem{\Gamma;
    \emptyset \vdash \Nat^{\ol{\beta}, \ol{\gamma}}\,(H[\phi :=
      F][\ol{\alpha := \beta}])\,F} \rho$. Thus
\[\begin{array}{lll}
 (\eta_{\ol{B}\,\ol{C}}\,,
\eta_{\ol{B'}\,\ol{C'}}) & \in & 
...
so that
\[\begin{array}{ll}
 & (\; \lambda \ol{Y}\,\ol{Z}.\,(\mathit{fold}_{T_\rho[\ol{\gamma := Z}]}\,(\lambda
\ol{X}.\,\eta_{\ol{X}\,\ol{Z}})_{\ol{Y}})_{\ol{B}\,\ol{C}}, \;
\lambda \ol{Y}\,\ol{Z}.\,(\mathit{fold}_{T_\rho[\ol{\gamma := Z}]}\,(\lambda
\ol{X}.\,\eta_{\ol{X}\,\ol{Z}})_{\ol{Y}})_{\ol{B'}\,\ol{C'}}\;)\\
= & (\; \mathit{fold}_{T_\rho[\ol{\gamma := C}]}\,(\lambda
\ol{X}.\,\eta_{\ol{X}\,\ol{C}})_{\ol{B}}, \;
\mathit{fold}_{T_\rho[\ol{\gamma := C'}]}\,(\lambda
\ol{X}.\,\eta_{\ol{X}\,\ol{C'}})_{\ol{B'}}\; )
\end{array}\]
has type
...
= & \relsem{\Gamma; \ol{\gamma},\ol{\beta} \vdash (\mu \phi. \lambda
  \ol{\alpha}. H)\ol{\beta}}\Eq_\rho[\ol{\gamma := S}][\ol{\alpha := R}] \to
\relsem{\Gamma;\ol{\gamma},\ol{\beta} \vdash F}\Eq_\rho[\ol{\gamma := 
    S}][\ol{\beta:= R}]
\end{array}\]
as desired.
\item {\color{red} The proofs that $\relsem{\Gamma;\emptyset \,|\,
    \Delta \vdash \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}}
    }\,(\mu \phi.\lambda \overline \alpha.H)\overline \beta\;F}$ is a
  natural transformation from $\relsem{\Gamma;\emptyset \vdash
    \Delta}$ to $\relsem{\Gamma;\emptyset \vdash
    \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu \phi.\lambda \overline
    \alpha.H)\overline \beta\;F}$ and that, for all $\rho : \relenv$
  and $d : \relsem{\Gamma;\emptyset \vdash \Delta}$,
  \[\relsem{\Gamma;\emptyset \,|\,
    \Delta \vdash \fold_{H,F}\, t : \Nat^{{\ol{\beta},\ol{\gamma}}
    }\,(\mu \phi.\lambda \overline \alpha.H)\overline
    \beta\;F}\,\rho\,d\] is a natural transformation from $\lambda
  \ol{R}\,\ol{S}.\,\relsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash (\mu
    \phi.\lambda {\overline \alpha}.H){\overline \beta}}\rho[\ol{\beta
      := R}][\ol{\gamma := S}]$ to\\ $\lambda
  \ol{R}\,\ol{S}.\,\relsem{\Gamma; \ol{\beta},\ol{\gamma} \vdash
    F}\rho[\ol{\beta := R}][\ol{\gamma := S}]$ that satisfies the
  additional condition necessary for it to be in $\relsem{\Gamma;
    \emptyset \vdash \Nat^{{\ol{\beta},\ol{\gamma}} }\,(\mu
    \phi.\lambda \overline \alpha.H)\overline \beta\;F}\,\rho$, are
  analogous.}

\item Finally, to see that
\[\begin{array}{ll}
  &  \pi_i(\relsem{\Gamma;\emptyset \,|\,
  \Delta \vdash \fold_{H,F}\, t : \Nat^{\ol{\gamma},\overline \beta}\,((\mu
  \phi.\lambda \overline \alpha.H)\overline \beta)\,F}\rho)\\
= & \setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
    \Nat^{\overline \gamma, \ol{\beta}}\,((\mu \phi.\lambda \overline
    \alpha.H)\overline \alpha)\,F}(\pi_i\rho)
\end{array}\]
we compute
\[\begin{array}{ll}
  & \pi_i(\relsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t
  : \Nat^{\overline \gamma,\ol{\beta}}\,((\mu \phi.\lambda \overline
  \alpha.H)\overline \alpha)\,F}\rho)\\
=& \pi_i (\lambda e\,\ol{R}\,\ol{S}.\,(\mathit{fold}_{T_{\rho[\ol{\gamma
          := S}]}} (\lambda \ol{U}.\, (\relsem{\Gamma;\emptyset \,|\,
    \Delta \vdash t : \Nat^{\ol \gamma, \ol \beta}\,H[\phi :=
      F][\ol{\alpha := \beta}]\;F}\rho\,e)_{\ol{U}\,\ol{S}}))_{\ol{R}})\\
=& \lambda e\,\ol{R}\,\ol{S}.\,(\mathit{fold}_{T_{(\pi_i\rho)[\ol{\gamma
          := \pi_i S}]}} (\lambda \ol{U}.\, (\setsem{\Gamma;\emptyset \,|\,
    \Delta \vdash t : \Nat^{\ol \gamma, \ol \beta}\,H[\phi :=
      F][\ol{\alpha := \beta}]\;F}(\pi_i \rho)\,(\pi_ie))_{(\pi_i
  \ol{U})\,(\pi_i \ol{S})}))_{\pi_i \ol{R}}\\
=& \lambda
d\,\ol{B}\,\ol{C}.\,(\mathit{fold}_{T_{(\pi_i\rho)[\ol{\gamma := C}]}}
(\lambda \ol{A}.\, (\setsem{\Gamma;\emptyset \,|\, \Delta \vdash t :
  \Nat^{\ol \gamma, \ol \beta}\,H[\phi := F][\ol{\alpha :=
      \beta}]\;F}\,(\pi_i \rho)\,d)_{\ol{A}\,\ol{C}}))_{\ol{B}}\\
= & \setsem{\Gamma;\emptyset \,|\, \Delta \vdash \fold_{H,F}\, t :
    \Nat^{\overline \gamma, \ol{\beta}}\,((\mu \phi.\lambda \overline
    \alpha.H)\overline \alpha)\,F}(\pi_i\rho)
\end{array}\]
