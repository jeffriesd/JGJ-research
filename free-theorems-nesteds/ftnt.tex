...
\item An object of $\rel$ is a triple $(A,B,R)$ where $R$ is a
  relation between the objects $A$ and $B$ in $\set$. We identify
  $(A,B,R)$ with $R$, and write $R : \rel(A,B)$ when convenient.
...
&\mid \left(\mu \phi^{k}. \lambda \ol{\alpha}.
...
\DisplayProof
\\
\AXC{$\Gamma;\Phi,\ol{\alpha}
  %  ^0_1,...,\alpha^0_k,\phi^k \vdash \tau
  : \mcF$}
\AXC{$\quad\quad\ol{\Gamma;\Phi \vdash \tau : \mcF}$}% \mbox{\,for\,} i = 1,...,k$}
...
(\sigma \times \tau)[\phi := F] & = & \sigma[\phi := F] \times \tau[\phi
  := F]\\[0.5ex]  
((\mu \psi^n. \lambda \ol{\beta}.\, G)\ol{\tau}[\phi :=
  F] & = & (\mu \psi^n. \lambda \ol{\beta}. \,G[\phi :=
  F])\, \ol{\tau[\phi := F]}
\end{array}\]
\end{dfn}
...
\AXC{$\Gamma;\Phi \,|\, \Delta \vdash t : H[\phi := \mu \phi.\lambda 
  \ol{\alpha}.H][\ol{\alpha := A}]$}
\AXC{$\ol{\Gamma;\Phi \vdash A}$}%_i \mbox{\,for\,}i = 1,...,k$}
...
otherwise has no impact on the fact that they are given by mutually
induction.
...
$\setenv$ for the collection of all set environments. 
...
from~\cite{jp19} that, for every $\Gamma; \ol{\alpha} \vdash E : \F$,
$\setsem{\Gamma; \ol{\alpha} \vdash E}$ is actually functorial in
...
T^\set_{\rho'}$ are functors and $f\phi : \rho\phi \to \rho'\phi$ is a
...
with the relation $(A,B,R)$. It will also be convenient to extend the
identification of a relation with its third component as in
Definition~\ref{def:rel} to identify a relation transformer $F =
(F^0,F^1,F^*)$ with its third component $F^*$. In this case we may
continue to write $F^0$ and $F^1$ for $\pi_1 F$ and $\pi_2 F$.  We
...
does a type variable.  We write $\rho[\alpha_1 := R_1,...,\alpha_k :=
  R_k]$ for the relation environment $\rho'$ such that $\rho' \alpha_i
\, = R_i$ for $i = 1,...,k$ and $\rho' \alpha = \rho\alpha$ if $\alpha
\not \in \{\alpha_1,...,\alpha_k\}$. We write $\relenv$ for the
collection of all relation environments.  If $\rho$ is a relation
environment, we write $\pi_1 \rho$ for the set environment mapping
each type variable $\beta$ to $\pi_1 (\rho \beta)$ and each type
constructor variable $\phi$ to the functor $(\rho\phi)^0$. The set
environment $\pi_2 \rho$ is defined analogously.

We define, for each $k$, the notion of an $\omega$-cocontinuous
functor from $\relenv$ to $RT_k$:
\begin{dfn}\label{def:relenv-functor}
An $\omega$-cocontinuous functor $H : [\relenv, RT_k]$ is a triple $H
= (H^0,H^1,H^*)$, where
\begin{itemize}
...
Definition~\ref{def:set-interp}. We have:
...
\begin{dfn}
If $F$ is a functor from $\set^k$ to $\set$, define $\Eq_F^*$
%: \rel^k \to \rel$
as follows.  Given $R_1 : \rel(A_1, B_1),$ $...,R_k : \rel(A_k,B_k)$,
let $\overline{i_{R_i} : R_i \hookrightarrow A_i \times B_i}$ be the
inclusion of $R_i$ as a subset of $A_i \times B_i$. By the universal
property of the product, there exists a unique $h_{\overline{A \times
...
\end{lemma}
\begin{proof}
Consider $\overline{(\alpha, \beta) : R \to S}$, where $\overline{R :
  \rel(A, B)}$ and $\overline{S : \rel(C, D)}$.  We want to show that
there exists a morphism $\epsilon : \Eq_F^*\overline{R} \to
\Eq_F^*\overline{S}$ such that
\[
\begin{tikzcd}
\Eq_F^*\overline{R}
...
\]
commutes.
By hypothesis, there exists $\overline{\gamma : R \to S}$
...
If $F : [\set^k,\set]$ and $A_1,...,A_k : \set$, then $\Eq_F^*
  \overline{\Eq^*_A} = \Eq^*_{F\overline{A}}$.
\end{lemma}
\begin{proof}
The relation $\overline{\Eq^*_A}$ corresponds to the subobject
$\overline{\Delta_{A} : A \to A \times A}$.
Since $h_{\overline{\Eq_A}}$ is the unique morphism making the diagram
\[
\begin{tikzcd}[row sep = large]
F\overline{A}
&F(\overline{A \times A})
...
\Eq_{\rho v}$ for all $v \in \V \cup \tvars$. The Identity Extension
Lemma can then be stated and proved as follows:
