...
If \,$\Gamma; \Phi,\phi^k \vdash F : \F$ with $k \geq 1$, and if\,
$\Gamma;\Phi, \ol{\alpha} \vdash H : \F$ with $|\ol\alpha| = k$, then
$\Gamma;\Phi \vdash F[\phi :=_{\ol{\alpha}} H] : \F$. {\color{blue}
  Similarly, if \,$\Gamma, \phi^k; \Phi \vdash F : \F$ with $k \geq
  1$, and if\, $\Gamma; \Psi,\ol{\alpha} \vdash H : \F$ with
  $|\ol\alpha| = k$ and $\Phi \cap \Psi = \emptyset$, then
  $\Gamma,\Psi;\Phi \vdash F[\phi :=_{\ol{\alpha}} H] : \F$, and if
  \,$\Gamma, \phi^k; \emptyset \vdash F : \T$ with $k \geq 1$, and
  if\, $\Gamma; \Phi,\ol{\alpha} \vdash H : \F$ with $|\ol\alpha| =
  k$, then $\Gamma,\Phi;\emptyset \vdash F[\phi :=_{\ol{\alpha}} H] :
  \T$.}  Here, the operation $(\cdot)[\phi := H]$ of {\em second-order
...
{\color{blue} (\Nat^{\ol\beta} G \,K)[\phi :=_{\ol{\alpha}} H]}
& = & {\color{blue} \Nat^{\ol\beta}\, (G[\phi :=_{\ol{\alpha}} H]) \,(K[\phi
  :=_{\ol{\alpha}} H])}\\
