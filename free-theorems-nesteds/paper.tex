...
popularized {\color{blue} for System F with a primitive list types}
%augmented with list types defined natively as type-level fixpoints 
as Wadler's so-called ``theorems for free''~\cite{wad89} because it
...
text of the programs involved. While most of the free theorems derived
by Wadler's are essentially {\color{blue} naturality properties} of
polymorphic list-processing functions, parametricity can also be used
to prove naturality properties for non-list ADTs, as well as
%used to prove
properties, like correctness of the program optimization
%for lists and other ADTs
known as {\em short cut fusion}~\cite{glp93,joh02,joh03}, that go
beyond simple naturality.
...
This paper is about parametricity for a variant of System F supporting
not just ADTs, but nested types as well. An ADT defines a {\em family
  of inductive data types}, one for each input type. For example, the
$\mathtt{List}$ data type definition above defines a collection of
data types $\mathtt{List\; A}$, $\mathtt{List\; B}$, $\mathtt{List\;
  (A \times B)}$, $\mathtt{List\; (\List\;A)}$, etc., each independent
of all the others. By contrast, a nested type~\cite{bm98} is an {\em
  inductive family of data types} that is defined over, or is defined
mutually recursively with, (other) such data types. Since the
structures of the data type at one type can depend on those at other
types, the entire family of types must be defined at once. Examples of
nested types include, trivially, ordinary ADTs, such as list and tree
types; simple nested types, such as the data type {\small
\[\begin{array}{l}
...
{\color{blue} Mention $\mathtt{ \{-\# TERMINATING \#-\} }$ in figure.}
...
perfect trees in Figure~\ref{fig:funs}, or for those on bushes in
...
\begin{verse}
{\em Can we prove properties of functions over (truly) nested types
  via paramertricity?}
\end{verse}
%if it isn't possible to prove properties of functions over (truly) nested
%types via parametricity, too.
This paper answers the above question in the affirmative.
% thus extending Wadler's party trick from ADTs to the richer family
%of nested types.
To achieve this, we first introduce in Section~\ref{sec:calculus} a
polymorphic calculus supporting nested types as generated by the
grammar of~\cite{jp19}. At the type level, the calculus is
%(because it also has $\Nat$-types),
the level-2-truncated version of the calculus from~\cite{jp19}; the
class of data types it includes as primitives
%supports
is very robust and includes all (truly) nested types known from the
literature. At the term level, our calculus features primitive
constructs for functors, their initial algebras, and structured
recursion ($\map$, $\tin$, and $\fold$, respectively). While our
calculus does not support general recursion at the term level, it does
provide strong termination guarantees since it is strongly
normalizing, {\color{blue} and it moves us toward the practical
  programming language based on ... sought by Wadler.} Next, in
Sections~\ref{sec:type-interp} and~\ref{sec:term-interp}, we construct
set and relational interpretations of our types and terms,
respectively.  Along the way, we prove that the model so constructed
is parametric, i.e., that it satisfies an Identity Extension Lemma
(Theorem~\ref{thm:iel}) and an Abstraction Theorem
(Theorem~\ref{thm:abstraction}). Our interpretations of types rely on
the result from~\cite{jp19} ensuring that all of the data types that
can be represented in our calculus have well-defined semantics
appropriately structured categories.  Finally, in Section~{sec:ftnt},
we formulate and prove in our calculus a variety of free theorems for
nested types. We prove not only {\color{blue} actually one generic
  theorem} naturality-style free theorems in Section~\ref{}, including
those described above, but also give a formal proof of the short cut
fusion for nested types first proposed in~\cite{gj07} in
Section~\ref{}.
...
calculus allows us to clearly delineate those standard consequences of
parametricity --- like most of Wadler's --- that are really
consequences of naturality and those --- like free theorems for the
...
terms of the type of the polymorphic identity function --- that
actually go beyond naturality. This is made explicit in Enrico's subst
theorem, which is an equality that is simply a consequence of our
semantics and would hold even in a non-parametric model (i.e., one
that didn't include the extra condition in the $\set$ interpretation
of $\Nat$-types), and does not in any way reply on the abstraction
...
{\color{red} Compare with Bob's paper.  What's new? No ADTs, etc., at
  type level; no intro/elim rules for them at term level. Treatment of
in Secs 4.4 and 4.5 therefore hand-wavey. Similar ideas, but
hand-wavey. (Only gets one half of graph lemma, too.}



mention Pitts categorical/denotational vs. operational
semantics. Pitts has ADTs as primitives.



Mention that we don't have full polymorphism (like Bob) but just one
level of foralls, but most free theorems only use that anyway (even
short cut).  We do have fixpoints at the term level, though --- even
h-o ones.
...
Definition~\ref{def:well-formed-terms} lets us represent, e.g., the
$\mathtt{reversePTree}$ function from Figure~\ref{fig:funs} in our
calculus as
\[\vdash \fold_{\beta + \phi(\beta \times \beta)}^{\PTree\,
...
function from Figure~\ref{fig:funs2} as {\color{blue} code goes here}.
...
aforementioned typing conflict.) {\color{blue} Add Daniel's commentary
  ...
  paragraph. Not a good restriction.}
...
trees to lists. Even in the absence of extra variables the term
representing the mapping of each {\em monomorphic instance} of
$\mathit{flatten}$ at types involving no functorial variables is
well-typed:
\[\begin{array}{l}
...
so that $\map \,\mathit{flatten}$ is as well.


But because the functorial contexts for the expressions $F$ and $G$ in
the rule for $\map^{F,G}_H$ are necessarily empty, the term
representing the mapping of the {\em polymorphic} $\mathit{flatten}$
function is not. Indeed, the polymorphic $\mathit{flatten}$ function,
whose type contains the functorial variable $\beta$, say, cannot have
a type of the form $\Nat^\emptyset F'\, G'$ for any $F'$ and $G'$ as
is required in the conclusion of the rule for
$\map^{F,G}_{\mathit{List}}$. However, in a polymorphic calculus of
...
instances. The rules with the ``extra'' $\gamma$s recover this
...
Abstraction Theorem vs ``just'' logical relations, etc.
