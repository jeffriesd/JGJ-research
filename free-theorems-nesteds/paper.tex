...
$\Gamma;\Phi \vdash F[\phi :=_{\ol{\alpha}} H] : \F$, {\color{blue}
  Similarly, if \,$\Gamma, \phi^k; \emptyset \vdash F : \T$ with $k
  \geq 1$, and if\, $\Gamma; \Phi,\ol{\alpha} \vdash H : \F$ with
  $|\ol\alpha| = k$, then $\Gamma,\Phi;\emptyset \vdash F[\phi
    :=_{\ol{\alpha}} H] : \T$.} Here, the operation $(\cdot)[\phi :=
  H]$ of {\em second-order type substitution} is defined by:
%\[\begin{array}{lll}
