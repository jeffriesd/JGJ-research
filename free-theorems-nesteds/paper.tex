...
To illustrate, consider the nested type of perfect trees, coded in
Agda as
\[\begin{array}{l}
...
\end{array}\]
and the following $\mathtt{reversePTree}$ function that performs a
``deep reverse'' of them:
\[\begin{array}{l}
...
\mathtt{mapPTree\; f \;(pnode\; p) = pnode\; (mapPTree \;(\lambda (x,y)
  \rightarrow (f x, f y))\; p)}\\ 
\\
...
Intuitively, it is not hard to see that $\mathtt{reversePTree}$
commutes with $\mathtt{reversePTree}$. To prove this formally, we
would proceed by induction on the structure of the input perfect tree
--- something that would probably have been done in practice, but is
now is entirely justified in the wake of~\cite{jp19}.
...
The issue is not just {\em how} to prove properties, but also knowing
{\em that they hold at all}.

To prove properties of functions on ADTs we usually induct on the
structure of the data type. After~\cite{jp20}, we can now do the same
for (truly) nested types. This requires knowing not just the
definition of the data type, but also the definition of the function
whose properties are to be proved.

But often for ADTs we get such results by parametricity instead. In
this case, we do not need to know the explicit definition of the
function. In this paper we show that this party trick can be extended
to (truly) nested types. We do this by constructing a parametric model
of a calculus for (truly) nested types. At the type level, the
calculus is, at its core (because it also has $\Nat$-types), a
level-2-truncated version of the data type calculus --- with fixed
points at the type level --- from~\cite{jp19}. As shown there, this
ensures that all of the data types that can be represented in our
calculus have well-defined semantics appropriately structured
categories. The term calculus is, as far as we know, entirely new. It
features primitive constructs for functors, their initial algebras,
and structured recursion ($\map$, $\tin$, and $\fold$, respectively),
but does not support general recursion at the term level. It is
strongly normalizing. (This is a level-2 version of the ADL sought by
OGI all those years ago.)
...
All this infrastructure could serve as a basis for adding nested types
to, say, Coq.
