...
$\Nat$ types in Definitions~\ref{def:set-sem} and~\ref{def:rel-sem}
ensure $\setsem{\Gamma \vdash \sigma \to \tau}$ and
$\setsem{\Gamma \vdash \forall \alpha. \tau}$ are as expected in any
parametric model.
...
\subsection{Free Theorems for polymorphic functions over ADTs}
...
\graph{\mathit{map}_{\mathit{List}} \, g}$ by
Lemma~\ref{lem:list-graph}. Moreover, $xs' =
\mathit{map}_{\mathit{List}} \,g \,xs$ and $(s', s) \in \graph{g} \to
\Eq_{\mathit{Bool}}$, so $s' = s \circ g$, so instantiating
Equation~\ref{eq:filter-thm-list} gives the desired result.
% \begin{align*}
...
{\color{blue} Check this!}
Using an analog of Lemma~\ref{lem:list-graph} for generalized rose
trees we can prove
\begin{thm} 
 If $g : A \to B$, $\eta : F \to G$ in $\set$, $\rho : \relenv$, $\rho \alpha =
...
We can recover standard short cut fusion for lists in our system:
\begin{thm}
...
Using Equation~\ref{thm:demotion-objects} {\color{blue} CHECK!}, we
can actually prove short cut fusion results for arbitrary ADTs.
...
Short cut fusion can, in fact, be generalized from ADTs to nested
...
{\color{red} Can take $\emptyset;\alpha \vdash c$ with
  ...
  in_{H} = G \,\setsem{\emptyset;\alpha \vdash K}\, B$ as in~\cite{gj??}.}
