...
{\color{red} Compare with Bob's paper.  What's new? No ADTs, etc., at
  type level; no intro/elim rules for them at term level, i.e., no
  term calculus for nested types. Treatment of in Secs 4.4 and 4.5
  therefore hand-wavey. Similar ideas, but hand-wavey. (Only gets one
  half of graph lemma, too.

Bob has relation transformers, but since he has no $\mu$-types, there
is no need for $\omega$-cocont, and this need not be propagated
through entire semantic set up. This is what takes most of the effort
in our setting.

We merge Bob and~\cite{jp219}.}
...
\section{The Identity Extension Lemma}
