...
(Theorem~\ref{thm:abstraction}). {\color{blue} Free theorems are just
  instantiations of the Abstraction Theorem.} Our interpretations of
types rely on the result from~\cite{jp19} ensuring that all of the
data types that can be represented in our calculus have well-defined
semantics appropriately structured categories.  Finally, in
Section~{sec:ftnt}, we formulate and prove in our calculus a variety
of free theorems for nested types. We prove not only {\color{blue}
  actually one generic theorem} naturality-style free theorems in
Section~\ref{}, including those described above, but also give a
formal proof of the short cut fusion for nested types first proposed
in~\cite{gj07} in Section~\ref{}.
...
If $\C$ and $\D$ are locally finitely presentable
categories~\cite{ar94}, write $[\C,\D]$ for the set of
$\omega$-cocontinuous functors from $\C$ to $\D$. Both $\set$ and
$\rel$ are locally finitely presentable.
...
states that if $R$ is the equality relation on $A$ then $\tau_1(R)$ is
the equality relation on $\tau_0(A)$, and is the essential property
that makes a model relationally parametric, rather than just induced
by a logical relation. The second is an \emph{Abstraction Theorem},
which states that, for any $R :\rel(A, B)$, $(t_0(A),t_0(B))$ is a
morphism in $\rel$ from $(\sigma_0(A),\sigma_0(B),\sigma_1(R))$ to
$(\tau_0(A),\tau_0(B),\tau_1(R))$. The Identity Extension Lemma is
similar to the Abstraction Theorem except that it applies to, and thus
can be used to reason about, {\em all} elements of a type's
interpretation, not just those that are interpretations of terms.
%i.e., $t_0(A)$ and $t_0(B)$ map related arguments to
%related results.
Similar results are expected to hold for types and terms with any
number of free variables.

As usual, parametricity in our setting requires that set
interpretations of types are defined simultaneously with their
relational interpretations. This allows us to cut down the
interpretations of $\Nat$ types to include only the ``parametric''
elements, as discussed in, e.g.,~\cite{agj14,atk??,bfss90,rey83},
which is crucial to obtaining an Identity Extension Lemma
(Theorem~\ref{thm:iel}) in the present setting.  We give set
...
well-behaved. We take some effort to develop conditions in
...
intepretations in the $\set$ and $\rel$ --- we interpret all type
...
in the second clause of Definition~\ref{def:set-sem} are given in
Definition~\ref{def:rel-sem}.
%As usual, this condition ensures that the interpretations of such
%types are sufficiently uniform to ensure parametricity.
...
functors) are as defined in Definition~\ref{def:set-sem}. The action
of $T^\set_H$ on a morphism $f : \rho \to \rho'$ is the higher-order
natural transformation $T^\set_{H,f} : T^\set_{H,\rho} \to
T^\set_{H,\rho'}$ whose action on $F : [\set^k,\set]$ is the natural
transformation $T^\set_{H,f}\, F : T^\set_{H,\rho}\,F \to
T^\set_{H,\rho'}\,F$ whose component at $\ol{A}$ is $(T^\set_{H,f}\,
F)_{\ol{A}} = \setsem{\Gamma; \Phi,\phi,\ol{\alpha} \vdash H}f[\phi :=
...
\item If $\Gamma;\Phi \vdash (\mu \phi.\lambda
 ...
 = & (\mu T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi \vdash \tau}\rho}\,
 \to \, (\mu T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi \vdash \tau}\rho'}
\end{array}\]
\noindent
by
\[\begin{array}{ll}
& (\mu T^\set_{H,f})\ol{\setsem{\Gamma;\Phi \vdash
  \tau}\rho'} \circ (\mu T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi \vdash \tau}f}\\
= & (\mu T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi \vdash \tau}f} \circ (\mu
T^\set_{H,f})\ol{\setsem{\Gamma;\Phi \vdash \tau}\rho} 
\end{array}\]
This equality holds because $\mu T^\set_{H,\rho}$ and $\mu
T^\set_{H,\rho'}$ are functors and $\mu T_{H,f}^\set : \mu
T_{H,\rho}^\set \to \mu T_{H,\rho'}^\set$ is a natural transformation,
so that the following naturality square commutes:
...
F$ for $F^1$ and $\pi_2 F$ for $F^2$. We further extend these
conventions to relation environments, introduced in
Definition~\ref{def:reln-env} below.
...
\item if $(\delta^1,\delta^2) : (F^1,F^2,F^*)\to (G^1,G^2,G^*)$ and
  ...
  H^*(G^1,G^2,G^*)\ol{R}\] Of course this condition is
...
Next, we observe that we can compute {\color{blue} $\omega$-directed}
colimits in $RT_k$. It is straightforward to check that if $\cal D$ is
an $\omega$-directed set, then $\colim{d \in {\cal D}}{(F^1_d,
  F^2_d,F^*_d)} = (\colim{d \in {\cal D}}{F^1_d}, \colim{d \in {\cal
    D}}{F^2_d}, \colim{d \in {\cal D}}{F^*_d})$.
