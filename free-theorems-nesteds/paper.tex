...
there are only $\set$-many choices of morphisms between these (or any)
two objects because $\set$ is locally small.
...
Theorem~\ref{thm:subst} gives a family of free theorems that are
consequences of naturality, and thus do not require the full power of
parametricity. Specific instances of Theorem~\ref{thm:ft-nat} include
{\color{blue} Wadler free theorems, map-reverse theorems from intro,
  ???}
...
following theorem is immediate from Theorem~\ref{thm:subst}, by taking
...
$\eta$ also satisfy the additional condition necessary for $\eta$ to
...
that each $\eta$ satisfies the extra condition necessary for it to be
