...
polymorphic languages. Parametricity was originally put forth by
Reynolds~\cite{rey83} for System F~\cite{gr89}, the formal calculus at
the core of all polymorphic functional languages. It was later
popularized --- for an informal Haskell-like language that is
essentially System F augmented with list types defined natively as
type-level fixpoints --- by Wadler as his so-called ``theorems for
free''~\cite{wad89} because it allows the deduction of many properties
of programs in such languages solely from their types. In particular,
the deductions require no knowledge whatsoever of the text of the
programs involved. While most of the free theorems appearing in
Wadler's paper are essentially naturality properties of polymorphic
functions, parametricity can also be used to prove properties, like
correctness of the program optimization for lists and other ADTs known
as {\em short cut fusion}~\cite{glp93,joh02,joh03}, that go beyond
simple naturality. In addition, parametricity in the presence of ADTs
...
This paper is about parametricity for a polymorphic language
supporting not just ADTs, but nested types as well. An ADT defines a
{\em family of inductive data types}, one for each input type. For
example, the $\mathtt{List}$ data type definition above defines a
collection of data types $\mathtt{List\; A}$, $\mathtt{List\; B}$,
$\mathtt{List\; (A \times B)}$, $\mathtt{List\; (\List\;A)}$, etc.,
each independent of all the others. By contrast, a nested
type~\cite{bm98} is an {\em inductive family of data types} that is
defined over, or is defined mutually recursively with, (other) such
data types. Since the structures of the data type at one type can
depend on those at other types, the entire family of types must be
defined at once. Examples of nested types include, trivially, ordinary
ADTs, such as list and tree types; simple nested types, such as the
data type
{\small
\[\begin{array}{l}
...
\hspace*{0.4in}\mathtt{fnode\;:\; A \rightarrow PTree\;(Forest\;A)}
\end{array}\]}
...
Suppose we wanted to prove properties of functions over nested
types. We might, for example, want to prove a
$\mathtt{map}$-$\mathtt{reverse}$ property for the functions on
perfect trees in Figure~\ref{fig:funs} or those on bushes in
Figure~\ref{fig:funs2}. A few well-chosen examples quickly convince us
that such a property should indeed hold for perfect trees and, taking
inspiration from the ADT world, we could construct a proof by
induction on the input perfect tree. In fact, to formally establish
this result, we could prove it in Coq or Agda; indeed, both actually
generate an induction rule for $\mathtt{PTree}$s, and the generated
rule gives the expected result because proving properties of perfect
trees requires only that we induct over the top-level perfect tree in
the recursive position, leaving any data internal to the input tree
untouched.
\begin{figure*}
\hspace*{-0.5in}
...
\hspace*{0.8in} \mathtt{(\{B : Set\} \rightarrow F (B \times B) \rightarrow F B)}\\
\hspace*{0.8in}\mathtt{\rightarrow PTree \;A \rightarrow F\;A}\\
...
\[\begin{array}{l}
  ...
  \rightarrow Bush\; A}\\ 
\mathtt{reverseBush \;\{A\} = bfold\; \{A\}\; \{Bush\}\; bnil \;balg}\\
\\
\mathtt{bfold : \forall \{A : Set\} \rightarrow \{F : Set \rightarrow
  Set\} \rightarrow}\\
...
\mathtt{bfold\; bn\; bc\; bnil = bn}\\
\mathtt{bfold \;bn \;bc \;(bcons\; x \;bb) =}\\
\hspace*{0.5in} \mathtt{bc\; x \;(bfold \;bn\; bc
  \;(bmap \;(bfold\; bn\; bc) \;bb))}\\
\\
...
It is nowhere near as clear that analogous plausibility or inductive
arguments can be made for the analogous property for bushes, since a
proof by induction must recursively induct over the bushes that are
internal to the top-level bush in the recursive position,
...
since
a proof by induction must recursively induct over the bushes that are
internal to the top-level bush in the recursive position. This is
sufficiently delicate that no induction rule for bushes or other truly
nested types was known until very recently, when the {\em deep
  induction rules} of~\cite{jp20} finally gave a way to induct over
{\em all} of the structured data present in a structure. Deep
induction thus not only gave the first principled, data type generic,
and practically useful structural induction rules for bushes and other
truly nested types, but has also opened the way for incorporating
automatic generation of useful induction rules for (truly) nested data
types --- and, eventually, even GADTs --- into modern proof
assistants.

{\color{blue} Mention $\mathtt{ \{-\# TERMINATING \#-\} }$}


But recalling that inductive proofs over ADTs can sometimes be
circumvented in the presence of parametricity, we might naturally ask
if it isn't possible to prove properties of functions over nested
types via parametricity. This paper answers this question in the
affirmative, thus extending Wadler's party trick from ADTs to the
richer family of nested types. To achieve this, we first introduce in
Section~\ref{sec:caclculus} a polymorphic calculus supporting nested
types as generated by the grammar of~\cite{jp19}. At the type level,
the calculus is
%(because it also has $\Nat$-types),
the level-2-truncated version of the data type calculus
from~\cite{jp19}; the class of nested types it supports is very robust
and includes all (truly) nested types known from the literature.  We
then construct a parametric model for our calculus. Our
interpretations of types rely on the result from~\cite{jp19} ensuring
that all of the data types that can be represented in our calculus
have well-defined semantics appropriately structured categories. Our
term calculus is, as far as we know, entirely new. It features
primitive constructs for functors, their initial algebras, and
structured recursion ($\map$, $\tin$, and $\fold$, respectively), but
does not support general recursion at the term level. It is strongly
normalizing. Finally, we formulate in our calculus, and prove, a
variety of free theorems for nested types. We prove not only
{\color{blue} actually one generic theorem} naturality-style free
theorems in Section~\ref{}, including those described above, but also
give a formal proof of the short cut fusion for nested types first
proposed in~\cite{gj07} in Section~\ref{}.
...
Reynolds, parametricity, Abstraction Theorem vs logical relations,
etc.



Restriction of recursion to that expressible via $\fold$ limits the
functions that can be written, but also provides strong termination
guarantees.

In fact, we do much more than just build a parametric model for this
calculus. The relationship between parametricity and naturality has
long been of interest. By incorporating explicit $\Nat$-types at the
object level and interpreting them as natural transformations, our
calculus allows us to clearly distinguish between what standard
consequences of parametricity --- like most of Wadler's --- are really
consequences of naturality and which --- like free theorems for the
type of filter for lists, ADTs, and even nested types, or the standard
short cut fusion for ADTs and its extension, new here, to nested
types, or non-existence of terms of bottom type, or uniqueness of
terms of the type of the polymorphic identity function --- actually go
beyond naturality. This is made explicit in Enrico's subst theorem,
which is an equality that is simply a consequence of our semantics and
would hold even in a non-parametric model (i.e., one that didn't
include the extra condition in the $\set$ interpretation of
$\Nat$-types), and does not in any way reply on the abstraction
theorem. (We will undoubtedly have to mention Bernardy et al. if we
mention internalizing parametricity...)

Enrico's subst theorem is actually very general. Generic over data
types, functors over which data types are built in $\Nat$-types, and
functions of $\Nat$-type.
...
{\color{red} Compare with Bob's paper.  What's new?}



mention Pitts
categorical/denotational vs. operational semantics



Mention that we don't have full polymorphism but just one level of
foralls, but most free theorems only use that anyway (even short cut).
We do have fixpoints at the term level, though --- even h-o ones.




It's the ADTs that make free theorems interesting. Pitts considered
lists in parpoe. I extended Pitts' to all polynomial such types.


\subsection{Preliminaries}\label{sec:prelims}

We begin with some categorical preliminaries. For a more detailed
treatment of definitions and conventions the reader is referred
to~\cite{}.
...
\section{The Calculus}\ref{sec:calculus}

\subsection{Types}

For each $k \ge 0$, we assume countable sets $\tvars^k$ of \emph{type
  constructor variables of arity $k$}, $\fvars^k$ of \emph{functorial
  variables of arity $k$}, disjoint for distinct $k$ and disjoint from
each other.  We use lower case Greek letters for both sets of
variables, and write $\phi^k$ to indicate that $\phi \in \tvars^k \cup
\fvars^k$.  When convenient we may write $\alpha, \beta$, etc., rather
than $\alpha^0, \beta^0$, etc., for elements of $\tvars^0 \cup
\fvars^0$. The sets of all type constructor variables and functorial
variables are $\tvars = \bigcup_{k \ge 0} \tvars^k$ and $\fvars =
\bigcup_{k \ge 0} \fvars^k$, respectively, and a \emph{type variable}
is any element of $\tvars \cup \fvars$. We write $\overline{\zeta}$
for either a set $\{\zeta_1,...,\zeta_n\}$ of type constructor
variables or a set of functorial variables when the cardinality $n$ of
the set is unimportant. If $\Pos$ is a set of type variables then we
write $\Pos, \overline{\phi}$ for $\Pos \cup \overline{\phi}$ when
$\Pos \cap \overline{\phi} = \emptyset$.  We omit the vector notation
for a singleton set, thus writing $\phi$, instead of
$\overline{\phi}$, for $\{\phi\}$.
...
consider $\beta$-conversion at the level of type constructors.

In a subexpression $\Nat^{\ol{\alpha}}\,\sigma\,\tau$, the $\Nat$
operator binds all occurrences of the variables in $\ol{\alpha}$ in
$\sigma$ and $\tau$. Similarly, in a subexpression $\mu \phi^k.\lambda
...
non-functorial in the variables in $\Gamma$ and functorial in the
variables in $\Phi$.
...
\UIC{$\Gamma,v^0;\emptyset \vdash v^0 : \T$}
\DisplayProof
\\
\\
...
{\color{blue} Include some examples of types, e.g., lists and perfect
  trees and bushes, etc. Include those that will be used in
  discussions below.

  Note that we cannot write as folds functions --- such as concat ::
  PTree a -> PTree a -> PTree a --- that take as input more than one
  non-algebraic nested type. This is because the restriction on the
  contexts in which $\Nat$-types can be formed conflicts with the need
  to feed $\fold$s algebras. (Can't fold over pairs to get the right
  functions; can't get them by using continuation style because of the
  aforementioned typing conflict.)}
...
weakening rules for well-formed types hold. (Note, however, that
weakening does not change the contexts in which $\Nat$-types can be
formed.) If $\Gamma;\emptyset \vdash \sigma : \T$ and
$\Gamma;\emptyset \vdash \tau : \T$, then our rules allow formation of
the type $\Gamma;\emptyset \vdash \Nat^\emptyset\sigma \,\tau$. This
type can be thought of as representing the standard arrow type $\Gamma
\vdash \sigma \to \tau$ in our calculus. We can similarly represent a
standard $\forall$-type $\Gamma; \emptyset \vdash \forall \ol\alpha
. \tau$ as the well-formed type $\Gamma; \emptyset \vdash
\Nat^{\ol\alpha} \,\onet \,\tau :\F$.  However, if $\ol\alpha$ is
non-empty then $\tau$ cannot be of the form $\Nat^{\ol\beta} H \, K$;
indeed, $\Gamma; \ol\alpha \vdash \Nat^{\ol\beta} H \, K$ is an
invalid typing judgment in our calculus (except by weakening).

Our formation rules also allow a type like $\List\gamma = (\mu
\beta. \lambda \alpha. \onet + \alpha \times \beta)\gamma$ either to
be natural in $\gamma$ or not, according to whether it is well-formed
in the context $\emptyset; \gamma$ or $\gamma;\emptyset$. If the
former, then the type $\vdash \Nat^\gamma \onet\, (\List\gamma) : \T$
is well-formed; if the latter, then it is not. Our formation rules
also allow the derivation of, e.g., $\delta; \emptyset \vdash
\Nat^\gamma (\List \gamma) \, (\Tree \gamma \delta)$, which represents
a natural transformation between lists and trees that is natural in
$\gamma$ but not in $\delta$.  We explicitly note that types can be
functorial in variables of arity greater than $0$. For example, the
type $\GRose \phi \, \alpha = \mu\beta . \alpha \times \phi \beta$ can
be functorial in $\phi$ or not according to whether $\phi \in \Gamma$
or $\phi \in \Phi$. This choice of context for $\phi$ determines
whether types such as $\Nat^\alpha (\GRose \phi \, \alpha)\, (\List
\alpha)$ are well-formed. If $\GRose$ is functorial in $\phi$, i.e. if
$\emptyset; \alpha, \phi \vdash \mu \beta . \alpha \times \phi \beta$,
then $\GRose$ cannot be used as a (co)domain in $\Nat$ types.  This is
because our calculus does not allow naturality in variables of arity
greater than $0$.

\subsection{A Demotion Theorem}

{\color{blue} Move statement of theorem earlier?} There is a subtlety
in the well-formedness rules for types.

\begin{thm}
If $\Gamma; \Phi, \phi^k \vdash \tau : \F$, then one can derive
$\Gamma, \psi^k; \Phi \vdash \tau[\phi^k :== \psi^k]$, where
$\tau[\phi :== \psi]$ is the textual replacement of $\phi$ in $\tau$.
In other words, all occurences of $\phi\ol\sigma$ in $\tau$ become
$\psi\ol\sigma$.
\end{thm}
\begin{proof}
By induction on the structure of $\tau$.
\begin{itemize}
\item Case $\Gamma; \Phi, \alpha \vdash \alpha : \F$. We must derive
  $\Gamma, \beta; \Phi \vdash \beta : \F$.
\[\begin{array}{c}
...
\DP
\end{array}\]  
\item Case $\Gamma; \Phi, \phi \vdash \phi \ol\tau : \F$. We must
  derive $\Gamma, \psi; \Phi \vdash (\phi \ol\tau) [\phi :== \psi] :
  \F$. The induction hypothesis gives that $\Gamma, \psi; \Phi \vdash
  \tau[\phi :== \psi] : \F$ for each $\tau$.
\[\begin{array}{c}
...
\DP
\end{array}\]  
The case for $\Gamma; \Phi, \phi' \vdash \phi \ol\tau : \F$, i.e., the
case in which the variable being ``demoted'' only appears in the
arguments, works by the same induction. \\
\item Case $\Gamma; \Phi, \phi \vdash (\mu \phi'. \lambda
  \ol\alpha. H)\ol\tau : \F$.  We must derive $\Gamma, \psi; \Phi
  \vdash ((\mu \phi'. \lambda \ol\alpha. H) \ol\tau)[\phi :== \psi] :
  \F$. The induction hypothesis gives that $\Gamma, \psi; \Phi \vdash
  \tau[\phi :== \psi] : \F$ for each $\tau$ and $\Gamma, \psi; \Phi,
  \ol\alpha, \phi' \vdash H[\phi :== \psi] : \F$.
\[\begin{array}{c}
\AXC{$\Gamma, \psi; \Phi, \ol\alpha, \phi' \vdash H[\phi :== \psi] : \F$}
\AXC{$\ol{\Gamma, \psi; \Phi \vdash \tau[\phi :== \psi] : \F}$}
\BIC{$\Gamma, \psi; \Phi \vdash (\mu \phi'. \lambda \ol\alpha. H[\phi :== \psi])
       \ol{\tau[\phi :== \psi]} : \F$}
\UIC{$\Gamma, \psi; \Phi \vdash ((\mu \phi'. \lambda
...
\subsection{Substitution for Types}

If $\tau$ is a type, $\Pos$ and $V$ contain only type variables of
arity $0$, and $k=0$ for every occurrence of $\phi^k$ bound by $\mu$
in $\tau$, then we say that $\tau$ is {\em first-order}. Otherwise we
say that $\tau$ is {\em second-order}.  Substitution for first-order
types is the usual capture-avoiding textual substitution. We write
$\tau[\alpha := \sigma]$ for the result of substituting $\sigma$ for
$\alpha$ in $\tau$, and $\tau[\alpha_1 := \tau_1,...,\alpha_k :=
  \tau_k]$, or $\tau[\ol{\alpha := \tau}]$ when convenient, for
$\tau[\alpha_1 := \tau_1][\alpha_2 := \tau_2,...,\alpha_k := \tau_k]$.
Substitution for second-order types is defined as follows.

\begin{dfn}\label{def:second-order-subst}
If \,$\Gamma; \Phi \vdash F[\phi] : \F$, $\phi^k \in \Gamma \cup
\Phi$, $k \geq 1$, and $\Gamma, \ol\beta;\Phi, \ol{\alpha} \vdash H :
\F$ with $|\ol\alpha| + |\ol\beta| = k$, then $\Gamma \setminus
\phi^k;\Phi \setminus \phi^k\vdash F[\phi :=_{\ol\beta,\ol{\alpha}} H]
: \F$.  The operation $(\cdot)[\phi := H]$ of {\em second-order type
...
substitution when clear from context or otherwise convenient.
...
\begin{dfn}
Let $\Gamma$ be a type constructor context and $\Phi$ be a functorial
context. A {\em term context for $\Gamma$ and $\Phi$} is a finite set
of bindings of the form $x : \tau$, where $x \in {\cal V}$ and
$\Gamma; \Phi \vdash \tau : \F$.
\end{dfn}
We adopt the same conventions for denoting disjoint unions in term
contexts as for type constructor contexts and functorial contexts.
...
and $\fold^F_H$ to be well-formed, the number of $\alpha$s must equal
the arity of $\phi$.)

Substitution for terms is the obvious extension of the usual
capture-avoiding textual substitution.
Definition~\ref{def:well-formed-terms} ensures that the expected
weakening rules for well-formed terms hold.

{\color{blue} Let $\PLeaves$ be the type $\emptyset; \beta \vdash (\mu \phi. \lambda \alpha. \alpha + \phi(\alpha \times \alpha))\beta$.
...
merit special mention. It allows us to, e.g., map or fold polymorphic
functions over data types. For example, consider the function
$\mathit{flatten} : \Nat^\beta
(\mathit{PTree}\,\beta)\,(\mathit{List}\,\beta)$ that maps perfect
trees to lists. Even without the ``extra'' $\gamma$s the term
representing the mapping of each {\em instance} of $\mathit{flatten}$
acting on perfect trees and lists of types involving no functorial
variables is well-typed:
\[\begin{array}{l}
...
\DisplayProof
\end{array}\]
But because the functorial contexts for the expressions instantiating
$F$ and $G$ in the rule for $\map^{F,G}_H$ are necessarily empty, the
term representing the mapping of the polymorphic function
$\mathit{flatten}$ is not. Indeed, the polymorphic function
$\mathit{flatten}$, whose type contains the functorial variable
$\beta$, say, cannot have a type of the form $\Nat^\emptyset F'\, G'$
for any $F'$ and $G'$ as is required in the conclusion of the rule for
$\map^{F,G}_{\mathtt{List}}$. However, in a polymorphic calculus of
course we want to be able to manipulate terms representing polymorphic
functions in their entirety rather than just their monomorphic
instantiations. The rules with the ``extra'' $\gamma$s recover this
capability. For example, with these rules the term representing the
mapping of polymorphic $\mathit{flatten}$ is typeable as follows:
\[\begin{array}{l}
...
parametricity is to give each type $T(\alpha)$ with a free type
variable $\alpha$ both an {\em object interpretation} $T_0$ taking
sets to sets and a \emph{relational interpretation} $T_1$ taking
relations $R \subseteq A \times B$ to relations $T_1(R) \subseteq
T_0(A) \times T_0(B)$, and to interpret each term $\alpha\,|\, x :
S(\alpha) \vdash t(\alpha,x) : T(\alpha)$ as a map $t_0$ associating
to each set $A$ a function $t_0(A) : S_0(A) \to T_0(A)$. These
interpretations were to be given inductively on the structure of $T$
and $t$ in such a way that they imply two key theorems: an
\emph{Identity Extension Lemma}, stating that if $R$ is the equality
relation on $A$ then $T_1(R)$ is the equality relation on $T_0(A)$;
and an \emph{Abstraction Theorem}, stating that, for any relation $R
\subseteq A \times B$, $t_0(A)$ and $t_0(B)$ map arguments related by
$S_1(R)$ to results related by $T_1(R)$. Similar results hold for
types and terms with any number of free variables.

Free theorems are just instantiations of the Abstraction
Theorem. 



Parametricity requires that set interpretations of types are defined
simultaneously with their relational interpretations. This is because
the underlying interpretations of $\Nat$ types must be cut down to
include only the ``parametric'' elements, as discussed in,
e.g.,~\cite{agj14,bfss90,rey83}. Indeed, without this cutting down it
will not be possible to prove the Identity Extension Lemma
(Theorem~\ref{thm:iel}), which is the key property that makes a model
relationally parametric, rather than just induced by a logical
relation. The Identity Extension Lemma is similar to the Abstraction
Theorem (Theorem~\ref{thm:abstraction}), except that it applies to
{\em all} elements of a type's interpretation, not just those that are
interpretations of terms. It can therefore be used to reason about
arbitrary elements of a type's interpretation, not just those that are
interpretations of terms.

In this section we give the set interpretations for types; in the next
section we give their relational interpretations. While the set
...
conditions we must impose to ensure that they are well-behaved. We
take some effort to develop these in Section~\ref{sec:rel-interp},
which separates Definitions~\ref{def:set-sem} and~\ref{def:rel-sem} in
...
To ensure that we stay in the setting of~\cite{jp19} --- and thus to
ensure that all of the types in our calculus have well-defined
intepretations in $\set$ and $\rel$ --- we interpret all type
variables as $\omega$-cocontinuous functors in
Definitions~\ref{def:set-env} and~\ref{def:reln-env}.
...
\rho'$ from a set environment $\rho$ to a set environment $\rho'$ with
$\rho|_\tvars = \rho'|_\tvars$ maps each type constructor variable
$\psi^k \in \tvars$ to the identity natural transformation on $\rho
\psi^k = \rho'\psi^k$ and maps each functorial variable $\phi^k \in
\fvars$ to a natural transformation from the $k$-ary functor $\rho
\phi^k$ on $\set$ to the $k$-ary functor $\rho' \phi^k$ on $\set$.
Composition of morphisms on set environments is given componentwise,
with the identity morphism mapping each set environment to
itself. This gives a category of set environments and morphisms
...
maps a type variable of arity $0$ to an $\omega$-cocontinuous functor
from $\set^0$ to $\set$ --- i.e., to a set.  If $\ol{\alpha} =
...
variable $v$; see Definition~\ref{def:reln-env} below for the complete
definition of a relation environment. The relational interpretations
referred to in the condition on the natural transformations in the
clause of Definition~\ref{def:set-sem} for types of the form
$\Nat^{\ol{\alpha}}\,F\,G$ are given in full in
Definition~\ref{def:rel-sem}. As usual, this condition ensures that
the interpretations of such types are sufficiently uniform to ensure
parametricity.
...
Definition~\ref{def:set-sem} does indeed define a set. Indeed, local
...
\setenv$, $T^\set_{\rho}$ is an $\omega$-cocontinuous endofunctor on
$[\set^k, \set]$, and thus admits a fixed point.  Since $T_\rho^\set$
is defined in terms of $\setsem{\Gamma;\Phi,\phi, \ol{\alpha} \vdash
  H}$, this means that set interpretations of types must be
$\omega$-cocontinuous functors.  This in turn means that the actions
of set interpretations of types on objects and on morphisms in
$\setenv$ are intertwined.  In fact, we know from~\cite{jp19} that,
for every $\Gamma; \ol{\alpha} \vdash \tau : \F$, $\setsem{\Gamma;
  \ol{\alpha} \vdash \tau}$ is actually functorial in $\ol{\alpha}$
and $\omega$-cocontinuous.

Indeed, for each $\setsem{\Gamma; \Phi, \phi^k, \ol{\alpha} \vdash
  H}$, the corresponding operator $T^\set$ {\color{blue} Should be
  $T^\set_H$, really, and similarly throughout.} can be extended to a
{\em functor} from $\setenv$ to $[[\set^k,\set],[\set^k,\set]]$. The
action of $T^\set$ on an object $\rho \in \setenv$ is given by the
higher-order functor $T_\rho^\set$, whose actions on objects (functors
from $\set^k$ to $\set$) and morphisms (natural transformations
between such functors) are as defined in
Definition~\ref{def:set-sem}. The action of $T^\set$ {\color{blue}
  $T_H^\set$} on a morphism $f : \rho \to \rho'$ is the higher-order
natural transformation {\color{blue} May want to use $T^\set_{H,f}$
  rather than $\sigma^\set_f$ in final version.} $\sigma^\set_f :
T^\set_\rho \to T^\set_{\rho'}$ whose action on any $F :
[\set^k,\set]$ is the natural transformation $\sigma^\set_f\, F :
T^\set_\rho\,F \to T^\set_{\rho'}\,F$ whose component at $\ol{A}$ is
\[(\sigma^\set_f \, F)_{\ol{A}}  =  \setsem{\Gamma;
  \Phi,\phi,\ol{\alpha} \vdash H}f[\phi := \id_F][\ol{\alpha :=
    \id_A}]\] The next definition uses this observation to define the
action of each functor interpreting a type on morphisms between set
environments.
...
equivalent to: if $\ol{(a,b) \in R}$ implies $\ol{(\alpha\,a,\beta\,b)
  \in S}$ then $(c,d) \in F^*\ol{R}$ implies $(F^1 \ol{\alpha}\,c,F^2
\ol{\beta}\,d) \in F^*\ol{S}$.

When convenient we identify a $0$-ary relation transformer $(A,B,R)$
with $R : \rel(A,B)$. We may also write $F^1$ and $F^2$ for $\pi_1 F$
...
\item if $R_1:\rel(A_1,B_1),...,R_k:\rel(A_k,B_k)$, then
  ...
  \ol{B})\] In other words, $\pi_1 (H^*(F^1,F^2,F^*) \ol{R}) = H^1F^1
...
transformer $F$ is given by $\sigma_F = (\sigma^1_{F^1},
\sigma^2_{F^2})$.
\end{dfn}
...
\in H^*F\overline{R}$, then $((\sigma^1_{F^1})_{\overline{A}} x,
...
if $T = (T^1,T^2,T^*) : RT_k \to RT_k$ is an endofunctor on $RT_k$
...
{\color{blue} Say realizing that not being able to define third
...
\cup \fvars^k$ to an $\omega$-cocontinuous $k$-ary relation
transformer.  A morphism $f : \rho \to \rho'$ from a relation
environment $\rho$ to a relation environment $\rho'$ with
$\rho|_\tvars = \rho'|_\tvars$ maps each type constructor variable
$\psi^k \in \tvars$ to the identity morphism on $\rho \psi^k = \rho'
\psi^k$ and maps each functorial variable $\phi^k \in \fvars$ to a
morphism from the $k$-ary relation transformer $\rho \phi$ to the
$k$-ary relation transformer $\rho' \phi$. Composition of morphisms on
...
relation environment maps a type variable of arity $0$ to a $0$-ary
relation transformer --- i.e., to a relation.  We write
...
environment mapping each type variable $\phi$ to the functor
$(\rho\phi)^1$. The set environment $\pi_2 \rho$ similarly maps each
type variable $\phi$ to the functor $(\rho\phi)^2$.
...
\item The action of $H$ on $\rho$ in $\relenv$ is given by $H \rho =
  (H^1 (\pi_1 \rho),\,H^2 (\pi_2 \rho),\,H^*\rho)$  
...
\begin{enumerate}
\item if $R_1:\rel(A_1,B_1),...,R_k:\rel(A_k,B_k)$, then
  \[H^*\rho\, \ol{R} : \rel(H^1(\pi_1 \rho)\, \ol{A}, H^2(\pi_2 \rho)\,
  \ol{B})\] In other words, $\pi_1 (H^*\rho \ol{R}) = H^1(\pi_1 \rho)
  \,\ol{A}$ and $\pi_2 (H^*\rho \ol{R}) = H^2 (\pi_2 \rho) \,\ol{B}$.
\item if $(\alpha_1, \beta_1) \in \Homrel(R_1,S_1),..., (\alpha_k,
  \beta_k) \in \Homrel(R_k,S_k)$, then
  \[H^*\rho\, \ol{(\alpha, \beta)} = (H^1(\pi_1 \rho)\,\ol{\alpha},
  H^2(\pi_2 \rho)\, \ol{\beta})\] In other words, $\pi_1 (H^*\rho\,
  \ol{(\alpha, \beta)}) = H^1(\pi_1 \rho)\,\ol{\alpha}$ and $\pi_2
  (H^*\rho\, \ol{(\alpha, \beta)}) = H^2(\pi_2 \rho)\,\ol{\beta}$.
\item if $f : \rho \to \rho'$ and
  $R_1:\rel(A_1,B_1),...,R_k:\rel(A_k,B_k)$, then
  \[\mbox{ if }(x, y) \in H^*\rho\,\ol{R} \mbox{ then }
  (H^1(\pi_1 f)\,{\ol{A}}\,x, H^2(\pi_2 f)\,{\ol{B}}\,y) \in
  H^*\rho'\,\ol{R}\]
...
Considering $\relenv$ as a product $\Pi_{\phi^k \in \tvars \cup
  \fvars} RT_k$, we extend Lemma~\ref{lem:colimits} to compute
colimits in $\relenv$ componentwise, and similarly extend
...
Let $\rho$ be a relation environment. The {\em relation
  interpretation} $\relsem{\cdot} : \F \to [\relenv, \rel]$ is defined
by
\begin{align*}
...
The interpretations in Definition~\ref{def:rel-sem} respect weakening,
i.e., a type and its weakenings all have the same relational
interpretations. The same holds for the actions of these
interpretations on morphisms in Definition~\ref{def:rel-sem-funcs}
below.  Moreover, the interpretation of $\Nat$ types ensures that
$\relsem{\Gamma \vdash \sigma \to \tau}\rho = \relsem{\Gamma \vdash
  \sigma} \rho \to \relsem{\Gamma \vdash \tau} \rho$, as expected. If
$\rho$ is a relational environment and $\vdash \tau : \F$, then we
write $\relsem{\vdash \tau }$ instead of $\relsem{\vdash \tau }\rho$
as for set interpretations.
...
Lemma~\ref{lem:fp}, it admits a fixed point.  Since $T_\rho$ is
...
\vdash \tau : \F$, $\relsem{\Gamma; \ol{\alpha} \vdash \tau}$ is
actually functorial in $\ol{\alpha}$ and $\omega$-cocontinuous.  We
first define the actions of each of these functors on morphisms
between environments, and then argue that the functors given by
Definitions~\ref{def:rel-sem} and~\ref{def:rel-sem-funcs} are
well-defined and have the required properties. As in the case of the
set interpretations, each operator $T$ {\color{blue} Should be $T_H$,
  really, and similarly throughout.} can be extended to a {\em
  functor} from $\relenv$ to $[[\rel^k,\rel],[\rel^k,\rel]]$. Its
action on an object $\rho \in \relenv$ is given by the higher-order
functor $T^\rel_\rho$ {\color{blue} $T_{H, \rho}^\rel$} whose actions on
objects and morphisms are as defined in
Definition~\ref{def:rel-sem-funcs}. The action of $T$ on a morphism $f
: \rho \to \rho'$ is the higher-order natural transformation
{\color{blue} May want to use $T_{H,f}$ rather than $\sigma_f$ in the
  final version.} $\sigma_f : T_\rho \to T_{\rho'}$ whose action on
any $F : [\rel^k,\rel]$ is the natural transformation $\sigma_f F :
T_\rho F \to T_{\rho'} F$ whose component at $\ol{R}$ is
    \[ (\sigma_f F)_{\ol{R}} = \relsem{\Gamma; \Phi,\phi,\ol{\alpha}
      \vdash H}f[\phi := \id_F][\ol{\alpha := \id_R}]\] The next
    definition uses this observation to define the action of each
    functor interpreting a type on morphisms between relation
    environments.
...
The standard definition of graph relation of a morphism in $\set$ is

\begin{dfn}\label{def:graph}
If $f : A \to B$ then the relation $\graph{f} : \rel(A,B)$ is defined
by $(x,y) \in \graph{f}$ iff $fx = y$.
\end{dfn}
Here, we are using angle bracket notation for both the graph relation
of a function and for the pairing of functions with the same domain.
This is justified by the relationship between the two notions observed
immediately after Lemma~\ref{dfn:graph-reln-functors}.

We extend the notion of a graph to natural transformations
between $k$-ary set functors by defining an associated $k$-ary
relation transformer. 

\begin{dfn}\label{dfn:graph-nat-transf}
    If $F, G: \Set^k \to \Set$ are $k$-ary set functors and $\alpha :
    F \to G$ is a natural transformation, we define the functor
    $\graph{\alpha}^*: \rel^k \to \rel$ as follows.  Given $R_1 :
    \rel(A_1, B_1),...,R_k : \rel(A_k,B_k)$, let $\iota_{R_i} : R_i
    \hookrightarrow A_i \times B_i$, for $i = 1,...,k$, be the
    inclusion of $R_i$ as a subset of $A_i \times B_i$. By the
    universal property of the product, there exists a unique
    $h_{\overline{A \times B}}$ making the diagram
    \[
    ...
    \end{tikzcd}
    \]
    Since $\alpha^\wedge\overline{R} : \rel(F\overline{A},
    G\overline{B})$ by construction, we define $\langle \alpha
    \rangle^* \overline{(A,B,R)} = (F\overline{A}, G\overline{B},
    \iota_{\alpha^\wedge \overline{R}}\alpha^\wedge\overline{R})$.
    Moreover, if $\overline{(\beta, \beta') : (A,B,R) \to (C,D,S)}$
    are morphisms in $\rel$, then we define
    $\graph{\alpha}^*\overline{(\beta, \beta')}$ to be
...
If $f : A \to B$ be a function with graph relation $\graph{f} = (A, B,
...
The graph relation transformer on a graph relation is easily computed:
...
bottom triangle of the following diagram commute
\[\begin{tikzcd}[row sep = large]
        &F\overline{A}
...
the universal property of the product
      ...
      \end{tikzcd}
      \]
gives that $h_{\graph{\overline{f}}} = \langle \id_{F \ol{A}},
...
is, so its epi-mono factorization gives that $\iota_{\alpha^\wedge
  \graph{\overline{f}}} = \langle \id_{F \ol{A}}, \alpha_{\ol{B}}
\circ F\ol{f} \rangle$, and thus that $\alpha^\wedge
\graph{\overline{f}}$, the domain of $\iota_{\alpha^\wedge
  \graph{\overline{f}}}$ is equal to $F\overline{A}$.  Then,
$\iota_{\alpha^\wedge \graph{\overline{f}}} \alpha^\wedge
\graph{\overline{f}} = \langle \id_{F \ol{A}}, \alpha_{\ol{B}} \circ
F\ol{f} \rangle (F \ol{A}) = \graph{ \alpha_{\ol{B}} \circ F\ol{f}
}^*$.
%(where the last equality is by Remark~\ref{rmk:graph-fn}).
We therefore conclude that $\graph{ \alpha }^* \graph{ \overline{f} }
...
Finally, note that $\alpha_{\ol{B}} \circ F\ol{f} = G\ol{f} \circ
...
We can extend the notions of the graph of a natural transformation
and the equality relation transformer to environments as follows:

\begin{definition}
Let $f : \rho \to \rho'$ is a morphism of set environments.  The graph
relation environment $\graph{f}$ is defined pointwise, i.e., for any
variable $\phi$, we define $\graph{f} \phi = \graph{f \phi}$. This
ensures that $\pi_1 \graph{f} = \rho$ and $\pi_2 \graph{f} = \rho'$.
Then the equality relation environment $\Eq_\rho$ for any set
environment $\rho$ is then defined to be $\graph{\id_{\rho}}$.
\end{definition}

With this definition in hand, we can prove an Identity Extension Lemma
for our type interpretations.
...
It follows from Theorem~\ref{thm:iel} that $\relsem{\Gamma \vdash
  \sigma \to \tau} \Eq_{\rho} = \Eq_{\setsem{\Gamma \vdash \sigma \to
    \tau}\rho}$, as expected.  Moreover, the Identity Extension Lemma
also allows us to prove a Graph Lemma.

\begin{lemma}[Graph Lemma]\label{lem:graph}
If $f : \rho \to \rho'$ is a morphism of set environments and $\Gamma;
\Phi \vdash F : \F$, then $\graph{\setsem{\Gamma; \Phi \vdash F} f} =
...
\subsection{Free Theorem for Type of Polymorphic Bottom}
...
\subsection{Free Theorem for Type of Polymorphic Identity}
...
\subsection{Free Theorem for Type of $\mathit{filter}$ for $\GRose$}
...
\subsection{Short Cut Fusion for Lists}
...
\subsection{Short Cut Fusion for Arbitrary ADTs}
...
\subsection{Short Cut Fusion for Arbitrary Nested Types}
