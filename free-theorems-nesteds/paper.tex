...
Suppose we wanted to prove some property of programs over an ADT such
as that of lists, coded in Agda\footnote{All code appearing in this
  paper are written in Agda.} as
\[\begin{array}{l}
...
definition of the data type of which the input is an instance, but
also the program text for the programs involved in the properties. For
example, to prove by induction that map then reverse is the same as
reverse then map, we need to have the program text for both the
reverse and the map functions. An alternative is to use parametricity,
popularized by Wadler as the folksy party trick known as ``theorems
for free''~\cite{wad89}. Parametricity allows the deduction of many
properties of programs solely from their types, with no knowledge
whatsoever of the text of the programs involved. It does this
by.....????? Although most of the theorems in Wadler's original paper
are essentially naturality properties, it has subsequently been used
%it can also be used 
to prove properties, like correctness of the program optimization for
lists and other ADTs known as short cut
fusion~\cite{glpj93,joh??,???}, that aren't. In various contexts,
parametricity has further been used to prove i) {\em properties of
  programs}, e.g., that they perform no illegal operations, satisfy
certain security criteria~\cite{ep03,rp10}, or are observationally
equivalent to their compiled forms~\cite{bh09,hd11}; ii) {\em
  whole-language properties}, e.g., that they support data abstraction
and modularity via representation independence,
...
important tool for reasoning about programs in polymorphic calculi
supporting algebraic data types.

An algebraic data type is a family of inductive types, one inductive
type for each input type. For example, the $\mathtt{List}$ data type
definition above defines a collection of data types $\mathtt{List\;
  A}$, $\mathtt{List\; A}$, $\mathtt{List\; C}$, etc., each
independent of all the others. By contrast, a nested type~\cite{bm98}
is an inductive family of data types that is defined over, or is
defined mutually recursively with, (other) such data types. Since the
structures of the data type at one type can depend on structures at
other types, the entire family must be defined at once. Examples of
nested types include, trivially, ordinary ADTs; simple nested types,
such as the type
\[\begin{array}{l}
...
\end{array}\]
of perfect trees, whose recursive occurrences never appear below their
own type constructors; and truly\footnote{Nested types that are
  defined over themselves are known as {\em truly nested types}.}
nested types, such as the type
...
\hspace*{0.4in}\mathtt{bcons\;:\;A \rightarrow Bush\;(Bush \; A)
  \rightarrow Bush\;A} 
\end{array}\]
of bushes (also called {\em bootstrapped heaps} by
Okasaki~\cite{oka99}), whose recursive occurrences do appear below
their own type constructors.

Now, suppose we wanted to prove properties of functions over nested
types. For example, suppose we have 
%Now, suppose we wanted to prove properties of functions over nested
%types. For example, suppose we have the nested type of perfect trees
%and the truly nested type of bushes~\cite{bm98} coded in Agda as
%\[\begin{array}{l}
%\mathtt{data\; PTree\;(A : Set)\;:\;Set\;where}\\
%\hspace*{0.4in}\mathtt{pleaf\;:\;A \rightarrow PTree\;A}\\
%\hspace*{0.4in}\mathtt{pnode\;:\;PTree\;(A \times A) \rightarrow PTree\;A}
...
functions $\mathtt{reversePTree}$ and $\mathtt{reverseBush}$ in
Figure~\ref{fig:funs} that act on these data types by reversing the
data at the leaves of perfect trees and ?????  on bushes, and 
\begin{figure*}
\centering
\resizebox{.4\linewidth}{!}{
  \begin{minipage}[b]{.6\textwidth}
\[\begin{array}{l}
\mathtt{reversePTree : \forall \{A : Set\}  \rightarrow
PTree\; A \rightarrow PTree \;A}\\ 
\mathtt{reversePTree \; \{A\} = foldPTree \; \{A\}\; \{PTree\}\;
pleaf\; ( \lambda p \rightarrow pnode\; (mapPTree\; swap\; p) )}\\
\\
\mathtt{foldPTree : \forall \{A : Set\} \rightarrow \{F : Set \rightarrow Set \}
 \rightarrow ( \{B : Set\} \rightarrow B \rightarrow F B) \rightarrow
 (\{B : Set\} \rightarrow F (B \times B) \rightarrow F B)}\\
\hspace*{2in}\mathtt{\rightarrow PTree \;A \rightarrow F\;A}\\
...
\mathtt{swap \;(x,y) = (y,x)}\\
\\
\mathtt{bushreverse : \forall \{A : Set\} \rightarrow Bush\; A
  \rightarrow Bush\; A}\\ 
\mathtt{bushreverse \;\{A\} = bfold\; \{A\}\; \{Bush\}\; bnil \;balg}\\
\\
\mathtt{bfold : \{A : Set\} \rightarrow \{F : Set \rightarrow Set\}
  \rightarrow (\{B : Set\} \rightarrow F B) \rightarrow (\{B : Set\}
  \rightarrow B \rightarrow F\; (F\; B) \rightarrow F\; B) \rightarrow
  Bush\; A \rightarrow F \;A}\\
\mathtt{bfold\; bn\; bc\; bnil = bn}\\
\mathtt{bfold \;bn \;bc \;(bcons\; x \;bb) = bc\; x \;(bfold \;bn\; bc
  \;(bmap \;(bfold\; bn\; bc) \;bb))}\\
\\
...
\mathtt{balg\; x \;(bcons \;(bcons\; y \;bx)\; bbbx) = bcons\; y\;
  (bcons\; (bcons \;x \;bx) \;bbbx)} 
\end{array}\]
\caption{$\mathtt{reversePTree}$, $\mathtt{reverseBush}$, and their
  auxiliary functions}\label{fig:funs}
\end{minipage}}
\end{figure*}
...
of the original perfect tree. But can the analogous argument can be
made for the latter? It turns out that it can, although this is
nowhere near as intuitively plausible.

To formally establish results such as these we might take inspiration
from the situation for ADTs and try a proof by induction on the input
data structure. For perfect trees we could actually prove this in Coq
or Agda, since it is necessary to induct only over the top-level
perfect tree in the recursive position, leaving any data internal to
that tree untouched. For bushes the story is different, however, since
