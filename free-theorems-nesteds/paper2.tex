...
calculus like the one presented here.
...
%\DisplayProof
%\AXC{$\phantom{\Gamma;\Phi}$}
%\UIC{$\Gamma, \alpha^0; \Phi \vdash \alpha^0$}
%\DisplayProof
%&
%\AXC{$\phantom{\Gamma;\Phi}$}
%\UIC{$\Gamma;\Phi, \alpha^0 \vdash \alpha^0$}
%\DisplayProof
%&
\AXC{\phantom{$\Gamma,\Phi$}}
...
\Nat^\emptyset\sigma \,\tau$. Since a type $\Gamma; \emptyset \vdash
\Nat^{\ol \alpha}\sigma\,\tau$ represents a natural transformation in
$\ol \alpha$ from $\sigma$ to $\tau$, the type $\Gamma;\emptyset
\vdash \Nat^\emptyset\sigma \,\tau$ represents the standard arrow type
$\Gamma \vdash \sigma \to \tau$ in our calculus. We similarly
represent a standard $\forall$-type $\Gamma; \emptyset \vdash \forall
\ol\alpha . \tau$ as $\Gamma; \emptyset \vdash \Nat^{\ol\alpha}
\,\onet \,\tau$ in our calculus.
...
= \emptyset$, then $\Gamma,\Psi;\Phi \vdash F[\phi :=_{\ol{\alpha}}
  H]$.
%, and if \,$\Gamma, \phi^k; \emptyset \vdash F$ with $k \geq 1$,
%and if\, $\Gamma; \Phi,\ol{\alpha} \vdash H$ with $|\ol\alpha| = k$,
%then $\Gamma,\Phi;\emptyset \vdash F[\phi :=_{\ol{\alpha}} H]$.
Here, the operation $(\cdot)[\phi := H]$ of {\em second-order type
...
\[\begin{array}{lll}
%\tau[\phi :=_{\ol\alpha} H] & = & \tau \mbox{ if } \tau \in \T
%    {\color{red} superceded below??????}\\[0.5ex]
...
\AXC{$\ol{\Gamma; \Phi, \ol{\beta},\ol{\gamma} \vdash G}$}
\TIC{$\Gamma; \Phi
  %\emptyset
...
{\color{blue} There's a difference between which folds we can write
  for ADTs when viewed as nesteds and when viewed as f-o (i.e., as
  ADTs) since the arity of $\phi$ must equal the number of arguments
  to which $\mu \phi. \lambda \ol \alpha. \tau$ is applied in $\fold$
  rule. This is precisely what allows us to have one nested type and
  as many ADTs as we want as arguments. See next paragraph.}

{\color{blue} Double-check this paragraph.}
...
\DisplayProof
\end{array}\]
{\color{blue} REVISE! But in the absence of $\ol \gamma$, the instance
...
We note that the second clause of
...
and $\Gamma; \ol{\alpha} \vdash G$, we can replace the 
...
study expressivity of folds for nesteds a la Graham, Bird and Paterson,

Mendler types

f-p recursion at term level

partial fixpoints (fp of spec is spec of fp)

Right KEs for gen folds. These need all limits? Set has all

left KEs for GADTs

See pp 40-end of our JFP nested types paper

Over the course of writing this paper we spent considerable time
thinking about ways to increase expressivity.

Expressibility of standard folds for nesteds is well-known to be a vexing
issue, since (in the absence of a construct for right Kan extensions) such
folds can only express computations whose results are natural
transformations. This issue long predates this paper, and our system inherits
these constraints. Many attempts to strengthen standard folds have been
made~\cite{bm??}. These include ????, as well as generalized folds, which are
shown in our JFP paper to be equivalent to the folds in this paper in the
presence of right Kan extensions. (Generalized folds could perhaps be used to
get partial specializations of nested types?) This latter observation
suggests extending our term calculus with a right Kan extension construct.
There are also reasons --- namely to represent GADTs --- to extend it with a
left Kan extension construct. So these will be the next directions this work
takes. For now we have a calculus
%with nested types as primitives,
%rather than Church  encodings, and standard folds to recurse over them.
with just standard folds to recurse over nested types, but the nested types
here are constructed via primitives rather than represented via Church
encodings.

Adding generalized folds or otherwise trying to fix the
well-known expressivity issue of folds for nested types in this paper would
detract from its clarity. Expressivity is orthogonal to our goal of
constructing a parametric model that includes nested types constructed from
primitives (rather than using their Church encodings).




One solution to expressivity is to have a way to track deep structure
of nested types when partially specializing, so that specialization of
fixpoint can be computed as fixp[oint of specialization.
