...
set of bindings of the form $x : \tau$, where $x \in {\cal V}$ and
$\Gamma; \Phi \vdash \tau$. We adopt the same conventions for denoting
...
\AXC{$\Gamma;\emptyset \vdash \mathit{List}\,\tau$ \hspace*{0.3in}
  $\Gamma;\emptyset~|~\Delta \vdash t
  : \Nat^\emptyset\,(\mathit{PTree}\, \sigma)\,(\mathit{List}\,
  \tau)$}
\TIC{$\Gamma;\emptyset \,|\, \Delta \vdash \map^{\mathit{PTree}\,\sigma,
    \,\mathit{List}\,\tau}_{\mathit{List}\,\alpha} \,t :
  \Nat^\emptyset\,(\mathit{List}\, (\mathit{PTree}\,
  \sigma))\,(\mathit{List}\, (\mathit{List}\, \tau))$} \DisplayProof
\end{array}\]
...
= \setsem{ \Gamma; \emptyset \,|\, x
  ...
  \circ (\map_F^{\overline{\sigma}, \overline{\tau}} \,
...
\begin{align*}
  ...
  &\setsem{\emptyset; \alpha~|~ y : \Nat^\emptyset \sigma \, \tau
    ...
    (\map_{\mathit{Bush} \, \alpha}^{{\sigma}, {\tau}}\,
...
{\color{red} Our semantics interprets $\map$ terms as semantic $\mathit{map}$
...
in~\cite{jac99}, is also a lpccc and is appropriately fibred over $C$.
This would
