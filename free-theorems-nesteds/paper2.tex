...
\DisplayProof
\\[3ex]
\AXC{$\Gamma;\Phi,\ol{\alpha},\phi^k \vdash F$}
\AXC{$\quad\quad\ol{\Gamma;\Phi \vdash F}$}
\BIC{$\Gamma;\Phi \vdash (\mu \phi^k.\lambda \ol{\alpha}. \,F)\ol{F}$}
\DisplayProof
\end{array}\]
...
\[\begin{array}{ll}
 & \relsem{\Gamma; \Phi, \phi, \ol{\alpha} \vdash H} \Eq_{\rho} [\phi
  := T^{n}_{H,\Eq_{\rho}} K_0] \overline{[\alpha := \Eq_{\setsem{\Gamma;
        \Phi \vdash F}\rho}]}\\
= & 
\relsem{\Gamma; \Phi, \phi,
  \ol{\alpha} \vdash H} \Eq_{\rho} [\phi := \Eq_{(T^\set_{H,\rho})^n K_0}]
\overline{[\alpha := \Eq_{\setsem{\Gamma; \Phi \vdash F}\rho}]}.
\end{array}\]
%\end{split}
%\end{equation}
With these results in hand, the proof is atraightforward. It is given
in the accompanying anonymous supplementary material.
...
$\textit{map}_G\,\ol f$ to denote the action of the semantic functor
$G$ on morphisms $\ol f$.
...
\[\textit{fold}_{1 + F \times \_}\, n\, c\; (G\; (\mathit{List}\,
...
\[\setsem{\vdash \fold_{\onet + F
  ...
  + F \times \alpha)\, H)} =
\textit{fold}_{1 + F \times \_}\] so that if $c \in
...
$\rho \beta = \graph{\textit{fold}_{1 + F \times \_}\, n\, c} :
...
n\, c) \in \graph{\textit{fold}_{1 + F \times \_}\, n\, c}$, i.e.,
that $\textit{fold}_{1 + F \times \_}\, n\, c\; (G\;
