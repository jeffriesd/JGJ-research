...
care is taken in the formation of fixpoints; all ADTs and all (truly)
nested types appearing in the literature can nevertheless be expressed
in our calculus. In addition, to ensure the existence of the fixpoints
interpreting nested types, cocontinuity conditions on their underlying
functors must be threaded throughout our model construction.  Overall,
this paper offers a blueprint for deriving principled parametricity
results for practical programming languages, such as Haskell and Agda,
that directly construct nested types as fixpoints rather than
representing them via Church encodings.
%encoding them.
...
e.g., using the deep induction rules for nested types
%for nested types
...
transformation functorial in $\phi$. Finally, the type $\mu
\phi. \lambda \alpha. \mathit{GRose}\,\phi\,\alpha$ is not well-formed
in our system. These restrictions ensure that the set and relational
interpretations of types in Section~\ref{sec:type-interp} are
functors. In Section~\ref{sec:conclusion} we discuss how GADTs and
other more expressive types might be accommodated.
...
expression $K$ for every functorial variable $\alpha$. In the rule for
$\map^{\ol{F},\ol{G}}_H$ there is one functorial expression $F$ and
one functorial expression $G$ for each functorial variable in
...
%argument.
In fact, even for ADTs there is a difference between which folds over them we
...
type $\alpha$. This is not possible for nested types, even if they are
...
\gamma$ does not. To remedy this, we would need to construct our
models from semantic categories that are locally $\lambda$-presentable
for $\lambda > \omega$.
...
%Nevertheless, improving expressivity is
These and other extensions are discussed in
Section~\ref{sec:conclusion}.
...
for GADTs to have well-defined interpretations. We note, however, that
%not necessarily in
...
%, as in~\cite{pit00},
to the calculus presented here. This would, however, require the
