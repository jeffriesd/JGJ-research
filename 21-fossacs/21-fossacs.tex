...
%in Section~\ref{sec:short-cut-nested}.
%Specifically, we state and prove
...
$\Gamma;\emptyset \vdash \Nat^\emptyset F \,G$,
%Since $\Gamma; \emptyset \vdash \Nat^{\ol
%  \alpha}F\,G$ represents the type of a natural transformation in $\ol
%\alpha$ from $F$ to $G$,
%Note that the type $\Gamma;\emptyset \vdash \Nat^\emptyset F \,G$
which represents the arrow type $\Gamma \vdash F \to G$ in our calculus.
...
%not all
Some System F types, such as $\forall \alpha. (\alpha \to \alpha) \to
\alpha$, are not
%only those System F types that can be interpreted as functors are
representable in our calculus.
%For example, $\forall \alpha. (\alpha
%\to \alpha) \to \alpha$ is not representable.
%
...
In addition to textual replacement, we also have substitution for
...
If $\Delta$ is a term context for $\Gamma$ and $\Phi$ then the formation
rules for the set of\, {\em well-formed terms over $\Delta$} are given
in Figure~\ref{fig:terms}.
%\end{definition}
%
An expression $L_{\ol{\alpha}}x.t$ binds all occurrences of the type
variables in $\ol{\alpha}$ in the types of $x$ and $t$, as well as all
occurrences of $x$ in $t$. In the rule for $t_{\ol K} s$ there is one
functorial expression in $\ol K$ for every variable in $\ol
\alpha$. In the rule for $\map^{\ol{F},\ol{G}}_H$ there is one
functorial expression in $\ol F$ and one functorial expression in $\ol
G$ for each variable in $\ol\phi$. Moreover, for each $\phi^k$ in
$\ol\phi$ the number of variables in $\ol\beta$ in the judgments for
functorial expresssions in $\ol F$ and $\ol G$ is $k$. In the rules
for $\tin_H$ and $\fold^F_H$, the variables in $\ol{\beta}$ are fresh
with respect to $H$, and there is one $\beta$ for every $\alpha$.
...
$\mathsf{fold}$ cannot produce a term of this type
...
take multiple nested types as arguments, provided they
%those nested types
...
viewed as ADTs (i.e., as fixpoints of first-order functors) versus
%and which folds we can type when they are viewed
as proper nested types (i.e.,
as fixpoints of higher-order functors). This is because, in the return
type of $\mathsf{fold}$, the arguments of the $\mu$-type must be
...
%In order to define such functions, we could add more expressive
%recursion combinators.  But since the expressivity issue for folds
%over nested types long pre-dates, and is somewhat orthogonal to, the
%issue of parametricity for languages that construct nested types as
%fixpoints, we do not consider it further here other than to offer, in
...
Expressivity of folds for nested types 
has long been
a vexing issue, and this is naturally inherited by our calculus.
Adding more expressive recursion combinators could help, but
%enhance the expressivity of our calculus
since this issue is somewhat orthogonal to the issue of parametricity
in the presence of primitive nested types we do not consider it
further.
%in this paper other than to identify, in
%Section~\ref{sec:conclusion}, some ideas for addressing it in future
%work.
...
The key to proving our Identity Extension Lemma
%(Theorem~\ref{thm:iel})
is a familiar ``cutting down'' of the interpretations of universally
quantified types to include only the ``parametric'' elements; the
relevant types for us are $\Nat$-types.
%(See, e.g.,~\cite{atk12,bfss90,gjfor15,rey83}).
This requires that the set interpretations of types
(Section~\ref{sec:set-interp}) are defined simultaneously with their
relational interpretations (Section~\ref{sec:rel-interp}).
...
%in Section~\ref{sec:rel-interp}.
While set interpretations are relatively straightforward, relational
interpretations are less so,
%mainly
because of the cocontinuity conditions needed to know they
are well-defined. We develop these conditions in
...
morphisms between them, denoted $\setenv$.
%\end{definition}
...
%But there are
and only $\set$-many
%choices of morphisms between any two objects
such morphisms since $\set$ is locally small.  Also,
$\setsem{\Gamma; \emptyset \vdash \Nat^{\ol\alpha} F\,G}$ is
$\omega$-cocontinuous since it is a constant functor.
%(and that allowing functorial
...
Interpretations of $\Nat$-types ensure that $\setsem{\Gamma \vdash F
  \to G}$ and $\setsem{\Gamma \vdash \forall \ol\alpha. F}$ are as
expected in parametric models.
...
$T_{H,\rho}^\set$, whose actions on objects (functors in $[\set^k,
  \set]$) and morphisms between them
%(natural transformations between such functors)
are given in Figure~\ref{fig:set-sem}. Its action on a
...
\begin{itemize}
\item
%  If \,$\Gamma;\Phi \vdash \zerot$ then 
 $\setsem{\Gamma;\Phi \vdash \zerot}f = \id_0$
\item
  %If \,$\Gamma;\Phi \vdash \onet$ then
...
%\end{CD}
%\end{equation}}
\item %If\, $\Gamma;\Phi \vdash F + G$ then
  $\setsem{\Gamma;\Phi
...
where $F^1,F^2 : [\set^k,\set]$ and $F^* : [\rel^k, \rel]$
are functors, if $
%R_1:\rel(A_1,B_1),...,
\ol{R_i:\rel(A_i,B_i)}$ for $i = 1,...,k$ then $F^*
\ol{R} : \rel(F^1 \ol{A}, F^2 \ol{B})$, and if
%$(\alpha_1, \beta_1) \in \Homrel(R_1,S_1),...,
$\ol{(\alpha_i, \beta_i) \in \Homrel(R_i,S_i)}$ for $i = 1,...,k$,
...
\in F^*\ol{S}$.
%When convenient
We identify a $0$-ary relation transformer $(A,B,R)$ with $R :
\rel(A,B)$,
%. We may also
and write $\pi_1 F$ for $F^1$ and $\pi_2 F$ for $F^2$.  Below we
extend these conventions to relation environments
...
$\rel^k$ to $\rel$.  Also let $0$ denote the initial object of either
$\set$ or $\rel$, as appropriate. Observing that, for every $k$,
$K^\set_0$ is initial in $[\set^k,\set]$, and $K^\rel_0$ is initial in
$[\rel^k,\rel]$, we have that, for each $k$, $K_0 =
...
\colim{n \in \nat}{(T^nK_0)^*})$.
%\end{equation}
If $T : [RT_k,RT_k]$ then $\mu T$ is a fixpoint for $T$, i.e., $\mu T
\cong T(\mu T)$.
%if $T$
%is $\omega$-cocontinuous, i.e., 
%\begin{lemma}\label{lem:fp} For any
%, then $\mu T \cong T(\mu T)$.
%\end{lemma}
%\noindent
...
$\rho|_\tvars = \rho'|_\tvars$ maps each
%type constructor variable
$\psi^k \in \tvars$ to the identity morphism on $\rho \psi^k = \rho'
\psi^k$ and each
%functorial variable
$\phi^k \in \fvars$ to a morphism
from the $k$-ary relation transformer $\rho \phi$ to the $k$-ary
relation transformer $\rho' \phi$. Composition of morphisms on
relation environments is componentwise, with the identity morphism
mapping each to itself; this gives a category $\relenv$ of relation
environments and their morphisms.
%\end{definition}
%When convenient w
We identify a $0$-ary relation transformer with
%the relation (transformer) that is
its codomain, and consider
...
\{\alpha_1,...,\alpha_k\}$.  If $\rho : \relenv$ we write $\pi_1 \rho$
and $\pi_2 \rho$ for the set environments mapping each type variable
$\phi$ to the functors $(\rho\phi)^1$ and $(\rho\phi)^2$,
respectively.
...
%\end{itemize}
%\end{definition}
The last two points above give:
  %the following analogues of the three conditions
%%immediately following Definition~\ref{def:RT-functor}:
%on functors on $\setenv$:
{\em i}) if $\ol{R_i : \rel(A_i,B_i)}$ for $i = 1,...,k$
%,...,R_k : \rel(A_k,B_k)$,
then $H^*\rho\, \ol{R} :
...
%  \,\ol{A}$ and $\pi_2 (H^*\rho \ol{R}) = H^2 (\pi_2 \rho) \,\ol{B}$;
    {\em ii}) if $\ol{(\alpha_i, \beta_i) \in \Homrel(R_i,S_i)}$ for
    $i = 1,...,k$
    %,..., (\alpha_k, \beta_k) \in \Homrel(R_k,S_k)$,
    then $H^*\rho\, \ol{(\alpha, \beta)} =
...
$\ol{R_i:\rel(A_i,B_i)}$ for $i = 1,...,k$,
%,...,R_k:\rel(A_k,B_k)$,
then if $(x, y) \in H^*\rho\,\ol{R}$ then
$(H^1(\pi_1
...
%Recalling from the start of this
...
\end{figure}
%Figure~\ref{fig:rel-sem}
It ensures that $\relsem{\Gamma \vdash F
...
on objects and on morphisms in $\relenv$ are intertwined.
%As for set interpretations,
We know from~\cite{jp19} that, for every $\Gamma;
...
%Definition~\ref{def:rel-sem-funcs}
For this definition and Figure~\ref{fig:rel-sem} to be well-defined we
need that, for every $H$, $T_{H,\rho}\,F$ is a relation transformer,
and $T_{H,f}\, F : T_{H,\rho}\, F \to T_{H,\rho'}\, F$ is a morphism
of relation transformers, whenever $F$ is a relation transformer $f :
\rho \to \rho'$ in $\relenv$. This is immediate from
...
\sem{\Gamma;\Phi \vdash K}^{\mathsf D}f}]$, and
%\label{eq:subs-const}
$\llbracket \Gamma; \Phi$
$\vdash F[\phi := H] \rrbracket^{\mathsf D} \rho
= \sem{\Gamma; \Phi, \phi \vdash F}^{\mathsf D}\rho
[\phi := \lambda \ol{A}.\, \sem{\Gamma;\Phi,\overline{\alpha}\vdash
    H}^{\mathsf D}\rho[\overline{\alpha := A}]]$, and, finally,
%\label{eq:subs-const-morph}
$\sem{\Gamma; \Phi \vdash F[\phi := H]}^{\mathsf D}f
= \sem{\Gamma; \Phi, \phi \vdash F}^{\mathsf D}f
[\phi := \lambda \ol{A}.\,\sem{\Gamma;\Phi,\overline{\alpha}\vdash
    H}^{\mathsf D}f[\overline{\alpha := \id_{\ol{A}}}]]$. 
...
In most treatments of parametricity, equality relations are
taken as {\em given}, either directly as diagonal relations or
perhaps via reflexive graphs.
%--- and the graph relations used to
%validate existence of initial algebras are defined in terms of
%them.
By contrast, we give a categorical definition of graph relations for
natural transformations and {\em construct} equality relations as
particular such relations. Our definitions specialize to the usual
ones for
%the graph relation for a
morphisms between sets and 
equality relations on sets.
%In light of its novelty, we spell out our construction in detail.

The standard definition $(x,y) \in \graph{f}$ iff $fx = y$ of the
graph $\graph{f}$ of a morphism $f : A \to B$ in $\set$
%is the relation $\graph{f} : \rel(A,B)$ defined by $(x,y) \in
%\graph{f}$ iff $fx = y$. This definition
...
%so graph relation transformers do extend graph relations
%to functors.
...
Theorem~\ref{thm:iel}: for any $H$, $\rho$, $A$, and subformula $J$ of
$H$, both
...
%\end{equation}
\end{comment}
hold. {\color{red} The full proof of the IEL is given in the appendix,
...
Then terms of $\Nat$ type behave as natural transformations with
respect to their source and target
%functorial
types:
%, i.e., we have
...
is an isomorphism. Further, our calculus admits no
%non-terminating terms, i.e.,
terms with the
%closed
type $\Nat^\alpha \onet \,\alpha$ of the polymorphic bottom, and every
closed term $g$ of type $\Nat^\alpha\alpha\,\alpha$ denotes the
polymorphic identity function {\color{red} (see appendix)}.
...
Theorem (Theorem~\ref{thm:abstraction}). As usual, % for such theorems,
we prove a more general result (Theorem~\ref{thm:at-gen}) for %possibly
open terms, and recover our Abstraction Theorem as its special case
%of Theorem~\ref{thm:at-gen}
...
induction on
%the type judgment for
$t$. It
%The challenge of our development
%manifests in this proof: it manifests in
requires showing that set and relational interpretations of term
judgments are natural transformations, and that all set
...
involved; the latter two especially require some rather delicate
diagram chasing. That it is possible provides strong evidence that our
development is sensible, natural, and at an appropriate level of
abstraction.
%{\color{red} Detailed proofs for the set interpretations for the five
%  interesting forms of terms are included in the accompanying
%  anonymous supplementary material. Even in the supplementary
%  material we omit the other, straightforward, cases.}
%The proofs for the relational interpretations of all term
%judgments are entirely analogous to the proofs for their set
%interpretations.
%The proof that the final equality in
%Theorem~\ref{thm:at-gen}
%%Equation~\ref{eq:projs}
%holds in each
...
Theorem~\ref{thm:at-gen}.
%The Abstraction Theorem is i
Its instantiation
%of Theorem~\ref{thm:at-gen-rel}
to closed terms of closed type gives
...
%Using the results of Sections~\ref{sec:Nat-type-terms}
%and~\ref{sec:thms} we can prove free theorems for nested
%types. {\color{red} (See appendix for proofs)}.  For example, we can
%prove that our calculus admits no non-terminating terms, i.e., terms
%with the closed type $\Nat^\alpha \onet \,\alpha$ of the polymorphic
%bottom, and the standard result that every closed term $g$ of type
...
%Theorem~\ref{thm:abstraction} can be used to prove more advanced free
%theorems than those of Section~\ref{sec:Nat-type-terms} in the
%presence of directly constructed nested types.
...
%\vdash C$ generalizes Theorem~\ref{thm:short-cut-nested} to a free
...
providing supporting primitive nested types,
and
%rather than representing them via their Church encodings. We
used its Abstraction Theorem to derive free theorems for nested types.
This was not possible before~\cite{jp19} because
%the nested types in our calculus
these types were not previously known to have well-defined
interpretations in locally finitely presentable categories (here,
$\set$ and $\rel$), and, to our knowledge, no term calculus for them
existed.
%The key to our parametric model is
...
schemes would be futile.
\end{comment}
We naturally expect the construction elaborated here to guide its
generalization to more advanced data types.
%
%the locally $\lambda$-presentable
%categories for $\lambda > \omega$ that will be needed to accommodate
%more general data types.
\begin{comment}
%, and recursion schemes for them.
%
...
\end{comment}
For example, GADTs can be represented using
%object-level
left Kan extensions,
%
%This suggests
...
%carefully incorporating a
%carefully designed adding a
%left Kan extension construct (even at higher-kinds)
%to a calculus like ours was
and it was shown in~\cite{jp19} that adding a $\mathsf{Lan}$ construct
to a calculus such as ours preserves the cocontinuity needed for the
data types it defines to have well-defined interpretations in locally
$\lambda$-presentable categories. (Interestingly,
%not necessarily in
%However, this will not, in general, be
%the case in locally finitely presentable categories. Indeed, as shown
%in~\cite{jp19},
$\lambda > \aleph_1$ is required to interpret even common
GADTs.) This suggests
%extending our calculus with Kan extension
%constructs and
%augmenting our type formation rules with such a construct.
%The constructions of this paper should all be possible
carrying out our model construction in locally $\lambda$-presentable
cartesian closed categories (lpcccs) $\cal C$ whose categories of
(abstract) relations, obtained by pullback as in~\cite{jac99}, are also
lpcccs and are  appropriately fibred over $\cal C$.
%This would
...
%, as in~\cite{pit00},
%to the calculus presented here. Of course, this
further requires
%the categories interpreting types
our semantic categories not just to be locally $\lambda$-presentable,
but also to support some kind of domain structure.
