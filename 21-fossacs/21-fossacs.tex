...
\author{Patricia Johann\,\Letter \and Enrico
  Ghiorzi\orcidID{0000-0001-5983-6230} \and Daniel Jeffries \vspace*{-0.12in}}  
...
\begin{figure*}
  ...
  \begin{adjustbox}{varwidth=5.4in, max width=5in, margin=-0.1in 0in -0.3in 0in, fbox, center}
       \[\begin{array}{ccc}
       \AXC{$\Gamma;\Phi \vdash F$}
...
substitution of $\phi$) allow us to map polymorphic functions over
nested types.  Suppose, for example, that we want to map the
...
\begin{figure}[t]
  \begin{adjustbox}{varwidth=5in, max width=4.6in, fbox, center}
...
$\setsem{\Gamma;\Phi \vdash \phi \ol{F}} f : \setsem{\Gamma;\Phi
  ...
  \phi\ol{F}}\rho' = (\rho\phi) \ol{\setsem{\Gamma;\Phi \vdash
    F}\rho}$ $\to (\rho'\phi) \ol{\setsem{\Gamma;\Phi \vdash F}\rho'}$
  is defined by $\setsem{\Gamma;\Phi \vdash \phi \ol{F}} f =
  ...
  \phi)_{\ol{\setsem{\Gamma;\Phi \vdash F}\rho}}$.  This holds 
  since $\rho\phi$ and $\rho'\phi$ are functors and
...
\item
  ...
  \setsem{\Gamma;\Phi \vdash F}f \times \setsem{\Gamma;\Phi \vdash
    G}f$
\item
$\setsem{\Gamma;\Phi \vdash (\mu \phi.\lambda \ol{\alpha}. H)\ol{G}} f
  \;: \;\setsem{\Gamma;\Phi \vdash (\mu \phi.\lambda
    \ol{\alpha}. H)\ol{G}} \rho \to$\\ $\setsem{\Gamma;\Phi \vdash (\mu
    \phi.\lambda\ol{\alpha}. H)\ol{G}} \rho'\! = \!(\mu
  T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi \vdash G}\rho} \to (\mu
  T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi \vdash G}\rho'}$ is defined
  by $(\mu T^\set_{H,f})\ol{\setsem{\Gamma;\Phi \vdash G}\rho'} \circ
  (\mu T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi \vdash G}f} =$\\ $(\mu
  T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi \vdash G}f}\, \circ\, (\mu
  T^\set_{H,f})\ol{\setsem{\Gamma;\Phi \vdash G}\rho}$.  This holds
  since $\mu T^\set_{H,\rho}$ and $\mu T^\set_{H,\rho'}$ are functors
  and $\mu T_{H,f}^\set : \mu T_{H,\rho}^\set \to \mu
  T_{H,\rho'}^\set$ is a natural transformation.
\end{itemize}
\end{definition}
...
$H^*(F^1,F^2,F^*) \ol{R} : \rel(H^1F^1 \ol{A}, H^2F^2 \ol{B})$ if
$R_1:\rel(A_1,B_1),...,R_k:\rel(A_k,B_k)$; {\em ii})
$H^*(F^1,F^2,F^*)\, \ol{(\alpha, \beta)} = (H^1F^1\ol{\alpha}, H^2F^2
\ol{\beta})$ if $(\alpha_1, \beta_1) \in \Homrel(R_1,S_1),...,
(\alpha_k, \beta_k) \in \Homrel(R_k,S_k)$; and {\em iii}) if
...
H^*(G^1,G^2,G^*)\ol{R}$ if $(x, y) \in H^*(F^1,F^2,F^*)\ol{R}$. Note,
however, that this last condition is automatically satisfied because
it is implied by the third condition on functors on relation
transformers.
...
define the relation transformer $\mu T$ to be 
$\colim{n \in \nat}{T^n K_0}$ $= (\mu T^1,\mu T^2, \colim{n \in \nat}{(T^nK_0)^*})$.  
If $T
...
\begin{figure}[t]
\begin{adjustbox}{varwidth=5in, max width=4.65in, fbox, center}
...
$[RT_k,RT_k]$. Its action on an object $\rho \in
...
\begin{figure}[t]
\begin{adjustbox}{varwidth=6in, max width=4.65in, fbox, center}
...
\vdash K$, 
$H : [\set,\set] \to [\set,\set]$ is defined by $Hfx =
