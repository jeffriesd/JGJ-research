...
$T_H$ to a {\em higher-order $\omega$-cocontinuous environment
  transformer} from $\relenv$ to the category of $\omega$-cocontinuous
higher-order relation transformers on $RT_k$:
...
\item $H^*$ is an object in $[\relenv,[[\rel^k,\rel],[\rel^k,\rel]]]$
\item $(H^1(\pi_1 \rho) F^1,H^2(\pi_2 \rho) F^2, H^* \rho F^*) \in
  RT_k$ whenever $(F^1,F^2,F^*) \in RT_k$
...
\item For all $(F^1,F^2,F^*) \in RT_k$, $\pi_1(H^* f
  \,F^*) = H^1 (\pi_1 f)\,F^1$ and $\pi_2(H^*f
  \,F^*) = H^2 (\pi_2 f)\,F^2$
\end{itemize}
\end{defi}
\noindent
...
that if $f : \rho \to \rho'$, if $(F^1,F^2,F^*) \in RT_k$, if $R_1 :
...
The action of $T_H$ on an object $\rho \in \relenv$ is

{\color{blue}
given by the higher-order functor
$T_{H,\rho}$ whose actions on objects and morphisms are given in
Definition~\ref{def:rel-sem-funcs}.}



The action of  $T_H$ on a morphism $f : \rho \to \rho'$ is

{\color{blue}
the higher-order natural transformation $T_{H,f} :
