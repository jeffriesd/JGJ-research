...
\def\ruleScoreFiller{\hrule height 0.8pt}
...
\address{Appalachian State University}	
\email{enrico.ghiorzi@gmail.com}  
...
this paper.  (See the past paragraph of this section for a more
...
distractions.
%It is important to note that different consequences of
...
%end of Section~7 of~\cite{bm98}. By contrast, in a semantic
...
%But since we can always use an ADT's (semantically equivalent)
...
\end{array}\]
respectively. Our calculus can similarly define a $\mathtt{rustle}$
function that changes the placement of the data in a bush. This
...
\item If \,$\Gamma;\Phi \vdash (\mu \phi.\lambda
  ...
  is
  defined by
  \[\begin{array}{ll} & (\mu T^\set_{H,f})_{\ol{\setsem{\Gamma;\Phi \vdash
      G}\rho'}} \circ (\mu T^\set_{H,\rho})\ol{\setsem{\Gamma;\Phi
      \vdash G}f}\\ = & (\mu T^\set_{H,\rho'})\ol{\setsem{\Gamma;\Phi  
      \vdash G}f} \circ (\mu T^\set_{H,f})_{\ol{\setsem{\Gamma;\Phi
...
%Term extensionality for both types
...
\end{tikzcd}\]}
by naturality of $\mathit{fold}_{T^\set_{H,\rho}}\eta$.  To see that,
for all $\rho : \setenv$, $\eta : \setsem{\Gamma; \emptyset \vdash
  \Nat^{\ol{\beta}}\,H[\phi :=_{\ol{\beta}} F][\ol{\alpha :=
      \beta}]\,F} \rho$, and $d \in \setsem{\Gamma; \emptyset \vdash
  \emptyset}\rho$,
...
needed to be in $\setsem{\Gamma;\emptyset \vdash
  \Nat^{{\ol{\beta}} }\,(\mu \phi.\lambda \overline
  \alpha.H)\overline \beta\;F}\,\rho$, let $\ol{R : \rel(B,B')}$.
Since $\eta$ satisfies the 
additional condition needed to be in $\setsem{\Gamma; \emptyset
...
$y$, then $L$ would have to be the entirety of
 $\lambda \ol B. \setsem{\Gamma; \Phi,\ol\beta
  \vdash (\Lan^{\ol\alpha}_{\ol K} F){\ol \beta}}\rho[\Phi := \ol
  N][\ol{\beta := B}]$.
%$\mathit{Lan}_{\ol{\lambda \ol A.\, \setsem{\Gamma; \ol\alpha \vdash
%      K}\rho {[\ol{\alpha := A} ]}}}\, \lambda \ol A.\,
...
suspect that it may not actually be possible to define a functorial
semantics for $\Lan$-types that gives rise to parametric models for
languages supporting even constant-free polynomial primitive GADTs.
