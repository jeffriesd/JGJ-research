...
$\Phi$ are applied to types involving variables from $\ol\alpha$ in
the body $F$ of $\mu \phi. \lambda \ol\alpha.F$. This will be critical
to proving the Identity Extension Lemma for our calculus.
...
function maps, e.g., the bush {\color{red} Change one 2 to 4? Find a
  better name for this operation than reverse.}
\[\begin{array}{l}
...
We can now define our set interpretations.  If $\rho$ is a set
environment we write $\Eq_\rho$ for the equality relation environment
such that $\Eq_\rho \phi = \Eq_{\rho \phi}$ for every type variable
$\phi$; see Definitions~\ref{def:rel-transf} and~\ref{def:reln-env}
for the definitions of a relation transformer and a relation
environment, and Equation~\ref{def:eq-transf} for the definition of
the relation transformer $\Eq_F$ on a functor $F$.  Equality relation
environments appear in the third clause of
Definition~\ref{def:set-sem}.  The relational interpretations also
appearing in the third clause of Definition~\ref{def:set-sem} are
