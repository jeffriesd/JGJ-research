...
represented as primitive data types.  Significantly, regardless of
which representation we choose, GADTs fail to have {\em all} expected
parametricity properties. Indeed, we can read the results of
Section~\ref{sec:par} as showing that i)
%some standard parametricity results (e.g.,
...
%some
naturality results
%(e.g., rearrange-transform properties)
...
in the usual ``container'' sense, and completing it so that it becomes
one destroys its relationally uniform --- i.e., parametric ---
behavior.
%required in a parametric model for parametricity.
...
paper clearly distinguishes those free theorems that are consequences
of naturality from those that are consequences of parametricity {\em
  per se}.
...
%and thus there being no models validating all
%rearrange-transform properties for these types,
...
an extension of $F_\omega$ that supports type equality, and can encode
GADTs. Functoriality is not guaranteed here, either; in fact, the
parametric properties of GADTs described in the preliminary
