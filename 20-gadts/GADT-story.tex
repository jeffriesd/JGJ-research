...
suitable\footnote{The $\mathsf{map}$ function for $\mathsf{F}$ should
  ...
  i.e., should preserve identities and compositions --- in
  $\mathit{Set}$ even though there is no mechanism in Haskell for
  enforcing this.} $\mathsf{map}$ function, i.e., must be an instance
of Haskell's $\mathsf{Functor}$ class.
...
$\mathsf{map}$ function as well, $F$ must again be the fixpoint $\mu
H$ of a functor $H$. In this case, however, $H$ must be a functor on
the category $\mathit{Func(Set)}$ of functors and natural
transformations on $\mathit{Set}$, i.e., $H$ must be a {\em
...
$\mathsf{H}$ that supports suitable\footnote{{\color{blue} The
    $\mathsf{map}$ function for $\mathsf{H}$ should satisfy syntactic
    reflections of the functoriality properties in
    $\mathit{Func(Set)}$ --- i.e., should preserve identity natural
    transformations and composition of natural transformations --- and
    the $\mathsf{map}$ function for $\mathsf{H\,F}$ should satisfy
    syntactic reflections of the functoriality properties in
    $\mathit{Set}$ as in the previous footnote. Again, there is no
    mechanism in Haskell for enforcing this.}} $\mathsf{map}$
functions.  {\color{blue} Mention naturality?}
...
is is modelled as the fixpoint $\mu F_{\mathit{List\,a}}$ of the
...
f$. The type constructor $\mathsf{List}$ is modelled by the
higher-order functor $H$ whose action on a functor $F$ is given by the
functor $H\,F$ whose actions on sets and functions between them are
given by $H\,F\,X = 1 + X \times F\,X$ and $H\,F\,f = \id_1 + f \times
F\,f$, respectively, and whose action on a natural transformation
$\eta$ is the natural transformation given by $H\,\eta = \id_1 + \eta
\times F\,\eta$. The type constructor $\mathsf{List}$ is then
modelled as the functor $\mu H$. Reflecting the functorial action of
$\mu H$ back into syntax gives Haskell's built-in $\mathsf{map}$
function as the type-and-data-uniform, structure-preserving,
data-changing function associated with $\mathsf{List}$. {\color{blue}
  HOW?}
...
of ADTs, it is simply the reflection back into syntax of the
{\color{blue} naturality property - right?} of the functorial action
of the interpretation of the type constructor associated with an ADT's
