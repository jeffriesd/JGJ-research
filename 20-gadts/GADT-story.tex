...
an element
\[\mathsf{eqElim\, (map_{Equal\,Void}\, absurd\, Refl)\, 1}\]
of type $\mathsf{Void}$, where $\mathsf{absurd :: 0 \to c}$ is the
syntactic reflection of the empty function from the empty set into the
interpretation of $\mathsf{c}$. {\color{blue} Is there a way to say
  this without passing to semantics?} But this should be
impossible. It is immediate that a GADT that is not actually a nested
type cannot support a $\mathsf{map}$ function (or a
...
$\mathcal{C}$, $\mathcal{D}$, and $\mathcal{E}$ must all be of theform $\mathit{Set}^n$ for some $n$. Using $\mathsf{Lan}$ we can then
...
Full Lan prop goes here. Explain cutting down. Or just argue intuitively?
\end{proposition}
\noindent
...
*\!.\,\mathit{Eq}_1) \, R \big)\;=\; (\mathit{Lan}_{\lambda
  *\!.\,\mathit{Eq}_1} \lambda *\!. \,1)\, 1\] (Note that the left Kan
  extension of $\lambda *\!. \,1$ along $\lambda \!.\,\mathit{Eq}_1$ 
 is a functor from $\mathit{Rel}$ to $\mathit{Set}$.)
