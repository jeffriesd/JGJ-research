...
``container'' may actually {\em depend} on (the type of) the data to
be contained. On other words, the shape and the data are not
independent! As a result, it may not always be possible to modify a
GADT's data while retaining its shape because changing (the type of)
the data may actually change the shape.
...
$\Eq$. Such a type can either be taken as primitive, as in
\[\begin{array}{l}
...
\noindent
or
%, in a higher-kinded calculus such as $F\omega$,
represented by its own Church encoding
\[\mathsf{Eq\,t_1\,t_2} = \mathsf{\forall f.\, (\forall a.\,
  f\,a\,a) \to f\,t_1\,t_2}\]
Using $\mathsf{Eq}$, the GADT $\mathsf{Seq}$ can be written as
...
This rewriting of $\mathsf{Seq}$'s definition shows that it can be
seen as a fixpoint $\mu H$ of the higher-order function $H$ defined on
objects by $H\,F\,a = a + Eq\,a\,(b \times c) \times F\,b \times
F\,c$. We note, however, that $Eq\,a$ cannot be made a functor, or,
equivalently, that $\mathsf{Eq\,a}$ cannot be an instance of Haskell's
$\mathsf{Functor}$ class. Indeed, if $\mathsf{Eq\,a}$ supported a
function $\mathsf{map_{Eq\,a} :: (b \to c) \to Eq\,a\,b \to Eq\,a\,c}$
then defining
\[\begin{array}{l}
\mathsf{eqElim :: Eq\, a\, b \to b \to a}\,{\color{red} wasEq\,a\,b\to b\to a}\\
\mathsf{eqElim\, Refl\, x = x} 
\end{array}\]
would allow us to construct the following element of the type
$\mathsf{Void}$:
\[\begin{array}{l}
...
\mathsf{emptyFromMapEqa \,map_{Eq\,a} = eqElim\, (map_{\Eq\,a}\,
  absurd\, Refl)\, ()}  
\end{array}\]
Of course this is impossible.  It follows immediately that
$\mathsf{Seq}$ cannot support a map function either. This is already
evident from the Haskell definition of $\mathsf{Seq}$, though. The
clause of $\mathsf{map}$ for the $\mathsf{Pair}$ constructor should
feed $\mathsf{map}$ a function $\mathsf{f :: (b \times c) \to e}$ and
a term of the form $\mathsf{Pair \,t_1\,t_2}$ for $\mathsf{x :: b}$
and $\mathsf{y :: c}$, and produce a term $\mathsf{Pair\,t_1'\,t_2'}$
for some appropriately typed terms $\mathsf{t_1'}$ and
...
of higher-order functors. The use of {\em higher-order functions} is
...
constructors; the use of higher-order funct{\em ors} is essential, as
for ADTs, to the very existence of the fixpoints being
computed~\cite{tfca}.

Consider, for example, the GADT $\mathsf{Seq}$ from above.  Because
its type argument varies in the instances of $\mathsf{Seq}$ appearing
in the types of its data constructor $\mathsf{Pair}$, $\mathsf{Seq}$
cannot be characterized by any first-order fixpoint equation. It can,
however, be viewed as a solution to the higher-order fixpoint equation
{\color{blue} syntax or semantics?}
\[\mathsf{H\,f\,a = a \,+\,Lan_{\lambda a\,b.\,a \times b}\,
  \lambda a\,b.\,f\,a \times f\,b}\] where $\mathsf{Lan_k\, g}$ is the
syntactic reflection of the {\em left Kan extension} $\Lan_K \,G$ of
the functor $G$ interpreting $\mathsf{g}$ along the functor $K$
interpreting $\mathsf{k}$~\cite{mac71}.


Other GADTs are similarly characterizable by higher-order functors.
The difference between ADTs and GADTs can thus be seen as a difference
in {\em orders} of their underlying functors. {\color{blue} GADTs
  make essential use of higher-order functors.}
...
Functorial comnpletion doesn't give nested types
