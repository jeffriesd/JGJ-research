...
$\mathsf{G\,a}$ defined in~\eqref{eq:G}. Despite its simplicity, this
GADT serves as an informative case study highlighting the differences
--- even if we only consider the data elements a GADT contains ---
between a GADT's Church encoding its representation as a primitive
data type.

\begin{example}\label{ex:g1}
Syntactically, the GADT $\mathsf{G\,a}$ defined in~\eqref{eq:G}
comprises a single data element, namely $\mathsf{c :: G \, 1}$. This
is captured by $\mathsf{G}$'s Church encoding
\[\mathsf{G\,a} = \mathsf{Equal\,1\,a}\]
from~\cite{atk12}, so $\mathsf{G}$'s effect is simply to test its
argument for equality against the unit type
$\mathsf{1}$. {\color{blue} Use same notation for unit type as in
  intro.}
%Recalling from Section~\ref{sec:CEs} that the
The interpretation of $\mathsf{G\,a}$ represented as the Chirch
encoding $\mathsf{Equal\,1\,a}$ is thus the function whose value is a
singleton set when the interpretation of $\mathsf{a}$ is $1$ and the
empty set otherwise. This perfectly mirrors $\mathsf{G}$'s syntax,
which indeed delivers just one data element when $\mathsf{a}$ is
$\mathsf{1}$ and no elements otherwise.

We now want to compute the interpretation of $\mathsf{G}$'s primitive
representation. The type of $\mathsf{G}$'s unique constructor
$\mathsf{c :: G\,1}$ can be equivalently expressed as $\mathsf{c :: 1
  \to G\,1}$ or, using~\eqref{eq:nat-transfs}, as $\mathsf{c ::
  (Lan_{\lambda *. 1}\,\lambda *. 1)\,a \to G\, a}$, where
...
$\mathsf{G\, a}$ is therefore just the application to $\mathsf{a}$ of
the fixpoint of the interpretation of the body $\mathsf{(Lan_{\lambda
    *. 1} \,\lambda *. 1)\,b}$ of the syntactic fixpoint.  But since
the recursion variable $\mathsf{\phi}$ does not appear in this body,
the interpretation of the fixpoint is just the interpretation of the
body itself.  The interpretation of $\mathsf{G\,a}$ is therefore
$(\mathit{Lan}_{\lambda *. 1} \lambda *. 1)\, A$, where $A$ interprets
$\mathsf{a}$.
...
$\mathsf{G}$ and its primitive representation are not equivalent: the
interpretation of the former has exactly one data element at instance
$\mathsf{G\,1}$ and no elements at any other instances, whereas the
interpretation of the latter has data elements at every instance other
than $\mathsf{G\,0}$.
%Indeed,
...
very different parametricity properties. We explore the differences in
parametricity results for the two representations in the next section.
