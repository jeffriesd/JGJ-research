...
function $\mathit{Equal}$. Such a type can either be taken as primitive, as in
\[\begin{array}{l}
...
natural transformation $\mu : \mathit{Lan}_K\,G \to H$ such that $(\mu
...
interpreting $\mathsf{k}$. {\color{blue} This was elided in ADT
  case. Why? Tie together better.}  The type constructor
...
Note, however, that the functorial completion of a GADT need not be a
nested type. As mentioned in the introduction, only those instances of
the GADT reachable from the syntax of the GADT are in the functorial
completion.

The representation choices are the same for GADTs as for ADTs. The
difference is that, although these two representations are isomorphic
for ADTs, they are not isomorphic for GADTs.  We will see this in
Section~\ref{??} below.
