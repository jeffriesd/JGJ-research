...
otherwise, i.e., the syntactic reflection of semantic equality. Such a
type can either be taken as primitive, as in
\[\begin{array}{l}
...
\[\mathsf{Eq\,t_1\,t_2} = \mathsf{\forall f.\, (\forall a.\,
  f\,a\,a) \to f\,t_1\,t_2}\] 
Using $\mathsf{Eq}$, the GADT $\mathsf{Seq}$, e.g., can be written as 
{\color{red} START HERE}
\[ {\color{blue} WHAT GOES HERE??} \]



Note that neither Eq nor Seq has a syntactic map (because one for Eq
leads to a contradition as in Daniel's exampe below, and because one
for Seq can't exist because of Pair), i.e., is not an instance of the
Functor class, and is not required to in this representation.
...
eqElim :: Equal a b -> a -> b
eqElim Refl x = x

emptyFromMapEq :: (forall a b c. (a -> b) -> Equal a c -> Equal b c) -> Void
emptyFromMapEq fmap = eqElim (fmap absurd Refl) ()
...
Moreover, because its type argument varies in the instances of
$\mathsf{Seq}$ appearing in the types of its data constructors,
$\mathsf{Seq}$ cannot be characterized by any first-order fixpoint
equation. It can, be viewed as a solution to the higher-order fixpoint
equation
...
Other GADTs
are similarly characterizable by higher-order functors.  The
difference between ADTs and GADTs can thus be seen as a difference in
{\em orders} of their underlying functors. {\color{blue} The role of
  higher-order functors here is essential.}
