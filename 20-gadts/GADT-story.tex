...
instance.
%Reflecting this property back into syntax gives that
In terms of syntax the parametricity property for a GADT's Church
encoding gives that the GADT is inhabited at any instance for which
data elements can be formed using its data constructors. In
...
none otherwise:
%gives exactly the expected
%%syntactic counterpart to the
...
$G_1\,R$ of $\mathsf{G}$ is {\color{blue} $\mathit{Eq}_1$ when $R$ is
  ...
  $\mathit{Eq}_1$
  %of $\mathsf{1}$
  }. Reflecting back into syntax we
deduce that there can be no term in the type that is the Church
