...
{\color{red} Need to describe $\omega_1$-directed colimits in
  $\oCPOenv$ for the type of $\oCPOsem{\cdot}$ to make sense. We do it
  pointwise.}
...
an $\oCPO$. \new{Does this use functorial actions of interps? I don't
  think so, but double-check with Enrico.} {\color{red} That each of
  the above interpretations is $\omega_1$-cocontinuous follows from
  Corollary~12 of~\cite{jp19} if we APPROPRIATELY RESTRICT THE
  SUBSCRIPTS OF $\Lan$s.  For $\Nat$-types, we note that
  $\oCPOsem{\Gamma; \emptyset \vdash \Nat^\Phi F\,G}$ is an
  $\omega_1$-cocontinuous functor because it is constant on
  $\omega_1$-directed sets.} Interpretations of $\Nat$-types ensure
that $\oCPOsem{\Gamma \vdash F \to G}$ and $\oCPOsem{\Gamma \vdash
...
is in $\oCPOR$. \new{Same comments here as above.} {\color{red} It is
  indeed a relation between $\oCPO$s, and it preserves sups of
  $\omega$-chains of natural transformations because their sups are
  computed pointwise. Concretely, if $(t_i, t_i') \colon
  \oCPORsem{\Gamma; \emptyset \vdash \Nat^\Phi F\,G} \rho$ for all $i
  < \omega$, then for all $\ol{K = (K^0, K^1, K^*) : \oCPORT_{k}}$ we
  have that
\[((t_i)_{\ol{K^0}},\, (t'_i)_{\ol{K^1}}) \in (\oCPORsem{\Gamma; \Phi
  \vdash G}\rho[\ol{\Phi := K}])^{\oCPORsem{\Gamma;\Phi\vdash
    F}\rho[\ol{\Phi := K}]}\] for every $i < \omega$. In addition,
$((t_i)_{\ol{K^0}})_{i < \omega}$ is an $\omega$-chain in
$(\oCPOsem{\Gamma; \Phi \vdash G}\rho[\ol{\Phi :=
    K^0}])^{\oCPOsem{\Gamma;\Phi\vdash F}\rho[\ol{\Phi := K^0}]}$ and
$((t'_i)_{\ol{K^1}})_{i < \omega}$ is an $\omega$-chain in
$(\oCPOsem{\Gamma; \Phi \vdash G}\rho[\ol{\Phi :=
    K^1}])^{\oCPOsem{\Gamma;\Phi\vdash F}\rho[\ol{\Phi := K^1}]}$, so
their sups are not only well-defined but must also be related
in
$(\oCPORsem{\Gamma;
    \Phi \vdash G}\rho[\ol{\Phi := K}])^{\oCPORsem{\Gamma;\Phi\vdash
      F}\rho[\ol{\Phi := K}]}$. This gives that
\[
  \Big((\bigvee_{i < \omega} t_i)_{\ol{K^0}}, (\bigvee_{i < \omega}
  t'_i)_{\ol{K^1}} \Big) = \Big(\bigvee_{i < \omega} (t_i)_{\ol{K^0}},
  \bigvee_{i < \omega} (t'_i)_{\ol{K^1}} \Big) \in (\oCPORsem{\Gamma;
    \Phi \vdash G}\rho[\ol{\Phi := K}])^{\oCPORsem{\Gamma;\Phi\vdash
      F}\rho[\ol{\Phi := K}]}
\]
Moreover, as for $\oCPO$ interpretations, $\omega_1$-cocontinuity of
each of the above interpretations follows from Corollary~12
of~\cite{jp19} if we APPROPRIATELY RESTRICT THE SUBSCRIPTS of the
$\Lan$s. For $\Nat$-types, $\oCPORsem{\Gamma; \emptyset \vdash
  \Nat^\Phi F\,G}$ is an $\omega_1$-cocontinuous functor because it is
constant on $\omega_1$-directed sets.} If $\rho \in \oCPORenv$ and
$\vdash F$, then we write $\oCPORsem{\vdash F}$ instead of
...
be $\omega_1$-cocontinuous functors from $\oCPORenv$ to $\oCPORT_0$,
...
$\mu$-types. {\color{red} Anything needed for $\Lan$ types??}
