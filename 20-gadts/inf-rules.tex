...
\QIC{$\Gamma;\Phi~|~\Delta \vdash \int_{\ol K,F} t : (\Lan^{\ol\alpha}_{\ol{K}}\,  F)\, \ol{K[\ol{\alpha := A}]}$}
\DisplayProof
\\\\
...
\new{Note that we cannot have
\[
\AXC{$\Gamma; \Phi,\ol{\alpha}\vdash F$} \AXC{$\ol{\Gamma; \ol{\alpha}
    \vdash K}$} \BIC{$\Gamma;\emptyset~|~\emptyset \vdash \int_{\ol
    K,F} : \Nat^{\Phi,\ol\alpha}\,F\, (\Lan^{\ol\alpha}_{\ol K}\,
  F)\ol{K}$} \DisplayProof\] We cannot prove the additional property
needed to define an object semantics for this rule because we can only
introduce components of the natural transformation at representable
types, as in the rule above. We cannot produce the entire natural
transformation required in the conclusion of the alternative rule
here. This makes sense because not all parts of the functors
underlying GADTs are representable in syntax, which in turn means that
not all components of the associated natural transformations are
representable in syntax.}
...
\begin{equation}\label{eq:functorial-action-of-lan}
((\textit{Lan}_{\ol K} \alpha) \,\circ \,\langle K_1,...,K_n \rangle)
  \circ \eta_F = \eta_{F'} \,\circ\, \alpha
\end{equation}
\noindent
Here, $\eta_F : F \to (\textit{Lan}_{\ol K} F) \circ \langle
...
respectively. In $\set$, we have the slightly more general property
...
& = & = \iota_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\rho},
  \ol{\id_{\oCPOsem{\Gamma; \Phi \vdash K[\ol{\alpha := A}]}\rho}}}
\oCPOsem{\Gamma; \Phi~|~\Delta \vdash t : F[\ol{\alpha := A}]} \rho\, 
d \vspace*{0.05in}\\
\relsem{\Gamma;\Phi~|~\Delta \vdash \int_{\ol K,F} t :
  (\Lan^{\ol\alpha}_{\ol{K}}\,  F)\, \ol{K[\ol{\alpha := A}]}}\rho\;d
& = & \vspace*{0.05in} \\
...
In the fourth-to-last clause of Definition~\ref{def:set-interp},
  $\iota_{Z, f}$ is the injection into
\[\colim{\ol{Z : \Set_0},\, \ol{f : \oCPOsem{\Gamma; \ol\alpha \vdash K}
    \rho[\ol{\alpha := Z}] \to \oCPOsem{\Gamma; \Phi \vdash
      K[\ol{\alpha := A}]} \rho}}{\,\oCPOsem{\Gamma; \Phi, \ol\alpha
    \vdash F} \rho[\ol{\alpha := Z}]}\]
...
\new{In the final clause of Definition~\ref{def:set-interp},...}
...
\subsection{Object interpretations of $\Lan$-introduction
  are well-defined}

We must show that, for every $\rho : \setenv$ and $d :
\oCPOsem{\Gamma;\Phi \vdash \Delta}\rho$,
\[\oCPOsem{\Gamma;\Phi~|~\Delta \vdash \int_{\ol K,F} t :
  (\Lan^{\ol\alpha}_{\ol{K}}\, F)\, \ol{K[\ol{\alpha := A}]}}\rho\, d
\in \oCPOsem{\Gamma;\Phi \vdash (\Lan^{\ol\alpha}_{\ol{K}}\, F)\,
  \ol{K[\ol{\alpha := A}]}}\rho\] i.e., that there exist $\ol{Z :
  \oCPO_0}$, $t_1', t_2' : \oCPOsem{\Gamma; \Phi, \ol\alpha \vdash
  F}\rho[\ol{\alpha := Z}]$, and
\[\ol{f : \oCPORsem{\Gamma; \ol{\alpha} \vdash K}\Eq_{\rho}[\ol{\alpha
      := \Eq_Z}] \to \oCPORsem{\Gamma; \Phi \vdash K[\ol{\alpha :=
        A}]}\Eq_{\rho}} \]
such that
\[\begin{array}{rcl}
\iota_{\ol{Z},\ol{\pi_1 f}}t_1' &=& \iota_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\rho}, \ol{\id_{\oCPOsem{\Gamma; \Phi \vdash K[\ol{\alpha := A}]}\rho}}}\,
\oCPOsem{\Gamma; \Phi~|~\Delta \vdash t : F[\ol{\alpha := A}]} \rho\, d \\[2ex]
\iota_{\ol{Z}, \ol{\pi_2 f}}t_2' &=& \iota_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\rho}, \ol{\id_{\oCPOsem{\Gamma; \Phi \vdash K[\ol{\alpha := A}]}\rho}}}\,
\oCPOsem{\Gamma; \Phi~|~\Delta \vdash t : F[\ol{\alpha := A}]} \rho\,
d\\[2ex]
(t_1',t_2') &\in& \oCPORsem{\Gamma; \Phi, \ol\alpha \vdash
  F}\Eq_{\rho}[\ol{\alpha := \Eq_Z}] \;=\; \oCPORsem{\Gamma;
  \Phi, \ol\alpha \vdash F}\Eq_{\rho[\ol{\alpha := Z}]}
\end{array}\]
Let $\ol{Z = \oCPOsem{\Gamma; \Phi \vdash A} \rho}$, $t_1' = t_2' =
\oCPOsem{\Gamma; \Phi~|~\Delta \vdash t : F[\ol{\alpha := A}]} \rho\,
d$, and $\ol{f = \id_{\oCPORsem{\Gamma; \ol\alpha \vdash K[\ol{\alpha
          := A}]}\Eq_{\rho}}}$.  Then~\eqref{eq:subs-var} gives that
$t_1', t_2' : \oCPOsem{\Gamma; \Phi, \ol\alpha \vdash
  F}\rho[\ol{\alpha := Z}]$ as required, as well as that
$\oCPORsem{\Gamma; \ol{\alpha} \vdash K}\Eq_{\rho}[\ol{\alpha :=
    \Eq_{\oCPOsem{\Gamma; \Phi \vdash A} \rho}}] = \oCPORsem{\Gamma;
  \Phi \vdash K[\ol{\alpha := A}]}\Eq_{\rho}$. In light of the latter
we take $f$ to be
\[\ol{ \id_{\oCPORsem{\Gamma; \ol\alpha \vdash K[\ol{\alpha :=
          A}]}\Eq_{\rho}} : \oCPORsem{\Gamma; \ol{\alpha} \vdash
    K}\Eq_{\rho}[\ol{\alpha := \Eq_{\oCPOsem{\Gamma; \Phi \vdash A}
        \rho}}] \to \oCPORsem{\Gamma; \Phi \vdash K[\ol{\alpha :=
        A}]}\Eq_{\rho} }\] Then the first two properties above hold by
construction, and the third holds because $t_1' = t_2'$ and because
$\oCPORsem{\Gamma; \Phi, \ol\alpha \vdash F}\Eq_{\rho[\ol{\alpha :=
      Z}]} = \Eq_{\setsem{\Gamma; \Phi, \ol\alpha \vdash
    F}\rho[\ol{\alpha := Z}]}$ by the IEL.

\subsection{Relational interpretations of $\Lan$-introduction are
  well-defined}


\subsection{Object interpretations of $\Lan$-elimination are
  well-defined} We must show that, for every 
...
\ol{B}. (\mu_{\ol{N}})_{\ol{B}} |_{\setsem{\Gamma; \Phi, \ol\beta
    ...
    \beta := \ol N, \ol B]}$ is natural in both $\ol{N}$ and $\ol{B}$,
as well as that it satisfies the additional condition required for it
to be in $\setsem{\Gamma; \emptyset \vdash \Nat^{\Phi, \ol\beta}\,
...
\begin{lemma}
For all $\rho : \setenv$ and $d : \setsem{\Gamma; \emptyset \vdash
  \Delta} \rho$, the family of morphisms
\[ \lambda \ol{N} \, \ol{B}. 
  ((\mu_{\ol{N}})_{\ol{B}})|_{\setsem{\Gamma; \Phi, \ol\beta \vdash
    (\Lan^{\ol\alpha}_{\ol K} F)\ol \beta } \rho[\Phi, \ol \beta :=
    \ol N, \ol B]} \]
is natural in $\ol{N}$ and $\ol{B}$.
\end{lemma}
\begin{proof}
To prove that this family of morphisms is natural, it suffices to show
that the family of unrestricted morphisms $\lambda \ol{N} \,
\ol{B}. (\mu_{\ol{N}})_{\ol{B}}$ is natural in $\ol{N}$ and $\ol{B}$.
Since each $\mu_{\ol N}$ is already natural in $\ol B$ we need only
show that $\mu$ is natural in $\ol N$. To this end, let $\ol{h : N
  \to N'}$, and define functors and natural transformations as
follows:
\begin{align*}
...
\end{align*}
First note that \(\eta\) is natural, so that
\begin{equation}\label{eq:lan-elim-eta-nat}
\eta_{\ol{N'}} \circ F_{\ol h} = G_{\ol h} \ol{K} \circ \eta_{\ol N} :
F_{\ol N} \to G_{\ol{N'}} \ol{K}
\end{equation}
...
\end{tikzcd}
\]
Now, note that the collection $\{\epsilon_{\ol N}\}$ actually defines
a natural transformation in $\ol N$ since
\begin{equation}\label{eq:lan-elim-epsilon-nat}
	(\Lan_{\ol K} F_{\ol h}) \ol{K} \circ \epsilon_{\ol N} = \epsilon_{\ol{N'}} \circ F_{\ol h}
\end{equation}
holds by~\eqref{eq:functorial-action-of-lan}. That is, we have
\[
\begin{tikzcd}[column sep = huge, row sep = huge]
\Set^j
\ar[r, bend left, "{F_{\ol N}}", ""{name=F, below}]
...
\end{tikzcd}
\]
Since $\mu_{\ol N} : \Lan_{\ol K}\, F_{\ol N} \to G_{\ol{N}}$ is the unique
natural transformation such that
\begin{equation}\label{eq:lan-elim-mu-univ}
\mu_{\ol N} \ol{K} \circ \epsilon_{\ol N} = \eta_{\ol N}
\end{equation}
and since $\mu_{\ol{N'}} : \Lan_{\ol K}\, F_{\ol{N'}} \to G_{\ol{N'}}$ is
the unique natural transformation such that
\begin{equation}\label{eq:lan-elim-mu'-univ}
\mu_{\ol{N'}} \ol{K} \circ \epsilon_{\ol{N'}} = \eta_{\ol{N'}}
\end{equation}
...
\end{tikzcd}
\]
By the universal property of $\Lan_K \, F_{\ol{N}}$ there exists a
unique $\theta : \Lan_K \,F_{\ol{N}} \to G_{\ol{N'}}$ such that
\[\theta \ol{K} \circ \epsilon_{\ol N} = \eta_{\ol{N'}} \circ F_{\ol h} = G_{\ol h} \ol{K} \circ \eta_{\ol N}
\]
Here, the last equality is by~\eqref{eq:lan-elim-eta-nat}.
But $G_{\ol h} \circ \mu_{\ol N}$ satisfies this universal property because
\[
...
\]
where the second equality is by~\eqref{eq:lan-elim-mu-univ}, and 
$\mu_{\ol{N'}} \circ \Lan_{\ol K} F_{\ol{h}}$ satisfies it as well
because
\[
...
\]
Here, the second equality is by~\eqref{eq:lan-elim-epsilon-nat} and
the third equality is by~\eqref{eq:lan-elim-mu'-univ}. We must
therefore have that $G_{\ol h} \circ \mu_{\ol N} = \mu_{\ol{N'}}
\circ \Lan_{\ol K} F_{\ol{h}}$ as desired.
\end{proof}
...
\begin{align*}
&\relsem{\Gamma; \Phi,\ol\beta \vdash G}\Eq_\rho [\Phi, \ol \beta :=
     \ol N, \ol{\relsem{\Gamma; \ol\alpha \vdash K}\Eq_\rho[\ol{\alpha :=
          \Eq_Z}]}]\\
=~~~~&\relsem{\Gamma; \Phi, \ol\alpha \vdash G[\ol{\beta :=
        K}]}\Eq_{\rho}[\Phi, \ol \alpha := \ol N, \ol{\Eq_Z}]
\end{align*}
To do so we note that since $\setsem{\Gamma; \emptyset \,|\, \Delta
  \vdash \eta : \Nat^{\Phi, \ol\alpha} \, F \,\, G[\ol{\beta := K}]}
\rho \, d$ is in $\setsem{\Gamma; \emptyset \vdash \Nat^{\Phi,
    \ol\alpha} \, F \,\, G[\ol{\beta := K}]} \rho$ it must satisfy
    \begin{equation}\label{eq:P1}
    \begin{split}
      &\forall \ol{N = (N^1, N^2, N^*) : RT_{k_i}}, \forall \ol{M =
        (M^1, M^2, M^*) : RT_0}.\\ &((\setsem{\Gamma; \emptyset \,|\,
        \Delta \vdash \eta : \Nat^{\Phi, \ol\alpha} \, F \,\,
        G[\ol{\beta := K}]} \rho \, d)_{\ol{N^1}, \ol{M^1}},
      (\setsem{\Gamma; \emptyset \,|\, \Delta \vdash \eta} :
      \Nat^{\Phi, \ol\alpha} \, F \,\, G[\ol{\beta := K}] \rho \,
      d)_{\ol{N^2}, \ol{M^2}}) \\ :~~~~ & \relsem{\Gamma; \Phi,
        \ol\alpha \vdash F}\Eq_{\rho}[\Phi, \ol \alpha := \ol N, \ol
        M] \rightarrow \relsem{\Gamma; \Phi, \ol\alpha \vdash
        G[\ol{\beta := K}]}\Eq_{\rho}[\Phi, \ol \alpha := \ol N, \ol
        M]
    \end{split}
    \end{equation}
together with the universal properties of $\Lan_{\ol{\setsem{\Gamma;
      \ol\alpha \vdash K}\rho {[\ol{\alpha := \_} ]}}} \setsem{\Gamma;
  \Phi, \ol\alpha \vdash F}\rho {[\Phi, \ol \alpha := \ol{N^i}, \_ ]}$
for $i = 1, 2$, and the fact that $(t_1',t_2') \in \relsem{\Gamma;
  \Phi, \ol\alpha \vdash F}\Eq_{\rho}[\Phi, \ol\alpha := \ol N,
  \ol{\Eq_Z}]$. Indeed, applying~\eqref{eq:P1} with $\ol{M} = \ol{\Eq_Z}$
gives (eliding the type of $\eta$)
    ...
    \begin{split}
      ...
      &: \relsem{\Gamma; \Phi, \ol\alpha \vdash F}\Eq_{\rho}[\Phi, \ol
        \alpha := \ol N, \ol{\Eq_Z}]
      \rightarrow \relsem{\Gamma; \Phi, \ol\alpha \vdash
        G[\ol{\beta := K}]}\Eq_{\rho}[\Phi, \ol \alpha := \ol N, \ol{\Eq_Z}]
    \end{split}
    \end{equation}
Then since $(t_1',t_2') \in \relsem{\Gamma; \Phi, \ol\alpha \vdash
  F}\Eq_\rho[\Phi := \ol{N}][\ol{\alpha := \Eq_Z}]$, we can apply
~\eqref{eq:morph} to it to get
\begin{align*}
...
\end{align*}
%%
Applying the universal property~\eqref{eq:defn-of-mu-sub-N} at component $\ol{Z}$ gives
\[
...
so that
\begin{align*}
\big((\mu_{\ol{N^1}})_{\ol{\setsem{\Gamma; \ol\alpha \vdash K} \rho[\ol{\alpha := Z}]}} 
  ((\epsilon_{\ol{N^1}})_{\ol{Z}} \,\, t_1') &, 
  %%%
  (\mu_{\ol{N^2}})_{\ol{\setsem{\Gamma; \ol\alpha \vdash K} \rho[\ol{\alpha := Z}]}} 
  ((\epsilon_{\ol{N^2}})_{\ol{Z}} \,\, t_2')\big) \\
  & \in 
  \relsem{\Gamma; \Phi, \ol\alpha \vdash
        G[\ol{\beta := K}]}\Eq_{\rho}[\Phi, \ol \alpha := \ol N,\ol{\Eq_Z}] 
...
\end{align*}
\noindent
Finally, then, we can apply the morphism 
\begin{align*}
  \relsem{\Gamma; \Phi,\ol\beta \vdash G}\id_{\Eq_\rho} [\Phi, \ol
    \beta := \ol{\id_N}, \ol f] 
  : \,\,
  &\relsem{\Gamma; \Phi,\ol\beta \vdash G}\Eq_\rho
  [\Phi, \ol \beta := \ol N, \ol{\relsem{\Gamma; \ol\alpha \vdash K}\Eq_\rho[\ol{\alpha := \Eq_Z}]}] \\
  \rightarrow \,\,
  &\relsem{\Gamma; \Phi,\ol\beta \vdash G}\Eq_\rho [\Phi, \ol \beta :=
    \ol N, \ol B]
\end{align*}
...
\begin{align*}
\big( 
  &\setsem{\Gamma; \Phi,\ol\beta \vdash G}\id_{\rho} [\Phi, \ol \beta := \ol{\id_{N^1}}, \ol{\pi_1 f}] 
  ((\mu_{\ol{N^1}})_{\ol{\setsem{\Gamma; \ol\alpha \vdash K} \rho[\ol{\alpha := Z}]}} 
  ((\epsilon_{\ol{N^1}})_{\ol{Z}} \,\, t_1')) ,  \\
  %%%
  &\setsem{\Gamma; \Phi,\ol\beta \vdash G}\id_{\rho} [\Phi, \ol \beta := \o;{\id_{N^2}}, \ol{\pi_2 f}] 
  ((\mu_{\ol{N^2}})_{\ol{\setsem{\Gamma; \ol\alpha \vdash K} \rho[\ol{\alpha := Z}]}} 
  ((\epsilon_{\ol{N^2}})_{\ol{Z}} \,\, t_2'))\big) \\
  & \in 
...
\subsection{Relational interpretations of $\Lan$-elimination are
  well-defined}  
