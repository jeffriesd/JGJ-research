...
{\color{blue} ( (\eta_1)_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\rho }} t,
  ...
  &{\color{blue} \hspace{0.5in} (\textit{Lan}_{\ol{\oCPORsem{\Gamma; \ol\alpha
        \vdash K}\Eq_\rho[\ol{\alpha := \Eq_{\_}}]}}
  \oCPORsem{\Gamma; \Phi, \ol\alpha \vdash F}\Eq_\rho[\ol{\alpha :=
      \Eq_{\_}}]) \ol{\oCPORsem{\Gamma; \Phi \vdash A}\Eq_\rho} \}}\\
\end{align*}
\end{dfn}
{\color{blue} Here, $\eta_i : \textit{Lan}_{\ol{\oCPOsem{\Gamma;
        \ol\alpha \vdash K}\rho[\ol{\alpha := \_}]}} \oCPOsem{\Gamma;
    \Phi, \ol\alpha \vdash F}\rho[\ol{\alpha := \_}] \; \to \;
  \pi_i\,(\textit{Lan}_{\ol{\oCPORsem{\Gamma; \ol\alpha \vdash
        K}\Eq_\rho[\ol{\alpha := \Eq_{\_}}]}} \oCPORsem{\Gamma; \Phi,
...
\begin{align*}
  ...
  &\hspace{0.3in}|~{\color{blue}
    (\nu_1 ((\eta_1)_{\ol{\oCPOsem{\Gamma; \Phi \vdash A}\pi_1 \rho}} t_1),
\nu_2 ((\eta_2)_{\ol{\oCPOsem{\Gamma; \Phi \vdash A}\pi_2 \rho}} t_2)) \in}\\
  &\hspace{0.3in}{\color{blue}
(\textit{Lan}_{\ol{\oCPORsem{\Gamma;
          \ol\alpha \vdash K}\rho[\ol{\alpha := \Eq_{\_}}]}}
...
{\color{blue} where $\eta_i$ is for the (mixed $\set/\rel$) diagram
  top p2 of Daniel's notes for relational environment $\Eq_{\pi_i
    \rho}[\alpha := \Eq_{\_}]$ and $\nu_i$ is the unique morphism in
  $\set$ from \[(\textit{Lan}_{\ol{\oCPOsem{\Gamma; \ol\alpha \vdash
        K}\pi_i\rho[\ol{\alpha := \_}]}} \oCPOsem{\Gamma; \Phi,
    \ol\alpha \vdash F}\pi_i\rho[\ol{\alpha := \_}])
  \ol{\oCPOsem{\Gamma; \Phi \vdash A}\pi_i\rho}\] to
\[\pi_i(\textit{Lan}_{\ol{\oCPOsem{\Gamma; \ol\alpha \vdash
      K}\rho[\ol{\alpha := \Eq_{\_}}]}} \oCPOsem{\Gamma; \Phi,
  \ol\alpha \vdash F}\rho[\ol{\alpha := \Eq_{\_}}])
\ol{\oCPOsem{\Gamma; \Phi \vdash A}\pi_i\rho}\]
given by the colimit formulation of left Kan extensions.}
