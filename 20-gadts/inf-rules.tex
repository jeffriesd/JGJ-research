...
($\omega_1$-)(co)complete whenever $\C$ is.  {\color{red} In
  particular, if $F : [\C, \C]_{(\omega_1)}$ for a(n)
  ($\omega_1$-)cocomplete category $\C$, then we can define $F^0 =
  \Id$, $F^{\alpha + 1} = F \circ F^\alpha$ at successor ordinals, and
  $F^{\bigcup_{\beta < \alpha} \beta} = \colim{\beta < \alpha}
  F^\beta$ at limit ordinals.}
