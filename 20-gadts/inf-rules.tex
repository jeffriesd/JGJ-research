...
{\color{red} We should ask what, intuitively, it means to a GADT to be
  ``parametric'', i.e., uniform in its type indices. For ADTs and
  nested types it means that the datatype elements are constructed in
  exactly the same ways at each type. For GADTs, it means that as
  well, but here we have to take into account the inhabited instances
  as well. [Of course we did not have to do that ADTs and nested types
    because {\em all} instances of ADTs and nested types are
    inhabited.] So, intuitively, we consider a GADT to be
  ``parametric'' if we can construct the inhabitants of all of the
  inhabited type instances in a type-independent way. We capture this
  in our calculus by requiring that $\emptyset;\ol\beta \vdash K$ (and
  that $K$ does not involve any constants?) in $\Gamma;\Phi \vdash
  (\Lan^{\ol{\alpha^0}}_{\ol K} F)\, \ol A$. Then to get that the real
  live Lans interpreting $\Lan$-constructs are $\omega$-cocontinuous
  we also require that $K$ is a polynomial functor in the $\alpha$s
  that does not involve any constant functors. This means that our
  constructors have types of the form $F\ol\beta \to F(K\ol\beta)$
  with no constants in $K$, so that exactly the corresponding
  instances of the GADT are inhabited for every instantiation of the
  $\alpha$s, and the objects for each instantiation of the $\alpha$s
  are constructed in exactly the same way. That is, the inhabited
  instances and the construction of the data in those instances are
  type independent.}
...
\AXC{$\ol{{\color{blue} \emptyset};\ol\alpha^0 \vdash K}$}
...
\AXC{$\ol{{\color{blue} \emptyset}; \ol{\alpha} \vdash K}$}
...
$\omega_1$-directed colimits. We write $[\A,\C]_{\omega_1}$ for the
category of $\omega_1$-cocontinuous functors from $\A$ to $\C$, and
...
($\omega_1$-)(co)complete whenever $\C$ is. In particular, if $F :
[\C, \C]_{(\omega_1)}$ for a(n) ($\omega_1$-)cocomplete category $\C$,
...
\begin{align*}
  ...
  \oCPOsem{\Gamma;\Phi \vdash (\Lan^{\ol{\alpha}}_{\ol{K}} F)\ol{A}}\rho &=
    \{ t : (\textit{Lan}_{\ol{\oCPOsem{\emptyset; \ol\alpha \vdash
          K}[\ol{\alpha := \_}]}} \oCPOsem{\Gamma; \Phi, \ol\alpha
      \vdash F}\rho[\ol{\alpha := \_}]) \ol{\oCPOsem{\Gamma; \Phi
        \vdash A}\rho}\\
    &\hspace{0.3in}|~
%    (t,t) \in \oCPORsem{\Gamma;\Phi \vdash
%      (\Lan^{\ol{\alpha}}_{\ol{K}} F)\ol{A}}\Eq_\rho\}\\
{\color{blue} ( (\eta_1)_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\rho }} t,
  (\eta_2)_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\rho }} t) \in}\\
  &{\color{blue} \hspace{0.5in}
     (\textit{Lan}_{\ol{\oCPORsem{\emptyset; \ol\alpha 
        \vdash K}[\ol{\alpha := \Eq_{\_}}]}}
  \oCPORsem{\Gamma; \Phi, \ol\alpha \vdash F}\Eq_\rho[\ol{\alpha :=
      \Eq_{\_}}]) \ol{\oCPORsem{\Gamma; \Phi \vdash A}\Eq_\rho} \}}\\
\end{align*}
\end{dfn}
{\color{blue} Here, $\eta_i : \textit{Lan}_{\ol{\oCPOsem{\emptyset;
        \ol\alpha \vdash K}[\ol{\alpha := \_}]}} \oCPOsem{\Gamma;
    \Phi, \ol\alpha \vdash F}\rho[\ol{\alpha := \_}] \; \to \;
  \pi_i\,(\textit{Lan}_{\ol{\oCPORsem{\emptyset; \ol\alpha \vdash
        K}[\ol{\alpha := \Eq_{\_}}]}} \oCPORsem{\Gamma; \Phi,
    \ol\alpha \vdash F}\Eq_\rho[\ol{\alpha := \Eq_{\_}}])$ for $i =
  1,2$, i.e., $\eta_i$ is for the (mixed $\set/\rel$) diagram top p2
  of Daniel's notes for relational environment $\Eq_\rho[\alpha :=
    \Eq_{\_}]$. (Note that the codomains of $\eta_1$ and $\eta_2$
  should be the same so we should have $\eta_1=\eta_2$). The condition
  in the final clause should therefore simplify by writing $\eta$ for
  $\eta_1 = \eta_2$.)}
...
follows from Corollary~12 of~\cite{jp19} if we \new{APPROPRIATELY
  ...
  \emptyset \vdash \Nat^\Phi F\,G}$ is an $\omega_1$-cocontinuous
...
\[\begin{array}{ll}
 & \big(\textit{Lan}_{\ol{\oCPOsem{\emptyset; \ol\alpha \vdash K}[\ol{\alpha := \_}]}}\,
...
$\textit{Lan}_{\ol{\oCPOsem{\emptyset; \ol\alpha \vdash
        K}[\ol{\alpha := \_}]}}\,\oCPOsem{\Gamma; \Phi, \ol\alpha
...
{\color{blue} Need to check that the extra condition in the interp of
  Lans is preserved for the last clause to be well-defined.}
...
T$ to be $\colim{i < \omega_1}{T^i K_0}$. It is not hard to see that
$\mu T$ is given explicitly as
\begin{equation}\label{eq:mu}
  \mu T = (\mu T^1,\mu T^2,
\colim{i < \omega_1}{(T^iK_0)^*})
\end{equation}
...
\begin{align*}
  ...
  \oCPOsem{\Gamma;\Phi \vdash (\Lan^{\ol{\alpha}}_{\ol{K}}
    F)\ol{A}}\pi_2 \rho\\
  &\hspace{0.3in}|~{\color{blue}
    ((\eta_1)_{\ol{\oCPOsem{\Gamma; \Phi \vdash A}\pi_1 \rho}} t_1,
(\eta_2)_{\ol{\oCPOsem{\Gamma; \Phi \vdash A}\pi_2 \rho}} t_2) \in}\\
  &\hspace{0.3in}{\color{blue}
(\textit{Lan}_{\ol{\oCPORsem{\emptyset;
          \ol\alpha \vdash K}[\ol{\alpha := \Eq_{\_}}]}}
    \oCPORsem{\Gamma; \Phi, \ol\alpha \vdash F}\rho[\ol{\alpha :=
        \Eq_{\_}}])
    \ol{\oCPORsem{\Gamma; \Phi \vdash A}\rho}} \}  
  %\forall \sigma : \setenv, (
  %(\eta^\sigma_1)_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\sigma }} t_1,
  %(\eta^\sigma_2)_{\ol{\oCPOsem{\Gamma;\Phi \vdash A}\sigma }} t_2) \in\\
  %&\hspace{0.5in} (\textit{Lan}_{\ol{\oCPORsem{\Gamma; \ol\alpha
  %      \vdash K}\Eq_\sigma[\ol{\alpha := \Eq_{\_}}]}}
  %\oCPORsem{\Gamma; \Phi, \ol\alpha \vdash F}\Eq_\sigma[\ol{\alpha :=
  %    \Eq_{\_}}]) \ol{\oCPORsem{\Gamma; \Phi \vdash A}\Eq_\sigma} \}\\
\end{align*}
\end{dfn}
{\color{blue} where $\eta_i$ is for the (mixed $\set/\rel$) diagram
  top p2 of Daniel's notes for relational environment $\Eq_{\pi_i
    \rho}[\alpha := \Eq_{\_}]$.}


%{\color{blue} In the final clause of Definition~\ref{def:rel-sem},
%  $\eta^\sigma_i : \textit{Lan}_{\ol{\oCPOsem{\Gamma; \ol\alpha \vdash
%        K}\sigma[\ol{\alpha := \_}]}} \oCPOsem{\Gamma; \Phi, \ol\alpha
%    \vdash F}\sigma[\ol{\alpha := \_}] \; \to \;
%  \pi_i\,(\textit{Lan}_{\ol{\oCPORsem{\Gamma; \ol\alpha \vdash
%        K}\Eq_\sigma[\ol{\alpha := \Eq_{\_}}]}} \oCPORsem{\Gamma;
%    \Phi, \ol\alpha \vdash F}\Eq_\sigma[\ol{\alpha := \Eq_{\_}}])$ for
%  each $\sigma : \setenv$ and $i = 1,2$.}
...
{\color{blue} This is not true for the relational interps of
  Lans. Define explicitly.}
...
that, since $\oCPO$ is a locally $\omega_1$-presentable category,
...
If $F,G : [\oCPO^k,\oCPO]_{\omega_1}$, and if $\alpha : F \to G$ is a natural
transformation, then $\graph{\alpha}$ is in $\oCPORT_k$.
\end{lemma}
\begin{proof}
Clearly, $\graph{\alpha}^*$ is $\omega_1$-cocontinuous, so
$\graph{\alpha}^* : [\oCPOR^k,\oCPOR]_{\omega_1}$. Now, suppose
...
If $\alpha : F \to G$ is a morphism in $[\oCPO^k, \oCPO]_{\omega_1}$
...
[\oCPO^k,\oCPO]_{\omega_1}$ is defined to be $\Eq_F =
...
application, fixpoint, and $\Lan$ cases are non-routine. The latter
two use Equation~\ref{eq:eq-pres-eq}. The fixpoint case also uses the
observation that, for every $i < \omega_1$, the following intermediate
