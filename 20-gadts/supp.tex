...
\eta'_{\overline{K^1}})$ is a morphism from $\oCPORsem{\Gamma; \Phi
...
\item The application case is proved by the following sequence of
  ...
  Definition~11, and the fourth is by
  Lemma~23:
\[
\begin{split}
...
Lemma~23 and the fourth equality is because, for every $i < \omega_1$,
...
and the last is by Lemma~23. {\color{red} Finally, the inductive step
  for limit ordinals is...}
...
\[T^0_{G,
  \Eq_{\rho}[\phi := T^{i}_{H,\Eq_{\rho}} K_0][\overline{\alpha :=
    ...
    \Eq_{(T^{\oCPO}_{H,\rho})^i K_0}][\overline{\alpha :=
      \Eq_A}]}\,K_0\]
The inductive case for successor ordinals is proved by: \\
\begin{align*}
...
{\color{red} The inductive case for limit ordinals is proved by...}
