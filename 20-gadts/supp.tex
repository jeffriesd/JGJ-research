...
and the last is by Equation~10. {\color{blue} Finally, the inductive
...
& \hspace*{0.5in}(\textit{Lan}_{\ol{\oCPORsem{\emptyset; \ol\beta 
        \vdash K}[\ol{\beta := \Eq_{\_}}]}}
...
%        \vdash K}\Eq_\rho[\ol{\beta := \Eq_{\_}}]}}
%  \oCPORsem{\Gamma; \Phi',\phi,
%    \ol\alpha,\ol\beta \vdash F}\Eq_\rho[\phi := T^i_{H,\Eq_{\rho}} K_0][\ol{\a%lpha
%      := \Eq_A}][\ol{\beta := \Eq_{\_}}])}\\
...
& \hspace*{0.5in}(\textit{Lan}_{\ol{\oCPORsem{\emptyset; \ol\beta 
  ...
  \oCPORsem{\Gamma; \Phi', \phi,\ol\alpha,\ol\beta \vdash F}\Eq_\rho[\phi
    := \Eq_{(T^\oCPO_{H,\rho})^i 
      K_0}][\ol{\alpha := \Eq_A}][\ol{\beta := \Eq_{\_}}])\\
&\hspace*{1in}\Eq_{\ol{\oCPOsem{\Gamma; \Phi',
...
\end{align*}
The second equality uses~\eqref{eq:helper-lan} four times, the
induction hypothesis for ~\eqref{eq:iel-fix-point-intermediate2}
twice, and the induction hypothesis for the IEL once, together with
two applications of the observation that $\pi_1 (\Eq_{\rho} [\phi :=
  (T^\oCPO_{H,\Eq_\rho})^i K_0] [\overline{\alpha := \Eq_A}]) =
\pi_1(\Eq_{\rho} [\phi := \Eq_{(T^\oCPO_{H,\rho})^i K_0}]
   [\overline{\alpha := \Eq_A}]) = \rho [\phi := (T^\oCPO_{H,\rho})^i
     K_0] [\overline{\alpha := A}]$, so that natural transformations
   $\eta_1$ and $\eta_2$ appearing in the conditions for the first and
   last expressions are indeed the same. REFER TO ELSEWHERE FOR
   DIAGRAMS FOR $\eta_i$s.}
\end{itemize}
...
& \hspace*{0.3in}(\textit{Lan}_{\ol{\oCPORsem{\Gamma; \ol\alpha
      \vdash K}\Eq_\rho[\ol{\alpha := \Eq_{\_}}]}} \oCPORsem{\Gamma; \Phi,
  \ol\alpha \vdash F}\Eq_\rho[\ol{\alpha := \Eq_{\_}}]) \ol{\oCPORsem{\Gamma;
    \Phi \vdash A}\Eq_\rho}\\
